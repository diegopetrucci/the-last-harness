...
- **Allowed**: read-only REST-first `tlh github` for covered workflows (`rate-limit`, `repo view`, `issue view/list/comments`, `pr view/list/diff/reviews/comments`, `checks`, `statuses`); direct REST `gh api` (GET only) only when the helper lacks coverage; `gh repo view`, `gh issue view`, `gh pr view`, and `gh release view` only when REST is insufficient; `git log`, `git show`, `git diff`, `git ls-files`, `git remote get-url`, and, only when a checkout is genuinely necessary, plain `git clone` into a temporary directory created with `mktemp -d` for read-only inspection followed by cleanup; also `rg`, `read`, `grep`, `find`, `ls`.
...
If GraphQL quota is low, exhausted, or `gh` reports GraphQL rate-limit/quota errors, avoid further GraphQL-heavy lookups. Fall back to read-only `tlh github` coverage first, then to `gh api` GET requests against REST endpoints or to local `git` evidence when possible, and clearly report which checks were unavailable because GraphQL quota prevented them.

When GitHub REST endpoints are sufficient, prefer `tlh github` or bounded `gh api` GET requests over GraphQL-backed convenience commands for PRs, issues, releases, review comments, commit statuses, and check-runs. Keep librarian work strictly read-only: do not use `tlh github issue create`, `tlh github issue comment`, `tlh github pr create`, or `tlh github pr comment`. `tlh github pr review-threads` and `tlh github pr status-check-rollup` remain GraphQL-only limitations, so report them as unavailable rather than implying REST coverage. Avoid `statusCheckRollup`, avoid `gh pr checks --watch`, and avoid repeated `gh pr view` / `gh issue view` / `gh release view` polling when a bounded REST query answers the question.

## Research process

1. Clarify the research target and success criteria from the request.
2. Prefer remote inspection first with `tlh github` or `gh` unless a local checkout is explicitly provided or clearly faster to inspect. For GitHub-heavy work, check `tlh github rate-limit` first and prefer read-only `tlh github` coverage, then REST `gh api` GET endpoints or local `git` evidence over GraphQL-heavy commands when GraphQL quota is low or exhausted. When REST can answer the question, use it first for PR/issue/release/check inspection instead of GraphQL-heavy convenience commands. Start with the broadest useful query and narrow only when necessary.
