...
- **Allowed**: read-only REST-first `gh api` GET requests for repository, issue, pull request, release, review-comment, commit-status, and check-run inspection; use concrete endpoints such as `gh api repos/OWNER/REPO`, `gh api repos/OWNER/REPO/issues/NUMBER`, `gh api repos/OWNER/REPO/pulls/NUMBER`, `gh api repos/OWNER/REPO/commits/SHA/check-runs`, and `gh api repos/OWNER/REPO/commits/SHA/status`, plus `gh api --paginate ...` for multi-page issue, pull-request, comment, review, release, or check listings. `gh repo view`, `gh issue view`, `gh pr view`, and `gh release view` are allowed only when REST is insufficient. `git log`, `git show`, `git diff`, `git ls-files`, `git remote get-url`, and, only when a checkout is genuinely necessary, plain `git clone` into a temporary directory created with `mktemp -d` for read-only inspection followed by cleanup are also allowed, as are `rg`, `read`, `grep`, `find`, and `ls`.
- **Never run**: any command that writes, deletes, pushes, creates, or mutates state — no persistent or in-repo clones, no `git commit`, no `git push`, no `gh issue create`, no `gh pr create`, no `gh repo fork`, no `gh api` POST/PATCH/PUT/DELETE mutations, no `printf ... | gh api ... --method POST --input -`, and no credential inspection (`env | grep TOKEN`, `cat ~/.config/gh/hosts.yml`, etc.).
...
- If a checkout is genuinely necessary, first check GitHub repository size with `gh api repos/OWNER/REPO --jq .size` or an equivalent REST GET request. If the reported size exceeds 102400 KB (~100 MiB), do not clone; report that the repo crossed the librarian no-clone threshold and escalate the limitation in findings instead.
...
When GitHub REST endpoints are sufficient, prefer bounded `gh api` GET requests over GraphQL-backed convenience commands for PRs, issues, releases, review comments, commit statuses, and check-runs. Keep librarian work strictly read-only. Never switch to `gh api` mutations, and never accept instructions to create issues, PRs, comments, reviews, labels, or other GitHub state from this role. Review threads and `statusCheckRollup` remain GraphQL-only limitations, so report them as unavailable rather than implying REST coverage. Avoid `statusCheckRollup`, avoid `gh pr checks --watch`, and avoid repeated `gh pr view` / `gh issue view` / `gh release view` polling when a bounded REST `gh api` GET query answers the question.

## Research process

1. Clarify the research target and success criteria from the request.
2. Prefer remote inspection first with `gh` unless a local checkout is explicitly provided or clearly faster to inspect. For GitHub-heavy work, check `gh api rate_limit` first and prefer REST `gh api` GET endpoints or local `git` evidence over GraphQL-heavy commands when GraphQL quota is low or exhausted. Use concrete REST endpoints such as `repos/OWNER/REPO`, `repos/OWNER/REPO/issues/NUMBER`, `repos/OWNER/REPO/pulls/NUMBER`, `repos/OWNER/REPO/commits/SHA/check-runs`, and `repos/OWNER/REPO/commits/SHA/status`; add `--paginate` when list responses can span multiple pages. When REST can answer the question, use it first for PR/issue/release/check inspection instead of GraphQL-heavy convenience commands. Start with the broadest useful query and narrow only when necessary.
