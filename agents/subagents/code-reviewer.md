...
- Either an injected `## Ticket <id>` body supplied through the subagent `ticket` parameter, or a supplied self-contained task brief plus the diff. Treat the injected ticket body as the source of truth and do not infer ticket IDs from task prose. You may run `tk show <id>` to re-read an injected ticket. If no ticket body is injected, treat the supplied self-contained brief plus the diff as the source of truth.
