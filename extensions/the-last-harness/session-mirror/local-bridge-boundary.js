...
function sameFile(first, second) {
    return (first.dev !== undefined &&
        first.ino !== undefined &&
        first.dev === second.dev &&
        first.ino === second.ino);
}
function modeMatches(s, k, p, u) {
...
function privateDirectory(directory, fs, expectedUid) {
    if (directory === "/")
        return false;
    let current = "";
    for (const part of directory.slice(1).split("/")) {
        ...
        if ((stat.mode & 0o022) !== 0 && !stickyWorldWritable)
            return false;
        if (current === directory) {
            if (stat.uid !== expectedUid || (stat.mode & 0o7777) !== 0o700)
                return false;
        }
        else if (stat.uid !== expectedUid && stat.uid !== 0)
            return false;
    }
    return true;
}
...
function select(value, key, fallback) {
    const candidate = option(value, key);
    return isFunction(candidate) ? candidate : fallback;
}
function read(fd, buffer, offset, length, position) {
    return readSync(fd, buffer, offset, length, position);
}
function fileSystem(controls) {
...
export function readRendezvous(directory, controls) {
    const safeDirectory = validPath(directory);
    if (safeDirectory === undefined)
        return undefined;
    const expectedUid = currentUid();
    if (expectedUid === undefined || O_RDONLY_NOFOLLOW === undefined)
        return undefined;
    const fs = fileSystem(controls);
    if (!privateDirectory(safeDirectory, fs, expectedUid))
        return undefined;
    const path = `${safeDirectory}/rendezvous.json`;
    let fd;
    try {
        const stat = metadata(fs.lstat(path));
        if (!stat || !modeMatches(stat, 0o100000, 0o600, expectedUid))
            return undefined;
        ...
        while (length < bytes.length) {
            ...
            if (count === undefined || count > bytes.length - length)
                return undefined;
            if (count === 0)
                break;
            length += count;
        }
        if (length > MAX_RENDEZVOUS_BYTES)
            return undefined;
...
export function socketPath(directory, name) {
    const safeDirectory = validPath(directory);
...
export function socketIsSafe(path, controls) {
    const safePath = validPath(path);
    const expectedUid = currentUid();
    if (safePath === undefined || expectedUid === undefined)
        return false;
    try {
...
export function parseResult(body) {
    ...
    if (["ready", "disconnected", "accepted", "duplicate", "deduplicated", "evicted"].includes(code)) {
        if (typeof revision !== "number" || typeof epoch !== "number")
            return undefined;
...
export function sourceInstanceId(value) {
