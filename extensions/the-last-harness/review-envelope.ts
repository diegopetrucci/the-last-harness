...
export function buildReviewEnvelope(
  ...
  if (ctx?.checkout?.performed) {
    ...
    lines.push(
      `note: previously on ${ctx.checkout.priorBranch}; run \`git checkout -\` to return.`,
    );
  }
  ...
  const bodyText = hasBody
    ? escapeEnvelopeFenceLines(ctx?.body as string, fenceKind)
    : "(no body gathered)";
...
function escapeEnvelopeFenceLines(body: string, fenceKind: string): string {
  ...
  return body
    .split("\n")
    .map((line) =>
      line === beginFence || line === endFence ? escapeDelimitedContentLine(line) : line,
    )
    .join("\n");
}
 ...
 */
export async function buildSnapshotParts(
  cwd: string,
  filePaths: string[],
  label: string,
): Promise<string[]> {
