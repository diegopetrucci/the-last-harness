...
function hasInlineLikeGitCommitMessageOrFileArgument(commitArguments: string[]): boolean {
  return (
    hasInlineGitCommitMessageArgument(commitArguments) ||
    hasInlineGitCommitFileArgument(commitArguments)
  );
}

function getInlineGitCommitFileArgument(
  commitArguments: string[],
): "stdin" | "process-substitution" | undefined {
...
function processSubstitutionIncludesTlhCommitAttributionFooter(
  fileArgument: string,
  footer: string,
): boolean {
...
function areInlineGitCommitArgumentsAttributed(
  commitArguments: string[],
  footer: string,
  segment?: string,
): boolean {
...
export function buildTlhCommitAttributionPrompt(
...
type AttributionCommandModule = {
  handleToggleTlhGitAttributionCommand(
    pi: ExtensionAPI,
    args: string,
...
export function registerToggleTlhGitAttributionCommand(
  ...
  const loadModule = createRetryableLazyImport(
    options.loadModule ??
      (() => import("./attribution-command.js") as Promise<AttributionCommandModule>),
  );
