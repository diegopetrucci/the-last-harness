...
function isSupportedTlhSubscriptionUsageProvider(
  ...
  return (
    ...
    SUPPORTED_PROVIDERS.has(provider as TlhSubscriptionUsageProvider)
  );
}
...
function subscriptionUsageCacheKey(
  provider: TlhSubscriptionUsageProvider,
  identity: string,
): string {
...
function cacheKeyMatchesProvider(
  cacheKey: string,
  provider: TlhSubscriptionUsageProvider,
): boolean {
...
export async function fetchTlhSubscriptionUsage(
  ...
  if (
    !isSupportedTlhSubscriptionUsageProvider(provider) ||
    !accessToken ||
...
async function resolveTlhSubscriptionUsageTarget(
  resolved: EligibleProviderContext,
