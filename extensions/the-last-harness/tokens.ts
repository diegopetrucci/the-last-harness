...
export type TokensReportSessionManager = Pick<
  ExtensionContext["sessionManager"],
  "getSessionDir" | "getSessionFile"
>;

export function createTokensCommandHandler(
  pi: ExtensionAPI,
  ...
  return [
    ...
    renderSection(
      ...
      [
        ...
        renderMetricCard(
          ...
          `${formatCurrency(analysis.totals.combined.costUsd)} • ${formatInteger(analysis.totals.combined.turns)} assistant turns`,
        ),
        renderMetricCard(
          primaryLabel,
          formatInteger(analysis.totals.primary.totalTokens),
          `${formatCurrency(analysis.totals.primary.costUsd)} • ${formatCoverage(coverage)}${formatCacheWarmCardDetail(primaryUsage)}`,
        ),
    ...
    renderSection(
      ...
      [
        ...
        renderMetricCard(
          "Source precision",
          analysis.tools.precision,
          "Estimated from tool names and current catalog",
        ),
        renderMetricCard(
          "MCP est. tokens",
    ...
    renderSection(
      ...
      [
        '<ul class="caveats">',
        ...[privacyCaveat, coverageCaveat, ...analysis.caveats].map(
...
function renderUsageTotalsTable(
  analysis: TlhSessionUsageAnalysis,
  primaryLabel = "Primary assistant",
): string {
  const em = "\u2014";
  const primaryUsage = getPrimaryUsageBreakdown(analysis);
...
function renderModelUsageTable(title: string, models: TlhModelUsage[]): string {
  return [
    ...
    renderTable(
      ["Model", "Provider", "Tokens", "Cost", "Assistant turns", "Assistant messages"],
      models.map((model) => [
        model.modelId,
        model.provider ?? "—",
...
function renderCacheTotalsTable(
  analysis: TlhSessionUsageAnalysis,
  primaryLabel = "Primary assistant",
): string {
  const primaryUsage = getPrimaryUsageBreakdown(analysis);
  const rows: Array<[string, TlhUsageTotals]> = [[primaryLabel, analysis.totals.primary]];
  if (hasCacheWarmUsage(primaryUsage)) {
    rows.push(
      ["Assistant-turn usage", primaryUsage.assistantTurnUsage],
      ["Pi-native cache_warm (non-turn)", primaryUsage.cacheWarmUsage],
    );
  }
...
function buildOpenReportCommands(
  path: string,
...
function hasUsage(usage: TlhUsageTotals): boolean {
  return (
    usage.inputTokens !== 0 ||
...
function hasCacheWarmUsage(usage: PrimaryUsageBreakdown): boolean {
  return usage.cacheWarmRequestCount > 0 || hasUsage(usage.cacheWarmUsage);
}

function formatCacheWarmCardDetail(usage: PrimaryUsageBreakdown): string {
...
function formatCacheWarmSummary(usage: PrimaryUsageBreakdown): string {
  if (!hasCacheWarmUsage(usage)) {
...
function formatCoverage(
  coverage: TlhSessionUsageAnalysis["primaryAssistant"]["usageCoverage"],
): string {
