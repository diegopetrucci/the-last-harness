...
import { ModelSelectorComponent, SettingsManager, type ExtensionContext } from "@earendil-works/pi-coding-agent";
...
function modelMatches(left: Pick<ModelOperation, "provider" | "modelId">, right: TlhModelSelection): boolean {
	return left.provider === right.provider && left.modelId === right.id;
}

function operationsMatch(left: ImmediateModelWrite | ModelOperation, right: CombinedModelWrite): boolean {
...
function replayWrites(patch: TlhModelSelectionPersistencePatch, writes: readonly SuppressedDefaultWrite[]): void {
...
function targetIsTrackedActiveModel(active: TrackedActiveModel, write: CombinedModelWrite): boolean {
	return (
		active.known &&
...
function replayImmediateModelWrite(patch: TlhModelSelectionPersistencePatch, write: CombinedModelWrite): void {
...
function interceptCombinedModelWrite(patch: TlhModelSelectionPersistencePatch, write: CombinedModelWrite): void {
...
export function installTlhModelSelectionPersistenceOverride(): boolean {
	...
	const modelSelectorPrototype = ModelSelectorComponent.prototype as unknown as ModelSelectorRuntimePrototype;
	const originals = {
...
export function claimTlhModelSelectionDefaults(model: TlhModelSelection): TlhModelSelectionDefaultsClaim | undefined {
	const patch = getInstalledPatch();
	if (!patch) {
		return undefined;
	}
...
export function isTlhNativeModelSelectorClaim(claim: TlhModelSelectionDefaultsClaim | undefined): boolean {
	return claim?.nativeSelector === true;
}

function takeClaimWrites(claim: TlhModelSelectionDefaultsClaim | undefined): SuppressedDefaultWrite[] {
