...
export function readHereDocBodies(command, startIndex, specs) {
    const bodies = [];
...
export function extractHereDocBodies(segment) {
    let quote;
    let escaped = false;
    const hereDocs = [];
...
export function normalizeTrailingLineEnding(value) {
    return value.replace(/\r?\n$/, "");
}
export function readProcessSubstitutionBody(command, startIndex) {
    let quote;
    let escaped = false;
    const hereDocs = [];
    let depth = 1;
...
function readShellCommandSegment(command, startIndex) {
    let quote;
    let escaped = false;
    const hereDocs = [];
    for (let index = startIndex; index < command.length; index += 1) {
...
export function isShellVariableAssignmentToken(token) {
    return /^[A-Za-z_][A-Za-z0-9_]*=/.test(token);
}
export function stripLeadingShellCommandPrefixes(tokens) {
    let startIndex = 0;
...
export function isSupportedEnvCommand(command) {
    return commandBasename(command) === "env";
}
function normalizeCaseInsensitiveLongOptionToken(token) {
    return token.startsWith("--") ? token.toLowerCase() : token;
}
function isSupportedEnvOptionWithoutValue(token) {
...
export function stripLeadingOptionTerminator(tokens) {
    return tokens[0] === "--" ? tokens.slice(1) : tokens;
}
export function buildEnvSplitStringEffectiveTokens(payload, remainingTokens) {
    return stripLeadingOptionTerminator([...tokenizeShellWords(payload), ...remainingTokens]);
}
export function getEnvSplitStringEffectiveTokens(tokens, splitStringIndex) {
...
function getShortEnvLeadingOptionParseResult(tokens, index) {
...
export function getEnvLeadingOptionParseResult(tokens, index) {
    const token = tokens[index] ?? "";
...
export function unwrapLeadingEnvCommandTokens(tokens) {
...
export function normalizeShellCommandTokensFromTokens(tokens) {
...
export function normalizeShellCommandTokens(segment) {
    return normalizeShellCommandTokensFromTokens(tokenizeShellWords(segment));
}
function getShellCommandPrefix(command) {
    let quote;
    let escaped = false;
    for (let index = 0; index < command.length; index += 1) {
        const character = command[index];
...
function renderPrintfEscape(format, index) {
...
function renderPrintfCycle(format, args, startIndex) {
    let output = "";
...
export function renderPrintfOutput(args) {
    let formatIndex = 0;
    if (args[formatIndex] === "--") {
        formatIndex += 1;
    }
...
export function renderEchoOutput(args) {
    let index = 0;
...
export function getObviousShellSegmentOutput(segment) {
...
export function getProcessSubstitutionOutput(command) {
    let output = "";
...
function isSupportedShellCommandWrapper(command) {
    return SHELL_COMMAND_WRAPPERS.has(commandBasename(command));
}
export function getWrappedShellCommandFromTokens(tokens) {
...
export function getWrappedShellCommand(segment) {
