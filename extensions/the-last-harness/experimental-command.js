...
function ensureMutableExperimentalSettings(settings) {
    validateTlhExperimentalSettings(settings);
    settings.tlh ??= {};
    settings.tlh.experimental ??= {};
}
function formatExperimentalFeatureStatus(featureId, enabled) {
...
function experimentalFeaturePickerOption(featureId, enabled) {
    const feature = getExperimentalFeature(featureId);
    if (!feature) {
        return featureId;
    }
...
function notifyExperimentalWriteResult(pi, ctx, featureId, result) {
...
async function showExperimentalFeaturePicker(pi, ctx) {
    ...
    while (true) {
        ...
        const featureIdsByOption = new Map(TLH_EXPERIMENTAL_FEATURES.map((feature) => {
...
function nextEnabledState(currentEnabled, action) {
    if (action === "enable")
        return true;
    if (action === "disable")
        return false;
...
function writeExperimentalFeaturePreference(cwd, featureId, action) {
    const feature = getExperimentalFeature(featureId);
    if (!feature) {
