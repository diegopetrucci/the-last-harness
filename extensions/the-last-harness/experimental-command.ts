...
function validateTlhExperimentalSettings(settings: unknown): asserts settings is TlhSettings {
  ...
  if (enabledFeatures !== undefined && !Array.isArray(enabledFeatures)) {
    throw new Error(
      "settings field 'tlh.experimental.enabledFeatures' must be an array if present",
    );
  }
  if (
...
function ensureMutableExperimentalSettings(
...
function formatExperimentalStatusMessage(
  ...
  if (featureId) {
    ...
    return formatExperimentalFeatureStatus(
      feature.id,
  ...
  return [
    ...
    ...TLH_EXPERIMENTAL_FEATURES.map((feature) =>
      formatExperimentalFeatureStatus(
        feature.id,
        isTlhExperimentalFeatureEnabled(config, feature.id),
      ),
    ),
...
function experimentalFeaturePickerOption(
  featureId: TlhExperimentalFeatureId,
  enabled: boolean,
): string {
  const feature = getExperimentalFeature(featureId);
...
function notifyExperimentalWriteResult(
  ...
  ctx.ui.notify(
    `${changedLabel} TLH experimental feature ${featureId} at ${formatHomePath(result.settingsPath)}. It is now ${stateLabel}.${activationNotice} ${undoLabel}${backupLabel}`,
    "info",
  );
}

async function showExperimentalFeaturePicker(
  pi: ExtensionAPI,
  ctx: ExtensionCommandContext,
): Promise<void> {
  if (
  ...
  while (true) {
    ...
    const featureIdsByOption = new Map(
      TLH_EXPERIMENTAL_FEATURES.map((feature) => {
        const option = experimentalFeaturePickerOption(
          feature.id,
          isTlhExperimentalFeatureEnabled(config, feature.id),
        );
    ...
    try {
      ...
      ctx.ui.notify(
        `Could not update TLH experimental feature ${selectedFeatureId}: ${message}`,
        "error",
      );
    }
  }
...
function writeExperimentalFeaturePreference(
  ...
  return withLockedTlhSettingsWrite(
    ...
    (current) => {
      ...
      const currentEnabledFeatures = normalizeEnabledFeatures(
        settings.tlh?.experimental?.enabledFeatures,
      );
...
export async function handleExperimentalCommand(
  ...
  if (!command) {
    ctx.ui.notify(
      `${EXPERIMENTAL_COMMAND_HELP} Available: ${availableExperimentalFeatureList()}.`,
      "error",
    );
    return;
  }
  ...
  if (command.type === "list" || command.type === "status") {
    ...
    ctx.ui.notify(
      formatExperimentalStatusMessage(getTlhExperimentalConfig(ctx.cwd), featureId),
      "info",
    );
    return;
  }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    ctx.ui.notify(
      `Could not update TLH experimental feature ${command.featureId}: ${message}`,
      "error",
    );
  }
}
