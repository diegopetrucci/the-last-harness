...
const TLH_MODEL_VISIBILITY_GET_AVAILABLE_ORIGINAL = Symbol.for(
  "tlh.modelVisibilityGetAvailableOriginal",
);
...
const TLH_MODEL_VISIBILITY_EXACT_LOOKUP_PATCHED = Symbol.for(
  "tlh.modelVisibilityExactLookupPatched",
);
...
export function normalizeTlhModelVisibilityConfig(
  config: unknown,
): ResolvedTlhModelVisibilityConfig {
  if (!isRecord(config)) {
...
export function matchesTlhModelVisibilityPattern(
...
export function isTlhModelHidden(
  ...
  return (
    matchesAnyPattern(model, TLH_HIDDEN_MODEL_DEFAULTS) || matchesAnyPattern(model, config.hidden)
  );
}
...
export function installTlhModelVisibilityFilter(): void {
  ...
  interactiveModePrototype.findExactModelMatch = async function tlhFindExactModelMatch(
    ...
    try {
      ...
      const refreshedMatch = findExactModelReferenceMatch(
        searchTerm,
        getUnfilteredAvailableModels(this.session),
      );
...
export function getUnfilteredAvailableModels(
  ...
  if (
    "getAvailableSnapshot" in modelSource &&
  ...
  if ("getAvailable" in modelSource && typeof modelSource.getAvailable === "function") {
    const modelRegistryPrototype = Object.getPrototypeOf(
      modelSource,
    ) as ModelRegistryPrototype | null;
