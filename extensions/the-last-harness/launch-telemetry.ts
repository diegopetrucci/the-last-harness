...
function extractBundledSubagentOverrides(
  ...
  for (const name of BUNDLED_SUBAGENT_NAMES) {
    ...
    if (typeof entry.model === "string" || entry.model === false)
      overrideEntry.model = entry.model as string | false;
 ...
 */
function readTlhProjectSubagentOverrides(
  cwd: string,
): Record<string, SubagentOverrideEntry> | undefined {
  try {
...
function readTlhLaunchSettings(): { ok: true; config: TlhLaunchSettings } | { ok: false } {
  ...
  const experimental =
    isPlainObject(tlh) && isPlainObject(tlh.experimental)
      ? (tlh.experimental as TlhExperimentalConfig)
      : undefined;
  ...
  return {
    ok: true,
    config: {
      telemetry: telemetry as TlhTelemetryConfig | undefined,
      experimental,
      subagentOverrides,
    },
  };
...
function shouldSkipTlhLaunchTelemetry(
  ...
  if (
    !configuredTlhTelemetryNamespace() ||
...
function getOrCreateTlhTelemetryInstallId(): string | undefined {
  ...
  if (
    existing?.schemaVersion === TLH_TELEMETRY_STATE_SCHEMA_VERSION &&
    ...
    [
      {
        ...
        payload: {
          ...
          "Tlh.PrimaryAgent.name": privacySafeTlhTelemetryPrimaryAgentName(
            snapshot.primaryAgentName,
          ),
