...
export function formatTlhSubscriptionUsageFooterSegment(
  ...
  const sessionSegment = formatUsageWindow(
    snapshot.provider,
    snapshot.windows?.session,
    "session",
    nowMs,
  );
  if (!sessionSegment) {
  ...
  if (options.showWeekly) {
    const weeklySegment = formatUsageWindow(
      snapshot.provider,
      snapshot.windows?.weekly,
      "weekly",
      nowMs,
    );
    if (weeklySegment) {
...
function getWeeklyVisibilityPreference(
  shouldShowWeekly: (() => boolean | undefined) | undefined,
): boolean | undefined {
  try {
...
function deriveWeeklyRemainingPercent(
  window: TlhSubscriptionUsageWindow | undefined,
): number | undefined {
  if (!window) {
    return undefined;
...
export function getTlhSubscriptionUsageFooterState(
  ...
  const subscriptionEligible = isSubscriptionUsageEligible(
    ctx,
    provider,
