...
function getTlhUsageLimitsConfig(cwd: string): TlhUsageLimitsConfig | undefined {
  try {
...
export function shouldShowTlhUsageWeekly(
  config: TlhUsageLimitsConfig | undefined,
): boolean | undefined {
...
function usageCommandCompletions(prefix: string) {
  ...
  const completions = USAGE_COMMAND_COMPLETIONS.filter((option) =>
