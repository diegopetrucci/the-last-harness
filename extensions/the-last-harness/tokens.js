...
export function createTokensCommandHandler(pi, dependencies = {}) {
    ...
    return async (args, ctx) => {
        ...
        try {
            ...
            try {
                ...
                ctx.ui.notify(`Opened local TLH token report at ${formatHomePath(report.path)}. Delete ${formatHomePath(report.directory)} when you no longer need it.`, "info");
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                ctx.ui.notify(`Generated local TLH token report at ${formatHomePath(report.path)}, but could not open it automatically: ${message}. Open the file manually and delete ${formatHomePath(report.directory)} when you no longer need it.`, "warning");
            }
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
...
export function buildTokensReportHtml(analysis, options = {}) {
    ...
    const primaryLabel = options.primaryAgentLabel && options.primaryAgentLabel.trim()
    ...
    const coverageCaveat = coverage.withoutUsage > 0
...
function renderSubagentRunsTable(analysis) {
...
function renderToolSourceTable(sources) {
    return [
        "<h3>Tool sources</h3>",
        renderTable(["Source", "Kind", "Calls", "Est. tokens", "Tools", "Scope", "Origin"], sources.map((bucket) => [
            bucket.source.label,
            bucket.source.kind,
...
function renderCacheTimelineTable(turns) {
    return [
        "<h3>Assistant turns with cache activity</h3>",
        renderTable(["Turn", "Timestamp", "Model", "Cache read", "Cache write", "Cache share"], turns.map((turn) => {
...
function renderWorstMissesTable(worst) {
    return renderTable(["Turn", "Idle gap", "Model changed", "Missed tokens", "Extra cost"], worst.map((event) => [
...
function renderReferenceSummary(title, refs) {
    return [
        `<h3>${escapeHtml(title)}</h3>`,
        refs.length > 0
...
function renderIntercomSummary(targets) {
    return [
...
function preferredReportParent(sessionManager) {
...
function createPrivateReportDirectory(preferredParent) {
...
function buildOpenReportCommands(path, platform = process.platform) {
    switch (platform) {
        case "darwin":
...
function setPrivateMode(path, mode) {
...
function formatTimelineTools(turn) {
...
function formatTimelineDiscoveries(turn) {
    const parts = [];
...
function formatRunStatus(success, exitCode) {
...
function formatCacheShare(cacheTokens, totalTokens) {
...
function formatErrorRate(errors, results) {
