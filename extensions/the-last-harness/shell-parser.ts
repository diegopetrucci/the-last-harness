...
export function readShellCommandSegment(
  ...
  for (let index = startIndex; index < command.length; index += 1) {
    ...
    const separatorLength =
      (character === "&" && next === "&") || (character === "|" && (next === "|" || next === "&"))
        ? 2
        : 1;
    return {
...
export function buildEnvSplitStringEffectiveTokens(
  payload: string,
...
export function getEnvSplitStringEffectiveTokens(
  tokens: string[],
...
export function getEnvLeadingOptionParseResult(
  tokens: string[],
  index: number,
): EnvLeadingOptionParseResult {
  const token = tokens[index] ?? "";
