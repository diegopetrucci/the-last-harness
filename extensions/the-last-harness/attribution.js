...
function commitMessageEndsWithFooter(message, footer) {
...
function getGitCommitArgumentsFromTokens(tokens) {
...
function getGitCommitArguments(segment) {
    return getGitCommitArgumentsFromTokens(normalizeShellCommandTokens(segment));
}
function shortGitCommitMessageOptionConsumesFollowingValue(token) {
...
function gitCommitOptionConsumesFollowingValue(commitArguments, index) {
...
function splitGitCommitArgumentsAtPathspecTerminator(commitArguments) {
...
function getInlineGitCommitMessageParts(commitArguments) {
    const messages = [];
...
function hasInlineGitCommitMessageArgument(commitArguments) {
    return getInlineGitCommitMessageParts(commitArguments).length > 0;
}
function getInlineGitCommitFileArgumentValue(commitArguments) {
    let value;
    const { optionArguments } = splitGitCommitArgumentsAtPathspecTerminator(commitArguments);
    for (let index = 0; index < optionArguments.length; index += 1) {
        const token = optionArguments[index];
...
function processSubstitutionIncludesTlhCommitAttributionFooter(fileArgument, footer) {
...
function hasInlineGitCommitFileArgument(commitArguments) {
    return getInlineGitCommitFileArgument(commitArguments) !== undefined;
}
function areInlineGitCommitArgumentsAttributed(commitArguments, footer, segment) {
...
function getEnvContextGitCommitAttributionBlockReasonFromTokens(tokens, footer, sourceSegment, depth = 0, failClosedOnCommitLike = false) {
    if (depth > MAX_WRAPPED_SHELL_GIT_COMMIT_RECURSION_DEPTH) {
        return buildTlhGitCommitAttributionBlockReason(footer);
    }
    ...
    while (index < envTokens.length) {
        ...
        if (parseResult.kind === "split-string") {
            return parseResult.effectiveTokens !== undefined
                ? getEnvContextGitCommitAttributionBlockReasonFromTokens(parseResult.effectiveTokens, footer, sourceSegment, depth + 1, failClosedOnCommitLike)
                : undefined;
        }
        if (parseResult.kind === "unknown-option") {
            return getEnvContextGitCommitAttributionBlockReasonFromTokens(stripLeadingOptionTerminator(envTokens.slice(index + 1)), footer, sourceSegment, depth + 1, true);
        }
        break;
    }
    return getWrappedShellGitCommitAttributionBlockReasonFromTokens(envTokens.slice(index), footer, depth + 1, failClosedOnCommitLike, sourceSegment);
}
function getUnsupportedEnvGitCommitAttributionBlockReason(segment, footer, depth = 0, sourceSegment = segment) {
    ...
    while (index < tokens.length) {
        ...
        if (parseResult.kind === "unknown-option") {
            return getEnvContextGitCommitAttributionBlockReasonFromTokens(stripLeadingOptionTerminator(tokens.slice(index + 1)), footer, sourceSegment, depth + 1, true);
        }
        break;
    }
    return undefined;
}
function getWrappedShellGitCommitAttributionBlockReason(command, footer, depth = 0, failClosedOnCommitLike = false, sourceSegment) {
    if (depth > MAX_WRAPPED_SHELL_GIT_COMMIT_RECURSION_DEPTH) {
        return buildTlhGitCommitAttributionBlockReason(footer);
    }
    for (const { segment } of splitShellCommandSegments(command)) {
        ...
        const blockReason = getWrappedShellGitCommitAttributionBlockReason(wrappedCommand, footer, depth + 1, failClosedOnCommitLike, effectiveSourceSegment);
        if (blockReason) {
            return blockReason;
        }
    }
...
export function buildTlhCommitAttributionPrompt(state) {
...
export function getTlhGitCommitAttributionBlockReason(command, state) {
    if (!state.enabled || !state.footer) {
        return undefined;
    }
