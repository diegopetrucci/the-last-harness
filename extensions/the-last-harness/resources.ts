...
function labelPrompt(resource: ResolvedResource): string {
  const content = readText(resource.path);
  const name =
    parseFrontmatterValue(content, "name") ?? basename(resource.path, extname(resource.path));
  return `/${name}`;
}
...
export async function collectStartupResourceSnapshot(
  ...
  const enabled = (resources: ResolvedResource[]) =>
    filterVisibleResources(resources, projectTrusted);
