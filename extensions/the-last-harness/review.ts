...
function currentReviewPrimaryAgentSelection(
  ctx: ExtensionCommandContext,
): ReviewPrimaryAgentSelection {
  const defaultResolution = resolvePrimaryAgentConfig(
    getTlhGlobalSettings(ctx.cwd).tlh?.primaryAgent,
  ) as {
 ...
 */
async function gatherUncommitted(
  pi: ExtensionAPI,
  ...
  const untrackedParts = await buildSnapshotParts(
    repoRootResult.root,
    untrackedFiles,
    "untracked file",
  );
...
async function gatherFolder(
  ...
  for (let i = 0; i < absPaths.length; i++) {
    ...
    if (pathStat.isDirectory()) {
      ...
      if (lsResult.code === 0) {
        filePaths = parseNullDelimitedGitPaths(lsResult.stdout).map((filePath) =>
          join(absPath, filePath),
        );
...
async function showBranchSwitchConfirm(
  ...
  const confirmed = await cmdCtx.ui.custom<boolean>((_tui, theme, _kb, done) => {
    ...
    container.addChild(border);
    container.addChild(
      new Text(theme.fg("accent", theme.bold("Switch Branch to Run Review")), 1, 0),
    );
    ...
    container.addChild(
      new Text(theme.fg("dim", "Enter or y to confirm  n or Esc to cancel"), 1, 0),
    );
    container.addChild(border);
