...
function configuredTlhTelemetryAppId() {
...
function shouldSkipTlhLaunchTelemetry(launchSettings = readTlhLaunchSettings()) {
    ...
    if (isTruthyEnvFlag(process.env.PI_OFFLINE))
        return true;
    if (isTruthyEnvFlag(process.env.TLH_SKIP_TELEMETRY))
        return true;
    if (isTruthyEnvFlag(process.env.TLH_TELEMETRY_DISABLED))
        return true;
    if (isFalseyEnvFlag(process.env.PI_TELEMETRY))
        return true;
...
function writeTlhTelemetryState(state) {
    try {
        const statePath = tlhTelemetryStatePath();
        if (!statePath)
            return false;
...
const PUBLIC_MODEL_ID_PATTERNS = [
    /^gpt-[a-z0-9._-]+$/,
    /^o[0-9][a-z0-9._-]*$/,
    /^chatgpt-[a-z0-9._-]+$/,
    /^claude-[a-z0-9._-]+$/,
...
async function getTlhOsMetadata() {
