...
const authorizationByToolCallId = new Map();
const authorizationByInput = new WeakMap();
function pruneProjectCustomAgentAuthorizations(now = Date.now()) {
...
export function isProjectCustomAgentBinding(value) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        return false;
    const candidate = value;
    if (typeof candidate.runtimeName !== "string" ||
...
function inspectDirectory(parentDirectory, name, diagnostics) {
    let entries;
    try {
...
function parseSimpleFrontmatter(content) {
    ...
    for (const line of lines.slice(1, closing)) {
        ...
        if (!match || match[1]?.length !== 0)
            continue;
        const key = match[2];
        if (!key || !FRONTMATTER_KEY_PATTERN.test(key))
            continue;
...
function validateCustomFrontmatter(file, content, diagnostics) {
    ...
    if (Object.hasOwn(frontmatter, "extensions") ||
    ...
    if (frontmatter.acceptanceRole !== undefined &&
...
function readBoundedCustomFile(input) {
    const { filePath, root, directories, initialIdentity, runtimeName, filename, diagnostics } = input;
    let descriptor;
    try {
...
function inspectTrust(cwd, root, agentDir, diagnostics) {
    try {
...
export function inventoryProjectCustomAgents(cwdInput, agentDirInput = getAgentDir()) {
    const diagnostics = [];
    ...
    ].filter((directory) => directory.identity !== undefined);
    for (const entry of candidateEntries) {
...
export function isProjectCustomAgentRuntimeName(value) {
    return typeof value === "string" && EMBEDDED_RUNTIME_NAME_PATTERN.test(value.trim());
}
export function customAgentFileForRuntimeName(inventory, runtimeName) {
    if (!EMBEDDED_RUNTIME_NAME_PATTERN.test(runtimeName))
        return undefined;
    return inventory.files.find((file) => file.runtimeName === runtimeName);
}
export function sameProjectCustomAgentBinding(left, right) {
    if (!isProjectCustomAgentBinding(left) || !isProjectCustomAgentBinding(right))
        return false;
    return Boolean(left.runtimeName === right.runtimeName &&
        left.filename === right.filename &&
        left.worktreeRoot === right.worktreeRoot &&
        left.canonicalPath === right.canonicalPath &&
        sameIdentity(left.identity, right.identity));
}
export function validateProjectCustomAgentBinding(bindingInput, cwdInput, agentDirInput = getAgentDir()) {
    ...
    if (!current?.binding || !sameProjectCustomAgentBinding(current.binding, binding)) {
        const diagnostic = inventory.diagnostics.find((candidate) => candidate.runtimeName === binding.runtimeName);
        return {
            valid: false,
...
function normalizeDispatchPath(value, baseCwd = process.cwd()) {
...
function targetEntries(input, runtimeCwd) {
    ...
    if (Array.isArray(record.tasks)) {
        record.tasks.forEach((task, taskIndex) => {
            ...
            if (typeof taskRecord.agent !== "string")
                return;
            const target = taskRecord.agent.trim();
            if (!EMBEDDED_RUNTIME_NAME_PATTERN.test(target))
                return;
            const cwd = typeof taskRecord.cwd === "string" && taskRecord.cwd.trim()
...
export function authorizeProjectCustomAgentInput(input, runtimeCwd, agentDirInput = getAgentDir()) {
    ...
    for (const entry of entries) {
        ...
        if (!file?.binding) {
            const detail = inventory.diagnostics.find((diagnostic) => diagnostic.runtimeName === entry.target)?.message;
            const trustGuidance = inventory.trust === "denied"
...
export function setProjectCustomAgentAuthorization(toolCallId, input, authorization) {
...
export function takeProjectCustomAgentAuthorization(toolCallId, input) {
    pruneProjectCustomAgentAuthorizations();
    let authorization;
    if (typeof toolCallId === "string" && toolCallId.trim()) {
...
export function clearProjectCustomAgentAuthorization(toolCallId) {
    pruneProjectCustomAgentAuthorizations();
    if (typeof toolCallId === "string" && toolCallId.trim()) {
        authorizationByToolCallId.delete(toolCallId);
    }
