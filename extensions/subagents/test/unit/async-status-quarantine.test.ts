...
function createIssue(asyncRoot: string, entry: string, content: string, kind: "json_parse" | "persisted_validation") {
...
describe("async status quarantine helper", () => {
	it("quarantines confirmed json-parse corruption and preserves run artifacts", () => {
		...
		try {
			...
			const result = quarantineCorruptAsyncRun(asyncRoot, issue, { createUniqueSuffix: () => "fixed-suffix" });
			...
			assert.equal(fs.readFileSync(path.join(result.quarantineDir, "status.json"), "utf-8"), "{bad json");
			assert.equal(fs.readFileSync(path.join(result.quarantineDir, "events.jsonl"), "utf-8"), '{"type":"event"}\n');
			assert.equal(fs.readFileSync(path.join(result.quarantineDir, "output.log"), "utf-8"), "private output\n");
			assert.equal(fs.readFileSync(path.join(result.quarantineDir, "session.jsonl"), "utf-8"), '{"secret":true}\n');
			assert.deepEqual(fs.readFileSync(path.join(result.quarantineDir, "extra.bin")), Buffer.from([0, 1, 2, 3]));
		} finally {
	...
	it("quarantines confirmed persisted-validation corruption", () => {
		...
		try {
			...
			const result = quarantineCorruptAsyncRun(asyncRoot, issue, { createUniqueSuffix: () => "validated" });
	...
	it("returns exact deferred reasons for unstable stats and stable fingerprint drift without moving the source", () => {
		...
		try {
			...
			assert.equal(fs.existsSync(path.join(baseDir, QUARANTINED_ASYNC_RUNS_DIRNAME, "unstable-race.unstable")), false);
			...
			assert.equal(fs.existsSync(path.join(baseDir, QUARANTINED_ASYNC_RUNS_DIRNAME, "changed-race.changed")), false);
		} finally {
			removeTempDir(baseDir);
		}
	});
	...
	it("revalidates stable current content and reports repaired when the issue fingerprint matches it", () => {
		...
		try {
			...
			assert.equal(fs.existsSync(path.join(baseDir, QUARANTINED_ASYNC_RUNS_DIRNAME, "repair-race.repair")), false);
		} finally {
			removeTempDir(baseDir);
		}
	});
