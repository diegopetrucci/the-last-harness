...
const sourceImportPattern =
  /from\s+["'](@earendil-works\/[^"']+)["']|import\s+["'](@earendil-works\/[^"']+)["']/g;
...
const removedSlashSurfaces = [
  ...
  {
    label: "/gather-context-and-clarify",
    pattern: /(^|[^\w-])\/gather-context-and-clarify(?![\w-])/,
  },
  { label: "/parallel-cleanup", pattern: /(^|[^\w-])\/parallel-cleanup(?![\w-])/ },
  { label: "/subagents-load-profile", pattern: /(^|[^\w-])\/subagents-load-profile(?![\w-])/ },
  {
    label: "/subagents-refresh-provider-models",
    pattern: /(^|[^\w-])\/subagents-refresh-provider-models(?![\w-])/,
  },
  {
    label: "/subagents-generate-profiles",
    pattern: /(^|[^\w-])\/subagents-generate-profiles(?![\w-])/,
  },
];
