...
describe("subagent live-detail controller", () => {
  it("defers and deduplicates probes before toggling confirmed live rows", async () => {
    ...
    assert.equal(
      liveInvalidations,
      1,
      "confirmed state was probed instead of returning immediately",
    );
    ...
    assert.equal(
      controller.registerToolRow("tool-row", nonLiveRendererState, nonLiveInvalidate),
      false,
    );
    assert.equal(
      controller.registerToolRow("tool-row", nonLiveRendererState, nonLiveInvalidate),
  ...
  it("reclaims a rebuilt live row only after its deferred invalidator re-enters", async () => {
    ...
    function rebuiltInvalidate(): void {
      ...
      reenteredAsLive = controller.registerToolRow(
        "tool-row",
        rebuiltRendererState,
        rebuiltInvalidate,
      );
    }

    assert.equal(
      controller.registerToolRow("tool-row", rebuiltRendererState, rebuiltInvalidate),
      false,
    );
