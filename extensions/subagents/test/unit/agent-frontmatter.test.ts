...
describe("agent tkTicketRequired frontmatter", () => {
	it("parses and serializes the TLH marker while preserving unknown fields", () => {
...
`,
		...
		const developer = discoverAgents(dir, "project").agents.find((agent) => agent.name === "developer");
		assert.ok(developer);
		...
		assert.match(serializeAgent(developer), /^tkTicketRequired: true$/m);
...
`,
		...
		const unmarkedDeveloper = discoverAgents(dir, "project").agents.find((agent) => agent.name === "developer");
		assert.ok(unmarkedDeveloper);
		assert.equal(unmarkedDeveloper.tkTicketRequired, false);
		assert.match(serializeAgent(unmarkedDeveloper), /^tkTicketRequired: false$/m);
...
`,
			...
			assert.equal(
				discoverAgents(dir, "both").agents.find((agent) => agent.name === "my-workflow.reviewer")?.source,
				"package",
			);
...
`,
			...
			assert.deepEqual(all.chains, []);
			assert.equal(
				discoverAgents(nested, "both").agents.find((agent) => agent.name === "nested-package-agent")?.source,
				"package",
			);
		}));
...
`,
			...
			assert.deepEqual(all.chains, []);
			assert.equal(
				discoverAgents(nested, "both").agents.find((agent) => agent.name === "home-package-agent")?.source,
				"package",
			);
		}));
...
`,
			...
			assert.deepEqual(all.chains, []);
			assert.equal(
				discoverAgents(nested, "both").agents.find((agent) => agent.name === "custom-profile-package-agent")?.source,
				"package",
			);
		}));
...
`,
			...
			assert.ok(
				packageAgents.find(
					(agent) => agent.name === "package-agent" && agent.filePath === path.join(packageRoot, "agent.md"),
				),
			);
			assert.ok(
				packageAgents.find(
...
`,
		...
		const result = discoverAgentsAll(dir);
		assert.ok(
			result.project.find((agent) => agent.name === "scout" && agent.filePath === path.join(nestedDir, "scout.md")),
		);
		assert.deepEqual(result.chains, []);
		assert.equal(
...
`,
		...
		assert.ok(
			result.agents.find(
				(agent) => agent.name === "canonical" && agent.filePath === path.join(dir, ".pi", "agents", "canonical.md"),
			),
		);
		assert.ok(
			result.agents.find(
				(agent) => agent.name === "skill-named-agent" && agent.filePath === path.join(dir, ".pi", "agents", "SKILL.md"),
			),
		);
