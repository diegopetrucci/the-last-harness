...
describe("mergeAgentsForScope", () => {
  ...
  it("package agents override builtins but not user or project agents", () => {
    ...
    assert.equal(
      mergeAgentsForScope("both", [], [], builtinAgents, packageAgents)[0]?.source,
      "package",
    );
    assert.equal(
      mergeAgentsForScope("user", userAgents, [], builtinAgents, packageAgents)[0]?.source,
      "user",
    );
    assert.equal(
      mergeAgentsForScope("project", [], projectAgents, builtinAgents, packageAgents)[0]?.source,
      "project",
    );
  });
