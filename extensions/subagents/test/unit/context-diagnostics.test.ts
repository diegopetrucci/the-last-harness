...
describe("subagent context and termination diagnostics", () => {
  it("uses ordered 80/95 pressure boundaries and preserves crossed-band history", () => {
    ...
    for (const [tokens, history, expected] of cases) {
      const pressure = detectContextPressureCrossing(
        { contextTokens: tokens, contextWindow: 1000 },
        history,
        123,
      );
      assert.equal(pressure?.severity, expected);
    }
    const warning = detectContextPressureCrossing(
      { contextTokens: 800, contextWindow: 1000 },
      [],
      123,
    )!;
    ...
    assert.equal(
      detectContextPressureCrossing({ contextTokens: 800, contextWindow: 1000 }, ["warning"]),
      undefined,
    );
    const critical = detectContextPressureCrossing({ contextTokens: 950, contextWindow: 1000 }, [
      "warning",
    ])!;
    assert.equal(critical.crossedThreshold, "critical");
    assert.equal(
      detectContextPressureCrossing({ contextTokens: 800, contextWindow: 1000 }, [
        "warning",
        "critical",
      ]),
      undefined,
    ...
    assert.equal(
      detectContextPressureCrossing({ contextTokens: 700, contextWindow: 1000 }, ["warning"]),
      undefined,
    );
    assert.equal(
      detectContextPressureCrossing({ contextTokens: 800, contextWindow: 1000 }, ["warning"]),
    ...
    assert.deepEqual(
      detectContextPressureCrossing({ contextTokens: 900, contextWindow: 0 }),
      undefined,
    );
  });
  ...
  it("blocks exactly at the centralized unsafe threshold and uses the latest total, not peak", () => {
    ...
    const exhausted = assessDurableResumeContext(
      { contextTokens: 1200, contextWindow: 1000 },
      1000,
    );
  ...
  it("proceeds with unknown diagnostics instead of inventing measurements", () => {
    assert.deepEqual(
      assessDurableResumeContext({ peakTokens: 1000, contextPercent: 99 }, undefined),
      {
        blocked: false,
        measured: false,
      },
  ...
  it("seeds continuation diagnostics and keeps prior context when no response usage is valid", () => {
    ...
    assert.deepEqual(
      updateContextUsageDiagnostics(restored, assistant("error", { totalTokens: 100 }), {
        restored: true,
      }),
      restored,
    );
  ...
  it("updates the latest context while retaining the continuation start and maximum peak", () => {
    ...
    diagnostics = updateContextUsageDiagnostics(
      diagnostics,
      assistant("stop", { totalTokens: 1200 }),
      {
        restored: true,
      },
    );
  ...
  it("ignores aborted, error, all-zero, and malformed assistant usage", () => {
    ...
    assert.equal(
      assistantContextTokens(
        assistant("stop", { totalTokens: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }),
      ),
      undefined,
    );
    assert.equal(
      assistantContextTokens({ role: "assistant", stopReason: "stop", usage: "bad" }),
      undefined,
    );
  });
  ...
  it("maps orchestration conditions before terminal model stops", () => {
    assert.equal(
      resolveSubagentTerminationReason({ cancelled: true, assistantStopReason: "stop" }),
      "cancelled",
    );
    assert.equal(
      resolveSubagentTerminationReason({ paused: true, assistantStopReason: "error" }),
      "paused",
    );
    assert.equal(
      resolveSubagentTerminationReason({ timedOut: true, assistantStopReason: "stop" }),
      "timed_out",
    );
    assert.equal(
    ...
    assert.equal(
      resolveSubagentTerminationReason({ assistantStopReason: "length" }),
      "output_limit",
    );
    assert.equal(resolveSubagentTerminationReason({ assistantStopReason: "error" }), "model_error");
    assert.equal(
      resolveSubagentTerminationReason({ assistantStopReason: "aborted" }),
      "interrupted",
    );
    assert.equal(
  ...
  it("classifies only the narrow adjacent #456 sequence with canonical ids", () => {
    ...
    assert.equal(
      classifyContextExhaustedTermination({ ...base, contextUsage: { contextPercent: 94.99 } }),
      undefined,
    );
    assert.equal(
      classifyContextExhaustedTermination({
        ...base,
        contextUsage: { peakTokens: 999, contextPercent: undefined },
      }),
      undefined,
    );
    ...
    for (const intervening of [
      ...
      assert.equal(
        classifyContextExhaustedTermination({ ...base, messages: [call, intervening, empty] }),
        undefined,
      );
    }
        ...
        messages: [
          {
            ...
            content: [
              call.content[0],
              { type: "toolCall", id: "call-457", name: "read", arguments: {} },
            ],
          },
          empty,
        ],
    ...
    for (const nonCanonicalCall of [
      ...
      assert.equal(
        classifyContextExhaustedTermination({ ...base, messages: [nonCanonicalCall, empty] }),
        undefined,
      );
    }
