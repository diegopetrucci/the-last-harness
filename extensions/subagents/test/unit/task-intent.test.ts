...
describe("classifyTaskMutationIntent", () => {
	...
	it("treats scoped no-edit constraints as constraints, not task intent", () => {
		...
		assert.equal(
			classifyTaskMutationIntent("worker", "Must not touch the production database; implement the fix locally").kind,
			"implementation",
		);
	});
	...
	it("lets blanket no-edit prohibitions win over write verbs", () => {
		...
		assert.equal(
			classifyTaskMutationIntent("worker", "Report on the extraction pipeline. Do not modify project/source files.")
				.kind,
			"read-only",
		);
		assert.equal(
	...
	it("strips repeated prohibition phrases before testing write intent", () => {
		assert.equal(
			classifyTaskMutationIntent("worker", "Do not modify vendor/. Do not modify generated/. Summarize the build.")
				.kind,
			"read-only",
		);
		assert.equal(
	...
	it("classifies research agents and plain reviewer tasks as read-only", () => {
		...
		assert.equal(
			classifyTaskMutationIntent("reviewer", "Review this; regardless of findings, apply changes directly").kind,
			"implementation",
		);
	});
