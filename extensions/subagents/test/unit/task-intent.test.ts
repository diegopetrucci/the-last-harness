...
import {
  ...
  taskMayMutate,
} from "../../src/runs/shared/task-intent.ts";

describe("classifyTaskMutationIntent", () => {
  it("keeps write imperatives despite investigative wording", () => {
    assert.equal(
      classifyTaskMutationIntent("worker", "Inspect the failure and implement the fix").kind,
      "implementation",
    );
    assert.equal(
      classifyTaskMutationIntent("worker", "Research the current code path and patch the bug").kind,
      "implementation",
    );
  });
  ...
  it("treats scoped no-edit constraints as constraints, not task intent", () => {
    assert.equal(
      classifyTaskMutationIntent("worker", "Do not modify tests; implement the fix").kind,
      "implementation",
    );
    assert.equal(
      classifyTaskMutationIntent("worker", "Fix the bug. Do not edit files outside src/.").kind,
      "implementation",
    );
    assert.equal(
      classifyTaskMutationIntent(
  ...
  it("stops the prohibition object before a following implementation clause", () => {
    ...
    assert.equal(
      classifyTaskMutationIntent("worker", "Do not modify tests and fixtures").kind,
      "read-only",
    );
  });
  ...
  it("lets blanket no-edit prohibitions win over write verbs", () => {
    assert.equal(
      classifyTaskMutationIntent("worker", "Implement this. Do not edit files.").kind,
      "read-only",
    );
    assert.equal(
      classifyTaskMutationIntent("worker", "Do not edit files. Tell me how to fix the bug.").kind,
      "read-only",
    );
    assert.equal(
      classifyTaskMutationIntent(
  ...
  it("classifies research agents and plain reviewer tasks as read-only", () => {
    assert.equal(
      classifyTaskMutationIntent("researcher", "Research this and patch the bug").kind,
      "read-only",
    );
    assert.equal(
      classifyTaskMutationIntent("reviewer", "Review this and fix any real issues").kind,
      "read-only",
    );
    assert.equal(
      classifyTaskMutationIntent(
  ...
  it("keeps report-writing deliverables read-only", () => {
    assert.equal(
      classifyTaskMutationIntent("worker", "Write a report on the API").kind,
      "read-only",
    );
  ...
  it("expectsImplementationMutation keeps legacy main-branch guard behavior for prohibition-bearing tasks", () => {
    ...
    assert.equal(
      expectsImplementationMutation("worker", "Do not modify tests; implement the fix"),
      false,
    );
    assert.equal(
      expectsImplementationMutation("worker", "Do not modify tests but implement the fix"),
      false,
    );
    assert.equal(
      expectsImplementationMutation(
