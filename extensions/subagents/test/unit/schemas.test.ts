...
describe("SubagentParams schema", () => {
	...
	it("includes count and concurrency on top-level parallel mode", () => {
		...
		assert.equal((taskItemsSchema?.required as string[] | undefined)?.includes("ticket") ?? false, false);
		const tasksDescription = String(SubagentParams?.properties?.tasks?.description ?? "");
		assert.match(tasksDescription, /ticket is developer-only/i);
		assert.match(tasksDescription, /developer.*required/i);
		assert.match(tasksDescription, /non-developer.*omit/i);
		assert.match(tasksDescription, /(?:do not infer.*task text|task text.*not used.*infer)/i);
		...
		const topLevelTicketDescription = String(topLevelTicketSchema.description ?? "");
		assert.match(topLevelTicketDescription, /developer-only/i);
		assert.match(topLevelTicketDescription, /required.*bundled TLH developer/i);
		assert.match(topLevelTicketDescription, /non-developer.*omit.*rejected/i);
		assert.match(topLevelTicketDescription, /(?:do not infer.*task text|task text.*not used.*infer)/i);
		assert.doesNotMatch(topLevelTicketDescription, /task text (?:supplies|provides) dispatch metadata/i);
		assert.equal((SubagentParams as unknown as JsonSchemaNode).required, undefined);
