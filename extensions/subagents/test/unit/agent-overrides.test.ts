...
describe("builtin agent overrides", () => {
	...
	it("prefers project settings overrides over user settings overrides for custom agents", () => {
		...
		writeJson(path.join(tempProject, ".pi", "settings.json"), {
			subagents: { agentOverrides: { auditor: { model: "openai-codex/gpt-5.4-mini", thinking: "high" } } },
		});
		writeProjectAgent(tempProject, "auditor", `---\nname: auditor\ndescription: Audit code\n---\n\nAudit the code.\n`);
	...
	it("does not apply project settings overrides when scope is user", () => {
		...
		writeUserAgent(tempHome, "auditor", `---\nname: auditor\ndescription: Audit code\n---\n\nAudit the code.\n`);

		const auditor = discoverAgents(tempProject, "user").agents.find((agent) => agent.name === "auditor");
		assert.ok(auditor);
	...
	it("does not apply user settings overrides when scope is project", () => {
		...
		const auditor = discoverAgents(tempProject, "project").agents.find((agent) => agent.name === "auditor");
		assert.ok(auditor);
