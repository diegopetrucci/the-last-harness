...
describe("matchesScopePattern", () => {
  it("matches an exact provider/id", () => {
    assert.equal(
      matchesScopePattern("anthropic/claude-sonnet-4", "anthropic/claude-sonnet-4"),
      true,
    );
  });
  ...
  it("strips a thinking suffix before matching", () => {
    assert.equal(
      matchesScopePattern("anthropic/claude-sonnet-4:high", "anthropic/claude-sonnet-4"),
      true,
    );
  ...
  it("does not treat arbitrary colon text as a thinking suffix", () => {
    ...
    assert.equal(
      matchesScopePattern("anthropic/claude-sonnet-4:experimental", "anthropic/claude-sonnet-4"),
      false,
    );
  });
...
describe("checkModelScope", () => {
  ...
  it("returns undefined when no allow list is configured", () => {
    ...
    assert.equal(
      checkModelScope("deepseek/deepseek-v4", { enforce: true, allow: [] }, "explicit"),
      undefined,
    );
  });
...
describe("parseModelScopeConfig", () => {
  ...
  it("rejects a non-boolean enforce", () => {
    assert.throws(
      () => parseModelScopeConfig({ enforce: "yes" }, meta),
      /invalid 'modelScope.enforce'/,
    );
  });

  it("rejects a non-array allow", () => {
    assert.throws(
      () => parseModelScopeConfig({ allow: "anthropic/*" }, meta),
  ...
  it("rejects enforce without a non-empty allow list", () => {
    assert.throws(
      () => parseModelScopeConfig({ enforce: true }, meta),
      /without a non-empty 'allow'/,
    );
    assert.throws(
      () => parseModelScopeConfig({ enforce: true, allow: [] }, meta),
      /non-empty array of patterns/,
    );
  });
});
