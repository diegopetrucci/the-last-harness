...
describe("turn-budget module", () => {
  ...
  describe("appendTurnBudgetSystemPrompt", () => {
    it("returns the system prompt unchanged when no budget is configured", () => {
      assert.equal(
        appendTurnBudgetSystemPrompt("You are a reviewer.", undefined),
        "You are a reviewer.",
      );
    ...
    it("appends a turn-budget block after an existing system prompt", () => {
      const result = appendTurnBudgetSystemPrompt(
        "You are a reviewer.",
        budget({ maxTurns: 5, graceTurns: 2 }),
      );
    ...
    it("pluralizes maxTurns for single and multiple turns", () => {
      assert.match(
        appendTurnBudgetSystemPrompt("", budget({ maxTurns: 1 })),
        /soft budget of 1 assistant turn\./,
      );
      assert.match(
        appendTurnBudgetSystemPrompt("", budget({ maxTurns: 4 })),
        /soft budget of 4 assistant turns\./,
      );
    });
  ...
  describe("initialTurnBudgetState", () => {
    it("starts within budget at turn zero and copies the resolved budget", () => {
      ...
      const expected: TurnBudgetState = {
        maxTurns: 5,
        graceTurns: 2,
  ...
  describe("shouldAbortForTurnBudget", () => {
    it("allows a terminal assistant response on the final grace turn", () => {
      assert.equal(
        shouldAbortForTurnBudget(budget({ maxTurns: 3, graceTurns: 1 }), 4, true),
        false,
      );
    });
    ...
    it("aborts a non-terminal assistant response on the final grace turn", () => {
      assert.equal(
        shouldAbortForTurnBudget(budget({ maxTurns: 3, graceTurns: 1 }), 4, false),
        true,
      );
    });
    ...
    it("allows a terminal response at the soft limit when grace turns are zero", () => {
      assert.equal(
        shouldAbortForTurnBudget(budget({ maxTurns: 2, graceTurns: 0 }), 2, true),
        false,
      );
      assert.equal(
        shouldAbortForTurnBudget(budget({ maxTurns: 2, graceTurns: 0 }), 2, false),
        true,
      );
    });
  });
