...
test("read-only issue drafting tasks do not trigger on suggested fix wording", () => {
  ...
  const result = evaluateCompletionMutationGuard({
    ...
    messages: [
      assistantText(
        "Title: completionGuard false positive\n\nSuggested fix: model read-only intent.",
      ),
    ],
  ...
  assert.equal(
    expectsImplementationMutation(
      "worker",
      "Draft GitHub issue for a bug. Include suspected cause and suggested fix.",
    ),
    false,
  );
});
...
test("omitted, empty, bash, unknown, write, and MCP tool capabilities stay conservative", () => {
  ...
  assert.equal(
    evaluateCompletionMutationGuard({ ...base, tools: ["read", "custom_lookup"] }).triggered,
    true,
  );
  assert.equal(
    evaluateCompletionMutationGuard({ ...base, tools: ["read", "write"] }).triggered,
    true,
  );
});
...
test("review-only, research, and framework output instructions do not expect mutation", () => {
  assert.equal(
    expectsImplementationMutation("worker", "Review only: return findings, do not edit"),
    false,
  );
  assert.equal(
    expectsImplementationMutation("worker", "Do not edit files. Tell me how to fix the bug."),
    false,
  );
  assert.equal(
    expectsImplementationMutation(
  ...
  assert.equal(
    expectsImplementationMutation("researcher", "Research this and patch the bug"),
    false,
  );
  assert.equal(
    expectsImplementationMutation("reviewer", "Review this and fix any real issues"),
    false,
  );
  assert.equal(
    expectsImplementationMutation(
  ...
  assert.equal(
    expectsImplementationMutation("worker", "[Write to: /tmp/result.md]\n\nSummarize findings"),
    false,
  );
  assert.equal(
    expectsImplementationMutation(
      "worker",
      injectSingleOutputInstruction("Summarize findings", "/tmp/fix.md"),
    ),
    false,
  );
...
test("worker implementation verbs win over investigative wording", () => {
  ...
  assert.equal(
    expectsImplementationMutation("worker", "Research the current code path and patch the bug"),
    true,
  );
  assert.equal(
    expectsImplementationMutation("worker", "Fix the bug where no edits were made"),
    true,
  );
  ...
  assert.equal(
    expectsImplementationMutation("worker", "Implement the fix and return findings."),
    true,
  );
});
...
test("non-worker implementation tasks still expect mutation", () => {
  assert.equal(
    expectsImplementationMutation("delegate", "Fix the bug where no edits were made"),
    true,
  );
  assert.equal(
    expectsImplementationMutation(
      "delegate",
      "Apply the suggested fix to src/runs/shared/completion-guard.ts",
    ),
    true,
  );
  assert.equal(
    expectsImplementationMutation("worker", "Draft a GitHub issue, then implement the fix"),
    true,
  );
});
...
test("worker edit intent covers common docs, config, and source tasks", () => {
  assert.equal(
    expectsImplementationMutation("worker", "Update README to mention the native tool"),
    true,
  );
  assert.equal(
    expectsImplementationMutation("worker", "Remove share functionality and all Vercel references"),
    true,
  );
  assert.equal(
    expectsImplementationMutation("worker", "Replace the registered command with a render tool"),
    true,
  );
  ...
  assert.equal(
    expectsImplementationMutation("worker", "Implement the fix. Do not edit unrelated files."),
    true,
  );
});
...
test("obvious mutating bash commands count as mutation attempts", () => {
  assert.equal(
    hasMutationToolCall([
      assistantToolCall("bash", { command: "mkdir -p src && cat > src/file.ts <<'EOF'\nhi\nEOF" }),
    ]),
    true,
  );
  assert.equal(
    hasMutationToolCall([
      assistantToolCall("bash", { command: "cat <<'EOF' > src/file.ts\nhi\nEOF" }),
    ]),
    true,
  );
  assert.equal(
    hasMutationToolCall([
      assistantToolCall("bash", {
        command: "python3 -c \"from pathlib import Path; Path('x').write_text('hi')\"",
      }),
    ]),
    true,
  );
  ...
  assert.equal(
    hasMutationToolCall([
      assistantToolCall("bash", { command: "node <<'JS'\nfs.writeFileSync('x', 'hi')\nJS" }),
    ]),
    true,
  );
  assert.equal(
    hasMutationToolCall([assistantToolCall("bash", { command: "node script.js > generated.txt" })]),
    true,
  );
  assert.equal(
    hasMutationToolCall([assistantToolCall("bash", { command: "echo 'a > b'" })]),
    false,
  );
  assert.equal(
    hasMutationToolCall([assistantToolCall("bash", { command: 'node -e "console.log(a > b)"' })]),
    false,
  );
  assert.equal(
    hasMutationToolCall([
      assistantToolCall("bash", { command: "python3 <<'PY'\nprint('inspect only')\nPY" }),
    ]),
    false,
  );
  assert.equal(
  ...
  assert.equal(
    hasMutationToolCall([assistantToolCall("bash", { command: "echo 'rm file'" })]),
    false,
  );
  assert.equal(
    hasMutationToolCall([assistantToolCall("bash", { command: 'printf "mkdir x"' })]),
    false,
  );
  assert.equal(
    hasMutationToolCall([assistantToolCall("bash", { command: "git apply patch.diff" })]),
    true,
  );
  assert.equal(
    hasMutationToolCall([assistantToolCall("bash", { command: "patch -p0 < fix.patch" })]),
    true,
  );
});
...
test("oracle, librarian, web-scout, and contrarian advisory agents do not expect mutation regardless of task verbs", () => {
  ...
  assert.equal(
    expectsImplementationMutation("librarian", "Research this and patch the bug"),
    false,
  );
  ...
  assert.equal(
    expectsImplementationMutation("team.contrarian", "Fix the failing search results"),
    false,
  );
});
...
test("qualified agent names are classified by local name", () => {
  ...
  assert.equal(
    expectsImplementationMutation("code-analysis.reviewer", "Review and fix issues"),
    false,
  );
  assert.equal(
    expectsImplementationMutation(
