...
describe("attachPostExitStdioGuard", () => {
  ...
  it(
    "does not delay a clean exit",
    {
      skip: process.platform === "win32" ? "bash scripts unavailable on Windows" : undefined,
    },
    async () => {
  ...
  it(
    "cuts off a silent grandchild with the idle timer",
    {
      skip: process.platform === "win32" ? "bash scripts unavailable on Windows" : undefined,
    },
    async () => {
      ...
      assert.ok(
        result.resolvedMs < idleMs + 2000,
        `expected idle cutoff, got ${result.resolvedMs}ms`,
      );
    },
  );

  it(
    "cuts off a chatty grandchild with the hard timer",
    {
      skip: process.platform === "win32" ? "bash scripts unavailable on Windows" : undefined,
    },
    async () => {
      ...
      assert.ok(
        result.resolvedMs < hardMs + 2000,
        `expected hard cutoff, got ${result.resolvedMs}ms`,
      );
    },
  );
});
