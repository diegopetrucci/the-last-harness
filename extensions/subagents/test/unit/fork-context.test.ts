...
describe("createForkContextResolver", () => {
  ...
  it("creates isolated branched sessions per index (parallel and chain compatible)", () => {
    ...
    try {
      ...
      assert.deepEqual(parallelSessions, [
        path.join(tempDir, "fork-2.jsonl"),
        path.join(tempDir, "fork-3.jsonl"),
      ]);
      assert.deepEqual(chainSessions, [
        path.join(tempDir, "fork-4.jsonl"),
        path.join(tempDir, "fork-5.jsonl"),
      ]);
