...
describe("resolveSingleOutputPath", () => {
  ...
  it("resolves relative output paths against an explicit artifact base", () => {
    ...
    assert.equal(
      resolved,
      path.resolve("/repo/.pi-subagents/artifacts/outputs/run-1", "reviews/report.md"),
    );
  });
});
...
describe("injectSingleOutputInstruction", () => {
  ...
  it("extracts current and legacy output instruction targets", () => {
    ...
    assert.equal(
      extractSingleOutputInstructionTarget(
        "**Output:** The harness will save your final response to: /tmp/legacy.md",
      ),
      "/tmp/legacy.md",
    );
    assert.equal(
...
describe("formatSavedOutputReference", () => {
  it("includes absolute path, human-readable size, and line count", () => {
    ...
    assert.equal(
      ref.message,
      `Output saved to: ${ref.path} (13 B, 2 lines). Read this file if needed.`,
    );
  });
...
describe("validateFileOnlyOutputMode", () => {
  it("requires an output path for file-only mode", () => {
    ...
    assert.equal(
      validateFileOnlyOutputMode("file-only", "/tmp/report.md", "Single run"),
      undefined,
    );
