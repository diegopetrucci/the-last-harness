...
interface ControlChannelModule {
  requestAsyncInterrupt(
    asyncDir: string,
    payload?: { ts?: number; source?: string; reason?: string },
  ): string;
}
...
export async function waitForAsyncState(
  ...
  while (readStatus(asyncDir)?.state !== state) {
    if (Date.now() > deadline)
      assert.fail(`Timed out waiting for async state '${state}' in ${asyncDir}`);
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
}
...
export async function waitForAsyncStatusPredicate(
  ...
  while (true) {
    ...
    if (Date.now() > deadline)
      assert.fail(`Timed out waiting for async status predicate '${label}' in ${asyncDir}`);
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
}
...
export async function waitForAsyncControlCondition(
  ...
  for (;;) {
    ...
    if (Date.now() > deadline)
      assert.fail(`Timed out waiting for control condition in ${asyncDir}`);
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
}
...
export async function waitForMockPiSignal(
  ...
  while (true) {
    ...
    if (Date.now() > deadline)
      assert.fail(`Timed out waiting for mock pid ${pid} to record ${signal}`);
