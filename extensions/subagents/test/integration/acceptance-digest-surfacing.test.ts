...
describe("acceptance-report digest surfacing", { skip: !available ? "pi packages not available" : undefined }, () => {
	let tempDir: string;
	let mockPi: MockPi;
	...
	it("leaves the artifact byte-exact when the run saved a user-requested output file", async () => {
		...
		assert.ok(result.artifactPaths, "expected artifact paths");
		assert.equal(fs.readFileSync(result.artifactPaths.outputPath, "utf-8"), "saved deliverable body");
	});
