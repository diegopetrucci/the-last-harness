...
const asyncExecutionMod = await tryImport<AsyncExecutionModule>(
  "./src/runs/background/async-execution.ts",
);
const createSubagentExecutor = executorMod.createSubagentExecutor;
assert.equal(
  asyncExecutionMod.isAsyncAvailable(),
  true,
  "required async runner module is unavailable",
);
...
describe("fork context execution wiring", () => {
  ...
  it("detaches parallel child runs cleanly on intercom handoff", async () => {
    ...
    assert.match(
      ...
      /Legacy detached parallel child \(echo\)\. Inspect status\/artifacts, then resume or replace work explicitly if needed\./,
    );
    assert.equal(detachEmitted, true);
    assert.equal(
      result.details?.results?.some((entry) => entry.detached === true && entry.exitCode === 0),
      true,
    );
  });
