...
import type { ExecutorDeps } from "./executor-types.ts";
...
import { resolveNestedAsyncDir } from "./nested-events.ts";
import { parseSubagentTerminalResult } from "../../shared/terminal-result.ts";
import { reconcileAsyncRun } from "../background/stale-run-reconciler.ts";
import { ASYNC_SUPERVISOR_LIFECYCLE_ERROR_MESSAGE } from "../background/cancellation-projection.ts";
...
/** Re-authorize embedded children before a nested steer or interrupt. */
export async function authorizeNestedProjectAgentControl(input: {
  ...
  for (const step of run.steps ?? []) {
    if (input.index !== undefined && run.steps?.indexOf(step) !== input.index) continue;
    if (
