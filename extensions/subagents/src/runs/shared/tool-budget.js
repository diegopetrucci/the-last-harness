...
export function normalizeToolBudgetBlock(block) {
    if (block === "*")
        return "*";
    if (block === undefined)
        return [...DEFAULT_TOOL_BUDGET_BLOCK];
    return [...new Set(block.map((tool) => tool.trim()).filter(Boolean))];
}
export function validateToolBudgetConfig(raw, label = "toolBudget") {
    if (raw === undefined)
        return {};
...
export function toolBudgetState(budget, toolCount, blockedTool) {
...
export function shouldBlockToolForBudget(budget, toolName, nextToolCount) {
    if (nextToolCount <= budget.hard)
        return false;
    return budget.block === "*" || budget.block.includes(toolName);
}
export function toolBudgetSoftNudge(budget, toolCount) {
...
export function toolBudgetBlockedMessage(budget, toolName, toolCount) {
...
export function encodeToolBudgetEnv(budget) {
    return budget ? JSON.stringify(budget) : undefined;
}
export function decodeToolBudgetEnv(value) {
    if (!value?.trim())
        return undefined;
