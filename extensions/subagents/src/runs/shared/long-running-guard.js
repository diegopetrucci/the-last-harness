...
export function resolveCurrentPath(toolName, args) {
    if (!toolName || !args)
        return undefined;
...
function hasUnquotedFileRedirection(command) {
    ...
    for (let i = 0; i < command.length; i++) {
        ...
        if (targetStart === "&" || targetStart === "|" || targetStart === ";")
            continue;
...
function isExecutableHereDocInterpreter(executable) {
    return executable !== undefined && EXECUTABLE_HEREDOC_INTERPRETER_PATTERN.test(executable);
}
function shouldScanHereDocBody(prelude) {
...
function collectHereDocDescriptors(line) {
    const descriptors = [];
    let inSingle = false;
    let inDouble = false;
    for (let i = 0; i < line.length; i++) {
        ...
        if (quote === "'" || quote === '"') {
            ...
            if (delimiter)
                descriptors.push({ delimiter, scanBody });
            continue;
        }
        const start = i;
...
function skipLeadingCliOptions(tokens, start, valueFlags, booleanFlags) {
    let index = start;
...
function isMutatingGitTagInvocation(args) {
    if (args.length === 0)
        return false;
    if (args.some((arg) => GIT_MUTATING_TAG_FLAGS.has(arg)))
        return true;
    if (args.some((arg) => GIT_READ_ONLY_TAG_FLAGS.has(arg)))
        return false;
    return args.some((arg) => !arg.startsWith("-"));
}
function isMutatingGitResetInvocation(args) {
    if (args.length === 0)
        return false;
    const mutatingModes = new Set(["--hard", "--mixed", "--soft", "--merge", "--keep"]);
    if (args.some((arg) => mutatingModes.has(arg)))
        return true;
    return args.some((arg) => !arg.startsWith("-"));
}
function isMutatingGitCleanInvocation(args) {
    let hasForce = false;
...
function isMutatingGitBranchInvocation(args) {
    if (args.length === 0)
        return false;
    if (args.some((arg) => GIT_MUTATING_BRANCH_FLAGS.has(arg) || arg.startsWith("--set-upstream-to=")))
        return true;
    if (args.some((arg) => GIT_READ_ONLY_BRANCH_FLAGS.has(arg) || /^-v+$/.test(arg)))
        return false;
    if (args.some((arg) => /^--(?:contains|format|merged|no-contains|no-merged|points-at|sort)=/.test(arg)))
        return false;
    return args.some((arg) => !arg.startsWith("-"));
}
function isMutatingGitStashInvocation(args) {
    if (args.length === 0)
        return true;
    const subcommand = args.find((arg) => !arg.startsWith("-"));
    if (!subcommand)
        return true;
    if (GIT_READ_ONLY_STASH_SUBCOMMANDS.has(subcommand) || subcommand === "help")
        return false;
    return GIT_MUTATING_STASH_SUBCOMMANDS.has(subcommand);
}
function hasTerminalReadOnlyFlag(args, readOnlyFlags) {
    return args.some((arg) => readOnlyFlags.has(arg));
}
function parseGhApiInvocation(args) {
    let explicitMethod;
...
function classifyStructuredShellSegment(segment) {
    ...
    if (executable === "git") {
        ...
        if (hasTerminalReadOnlyFlag(invocationArgs, GIT_TERMINAL_READ_ONLY_FLAGS))
            return "read-only";
        ...
        if (subcommand === "switch" && (subcommandArgs.includes("-c") || subcommandArgs.includes("-C")))
            return "mutating";
        if (subcommand === "stash" && isMutatingGitStashInvocation(subcommandArgs))
            return "mutating";
        if (subcommand === "tag" && isMutatingGitTagInvocation(subcommandArgs))
            return "mutating";
        if (subcommand === "apply" && !subcommandArgs.includes("--check"))
            return "mutating";
        if (subcommand === "reset" && isMutatingGitResetInvocation(subcommandArgs))
            return "mutating";
        if (subcommand === "clean" && isMutatingGitCleanInvocation(subcommandArgs))
            return "mutating";
    ...
    if (executable === "gh") {
        const invocationArgs = tokens.slice(commandIndex + 1);
        if (hasTerminalReadOnlyFlag(invocationArgs, GH_TERMINAL_READ_ONLY_FLAGS))
            return "read-only";
        ...
        if (!subcommand)
            return "read-only";
        if (subcommand === "pr" && GH_MUTATING_PR_SUBCOMMANDS.has(subcommandArgs[0] ?? ""))
            return "mutating";
        if (subcommand === "api" && isMutatingGhApiInvocation(subcommandArgs))
            return "mutating";
        if (subcommand === "release" && GH_MUTATING_RELEASE_SUBCOMMANDS.has(subcommandArgs[0] ?? ""))
            return "mutating";
        return "read-only";
    }
    return "unrecognized";
}
function hasMutatingExecutableHereDocBody(command) {
    return extractExecutableHereDocBodies(command).some((body) => EMBEDDED_FILE_WRITE_PATTERNS.some((pattern) => pattern.test(body)));
}
export function isMutatingBashCommand(command) {
    const shellSyntax = stripHereDocBodies(command);
    if (hasUnquotedFileRedirection(shellSyntax) || hasMutatingExecutableHereDocBody(command))
        return true;
    for (const segment of splitShellCommandSegments(shellSyntax)) {
        ...
        if (structuredClassification === "mutating")
            return true;
        if (structuredClassification === "read-only")
            continue;
...
export function didMutatingToolFail(text) {
...
export function createMutatingFailureState() {
    return {
...
export function recordMutatingFailure(state, input, windowMs) {
    ...
    if (state.recentFailures.length > 3)
        state.recentFailures.shift();
}
export function shouldEscalateMutatingFailures(state, threshold) {
    return state.consecutiveFailures >= threshold || state.repeatedPathFailures >= threshold;
}
export function summarizeRecentMutatingFailures(state) {
