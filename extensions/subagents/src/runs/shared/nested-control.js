...
import { canonicalSubagentModelIdentity, sanitizeSubagentModelIdentity, sanitizeSubagentModelResolution, } from "./model-fallback.js";
...
import { resolveNestedAsyncDir } from "./nested-events.js";
...
import { ASYNC_SUPERVISOR_LIFECYCLE_ERROR_MESSAGE } from "../background/cancellation-projection.js";
...
import { authorizePersistedProjectAgentRun, hasMalformedProjectAgentControlMarker, projectRunAuthorizationError, } from "./project-agent-control.js";
export const NESTED_ASYNC_RUNS_DIR = path.join(TEMP_ROOT_DIR, "nested-subagent-runs");
export async function authorizeNestedProjectAgentControl(input) {
    const run = input.target.match.run;
    if (hasMalformedProjectAgentControlMarker(run) || run.projectAgentMarker === true) {
        throw projectRunAuthorizationError(`Nested run '${run.id}' has a malformed project-agent marker.`);
    }
    for (const step of run.steps ?? []) {
        if (input.index !== undefined && run.steps?.indexOf(step) !== input.index)
            continue;
        ...
        if (!step.agent.startsWith("embedded."))
            continue;
        if (step.projectAgentMarker === true) {
            throw projectRunAuthorizationError(`Nested run '${run.id}' has a malformed project-agent marker.`);
        }
        await authorizePersistedProjectAgentRun({
            target: {
                ...
                agent: step.agent,
                cwd: run.cwd,
                projectAgent: step.projectAgent ?? run.projectAgent,
            },
            ctx: input.ctx,
            deps: input.deps,
            ...(input.action === "interrupt" ? { requireArchitect: false } : {}),
        });
    }
    if ((run.steps?.length ?? 0) === 0 && run.agent?.startsWith("embedded.")) {
        await authorizePersistedProjectAgentRun({
            target: {
                runId: run.id,
                agent: run.agent,
                cwd: run.cwd,
                projectAgent: run.projectAgent,
            },
            ctx: input.ctx,
            deps: input.deps,
            ...(input.action === "interrupt" ? { requireArchitect: false } : {}),
...
function readNestedResumeStatusStep(runId, asyncDir) {
    if (!asyncDir)
        return undefined;
    const parsed = readStatus(asyncDir);
    if (!parsed)
        return undefined;
    ...
    return {
        ...(parsed.lifecycle?.resumeBlockedReason === "supervisor_lifecycle_failure"
            ? { resumeBlockedReason: "supervisor_lifecycle_failure" }
            : {}),
        ...(typeof parsed.cwd === "string" ? { cwd: parsed.cwd } : {}),
        ...(typeof raw.status === "string" ? { status: raw.status } : {}),
        ...(parseSubagentTerminalResult(raw.terminalResult)
            ? { terminalResult: parseSubagentTerminalResult(raw.terminalResult) }
            : {}),
...
export function resolveNestedResumeTarget(match, trustedSessionRoots) {
    ...
    const projectAgentMarker = Object.hasOwn(run, "projectAgent")
        ? normalizeProjectAgentIdentity(run.projectAgent)
        : undefined;
    ...
    if (statusStep?.resumeBlockedReason === "supervisor_lifecycle_failure")
    ...
    if (projectAgentMarker) {
        ...
        const cwdValidation = validateProjectAgentCwdContainment(projectAgentMarker.root, persistedCwd);
    ...
    return {
        ...
        ...(statusStep?.terminalResult ? { terminalResult: statusStep.terminalResult } : {}),
...
export function steerNestedRun(input) {
    ...
    return {
        content: [
            {
                type: "text",
                text: `Nested run ${run.id} is not a live async child session with a steering inbox. action='steer' cannot target it.`,
            },
        ],
        isError: true,
        details: { mode: "management", results: [] },
