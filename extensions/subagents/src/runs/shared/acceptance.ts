...
function inferLegacyLevel(input: { agentName: string; task?: string; mode?: SubagentRunMode; async?: boolean }): {
	...
	const readOnlyTask =
		/\b(?:read[- ]only|review[- ]only|do not edit|don't edit|no edits|without edits|inspect|summari[sz]e)\b/.test(task);
	const writeTask =
...
export function resolveEffectiveAcceptance(input: {
	...
	let review: ResolvedAcceptanceConfig["review"] = explicit.review !== undefined ? explicit.review : inferred.review;
	if (
		level === "reviewed" &&
		explicitLevel !== "auto" &&
...
function mergeAcceptanceCriteria(
	...
	for (const criterion of extra) {
		...
		merged[index] = {
			...
			severity:
				merged[index]?.severity === "required" || criterion.severity === "required" ? "required" : "recommended",
		};
	}
	return merged;
}
	...
	if (
		...
		for (const command of acceptance.verify) {
			ledger.verifyRuns.push(
				await runVerifyCommand(command, input.cwd, { signal: input.signal, abortMessage: input.abortMessage }),
			);
