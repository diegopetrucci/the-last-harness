...
function extractExecutableHereDocBodies(command: string): string[] {
  ...
  for (const line of lines) {
    ...
    pending.push(
      ...collectHereDocDescriptors(line).map(({ delimiter, scanBody }) => ({
        delimiter,
        scanBody,
        lines: [],
      })),
...
function isMutatingGitBranchInvocation(args: string[]): boolean {
  if (args.length === 0) return false;
  if (
    args.some((arg) => GIT_MUTATING_BRANCH_FLAGS.has(arg) || arg.startsWith("--set-upstream-to="))
  )
    return true;
  if (args.some((arg) => GIT_READ_ONLY_BRANCH_FLAGS.has(arg) || /^-v+$/.test(arg))) return false;
  if (
    args.some((arg) =>
      /^--(?:contains|format|merged|no-contains|no-merged|points-at|sort)=/.test(arg),
    )
  )
    return false;
  return args.some((arg) => !arg.startsWith("-"));
...
function parseGhApiInvocation(args: string[]): {
  explicitMethod?: string;
  hasFieldParameters: boolean;
} {
...
export function nextLongRunningTrigger(
  ...
  if (
    config.activeNoticeAfterTokens !== undefined &&
