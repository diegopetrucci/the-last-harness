/**
 * Foreground result, acceptance, and artifact finalization.
 */
...
export function applyHealthProgressProjection(
  progress: Pick<
    AgentProgress,
...
export function transitionHealthForProgress(
...
export function clearHealthForProgress(
  box: HealthTransitionBox,
  progress: Pick<
    AgentProgress,
    "activityState" | "idleEpisodeId" | "durableAttentionReasons" | "compaction"
...
export type { HealthTransitionBox };
...
export const CONFIGURED_RUN_DEADLINE_TIMEOUT_MESSAGE =
...
export function setupForegroundArtifacts(
  runtimeCwd: string,
  agentName: string,
...
export function finalizeSingleAttempt(input: SingleAttemptFinalizationInput): SingleResult {
  const {
    result,
    progress,
    startTime,
...
export function finalizeForegroundArtifacts(input: ForegroundArtifactFinalizationInput): void {
  const {
    result,
    options,
    artifactPathsResult,
