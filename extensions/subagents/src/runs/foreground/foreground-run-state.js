...
import { PROJECT_AGENT_TERMINAL_RETENTION_MS, normalizeProjectAgentRunCapture, releaseProjectAgentRunReference, } from "../../agents/project-agent-snapshot.js";
...
export function resolveForegroundResumeTarget(params, state) {
    ...
    let continuationAcceptance;
    if (childState === "paused") {
        if (!child.acceptance ||
            !isWellFormedResolvedAcceptance(child.acceptance.effectiveAcceptance)) {
            throw new Error(`Foreground run '${run.runId}' child ${index} has a missing or malformed persisted acceptance ledger; refusing to resume with an unverified acceptance contract.`);
        }
        if (child.acceptance.status === "skipped") {
            if (child.acceptance.effectiveAcceptance.level === "none") {
                throw new Error(`Foreground run '${run.runId}' child ${index} has an incompatible persisted acceptance ledger: status 'skipped' cannot carry effective level 'none'.`);
            }
            continuationAcceptance = child.acceptance.effectiveAcceptance;
        }
        else if (child.acceptance.status === "not-required") {
            if (child.acceptance.effectiveAcceptance.level !== "none") {
                throw new Error(`Foreground run '${run.runId}' child ${index} has an incompatible persisted acceptance ledger: status 'not-required' must carry effective level 'none'.`);
            }
        }
        else {
            throw new Error(`Foreground run '${run.runId}' child ${index} has an incompatible persisted acceptance ledger status '${child.acceptance.status}'; expected 'skipped' or 'not-required'.`);
        }
    }
    return {
        runId: run.runId,
