...
import { scanAsyncRunsForRestore, validatePersistedAsyncStatus, type AsyncRunSummary } from "./async-status.ts";
						...
						if (status.steps?.length) {
							...
							const activeGroup =
								status.currentStep !== undefined
									? groups.find(
											(group) => status.currentStep! >= group.start && status.currentStep! < group.start + group.count,
										)
									: undefined;
							const visibleSteps = activeGroup
								? status.steps
