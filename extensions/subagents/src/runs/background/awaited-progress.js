...
import { canonicalLifecycleStepState } from "./async-status-boundary.js";
function progressStatusForStep(status) {
...
function progressTokensForStep(step) {
    return step?.tokens?.total ?? 0;
}
function emptyProgressUsage() {
...
function plannedStepsForPlan(plan) {
...
function buildProgressItem(input) {
    ...
    const progress = {
        ...
        task,
        lastActivityAt: step?.lastActivityAt !== undefined && step.startedAt !== undefined
...
function applyProgressOutcome(result, flags, step, skills, skillsWarning) {
    if (flags.paused)
    ...
    if (skillsWarning)
        result.skillsWarning = skillsWarning;
}
function applyProgressModelProjection(result, step, planned) {
    if (step?.model ?? planned?.model)
        result.model = step?.model ?? planned?.model;
    if (step?.thinking ?? planned?.thinking)
        result.thinking = step?.thinking ?? planned?.thinking;
    if (step?.modelIdentity ?? planned?.modelIdentity)
        result.modelIdentity = step?.modelIdentity ?? planned?.modelIdentity;
    if (step?.modelResolution)
        result.modelResolution = step.modelResolution;
    if (step?.attemptedModels)
        result.attemptedModels = [...step.attemptedModels];
    ...
    if (step?.modelFallbackNotice)
        result.modelFallbackNotice = step.modelFallbackNotice;
}
function applyProgressDiagnostics(result, step) {
    if (step?.totalCost)
        result.totalCost = { ...step.totalCost };
    if (step?.contextUsage)
        result.contextUsage = step.contextUsage;
    if (step?.contextPressure)
        result.contextPressure = step.contextPressure;
    if (step?.contextPressureCrossedThresholds)
        result.contextPressureCrossedThresholds = [...step.contextPressureCrossedThresholds];
}
function applyProgressLifecycle(result, status, flags, step, planned) {
    if (step?.terminationReason)
        result.terminationReason = step.terminationReason;
    if (step?.sessionFile ?? planned?.sessionFile)
        result.sessionFile = step?.sessionFile ?? planned?.sessionFile;
    if (flags.paused && step?.pause)
        result.pause = step.pause;
    if (step?.cancel)
        result.cancel = step.cancel;
    ...
    if (step?.children?.length)
        result.children = nestedChildren(step.children);
    if (status === "cancelled" && !result.terminationReason)
        result.terminationReason = "cancelled";
}
function buildProgressResult(input) {
    const { agent, task, status, step, planned, flags, skills, skillsWarning, usage, finalOutput } = input;
    const result = {
        agent,
        task,
...
function buildAwaitedProgressStep(status, index, planned, persisted, now) {
    ...
    const durationMs = persisted?.durationMs ??
    ...
    if (progress.lastActivityAt === undefined)
        progress.lastActivityAt = status.startedAt;
    return { progress, result };
}
export function buildAwaitedProgressUpdate(input, status, now) {
    ...
    for (let index = 0; index < stepCount; index++) {
        const projected = buildAwaitedProgressStep(status, index, plannedSteps[index], persistedSteps[index], now);
