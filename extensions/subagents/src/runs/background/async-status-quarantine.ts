...
function isValidFingerprint(
  ...
  return (
    issue.fingerprint?.algorithm === "sha256" && /^[a-f0-9]{64}$/u.test(issue.fingerprint.value)
  );
}
...
function validateIssuePaths(asyncDirRoot: string, issue: AsyncRunCorruptEntryIssue): boolean {
  ...
  if (
    ...
    return false;
  return (
    path.resolve(issue.asyncDir) === expectedAsyncDir &&
...
export function quarantineCorruptAsyncRun(
  ...
  const createUniqueSuffix =
    options.createUniqueSuffix ?? (() => `${now()}-${Math.random().toString(36).slice(2, 8)}`);
