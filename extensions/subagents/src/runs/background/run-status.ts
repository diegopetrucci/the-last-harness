...
function formatRememberedForegroundStatus(run: ForegroundResumeRun): string {
	...
	const resumable = run.children.find(
		(child) => child.status !== "detached" && !child.cancel?.cancelledAt && hasExistingSessionFile(child.sessionFile),
	);
...
export function inspectSubagentStatus(params: RunStatusParams, deps: RunStatusDeps = {}): SubagentToolResult<Details> {
