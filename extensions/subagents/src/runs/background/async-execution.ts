...
function validateAsyncExecutionAcceptance(
  params: Pick<AsyncSingleParams, "acceptance"> | Pick<AsyncChainParams, "chain">,
): string[] {
  const errors: string[] = [];
  if ("chain" in params) {
  ...
  const buildSeqStep = (
    ...
    if (ticketResolution.error) {
      throw new AsyncStartValidationError(
        `Invalid ticket for async step (${s.agent}): ${ticketResolution.error}`,
      );
    }
    ...
    const thinkingOverride =
      flatIndex === undefined ? undefined : thinkingOverridesByFlatIndex?.[flatIndex];
...
export function executeAsyncSingle(id: string, params: AsyncSingleParams): AsyncExecutionResult {
  ...
  if (ticketResolution.error) {
    return formatAsyncStartError(
      "single",
      `Invalid ticket for SINGLE mode: ${ticketResolution.error}`,
    );
  }
