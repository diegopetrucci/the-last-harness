...
function contentText(content: unknown): string {
  if (typeof content === "string") return safeTerminalText(content);
  ...
  return content
    .map((part) => {
      ...
      if (typeof entry.text === "string") return safeTerminalText(entry.text);
...
function readSessionTranscriptTail(
  ...
  const tail = readContainedTextTail(
    sessionFile,
    Math.max(maxLines * 4, maxLines),
    trustedRoots,
    "session",
  );
...
function formatActivityFacts(input: {
  ...
  if (input.currentPath) facts.push(safeTerminalText(shortenPath(input.currentPath)));
...
function formatForegroundFleetLines(controls: ForegroundControl[]): string[] {
  ...
  for (const control of ordered) {
    ...
    const current = currentAgent
      ...
      : "";
    lines.push(
      `- ${runId} | running | ${foregroundModeName(control)}${current}${activity ? ` | ${activity}` : ""}`,
    );
    lines.push(`  status: subagent({ action: "status", id: "${runId}" })`);
    lines.push(
      "  status transcript: live in the expanded foreground result; the canonical session appears after completion when sessions are enabled. This is not the optional _transcript.jsonl diagnostic artifact.",
    );
    lines.push(
...
function formatAsyncFleetLines(runs: AsyncRunSummary[]): string[] {
  ...
  for (const run of runs) {
    ...
    lines.push(
      `- ${runId} | ${safeTerminalText(run.state)}${activity ? ` | ${activity}` : ""} | ${mode} | ${safeProgress}${pending} | ${safeCwd}`,
    );
    lines.push(`  status: subagent({ action: "status", id: "${runId}" })`);
    lines.push(
    ...
    for (const step of run.steps) {
      ...
      if (fs.existsSync(output)) lines.push(`    output: ${safeTerminalText(shortenPath(output))}`);
      if (step.sessionFile)
      ...
      if (step.status === "running" || step.recentOutput?.length || fs.existsSync(output)) {
        lines.push(
          `    status transcript: subagent({ action: "status", id: "${runId}", index: ${step.index}, view: "transcript" })`,
        );
      }
      lines.push(
    ...
    lines.push(
      ...formatNestedRunStatusLines(unattached, { indent: "  ", commandHints: true, maxLines: 12 }),
    );
    if (run.error) lines.push(`  error: ${safeTerminalText(run.error)}`);
    ...
    if (outputPath) lines.push(`  output: ${safeTerminalText(shortenPath(outputPath))}`);
    if (run.sessionFile) lines.push(`  session: ${safeTerminalText(shortenPath(run.sessionFile))}`);
  }
  return lines;
}

export function inspectSubagentFleet(
  _params: FleetViewParams,
  deps: FleetViewDeps = {},
  ...
  lines.push(
    '  Tail status transcript: subagent({ action: "status", id: "<run-id>", view: "transcript" })',
  );
  lines.push(
    '  Tail child status transcript: subagent({ action: "status", id: "<run-id>", index: 0, view: "transcript" })',
  );
...
function validateTranscriptIndex(
  index: number | undefined,
  steps: AsyncJobStep[],
): number | undefined {
  if (index === undefined) return undefined;
  if (!Number.isInteger(index)) throw new Error("Status transcript index must be an integer.");
  if (index < 0 || index >= steps.length)
    throw new Error(
      `Status transcript index ${index} is out of range for ${steps.length} child step${steps.length === 1 ? "" : "s"}.`,
    );
  return index;
}
...
function appendKnownArtifacts(
  ...
  for (const artifact of artifacts) lines.push(`  ${safeTerminalText(artifact)}`);
}

function appendTranscriptBody(
  lines: string[],
  sourceLabel: string,
  sourceLines: string[],
  truncated: boolean,
  ...
  for (const line of sourceLines) lines.push(`  ${safeTerminalText(line)}`);
}
...
export function formatAsyncRunTranscript(
  ...
  const stepOutputPath =
    selected.index !== undefined ? path.join(asyncDir, `output-${selected.index}.log`) : undefined;
  ...
  const sessionFile =
    selected.index !== undefined ? selected.step?.sessionFile : status.sessionFile;
  ...
  if (transcriptLines.length === 0 && sessionFile) {
    const sessionTail = readSessionTranscriptTail(
      sessionFile,
      lineLimit,
...
export function formatNestedRunTranscript(
  ...
  if (sessionTail.warnings.length) {
    lines.push("Warnings:");
    for (const warning of sessionTail.warnings) lines.push(`  ${safeTerminalText(warning)}`);
  }
  appendTranscriptBody(
    lines,
    `Canonical session tail from ${run.sessionFile} (status transcript view)`,
    sessionTail.lines,
    false,
  );
  return safeTerminalDocument(lines.join("\n"));
...
export function formatAsyncResultTranscript(
  ...
  if (index !== undefined && (index < 0 || index >= children.length))
    throw new Error(
      `Status transcript index ${index} is out of range for ${children.length} result child${children.length === 1 ? "" : "ren"}.`,
    );
  ...
  appendTranscriptBody(
    lines,
    "Result status transcript tail (retained result output; not _transcript.jsonl)",
