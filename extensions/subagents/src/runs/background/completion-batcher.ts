...
export function resolveCompletionBatchConfig(
  ...
  const enabled =
    typeof config?.enabled === "boolean" ? config.enabled : DEFAULT_COMPLETION_BATCH_CONFIG.enabled;
  return {
    enabled,
    ...
    stragglerDebounceMs:
      parsePositiveInt(config?.stragglerDebounceMs) ??
      DEFAULT_COMPLETION_BATCH_CONFIG.stragglerDebounceMs,
    stragglerMaxWaitMs:
      parsePositiveInt(config?.stragglerMaxWaitMs) ??
      DEFAULT_COMPLETION_BATCH_CONFIG.stragglerMaxWaitMs,
    stragglerWindowMs:
      parsePositiveInt(config?.stragglerWindowMs) ??
 ...
 */
export function createCompletionBatcher<T>(
  options: CompletionBatcherOptions<T>,
): CompletionBatcher<T> {
