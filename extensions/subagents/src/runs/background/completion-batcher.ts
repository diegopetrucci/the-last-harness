...
export function resolveCompletionBatchConfig(config?: CompletionBatchConfig): ResolvedCompletionBatchConfig {
	...
	return {
		...
		stragglerDebounceMs:
			parsePositiveInt(config?.stragglerDebounceMs) ?? DEFAULT_COMPLETION_BATCH_CONFIG.stragglerDebounceMs,
		stragglerMaxWaitMs:
			parsePositiveInt(config?.stragglerMaxWaitMs) ?? DEFAULT_COMPLETION_BATCH_CONFIG.stragglerMaxWaitMs,
