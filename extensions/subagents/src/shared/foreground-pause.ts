...
export const FOREGROUND_SUPERVISOR_LIFECYCLE_ERROR_MESSAGE =
  "Foreground supervisor lifecycle update failed. Check status for the paused run and retry resume or cancel if needed.";

export function formatForegroundPauseMessage(input: ForegroundPauseMessageInput): string {
  const resumeLine =
    input.resume.kind === "single"
...
export function formatForegroundSupervisorPauseMessage(
  input: ForegroundSupervisorPauseMessageInput,
): string {
