...
export const MAX_ATTRIBUTION_STATUS_BYTES = MAX_SUBAGENT_TERMINAL_SNAPSHOT_BYTES * MAX_TOP_LEVEL_PARALLEL_TASKS * 2;
export const MAX_SUBAGENT_WORKSPACE_EVIDENCE_BYTES = 256 * 1024;
const MAX_SUBAGENT_EXIT_SIGNAL_BYTES = 64;
export function terminalResultForStatusStep(result, status) {
    if (!result.terminalResult)
        return undefined;
    const terminalState = status === "cancelled"
        ? "cancelled"
        : status === "paused"
            ? "paused"
            : status === "complete"
...
const WORKSPACE_ATTRIBUTIONS = new Set([
    "exclusive",
    "shared",
    "unknown",
]);
const WORKSPACE_UNAVAILABLE_REASONS = new Set([
    "not_git_repository",
    "command_failed",
    "not_captured",
]);
...
function boundedString(value, maxBytes, allowEmpty = true) {
...
function parseTokenUsage(value) {
    if (!isPlainObject(value) || !hasExactKeys(value, ["input", "output", "total"]))
        return undefined;
...
function parseProviderTokens(value) {
    ...
    if (value.status !== "available" || !hasExactKeys(value, ["status", "usage"]))
        return undefined;
    const usage = parseTokenUsage(value.usage);
    return usage ? { status: "available", usage: { ...usage } } : undefined;
}
function parseExit(value) {
    if (!isPlainObject(value) || !hasExactKeys(value, ["code", "signal"]))
        return undefined;
...
function parseRequestedToolCalls(value) {
    if (!isPlainObject(value) || !hasExactKeys(value, ["edit", "write", "bash"]))
        return undefined;
    ...
    if (edit === undefined || write === undefined || bash === undefined)
        return undefined;
...
function parseWorkspaceSnapshot(value) {
    ...
    if (value.status === "unavailable") {
        return hasExactKeys(value, ["status", "reason"]) &&
            ...
            WORKSPACE_UNAVAILABLE_REASONS.has(value.reason)
    ...
    if (value.status !== "available" ||
        ...
        return undefined;
    const statusPorcelainZ = boundedString(value.statusPorcelainZ, MAX_SUBAGENT_WORKSPACE_EVIDENCE_BYTES);
    const worktreeDiffStat = boundedString(value.worktreeDiffStat, MAX_SUBAGENT_WORKSPACE_EVIDENCE_BYTES);
    const indexDiffStat = boundedString(value.indexDiffStat, MAX_SUBAGENT_WORKSPACE_EVIDENCE_BYTES);
    if (statusPorcelainZ === undefined ||
...
function parseWorkspace(value) {
    ...
    if (!baseline ||
        ...
        !WORKSPACE_ATTRIBUTIONS.has(attribution))
        return undefined;
    return {
        baseline,
        post,
...
function boundedEvidence(value) {
    if (Buffer.byteLength(value, "utf-8") <= MAX_SUBAGENT_WORKSPACE_EVIDENCE_BYTES)
        return value;
...
function workspaceEvidenceBytes(snapshot) {
    if (snapshot.status !== "available")
        return 0;
    return (Buffer.byteLength(snapshot.statusPorcelainZ, "utf-8") +
        Buffer.byteLength(snapshot.worktreeDiffStat, "utf-8") +
        Buffer.byteLength(snapshot.indexDiffStat, "utf-8"));
}
function attemptWorkspaceEvidenceBytes(attempt) {
    return (workspaceEvidenceBytes(attempt.workspace.baseline) +
        workspaceEvidenceBytes(attempt.workspace.post));
}
function boundWorkspaceSnapshot(snapshot, remainingBytes) {
...
export function boundSubagentAttemptFacts(attempts) {
    ...
    return attempts.slice(0, MAX_SUBAGENT_TERMINAL_ATTEMPTS).map((attempt) => {
        ...
        return {
            ...
            providerTokens: attempt.providerTokens.status === "available"
                ? { status: "available", usage: { ...attempt.providerTokens.usage } }
                : { status: "unavailable" },
        };
    });
}
export function appendBoundedSubagentAttemptFact(attempts, attempt) {
...
function parseFacts(value) {
    ...
    for (const [index, valueAttempt] of value.attempts.entries()) {
        ...
        if (!attempt || attempt.attempt !== index + 1)
            return undefined;
        snapshotBytes += attemptWorkspaceEvidenceBytes(attempt);
        if (snapshotBytes > MAX_SUBAGENT_TERMINAL_SNAPSHOT_BYTES)
            return undefined;
        attempts.push(attempt);
    }
    return { attempts };
}
export function parseSubagentTerminalResult(value) {
    try {
        if (!isPlainObject(value) || !hasExactKeys(value, ["state", "facts"]))
            return undefined;
