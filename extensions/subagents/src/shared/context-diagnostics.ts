  ...
  try {
    ...
    while (bytesScanned < MAX_SESSION_HEADER_SCAN_BYTES) {
      const bytesToRead = Math.min(
        ...
        MAX_SESSION_HEADER_SCAN_BYTES - bytesScanned,
      );
...
export function parseContextPressureCrossedThresholds(
  value: unknown,
...
export function formatDurableResumeContextBlock(
  assessment: DurableResumeContextAssessment,
): string {
...
export function parseSubagentTerminationReason(
  value: unknown,
): SubagentTerminationReason | undefined {
  return typeof value === "string" &&
    KNOWN_TERMINATION_REASONS.has(value as SubagentTerminationReason)
