...
import { boundChildError } from "../runs/shared/child-protocol.js";
...
export function createChildTranscriptWriter(input) {
    ...
    const writeTruncatedMarker = () => {
        ...
        if (bytesWritten + markerBytes > maxBytes)
            return false;
        try {
    ...
    const writeStderrDecodedText = (text) => {
        if (!text || writeError || truncated)
            return;
