...
interface BridgedAgentMetadata {
  source: AgentConfig;
  instruction: string;
}

const BRIDGED_AGENT_CONFIGS = new WeakMap<AgentConfig, BridgedAgentMetadata>();
...
function resolveIntercomBridgeConfig(
  value: ExtensionConfig["intercomBridge"],
): Required<IntercomBridgeConfig> {
...
export function diagnoseIntercomBridge(
  input: ResolveIntercomBridgeInput,
): IntercomBridgeDiagnostic {
