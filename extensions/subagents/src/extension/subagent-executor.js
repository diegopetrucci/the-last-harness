...
import { buildDoctorReport, resolveProjectAgentDoctorTrust } from "./doctor.js";
import { authorizeNestedProjectAgentControl, authorizeProjectInterruptTarget, authorizeProjectSteerTarget, buildManagementActionParams, buildRunStatusParams, buildResumeModelResolution, cancelPersistedPausedAsyncRun, getRequestedModeLabel, interruptAsyncRun, readModelRegistrySnapshot, requestInterruptAllRunningSubagentRuns, interruptNestedRun, resolveSingleRunOutputBaseDir, selectInterruptTarget, steerAsyncRun, steerNestedRun, trustedSessionRootsForStatus, unknownAgentMessage, providerFallbackModelsForTarget, resumeAsyncRun, unsupportedSavedChainInput, unsupportedSavedChainInputResult, } from "../runs/shared/run-control.js";
import { hasMalformedProjectAgentControlMarker, isRecordValue, normalizeProjectAgentAccess, projectRunAuthorizationError, resolveProjectAgentExecution, } from "../runs/shared/project-agent-control.js";
export { buildResumeModelResolution, normalizeProjectAgentAccess, requestInterruptAllRunningSubagentRuns, };
...
import { executeAsyncParallel, executeAsyncSingle, isAsyncAvailable, } from "../runs/background/async-execution.js";
import { resolveCurrentSessionId } from "../shared/session-identity.js";
import { resolveControlConfig } from "../runs/shared/subagent-control.js";
...
function applyProjectAgentOpenRouterModel(params, identities, currentModel) {
    if (!identities?.length || currentModel?.provider !== "openrouter")
        return params;
...
function statusTranscriptOptionError(params) {
    if (params.view === undefined && params.lines === undefined)
        return undefined;
...
function normalizeExecutionTickets(params) {
    ...
    if (topLevel.error)
        return { error: topLevel.error };
    let taskError;
    const normalizedTasks = params.tasks?.map((task, index) => {
        const ticket = normalizeTicketId(task.ticket, `tasks[${index}].ticket`);
        if (ticket.error)
            taskError ??= ticket.error;
        const normalizedTask = { ...task };
        if (ticket.ticketId)
            normalizedTask.ticket = ticket.ticketId;
        else
            delete normalizedTask.ticket;
        return normalizedTask;
    });
    if (taskError)
        return { error: taskError };
    const normalizedParams = { ...params };
    if (topLevel.ticketId)
        normalizedParams.ticket = topLevel.ticketId;
    else
        delete normalizedParams.ticket;
    if (normalizedTasks)
        normalizedParams.tasks = normalizedTasks;
    return { params: normalizedParams };
}
function prepareTicketBodies(params, effectiveCwd) {
    const lookups = new Map();
    const ticketBodies = new Map();
    const prepare = (ticketId, cwd, field) => {
        ...
        if (cached) {
            ...
            if (cached.body !== undefined)
                ticketBodies.set(key, cached.body);
            return undefined;
        }
...
async function runAsyncPath(data, deps) {
    const { params, effectiveCwd, agents, ctx, shareEnabled, sessionRoot, sessionFileForTask, artifactConfig, artifactsDir, awaited, controlConfig, nestedRoute, ticketBodies, } = data;
    ...
    if (hasTasks && params.tasks) {
        ...
        const modelOverrides = params.tasks.map((task, index) => resolveSubagentModelOverride(task.model ?? agentConfigs[index]?.model, ctx.model, {
    ...
    if (hasSingle) {
        ...
        const modelOverride = resolveSubagentModelOverride(params.model ?? a.model, ctx.model, {
            scope: data.modelScope,
        ...
        return executeSingle(id, {
            ...
            nestedRoute,
            timeoutMs: data.timeoutMs,
            projectAgent: data.projectAgentIdentities?.find((identity) => `embedded.${identity.slug}` === params.agent),
        });
    }
    return null;
}
...
async function executeDoctorAction(params, requestCwd, ctx, deps) {
    ...
    let projectAgentTrustOptions = {};
    try {
        const access = normalizeProjectAgentAccess(deps.getProjectAgentAccess?.({
            cwd: requestCwd,
    ...
    const projectAgentTrust = await resolveProjectAgentDoctorTrust(requestCwd, projectAgentTrustOptions);
    return {
        content: [
            {
                type: "text",
...
function projectManagementError(text) {
    return {
...
function awaitedLifecycleFailure() {
    return {
        content: [
            {
                type: "text",
...
function projectManagementErrorFrom(error) {
    if (isAsyncStatusReadError(error))
        return awaitedLifecycleFailure();
...
async function executeSteerAction(params, ctx, deps) {
    ...
    if (params.dir) {
        try {
            ...
            const runId = location.resolvedId ?? params.id ?? path.basename(location.asyncDir ?? params.dir);
...
async function executeInterruptAction(params, ctx, deps) {
    ...
    if (resolved?.kind === "nested") {
        if (hasMalformedProjectAgentControlMarker(resolved.match.run)) {
            return projectManagementError(projectRunAuthorizationError("the nested target carries a malformed project-agent marker; refusing nested interrupt fallback.").message);
        }
        try {
            await authorizeNestedProjectAgentControl({
                target: resolved,
    ...
    if (params.id?.trim() && asyncInterruptTarget.location.asyncDir) {
        ...
        if (isPersistedCancellationState(persistedStatus)) {
            return cancelPersistedPausedAsyncRun(asyncInterruptTarget.location.asyncDir, asyncInterruptTarget.id, params.index);
        }
    }
    try {
        const result = interruptAsyncRun(deps.state, asyncInterruptTarget.id, deps.kill, asyncInterruptTarget.location);
        return (result ??
            projectManagementError("No interrupt-capable run found in this session."));
    }
    catch (error) {
        return projectManagementErrorFrom(error);
    }
...
export function createSubagentExecutor(deps) {
    ...
    const execute = async (_id, params, signal, onUpdate, ctx) => {
        ...
        deps.state.currentSessionId = resolveCurrentSessionId(ctx.sessionManager);
        const projectResolution = await resolveProjectAgentExecution(effectiveParams, requestedExecutionCwd, scope, deps.state.currentSessionId, deps);
        if ("error" in projectResolution) {
            return toExecutionErrorResult(effectiveParams, new Error(projectResolution.error));
        }
        effectiveParams = applyProjectAgentOpenRouterModel(projectResolution.params, projectResolution.projectAgentIdentities, ctx.model);
