...
const FallbackModelsOverride = Type.Array(Type.String(), {
  description:
    "Per-execution fallback models to try after the primary model and before any agent fallbackModels.",
});
...
const SubagentParamsSchema = Type.Object(
  {
    agent: Type.Optional(
      Type.String({ description: "Agent name for SINGLE mode or action='get'." }),
    ),
    task: Type.Optional(
    ...
    action: Type.Optional(
      Type.String({
        ...
        description:
          "Management action. One of: list, get, status, interrupt, resume, steer, doctor. Omit for execution mode (single agent or parallel tasks).",
      }),
    ),
    id: Type.Optional(
      Type.String({
    ...
    message: Type.Optional(
      Type.String({
        description:
          "Optional guidance for action='resume' (omit for unchanged resume), or required guidance for action='steer'.",
      }),
    ),
