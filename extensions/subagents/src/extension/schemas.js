...
const TaskItem = Type.Object({
    ...
    ticket: Type.Optional(Type.String({
        minLength: 1,
        description: "Explicit tk ticket ID for this parallel task, resolved from the effective task cwd/TICKETS_DIR; omit for legacy task-text inference.",
    })),
...
const SubagentParamsSchema = Type.Object({
    ...
    ticket: Type.Optional(Type.String({
        minLength: 1,
        description: "Explicit tk ticket ID for SINGLE mode, resolved from the effective task cwd/TICKETS_DIR; omit for legacy task-text inference.",
    })),
    tasks: Type.Optional(Type.Array(TaskItem, {
        description: "PARALLEL mode: [{agent, task, ticket?, count?, output?, outputMode?, reads?, progress?, model?}, ...]",
    })),
