...
function gitIdentity(source: string): string {
	...
	if (value.startsWith("git:") && !/^git:\/\//i.test(value)) value = value.slice("git:".length).trim();
...
function settingsMatches(settingsPath: string, scope: ExternalSubagentPackageMatch["scope"]): ExternalSubagentPackageMatch[] {
	...
	for (const entry of settings.packages) {
		...
		if (!source || !identity || !EXTERNAL_SUBAGENT_PACKAGE_IDENTITIES.has(identity) || seen.has(identity)) continue;
		seen.add(identity);
