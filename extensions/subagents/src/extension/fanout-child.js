...
import { childMessageAckPath, requestAsyncResume, waitForChildMessageAcceptance, writeChildMessageRequestToDir } from "../runs/background/control-channel.js";
import { reconcileAsyncRun } from "../runs/background/stale-run-reconciler.js";
import { SUBAGENT_CHILD_ENV, SUBAGENT_FANOUT_CHILD_ENV } from "../runs/shared/pi-args.js";
import { claimNestedControlRequest, findNestedRun, nestedControlRequestOwnedBy, NESTED_RUNNER_ACCEPTANCE_TIMEOUT_MS, projectNestedEvents, readNestedControlRequests, resolveNestedAsyncDir, resolveNestedParentAddressFromEnv, resolveNestedRouteFromEnv, writeNestedControlResult } from "../runs/shared/nested-events.js";
...
function boundedNestedControlResult(request, ok, message) {
...
function resolveLiveTargetIndex(input) {
    ...
    if (input.requestedIndex !== undefined) {
        if (eligible.includes(input.requestedIndex))
            return { index: input.requestedIndex };
        return { error: `${input.label} ${input.runId} child ${input.requestedIndex} is not live and cannot accept a resume follow-up.` };
    }
    if (eligible.length === 1)
        return { index: eligible[0] };
    if (eligible.length > 1)
        return { error: `${input.label} ${input.runId} has multiple live children (${eligible.join(", ")}); action='resume' requires index.` };
    return { error: `${input.label} ${input.runId} has no live child that can accept a resume follow-up.` };
}
async function routeNestedAsyncResume(route, request, run, kill = process.kill) {
    const asyncDir = resolveNestedAsyncDir(route.rootRunId, run);
    if (!asyncDir)
        return boundedNestedControlResult(request, false, `Nested async run ${run.id} has no valid live run directory to resume.`);
    ...
    if (!status || status.runId !== run.id || status.state !== "running")
        return boundedNestedControlResult(request, false, `Nested async run ${run.id} is not running and cannot accept a live resume follow-up.`);
    ...
    if (resolved.error)
        return boundedNestedControlResult(request, false, resolved.error);
...
export async function routeNestedResumeRequest(route, state, request) {
    if (!request.message?.trim())
        return boundedNestedControlResult(request, false, "Nested resume requires message.");
    const run = findNestedRun(projectNestedEvents(route).children, request.targetRunId);
    if (!run)
        return boundedNestedControlResult(request, false, `Nested run ${request.targetRunId} is no longer routed by this owner.`);
    ...
    if (run.ownerState && run.ownerState !== "live")
        return boundedNestedControlResult(request, false, `Nested run ${run.id} owner is ${run.ownerState} and cannot accept a live resume follow-up.`);
    if (run.asyncDir)
        return routeNestedAsyncResume(route, request, run);
    const control = state.foregroundControls.get(request.targetRunId);
    if (!control)
        return boundedNestedControlResult(request, false, `Nested run ${request.targetRunId} is not active in this fanout child.`);
    ...
    if (!inbox)
        return boundedNestedControlResult(request, false, `Nested run ${request.targetRunId} child ${resolved.index} is no longer live and cannot accept a resume follow-up.`);
    if (Date.now() >= request.deliveryDeadlineAt)
        return boundedNestedControlResult(request, false, `Nested resume delivery deadline expired for run ${request.targetRunId}; no follow-up was queued.`);
    const resumeRequest = { type: "resume", id: request.requestId, ts: Date.now(), message: request.message.trim(), targetIndex: resolved.index, deliveryDeadlineAt: request.deliveryDeadlineAt, source: "nested-resume" };
...
function startNestedControlInboxListener(_pi, state) {
    let route;
    try {
    ...
    const timer = setInterval(() => {
        try {
            ...
            for (const request of readNestedControlRequests(route)) {
                if (!nestedControlRequestOwnedBy(request, owner) || pending.has(request.requestId))
                    continue;
...
export default function registerFanoutChildSubagentExtension(pi) {
