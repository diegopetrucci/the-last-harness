...
import { frontmatterNameForConfig } from "./identity.js";
...
export function serializeAgent(config, options = {}) {
    const lines = [];
    const preserve = (...fields) => fields.some((field) => options.preserveFrontmatterFields?.has(field));
    ...
    if (config.model || preserve("model"))
    ...
    if (config.defaultContext || preserve("defaultContext"))
        lines.push(`defaultContext: ${config.defaultContext ?? ""}`);
    if (config.acceptanceRole || preserve("acceptanceRole"))
        lines.push(`acceptanceRole: ${config.acceptanceRole ?? ""}`);
    const skillsValue = joinComma(config.skills);
    if (skillsValue || preserve("skill", "skills"))
