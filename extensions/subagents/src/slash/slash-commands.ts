...
async function requestSlashRun(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onUpdate = (data: unknown) => {
      ...
      ctx.ui.setStatus(
        "subagent-slash",
        `${count} tools${tool} | ${liveDetailShortcutDisplay()} live detail`,
      );
    };
...
function buildSlashExportText(response: SlashSubagentResponse): string {
  const output =
    extractSlashMessageText(response.result.content) || response.errorText || "(no output)";
...
async function runSlashSubagent(
  ...
  const initialText =
    extractSlashMessageText(initialDetails.result.content) || "Running subagent...";
  pi.sendMessage({
