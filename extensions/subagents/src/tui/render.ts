...
function fitInlineThinkingActivity(prefix: string, phrase: string, freshness: string, theme: Theme, maxWidth: number): string {
...
function widgetInlineThinkingActivity(job: AsyncJobState): { phrase: string; freshness: string } | undefined {
	if (job.status !== "running" || job.interruptRequestedAt !== undefined || job.currentTool || widgetActiveStep(job) || widgetHasPausingStep(job)) return undefined;
...
function widgetActivityLines(job: AsyncJobState, expanded = false): string[] {
	...
	const facts: string[] = [];
	if (currentTool && currentToolStartedAt !== undefined && job.updatedAt !== undefined) facts.push(`${currentTool} ${formatDuration(Math.max(0, job.updatedAt - currentToolStartedAt))}`);
	...
	if (!currentTool && !expanded && job.status === "running") {
		return [compactThinkingPhrase(activityState, turnCount), activity, ...facts].filter((line): line is string => Boolean(line));
	}
	if (activity && facts.length) return [`${activity} · ${facts.join(" · ")}`];
	if (activity) return [activity];
	if (facts.length) return [facts.join(" · ")];
	if (job.status === "running") return [expanded ? "thinking…" : compactThinkingPhrase(activityState, turnCount) ?? "thinking…"];
...
function widgetStepActivity(step: NonNullable<AsyncJobState["steps"]>[number], snapshotNow?: number, expanded = false): string {
...
function widgetStepStats(theme: Theme, step: NonNullable<AsyncJobState["steps"]>[number], durationFallbackMs?: number, expanded = false): string {
	if (!expanded) return "";
...
function widgetStepActivityLines(step: NonNullable<AsyncJobState["steps"]>[number], width: number, expanded: boolean, snapshotNow?: number): string[] {
	if (step.interruptRequestedAt !== undefined) return ["pausing…"];
	...
	if (!expanded && step.status === "running") return [compactThinkingPhrase(step.activityState, step.turnCount), ...(activity ? [activity] : [])].filter((line): line is string => Boolean(line));
	if (activity) return [activity];
...
function nestedActivity(
	input: Pick<NestedRunSummary | NestedStepSummary, "activityState" | "lastActivityAt" | "currentTool" | "currentToolStartedAt" | "currentPath" | "turnCount" | "toolCount"> & { totalTokens?: NestedRunSummary["totalTokens"] },
	...
	if (activity) return activity;
	if (facts.length) return facts.join(" · ");
	if (state === "running") return expanded ? "thinking…" : compactThinkingPhrase(input.activityState, input.turnCount) ?? "thinking…";
	...
	const append = (items: NestedRunSummary[] | undefined, depth: number, prefix: string): void => {
		...
		for (let index = 0; index < items.length; index++) {
			...
			for (const step of child.steps ?? []) {
				...
				lines.push(theme.fg("dim", `${prefix}  ↳ ${nestedStatusGlyph(step.status, theme)} ${step.agent} · ${step.status} · ${nestedActivity(step, step.status, snapshotNow ?? child.lastUpdate, privacySafe, expanded)}`));
...
function compactSingleWidgetLines(job: AsyncJobState, theme: Theme, width: number): string[] {
	...
	for (const [index, step] of job.steps.entries()) {
		...
		for (const nestedLine of formatNestedWidgetLines(step.children, theme, contentWidth, false, job.updatedAt, 1, isProtectedWidgetLifecycle(step.status, step.interruptRequestedAt))) lines.push(`    ${nestedLine}`);
	}
...
function progressiveJobLine(job: AsyncJobState, theme: Theme, width: number): string {
	...
	if (thinkingActivity) return fitInlineThinkingActivity(prefix, thinkingActivity.phrase, thinkingActivity.freshness, theme, contentWidth);
	const runningStep = widgetRunningStep(job);
	const activityState = widgetActivityState(job, runningStep);
