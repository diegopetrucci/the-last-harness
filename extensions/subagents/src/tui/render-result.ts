/**
 * Engine-neutral subagent tool-result rendering.
 */
...
function getToolCallLines(
  ...
  return (
    result.toolCalls?.map((toolCall) =>
      safeTerminalDocumentLeaf(toolCall.expandedText ?? toolCall.text),
    ) ?? []
  );
}
...
function collapsedAwaitedLineBudget(): number {
  const rows = process.stdout.rows || 30;
  return Math.max(5, Math.min(14, Math.floor(rows * 0.4)));
}

function collapsedAwaitedSummaryLines(
  hiddenCount: number,
  theme: Theme,
  width: number,
  maxLines: number,
...
function fitCollapsedAwaitedLines(
  contentLines: string[],
  theme: Theme,
  width: number,
...
function collapsedAwaitedComponent(logicalLines: readonly string[], theme: Theme): Component {
  return {
...
function renderSingleCompact(
  ...
  if (isRunning && r.progress) {
    for (const [activityIndex, activity] of compactProgressActivityLines(
      r.progress,
      width,
      AWAITED_ACTIVITY_PREFIX,
      AWAITED_ACTIVITY_CONTINUATION_PREFIX,
    ).entries()) {
      const prefix =
        activityIndex === 0 ? AWAITED_ACTIVITY_PREFIX : AWAITED_ACTIVITY_CONTINUATION_PREFIX;
