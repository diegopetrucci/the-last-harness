...
function collapsedAwaitedLineBudget() {
    const rows = process.stdout.rows || 30;
    return Math.max(5, Math.min(14, Math.floor(rows * 0.4)));
}
function collapsedAwaitedSummaryLines(hiddenCount, theme, width, maxLines) {
...
function fitCollapsedAwaitedLines(contentLines, theme, width, footerLines = []) {
    const budget = collapsedAwaitedLineBudget();
    if (footerLines.length > budget) {
        const summaryLines = collapsedAwaitedSummaryLines(contentLines.length, theme, width, budget);
    ...
    const summaryLines = contentBudget > 0
        ? collapsedAwaitedSummaryLines(contentLines.length, theme, width, contentBudget)
        : [];
    return [...summaryLines.slice(0, contentBudget), ...footerLines];
}
function collapsedAwaitedComponent(logicalLines, theme) {
    return {
        render(width) {
            ...
            const footerLines = hasLiveDetailFooter
                ...
                : [];
            return fitCollapsedAwaitedLines(contentLines, theme, contentWidth, footerLines);
        },
        invalidate() { },
    };
}
...
const AWAITED_ACTIVITY_PREFIX = "  ⎿  ";
const AWAITED_ACTIVITY_CONTINUATION_PREFIX = "     ";
...
function renderSingleCompact(d, r, theme, frame) {
    ...
    if (isRunning && r.progress) {
        for (const [activityIndex, activity] of compactProgressActivityLines(r.progress, width, AWAITED_ACTIVITY_PREFIX, AWAITED_ACTIVITY_CONTINUATION_PREFIX).entries()) {
            const prefix = activityIndex === 0 ? AWAITED_ACTIVITY_PREFIX : AWAITED_ACTIVITY_CONTINUATION_PREFIX;
            lines.push(theme.fg("dim", `${prefix}${activity}`));
        }
        lines.push(theme.fg("dim", `  ${liveDetailHintText()}`));
        return collapsedAwaitedComponent(lines, theme);
    }
