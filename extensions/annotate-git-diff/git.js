...
function parseNameStatusLine(parts) {
    ...
    if (path == null)
        return null;
    if (code === "M")
        return { status: "modified", oldPath: path, newPath: path };
    if (code === "A")
        return { status: "added", oldPath: null, newPath: path };
    if (code === "D")
        return { status: "deleted", oldPath: path, newPath: null };
    return null;
}
...
function buildSnapshotFileId(path, hasWorkingTreeFile) {
...
function inspectWorkingTreePath(repoRoot, path) {
    ...
    for (let index = 0; index < parts.length; index += 1) {
        ...
        if (stat.isFile())
            return { kind: "file", absolutePath: currentPath };
        return { kind: "other" };
    }
    return { kind: "unsafe" };
}
async function getWorkingTreeContent(repoRoot, path) {
    ...
    if (entry.kind === "symlink")
        return WORKING_TREE_SYMLINK_CONTENT;
    if (entry.kind !== "file")
        return "";
    try {
...
function classifyFilePath(path) {
    ...
    if (binaryExtensions.has(extension))
        return { kind: "binary", mimeType: null };
...
async function loadBinarySideFromWorkingTree(repoRoot, path, mimeType) {
    if (path == null)
        return { exists: false, previewUrl: null };
    const entry = inspectWorkingTreePath(repoRoot, path);
    if (entry.kind === "symlink")
        return { exists: true, previewUrl: null };
    if (entry.kind !== "file")
        return { exists: false, previewUrl: null };
...
async function getTrackedBranchReviewChanges(pi, repoRoot, branchComparisonBase) {
    return parseNameStatus(await runGitAllowFailure(pi, repoRoot, [
        "diff",
        "--find-renames",
        "-M",
        "--name-status",
...
async function getWorkingTreeSnapshotChanges(pi, repoRoot, baseRevision) {
    ...
    scriptLines.push(baseRevision != null
...
async function listReviewableRepositoryPaths(pi, repoRoot) {
    ...
    for (const path of output.split("\0")) {
        if (path.length === 0 || seen.has(path) || !isIncludedReviewPath(path))
            continue;
        seen.add(path);
        paths.push(path);
    }
...
function hasWorkingTreeFile(repoRoot, path) {
    const entry = inspectWorkingTreePath(repoRoot, path);
    return entry.kind === "file" || entry.kind === "symlink";
}
function toSnapshotReviewFile(repoRoot, path) {
    const meta = classifyFilePath(path);
...
async function getAllReviewFiles(pi, repoRoot, branchChanges) {
...
export async function getCommitFiles(pi, repoRoot, sha) {
    if (isWorkingTreeCommitSha(sha)) {
        const repositoryHasHead = await hasHead(pi, repoRoot);
        const changes = (await getWorkingTreeReviewChanges(pi, repoRoot, repositoryHasHead)).filter((change) => isIncludedReviewPath(change.newPath ?? change.oldPath ?? ""));
        return changes
            .map((change) => {
            const comparison = toComparison(change);
            return toReviewFile(change, {
    ...
    const changes = parseNameStatus(output).filter((change) => isIncludedReviewPath(change.newPath ?? change.oldPath ?? ""));
    return changes
        .map((change) => {
        const comparison = toComparison(change);
        return toReviewFile(change, {
...
export async function loadReviewFileContents(pi, repoRoot, file, scope, commitSha = null, branchMergeBaseSha = null) {
    ...
    if (file.kind !== "text") {
        ...
        if (scope === "commits") {
            ...
            const originalSide = await loadBinarySideFromRevision(pi, repoRoot, `${commitSha}^`, comparison.oldPath, file.mimeType);
        ...
        if (!branchMergeBaseSha)
            return emptyBinaryContents;
        const originalSide = await loadBinarySideFromRevision(pi, repoRoot, branchMergeBaseSha, comparison.oldPath, file.mimeType);
        const modifiedSide = file.hasWorkingTreeFile
            ? await loadBinarySideFromWorkingTree(repoRoot, comparison.newPath, file.mimeType)
    ...
    if (scope === "commits") {
        ...
        if (isWorkingTreeCommitSha(commitSha)) {
            const repositoryHasHead = await hasHead(pi, repoRoot);
            const originalContent = repositoryHasHead && comparison.oldPath != null
