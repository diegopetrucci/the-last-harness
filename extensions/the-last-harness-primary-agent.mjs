...
export const SELECTABLE_PRIMARY_AGENTS = Object.freeze([
  "architect",
  "rush",
  "product",
  "bug-hunter",
...
export function resolvePrimaryAgentConfig(config) {
  ...
  if (hasOwn(config, "selected")) {
    ...
    return {
      ...
      invalidSelected: invalidSelectionLabel(config.selected),
    };
  }
...
export function resolvePrimaryAgentSessionState(data) {
  ...
  if (hasOwn(data, "selected")) {
    ...
    return {
      selection: DEFAULT_PRIMARY_AGENT,
      invalidSelected: invalidSelectionLabel(data.selected),
    };
  }
...
export function primaryAgentSelectionFromBranch(entries) {
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    if (
      !isRecord(entry) ||
      entry.type !== "custom" ||
      entry.customType !== PRIMARY_AGENT_SESSION_STATE_ENTRY
    ) {
      continue;
    }
