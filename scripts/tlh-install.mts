...
function assertSupportedNodeRuntime(currentVersion = process.versions.node): void {
  if (!parseNodeVersion(currentVersion)) {
    throw new Error(
      `unable to determine Node.js version; The Last Harness requires Node.js >= ${MIN_NODE_VERSION}.`,
    );
  }
...
function buildInstallConfig(
  ...
  const effectiveWrapperName =
    isMainRef && !parsedArgs.wrapperNameExplicit ? "tlh-main" : parsedArgs.wrapperName;
  ...
  const packageHelperRoot =
    resolvedPackageRoot || (packageSourceIsDefault ? defaultPackageRoot : "");
  ...
  return {
    ...
    rawBase:
      parsedArgs.rawBaseInput ||
      `https://raw.githubusercontent.com/${parsedArgs.repo}/${parsedArgs.ref}`,
    ...
    piCmd: "",
    supportFilePaths: Object.fromEntries(supportFiles.map((file) => [file.variable, ""])) as Record<
      string,
      string
    >,
    scriptPath,
...
function detailLog(
  config: Pick<InstallConfig, "dryRun" | "quiet" | "verbose">,
  message: string,
): void {
...
function preferBinDirOnPathForCurrentInstall(
  ...
  config.env.PATH = [binDir, ...currentEntries.filter((entry: string) => entry !== binDir)].join(
    delimiter,
  );
  if (alreadyPresent) {
...
async function installSupportFilesToProfile(config: InstallConfig): Promise<void> {
  ...
  const missingPrompts = missingTlhSubagentPrompts(subagentsSrc, {
    prompts: config.subagentPrompts,
  });
  ...
  const installedSubagentsDir = copyTlhSubagentPrompts(config, subagentsSrc, {
    prompts: config.subagentPrompts,
  });
  const installedMissing = missingTlhSubagentPrompts(installedSubagentsDir, {
    prompts: config.subagentPrompts,
  });
...
function installDefaultExtensions(config: InstallConfig): void {
  ...
  try {
    ...
    throw new Error(`failed to read bundled default extension sources: ${message}`, {
      cause: error,
    });
  }
  try {
    ...
    if (
      defaultExtensionsFileRequiresCriticalInstall(
        config.supportFilePaths.DEFAULT_EXTENSIONS_FILE,
        {
          noSettings: config.noSettings,
        },
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`failed to read critical bundled default extension sources: ${message}`, {
        cause: error,
      });
    }
    warn(
...
function configureGnosis(config: InstallConfig): void {
  ...
  if (
    !config.supportFilePaths.TLH_GNOSIS_SCRIPT ||
    !existsSync(config.supportFilePaths.TLH_GNOSIS_SCRIPT)
  ) {
    throw new Error(
      `required Gnosis support script not found for ref ${config.ref}: scripts/tlh-gnosis.mjs`,
    );
  }
...
function printSummary(config: InstallConfig): void {
  ...
  if (!config.noWrapper) {
    ...
    log(
      config,
      `\`PI_CODING_AGENT_DIR="${config.agentDir}" "${dirname(config.agentDir)}/runtime/bin/pi"\``,
    );
  }
...
async function runInstallFlow(config: InstallConfig): Promise<void> {
  ...
  const retiredSubagentNpmCommand =
    retiredSubagentPackages.length > 0
