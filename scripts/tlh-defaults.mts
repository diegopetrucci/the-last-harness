...
const RETIRED_DEFAULT_PACKAGE_IDENTITIES = new Set(
	RETIRED_TLH_DEFAULT_PACKAGE_SOURCES.map(packageIdentity).filter((value): value is string => Boolean(value)),
);
...
function orderedDisabledIds(ids: Set<string>, defaultExtensions: readonly DefaultExtensionEntry[]): string[] {
...
function removeDuplicatePackagesAfterIndex(settings: Settings, identity: string | undefined, firstIndex: number): void {
...
function replacedPackageSource(settings: Settings, extension: DefaultExtensionEntry): string | undefined {
...
function enablePackage(settings: Settings, extension: DefaultExtensionEntry): void {
	...
	const next: JsonObject = { ...cloneJsonObject(current), source: packageSourceOf(current) || extension.source };
...
function assertKnownExtension(defaultExtensions: readonly DefaultExtensionEntry[], id: string): DefaultExtensionEntry {
...
function commandList(settings: Settings, defaultExtensions: readonly DefaultExtensionEntry[]): void {
...
function commandDisable(settings: Settings, defaultExtensions: readonly DefaultExtensionEntry[], id: string): boolean {
...
function commandEnable(settings: Settings, defaultExtensions: readonly DefaultExtensionEntry[], id: string): boolean {
	const extension = assertKnownExtension(defaultExtensions, id);
	const disabledIds = disabledIdsFromSettings(settings, defaultExtensions);
...
function syncDefaultExtensionProvenance(settings: Settings, defaultExtensions: readonly DefaultExtensionEntry[]): void {
	const disabledIds = disabledIdsFromSettings(settings, defaultExtensions);
