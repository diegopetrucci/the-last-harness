...
function defaultTlhAgentDir(
  env: ProcessEnvMap = process.env,
  options: { homeDir?: string } = {},
): string | undefined {
...
function loadState(args: CliArgs): NormalizedInstallState | undefined {
  ...
  try {
    ...
    if (!args.quiet) {
      console.error(
        `warning: ${pathValue} does not contain usable The Last Harness update metadata; ignoring it`,
      );
    }
  } catch (error) {
    if (!args.quiet) {
...
function assertRecoveryTargetSafe(agentDir: string): void {
  if (pathIsProtectedPiConfig(agentDir)) {
    throw new Error(
      `refusing to recover The Last Harness update against normal Pi config root: ${agentDir}`,
    );
  }
}
