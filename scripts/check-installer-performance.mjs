...
import { buildPtyCommand, runProcess, } from "./lib/check-installer-performance-process.mjs";
import { assertOwnedWorkspacePath, buildChildEnvironment, createBenchmarkWorkspace, removeOwnedWorkspace, writeCredentialFreeTrustMetadata, BENCHMARK_ROOT_PREFIX, REPOSITORY, } from "./lib/check-installer-performance-workspace.mjs";
import { createReadinessObserver, hasFooterMarker, hasHeaderMarker, observeReadinessChunk, stripTerminalNoise, } from "./lib/tlh-startup-readiness.mjs";
...
function validateRef(ref, flag) {
    if (ref.length === 0 || ref.length > 256)
        throw new Error(`${flag} must be a valid ref`);
...
function diagnoseReadinessFailure(state, status) {
    ...
    if (state.headerMs === undefined)
        diagnostics.push("TLH header marker was not observed");
    if (state.footerMs === undefined)
        diagnostics.push("TLH footer marker was not observed");
    ...
    if (status.timedOut)
        diagnostics.push(`launch timed out after ${LAUNCH_TIMEOUT_MS}ms`);
    else if (status.signal)
        diagnostics.push(`launch ended by signal ${status.signal}`);
...
function recordBoundary(state, name, elapsedMs) {
    if (state.boundaries[name] === undefined)
        state.boundaries[name] = elapsedMs;
}
function processPhaseLine(state, line, elapsedMs) {
    const normalized = line.replace(/\r/g, "").trim();
    if (!normalized)
        return;
    if (/Pinning local Pi runtime|Installing TLH private Pi runtime|^Installing package\.\.\./u.test(normalized)) {
    ...
    if (/Installing bundled default extensions|Skipping bundled default extensions/u.test(normalized)) {
        recordBoundary(state, "defaults-start", elapsedMs);
    }
...
export function observeInstallerChunk(state, chunk, elapsedMs, stream = "stdout") {
    ...
    for (const line of lines)
        processPhaseLine(state, line, elapsedMs);
}
export function finishPhaseObserver(state, elapsedMs) {
    for (const stream of ["stdout", "stderr"]) {
        const pending = state.pendingByStream[stream];
        if (pending)
            processPhaseLine(state, pending, elapsedMs);
    }
    const phases = [
        {
            name: "bootstrap",
    ...
    return Object.fromEntries(phases.map(({ name, start, end, reason }) => {
...
function installerDiagnostics(result, phases) {
...
async function measureLaunch(workspace, env, cleanup) {
...
async function runInstaller(sourcePath, packageRef, workspace, sourceEnv, cleanup) {
...
async function prepareFullyLaunchedSeed(sourcePath, packageRef, workspace, sourceEnv, cleanup) {
    ...
    const launch = await measureLaunch(workspace, buildChildEnvironment(workspace, sourceEnv, { ref: packageRef }), cleanup);
    return {
        kind: "fully-launched-seed",
        excludedFromMeasurements: true,
        installerWallMs: installer.wallMs,
...
function copyNpmCache(source, destination) {
...
function findPackageCheckout(agentDir) {
    return join(agentDir, "git", "github.com", ...REPOSITORY.split("/"));
}
export async function readInstalledPackageRevision(workspace, sourceEnv, cleanup, runner = runProcess) {
    const packageRoot = findPackageCheckout(workspace.agentDir);
    if (!existsSync(join(packageRoot, ".git")) && !existsSync(join(packageRoot, "HEAD")))
        return null;
    const env = buildChildEnvironment(workspace, sourceEnv);
    const result = await runner("git", ["--no-optional-locks", "-C", packageRoot, "rev-parse", "HEAD"], {
        cwd: workspace.cwd,
        env,
        timeoutMs: 10_000,
        registerStop: cleanup.registerStop,
...
export async function readPiVersion(workspace, sourceEnv, cleanup, runner = runProcess) {
...
export function parseBundledNpmPins(jsonText) {
    let parsed;
    try {
        parsed = JSON.parse(jsonText);
    }
    catch (error) {
        throw new Error(`bundled default-extension manifest is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    for (const entry of parsed) {
        ...
        if (pin)
            packages.set(pin.name, pin.version);
    }
    if (packages.size === 0)
        throw new Error("bundled default-extension manifest contains no npm pins");
    return { packages };
}
export function compareBundledNpmPins(before, after) {
    const packages = [];
...
function rawFileUrl(ref, relativePath) {
...
async function fetchTextWithCurl(url, workspace, sourceEnv, cleanup) {
    ...
    if (result.code !== 0 || result.signal || result.timedOut) {
        throw new Error(`failed to download ${url}: ${formatStatus(result)}${result.stderr ? ` (${excerptOutput(result.stderr)})` : ""}`);
    }
    return result.stdout;
}
export async function readRemoteRefFile(ref, relativePath, workspace, sourceEnv, cleanup, fetcher = fetchTextWithCurl) {
    return fetcher(rawFileUrl(ref, relativePath), workspace, sourceEnv, cleanup);
}
export function installedRevisionMismatchDiagnostic(scenario, run, requestedRef, resolvedRevision, observedRevision) {
    if (!observedRevision || observedRevision === resolvedRevision)
        return null;
...
export function parseRemoteRevision(output, ref) {
    ...
    for (const line of output.split(/\r?\n/u)) {
        ...
        if (remoteRef === peeledTagRef)
            peeledTag = revision;
        else if (direct === null && directRefs.has(remoteRef))
            direct = revision;
    }
    return peeledTag ?? direct;
}
export async function resolveRemoteRefRevision(ref, workspace, sourceEnv, cleanup, runner = runProcess) {
    ...
    if (remote.code !== 0 || remote.signal || remote.timedOut)
        return null;
    return parseRemoteRevision(remote.stdout, ref);
}
async function currentCheckoutRevision(repoRoot, workspace, sourceEnv, cleanup) {
    const env = buildChildEnvironment(workspace, sourceEnv);
    const revisionResult = await runProcess("git", ["--no-optional-locks", "-C", repoRoot, "rev-parse", "HEAD"], {
        cwd: workspace.cwd,
        env,
        timeoutMs: 30_000,
        registerStop: cleanup.registerStop,
    });
    const statusResult = await runProcess("git", ["--no-optional-locks", "-C", repoRoot, "status", "--porcelain"], {
        cwd: workspace.cwd,
        env,
        timeoutMs: 30_000,
        registerStop: cleanup.registerStop,
...
function summarizeSamples(samples) {
    ...
    const installerValues = measuredSamples.flatMap((sample) => sample.installer?.success ? [sample.installer.wallMs] : []);
    const launchValues = measuredSamples.flatMap((sample) => sample.launch?.ready && sample.launch.wallMs !== null ? [sample.launch.wallMs] : []);
    const phaseSummary = {};
    for (const phase of [
        "bootstrap",
        "runtime",
    ...
    for (const scenario of new Set(samples.map((sample) => sample.scenario))) {
        ...
        const measuredScenarioSamples = scenarioSamples.filter((sample) => sample.failureDiagnostics.length === 0);
        byScenario[scenario] = {
            installerWallMs: numericSummary(measuredScenarioSamples.flatMap((sample) => sample.installer?.success ? [sample.installer.wallMs] : [])),
            firstUsableLaunchMs: numericSummary(measuredScenarioSamples.flatMap((sample) => sample.launch?.ready && sample.launch.wallMs !== null ? [sample.launch.wallMs] : [])),
...
export async function npmVersion(workspace, sourceEnv, cleanup, runner = runProcess) {
...
export async function prepareSource(options, root, workspace, sourceEnv, cleanup, selectedRevision, runner = runProcess) {
    ...
    if (result.code !== 0 || result.signal || result.timedOut || !existsSync(installScriptPath)) {
        throw new Error(`failed to download installer for ${options.ref}: ${formatStatus(result)}${result.stderr ? ` (${excerptOutput(result.stderr)})` : ""}`);
    }
    return {
        installScriptPath,
...
async function createWarmCacheSeed(sourcePath, root, sourceEnv, cleanup, selectedRevision) {
    const workspace = createBenchmarkWorkspace(root);
    const setup = await prepareFullyLaunchedSeed(sourcePath, selectedRevision, workspace, sourceEnv, cleanup);
    if (!setup.ready) {
...
async function runScenarioSample(sourcePath, options, scenario, runNumber, root, sourceEnv, cleanup, warmCacheSeed, selectedRevision, upgradeRevision) {
    const workspace = createBenchmarkWorkspace(root);
    ...
    try {
        if (scenario === "warm-cache-fresh") {
            if (!warmCacheSeed)
                throw new Error("warm-cache seed was not prepared");
            ...
            if (!seedRef || !seedRevision)
                throw new Error("changed-pin-upgrade requires --upgrade-from");
            setup = await prepareFullyLaunchedSeed(sourcePath, seedRevision, workspace, sourceEnv, cleanup);
            if (!setup.ready) {
        ...
        const installer = await runInstaller(sourcePath, selectedRevision, workspace, sourceEnv, cleanup);
        if (!installer.success) {
        ...
        writeCredentialFreeTrustMetadata(workspace);
        const launch = await measureLaunch(workspace, buildChildEnvironment(workspace, sourceEnv, { ref: selectedRevision }), cleanup);
        if (!launch.ready)
            failureDiagnostics.push(...launch.diagnostics);
        const observedInstalledRevision = await readInstalledPackageRevision(workspace, sourceEnv, cleanup);
        const revisionDiagnostic = installedRevisionMismatchDiagnostic(scenario, runNumber, selectedRef, selectedRevision, observedInstalledRevision);
        if (revisionDiagnostic)
            failureDiagnostics.push(revisionDiagnostic);
...
function benchmarkScope() {
    return {
        ...
        environment: "temporary HOME/profile/bin/npm cache/cwd with allowlisted environment, isolated npm/git/XDG configuration, and no inherited credentials",
        network: "remote mode and package installation require network; no live baseline is run by ordinary validation",
...
async function runBenchmark(options) {
    ...
    try {
        ...
        const selectedRevision = await resolveRemoteRefRevision(options.ref, workspace, sourceEnv, cleanup);
        if (!selectedRevision)
            throw new Error(`unable to resolve selected ref ${options.ref}`);
        ...
        if (options.scenarios === "all" && options.upgradeFrom) {
            if (!upgradeRevision)
                throw new Error(`unable to resolve upgrade ref ${options.upgradeFrom}`);
            const beforeText = await readRemoteRefFile(upgradeRevision, "config/default-extensions.json", workspace, sourceEnv, cleanup);
            const afterText = await readRemoteRefFile(selectedRevision, "config/default-extensions.json", workspace, sourceEnv, cleanup);
            pinChange = compareBundledNpmPins(parseBundledNpmPins(beforeText), parseBundledNpmPins(afterText));
            if (!pinChange.changed) {
                throw new Error(`--upgrade-from ${options.upgradeFrom} and --ref ${options.ref} do not change a bundled npm pin`);
            }
        }
        const source = await prepareSource(options, root, workspace, sourceEnv, cleanup, selectedRevision);
        ...
        for (const scenario of selectedScenarios(options.scenarios)) {
            if (scenario === "warm-cache-fresh" && !warmCacheSeed) {
                warmCacheSeed = await createWarmCacheSeed(source.installScriptPath, root, sourceEnv, cleanup, selectedRevision);
            }
            for (let runNumber = 1; runNumber <= options.runs; runNumber += 1) {
                samples.push(await runScenarioSample(source.installScriptPath, options, scenario, runNumber, root, sourceEnv, cleanup, warmCacheSeed, selectedRevision, upgradeRevision));
            }
        }
        const firstPiVersion = samples.find((sample) => sample.piVersion)?.piVersion || null;
        const failureDiagnostics = samples.flatMap((sample) => sample.failureDiagnostics.length > 0
...
function printTextResult(result) {
    ...
    console.log(`selected ref: ${result.selectedRef.requested} (${result.selectedRef.resolved || "unresolved"})`);
    if (result.upgradeFromRef) {
        console.log(`upgrade-from ref: ${result.upgradeFromRef.requested} (${result.upgradeFromRef.resolved || "unresolved"})`);
    }
    ...
    console.log(`versions: node ${result.toolVersions.node}, npm ${result.toolVersions.npm || "unavailable"}, pi ${result.toolVersions.pi || "unavailable"}`);
    console.log("");
    ...
    const installer = summary.installerWallMs;
    const launch = summary.firstUsableLaunchMs;
    console.log("");
    console.log(`summary installer median: ${formatMs(typeof installer.median === "number" ? installer.median : null)}`);
    console.log(`summary first usable launch median: ${formatMs(typeof launch.median === "number" ? launch.median : null)}`);
    console.log("phase timings are approximate progress observations; unavailable phases are not treated as zero");
...
export { DEFAULT_MODE, DEFAULT_REF, DEFAULT_RUNS, DEFAULT_SCENARIOS, INSTALL_TIMEOUT_MS, LAUNCH_TIMEOUT_MS, REPOSITORY, assertOwnedWorkspacePath, benchmarkScope, buildChildEnvironment, createBenchmarkWorkspace, createCleanupController, createReadinessObserver, diagnoseReadinessFailure, main, runProcess, parseArgs, printTextResult, runBenchmark, summarizeSamples, observeReadinessChunk, selectedScenarios, scenarioCacheCondition, usage, writeCredentialFreeTrustMetadata, };
