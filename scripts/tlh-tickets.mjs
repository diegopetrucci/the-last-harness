...
import { backupPathWithTimestamp, defaultTlhSettingsPath, expandHomePath, readJsonFile, readOptionValue, resolveTlhAgentDir, } from "./lib/tlh-install-utils.mjs";
...
function assignCliOptionValue(args, key, argv, index, flags) {
...
function normalizedInstallPath(path) {
    ...
    if (typeof expandedPath !== "string" || !expandedPath)
        return undefined;
...
function managedTkPinIsFresh(settings, expectedSha256) {
    ...
    if (typeof recorded !== "string" || typeof expectedSha256 !== "string")
        return false;
...
function normalizeValidCandidate(candidate) {
    if (candidate === "tk")
        return commandPath("tk") || candidate;
    return candidate;
}
function findValidTk(args, settings, agentDir) {
    for (const candidate of candidateCommands(args, settings, agentDir)) {
        if (!hasTkCommandName(candidate))
            continue;
        if (validateTkCommand(candidate, agentDir))
...
function cleanupCreatedEmptyFile(fd, path) {
    ...
    if (!fdStats.isFile() || !pathStats.isFile() || pathStats.isSymbolicLink())
        return false;
    if (!sameFileStats(fdStats, pathStats))
        return false;
    if (fdStats.size !== 0 || pathStats.size !== 0)
        return false;
...
function listTarGzipEntries(archivePath, agentDir) {
    ...
    if (result.error)
        throw result.error;
    if (result.status !== 0)
        throw new Error("failed to list ticket source archive");
...
function ticketArchiveEntry(archivePath, preferredEntry, agentDir) {
    ...
    if (entries.includes(preferredEntry))
...
function extractTicketScript(archivePath, extractDir, preferredEntry, agentDir) {
    ...
    if (result.error)
        throw result.error;
    if (result.status !== 0)
        throw new Error("failed to extract ticket script from source archive");
    const extracted = join(extractDir, ...entry.split("/"));
    const stats = lstatIfExists(extracted);
    if (!stats)
        throw new Error("ticket script was not extracted from source archive");
    if (stats.isSymbolicLink())
        throw new Error("ticket script in source archive is a symlink");
    if (!stats.isFile())
        throw new Error("ticket script in source archive is not a file");
    return extracted;
}
...
function logWriteResult(args, writeResult) {
    ...
    if (writeResult === "unchanged")
        detailLog(args, "No settings changes were needed.");
}
function setTicketsEnabled(args, settingsPath, settings, previousRaw, installPath, installedSha256) {
    ...
    if (normalized)
        settings.tlh.tickets.installPath = normalized;
...
function validTkForEnable(args, settings, agentDir) {
    ...
    if (requested)
        return validatedRequestedInstallPath(args, agentDir, requested);
