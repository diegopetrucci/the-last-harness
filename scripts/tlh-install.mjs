...
function buildInstallConfig(parsedArgs, env = process.env) {
    ...
    return {
        ...
        rawBase: parsedArgs.rawBaseInput ||
...
async function mergeSettings(config) {
    ...
    if (!config.supportFilePaths.KEYBINDINGS_MERGE_SCRIPT ||
...
function installDefaultExtensions(config) {
    ...
    if (!config.supportFilePaths.TLH_DEFAULTS_SCRIPT ||
...
function npmPreinstallIo(config) {
    return {
        log: (message) => log(config, message),
...
function preInstallNpmDefaultExtensions(config) {
    preInstallNpmDefaultExtensionsImpl(config, npmPreinstallIo(config));
}
