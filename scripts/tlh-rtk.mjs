...
function expandResolvedPath(path) {
...
function normalizeValidCandidate(candidate, agentDir) {
    if (candidate === "rtk")
        return commandPath("rtk", agentDir) || candidate;
    return candidate;
}
...
function assertNoSymlinkedManagedTargetParents(target, boundary) {
    ...
    if (!isPathInsideOrEqual(parent, boundary))
        return;
    ...
    for (const created of [...createdDirs].reverse()) {
        try {
            ...
            if (stats.isSymbolicLink() || !stats.isDirectory())
                continue;
            if (!sameFileStats(stats, created.stats))
                continue;
...
function managedRtkRelease(args) {
    ...
    if (!version)
        throw new Error("RTK version is empty");
    if (!assetName)
        throw new Error("RTK asset name is empty");
    if (!isHexSha256(assetSha256))
        throw new Error(`RTK asset SHA256 must be a 64-character hex digest (got ${JSON.stringify(assetSha256)})`);
    if (!isHexSha256(binarySha256))
        throw new Error(`RTK binary SHA256 must be a 64-character hex digest (got ${JSON.stringify(binarySha256)})`);
    if (!downloadUrl || typeof downloadUrl !== "string")
        throw new Error("RTK download URL is empty");
    return {
        key,
        version,
        assetName,
...
function listTarGzipEntries(archivePath) {
    ...
    if (result.error)
        throw result.error;
    if (result.status !== 0)
        throw new Error("failed to list RTK release archive");
    return result.stdout.split(/\r?\n/).filter(Boolean);
}
function isSafeArchiveEntry(entry) {
    if (!entry || entry.startsWith("/") || isAbsolute(entry) || entry.includes("\\"))
        return false;
...
function extractRtkBinary(archivePath, extractDir) {
    ...
    if (result.error)
        throw result.error;
    if (result.status !== 0)
        throw new Error("failed to extract RTK binary from release archive");
    const extracted = join(extractDir, ...entry.split("/"));
    const stats = lstatIfExists(extracted);
    if (!stats)
        throw new Error("RTK binary was not extracted from release archive");
    if (stats.isSymbolicLink())
        throw new Error("RTK binary in release archive is a symlink");
    if (!stats.isFile())
        throw new Error("RTK binary in release archive is not a file");
    return extracted;
}
function validateRtkCommand(command, agentDir, release) {
    try {
        ...
        if (!resolvedCommand || !isExecutableFile(resolvedCommand))
            return false;
        if (sha256File(resolvedCommand).toLowerCase() !== release.binarySha256)
            return false;
        ...
        if (versionResult.error || versionResult.status !== 0)
            return false;
        if ((versionResult.stdout || "").trim() !== `rtk ${release.version}`)
            return false;
        ...
        if (rewriteResult.error || (rewriteResult.status !== 0 && rewriteResult.status !== 3))
            return false;
...
function findValidRtk(args, agentDir, release) {
    for (const candidate of candidateCommands(args, agentDir)) {
        if (validateRtkCommand(candidate, agentDir, release))
            return normalizeValidCandidate(candidate, agentDir);
    }
    return undefined;
}
async function installManagedRtk(args, agentDir) {
    ...
    if (!release)
        throw new Error(unsupportedRtkPlatformMessage());
    const target = plan.target;
    if (validateRtkCommand(target, agentDir, release)) {
        detailLog(args, `Managed RTK already satisfies pin: ${target}`);
        if (args.dryRun)
            logStderr(args, `Would keep pinned RTK ${release.version} in isolated profile: ${target}`);
        return target;
    }
    if (args.dryRun) {
    ...
    try {
        ...
        if (installTempDir)
            rmSync(installTempDir, { recursive: true, force: true });
        if (downloadTempDir)
