...
export interface RuntimeCompileCacheCommandOptions {
  cwd: string;
...
export interface ProfileCleanupIo {
  ...
  runCommand(commandArgs: readonly string[], options: RuntimeCompileCacheCommandOptions): void;
}
...
export function pruneAndPrewarmRuntimeCompileCache(
  ...
  if (isSymlink(runtimePrefix)) {
    io.warn(
      `refusing to prune runtime compile cache through symlinked runtime directory: ${runtimePrefix}`,
    );
    return;
  }
  ...
  if (isSymlink(cacheDir)) {
    io.warn(
      `refusing to traverse symlinked runtime compile-cache path: ${cacheDir}; remove the symlink (not its target) and rerun the installer/update`,
    );
    return;
  }
  if (!existsSync(cacheDir)) {
    io.detailLog(
      `${config.dryRun ? "Would prepare" : "No existing"} runtime compile-cache directory: ${cacheDir}`,
    );
  } else {
    let entries;
    try {
      ...
      io.warn(
        `could not inspect runtime compile-cache directory ${cacheDir}; leaving it untouched: ${error instanceof Error ? error.message : String(error)}`,
      );
      return;
    }

    const recognizedSymlink = entries.find(
      (entry) => entry.isSymbolicLink() && RUNTIME_COMPILE_CACHE_KEY_PATTERN.test(entry.name),
    );
    if (recognizedSymlink) {
      io.warn(
        `refusing to prune runtime compile-cache because recognized key entry is a symlink: ${join(cacheDir, recognizedSymlink.name)}; remove the symlink and rerun the installer/update`,
      );
      return;
    }
    ...
    for (const entry of entries) {
      ...
      if (!recognized || symlinked || !entry.isDirectory()) {
        io.warn(
          `preserving unexpected runtime compile-cache entry: ${entryPath}; only recognized Node/Bun version-key directories are pruned. Inspect it and remove it manually if it is not needed`,
        );
        continue;
      }
      if (config.dryRun) {
      ...
      try {
        ...
        io.warn(
          `could not remove runtime compile-cache key directory ${entryPath}; leaving it in place: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
  ...
  try {
    ...
    throw new Error(
      `runtime compile-cache pre-warming failed after primary install/update work completed; rerun the installer/update: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
