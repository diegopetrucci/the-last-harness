...
function validateMode(mode, label) {
  ...
  if ((mode & 0o022) !== 0) {
    throw new Error(
      `refusing to write ${label} with unsafe group/world-writable file mode: ${mode.toString(8)}`,
    );
  }
  return mode;
}
...
function relativeProfilePath(agentDir, targetPath, label) {
  ...
  if (resolvedTargetPath === resolvedAgentDir) {
    throw new Error(
      `refusing to write ${label} over the configured TLH profile directory: ${resolvedTargetPath}`,
    );
  }
...
function validatedPlanPaths(plan, label) {
  ...
  if (
    ...
    resolve(agentRoot) !== resolve(legacyAgentDir)
  ) {
  ...
  if (
    intendedAgentDir !== undefined &&
    resolve(intendedAgentDir) !== realpathForCompare(agentDir)
  ) {
    throw new Error(
      `refusing to write ${label} with mismatched compatibility plan intended profile root`,
    );
  }
  ...
  if (
    intendedTargetParent !== undefined &&
    resolve(intendedTargetParent) !== realpathForCompare(lexicalTargetParent)
  ) {
    throw new Error(
      `refusing to write ${label} with mismatched compatibility plan intended target parent`,
    );
  }
...
export function createSafeTlhProfileWritePlan({
  ...
  if (pathIsProtectedPiConfig(normalizedTargetPath, { homeDir, alreadyNormalized: true })) {
    throw new Error(
      `refusing to write ${label} under normal Pi config root: ${resolvedTargetPath}`,
    );
  }
