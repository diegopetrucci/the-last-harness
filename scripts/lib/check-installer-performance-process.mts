...
export type CleanupStop = () => Promise<void>;
...
export interface ProcessOptions {
  ...
  timeoutMs: number;
  onOutput?: (chunk: string, stream: OutputStream, elapsedMs: number) => boolean | void;
...
function waitForProcessTreeGone(child: ChildProcess, timeoutMs: number): Promise<boolean> {
...
export async function stopProcessTree(
  ...
  if (!processExited(child) || processGroupExists(child.pid)) {
    ...
    if (await waitForProcessTreeGone(child, PROCESS_TERM_GRACE_MS)) return;
...
export async function runProcess(
  ...
  const requestStop = (signal: NodeJS.Signals): Promise<void> => {
    if (!stopPromise) stopPromise = stopProcessTree(child, signal);
    return stopPromise;
  };
  const unregisterStop = options.registerStop?.(() => requestStop("SIGTERM"));

  const consume = (stream: OutputStream, chunk: Buffer | string): void => {
    const text = chunk.toString();
    if (stream === "stdout") stdout = appendCapture(stdout, text);
    else stderr = appendCapture(stderr, text);
    try {
  ...
  if (!stopPromise) stopPromise = stopProcessTree(child, "SIGTERM");
  try {
    await stopPromise;
  } finally {
    unregisterStop?.();
...
export function buildPtyCommand(commandParts: readonly string[]): {
  command: string;
  args: string[];
} {
