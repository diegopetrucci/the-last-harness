...
type RetiredSubagentInstall = { kind: "npm"; path: string; packageName: string } | { kind: "git"; path: string };
...
function packageManagerCommand(config: RetiredSubagentCleanupConfig): {
	...
	const configured =
		config.npmCommand ?? (config.settingsPath ? captureRetiredSubagentNpmCommand(config.settingsPath) : undefined);
 ...
 */
export function managedRetiredSubagentPackages(settings: unknown): RetiredSubagentPackageCandidate[] {
	...
	for (const entry of settings.packages) {
		...
		if (!RETIRED_TLH_SUBAGENTS_DEFAULT_PACKAGE_SOURCES.some((known) => packageIdentity(known) === identity)) continue;
		seen.add(identity);
...
export function captureManagedRetiredSubagentPackages(settingsPath: string): RetiredSubagentPackageCandidate[] {
...
export function defaultExtensionsRequireCriticalInstall(
	...
	try {
		...
		return (
			Array.isArray(defaults) && defaults.some((extension) => isPlainObject(extension) && extension.critical === true)
		);
	} catch {
		return false;
	}
