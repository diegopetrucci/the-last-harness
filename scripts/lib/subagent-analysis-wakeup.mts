...
import type { SessionScanResult } from "./session-analysis.mjs";
import {
  BACKGROUND_COMPLETION_NUDGE,
  CONTROL_NOTICE_NUDGE,
  SUBAGENT_ACTIONS,
  SUBAGENT_CONTROL_TYPE,
  SUBAGENT_NOTIFY_TYPE,
  TARGETING_ACTIONS,
  controlSourceKey,
  hasOwn,
  isCompletionBatchCandidate,
  isLegacyCompletionText,
  isLegacyControlText,
  isObject,
  MAX_COMPLETION_BATCH_STATES,
  parseCompletionBatchDetails,
  parseDirectSubagentControlEvent,
  parseLegacyUsage,
  parseNormalizedTelemetry,
  parseSubagentControlEvent,
  parsedCallOperation,
  safeMetadataString,
  safeOpaqueRunId,
  safeRunId,
  type ParsedCompletionBatch,
  type ParsedCompletionBatchEntry,
  syntheticNudgeKind,
...
} from "./subagent-analysis-aggregation.mjs";
import type { SubagentUsageTotals, SyntheticWakeupRecord } from "./subagent-analysis-types.mjs";

export interface WakeupCounters {
  ...
  completionDetailsMissing: number;
  controlDetailsMissing: number;
  wakeupsObserved: number;
  wakeupsAttributed: number;
  wakeupsUnattributed: number;
  wakeupsInterruptedByHumanInput: number;
  wakeupsMissingAssistantTurn: number;
  wakeupsMissingRunId: number;
  wakeupsMissingNudge: number;
  ...
  /** Number of flush-level tombstones evicted under the bounded state limit. */
...
export interface WakeupAnalysisContext {
  ensureRun(runId: string): MutableRun;
  getRun(runId: string): MutableRun | undefined;
  addTelemetry(raw: unknown, sourceKind: string): NormalizedTelemetry | undefined;
  addCompletionDetailsToRun(run: MutableRun, details: Record<string, unknown>): void;
  counters: WakeupCounters;
  wakeups: SyntheticWakeupRecord[];
  seenCompletionSources: Set<string>;
  seenControlSources: Set<string>;
}
...
type WakeSource = {
  ...
  key: string;
  runIds: string[];
  nudgeKind: "completion" | "control";
};

type WakeupRegistration = {
  record: SyntheticWakeupRecord;
  finalized: boolean;
};
...
type CompletionBatchState = {
  ...
  sawFinalTrigger: boolean;
  invalid: boolean;
  structurallyInvalid: boolean;
  mixedLegacy: boolean;
  sourceRegistered: boolean;
  wakeSourceKey?: string;
  wakeupRegistration?: WakeupRegistration;
  coverageFinalized: boolean;
};

/** Flush-level membership survives batch-state eviction as a tombstone. */
type CompletionFlushState = {
  flushId: string;
  flushCount: number;
  members: Map<number, CompletionBatchState>;
  invalid: boolean;
  sourceRegistered: boolean;
  wakeSourceKey?: string;
  wakeupRegistration?: WakeupRegistration;
...
type WakeState = {
  pendingSource?: WakeSource;
  activeWake?: ActiveWake;
  completionBatches: Map<string, CompletionBatchState>;
  completionFlushes: Map<string, CompletionFlushState>;
};

function emptyManagementCalls(): Map<string, number> {
  return new Map<string, number>();
}

function assistantUsage(message: Record<string, unknown>): SubagentUsageTotals | null {
  const usage = parseLegacyUsage(message.usage);
  return usage ? cloneSubagentUsage(usage) : null;
}

function managementCallsForWakeup(
  message: Record<string, unknown>,
  runIds: ReadonlySet<string>,
): Map<string, number> {
  const managementCalls = emptyManagementCalls();
  if (runIds.size === 0 || !Array.isArray(message.content)) return managementCalls;
  for (const rawItem of message.content) {
    if (!isObject(rawItem) || rawItem.type !== "toolCall") continue;
    const toolName =
      typeof rawItem.toolName === "string"
        ? rawItem.toolName
    ...
    if (!action || !SUBAGENT_ACTIONS.has(action)) continue;
    const target = TARGETING_ACTIONS.has(action) ? operation.targetRunId : undefined;
    if (!target || !runIds.has(target)) continue;
    managementCalls.set(action, (managementCalls.get(action) ?? 0) + 1);
  }
  return managementCalls;
}

function makeWakeupRecord(
  source: WakeSource,
  attributed: boolean,
  reason: SyntheticWakeupRecord["reason"],
  usage: SubagentUsageTotals | null,
  managementCalls: Map<string, number>,
): SyntheticWakeupRecord {
  const byAction = Object.fromEntries(
    [...managementCalls.entries()].sort(([a], [b]) => a.localeCompare(b)),
  );
...
function finishWakeup(
  context: WakeupAnalysisContext,
  active: ActiveWake,
  reason: SyntheticWakeupRecord["reason"],
  usage: SubagentUsageTotals | null = null,
  managementCalls = emptyManagementCalls(),
  attributed = false,
): void {
  const record = active.registration.record;
  if (!context.wakeups.includes(record) || active.registration.finalized) return;
  Object.assign(
    record,
    makeWakeupRecord(active.source, attributed, reason, usage, managementCalls),
  );
  active.registration.finalized = true;
  if (attributed) context.counters.wakeupsAttributed++;
  else context.counters.wakeupsUnattributed++;
  if (reason === "interrupted-by-human-input") context.counters.wakeupsInterruptedByHumanInput++;
  if (reason === "missing-assistant-turn") context.counters.wakeupsMissingAssistantTurn++;
  if (reason === "missing-run-id") context.counters.wakeupsMissingRunId++;
}

function abandonPendingSource(context: WakeupAnalysisContext, state: WakeState): void {
...
function registerSource(
  ...
  if (state.pendingSource) abandonPendingSource(context, state);
  state.pendingSource = source;
}

function detailsIndicateAsync(details: Record<string, unknown>): boolean {
  return (
...
function extractRunIdFromDetails(
...
function processStructuredControlEventArray(
  context: WakeupAnalysisContext,
  state: WakeState,
  events: readonly unknown[],
  source: SubagentExecution,
): void {
  ...
  for (let index = 0; index < limit; index++) {
    ...
    if (agent) run.roles.add(agent);
    if (!run.telemetry && parsed.source !== "unknown") run.execution = parsed.source;
  ...
  if (events.length > limit) context.counters.malformedControlDetails++;
}

function processStructuredControlEvents(
  context: WakeupAnalysisContext,
  state: WakeState,
  details: Record<string, unknown>,
): void {
  const telemetry = parseNormalizedTelemetry(details.telemetry);
  const source: SubagentExecution =
    telemetry?.run.execution ?? (detailsIndicateAsync(details) ? "async" : "foreground");
  if (hasOwn(details, "controlEvents")) {
    ...
    else context.counters.malformedControlDetails++;
  }
  if (!Array.isArray(details.results)) return;
  const limit = Math.min(details.results.length, 64);
  for (let index = 0; index < limit; index++) {
    const result = details.results[index];
    if (!isObject(result) || !hasOwn(result, "controlEvents")) continue;
    if (Array.isArray(result.controlEvents))
      processStructuredControlEventArray(context, state, result.controlEvents, source);
    else context.counters.malformedControlDetails++;
  }
  if (details.results.length > limit) context.counters.malformedControlDetails++;
}

function completionBatchComplete(batch: CompletionBatchState): boolean {
  if (batch.invalid || batch.chunks.size !== batch.batchCount) return false;
  for (let index = 0; index < batch.batchCount; index++) {
    if (!batch.chunks.has(index)) return false;
  }
  return true;
}

function completionChunkDigest(parsed: ParsedCompletionBatch): string {
  return createHash("sha256").update(JSON.stringify(parsed)).digest("hex");
}

function completionFlushSourceKey(flushId: string): string {
  return tupleKey("completion-flush", flushId);
}

function completeCompletionFlush(
  state: WakeState,
  finalBatch: CompletionBatchState,
): CompletionBatchState[] | undefined {
  if (
    finalBatch.flushIndex >= MAX_COMPLETION_BATCH_STATES ||
    finalBatch.flushIndex !== finalBatch.flushCount - 1
  )
    return undefined;
  const flush = state.completionFlushes.get(finalBatch.flushId);
  if (!flush || flush.invalid || flush.flushCount !== finalBatch.flushCount) return undefined;
  if (flush.members.size !== finalBatch.flushCount) return undefined;

  const ordered: CompletionBatchState[] = [];
...
function processCompletionBatchEntry(
  context: WakeupAnalysisContext,
  batch: CompletionBatchState,
  entry: ParsedCompletionBatchEntry,
): boolean {
  ...
  if (invalid && !telemetry) return true;
  ...
  if (!runId && !invalid && entry.asyncId) {
    ...
    const run = context.ensureRun(runId);
    if (entry.agent) run.roles.add(entry.agent);
    run.sourceKinds.add("completion-batch");
  }
  if (runId) batch.runIds.add(runId);
  return invalid;
}

function revokeWakeup(context: WakeupAnalysisContext, registration: WakeupRegistration): void {
  ...
  if (registration.record.attributed) context.counters.wakeupsAttributed--;
  else context.counters.wakeupsUnattributed--;
  ...
  if (registration.record.reason === "missing-run-id") context.counters.wakeupsMissingRunId--;
}

function invalidateRegisteredFlushWakeup(
  context: WakeupAnalysisContext,
  state: WakeState,
  flush: CompletionFlushState,
  revokeActive = false,
): void {
  const sourceKey = flush.wakeSourceKey;
  if (!flush.sourceRegistered || !sourceKey) return;
  ...
  for (const candidate of flush.members.values()) {
    if (candidate.wakeSourceKey !== sourceKey) continue;
    candidate.wakeupRegistration = undefined;
    candidate.sourceRegistered = false;
  }
  if (registration && (!activeWake || revokeActive)) revokeWakeup(context, registration);
}

function invalidateRegisteredBatchWakeup(
  context: WakeupAnalysisContext,
  state: WakeState,
  batch: CompletionBatchState,
): void {
  const sourceKey = batch.wakeSourceKey;
  if (!batch.sourceRegistered || !sourceKey) return;
...
function invalidateCompletionFlush(
  context: WakeupAnalysisContext,
  state: WakeState,
  flush: CompletionFlushState,
  _structural = false,
...
function invalidateCompletionBatch(
  context: WakeupAnalysisContext,
  state: WakeState,
  batch: CompletionBatchState,
  structural = false,
  ...
  batch.invalid = true;
  if (structural) batch.structurallyInvalid = true;
  invalidateRegisteredBatchWakeup(context, state, batch);
}

function finalizeCompletionBatch(
  context: WakeupAnalysisContext,
  batch: CompletionBatchState,
): void {
  if (batch.coverageFinalized) return;
  let missing = 0;
  for (let index = 0; index < batch.batchCount; index++) {
    if (!batch.chunks.has(index)) missing++;
  }
  ...
  if (batch.mixedLegacy) context.counters.completionBatchesMixedLegacy++;
  batch.coverageFinalized = true;
}

function evictOldestCompletionBatch(context: WakeupAnalysisContext, state: WakeState): void {
  let candidate: [string, CompletionBatchState] | undefined;
...
function evictOldestCompletionFlush(context: WakeupAnalysisContext, state: WakeState): void {
...
function processCompletionBatchNotification(
  context: WakeupAnalysisContext,
  state: WakeState,
  ...
  if (batch && batch.flushId !== parsed.flushId) {
    ...
    if (previousFlush) invalidateCompletionFlush(context, state, previousFlush, true);
    else invalidateCompletionBatch(context, state, batch, true);
    context.counters.completionBatchChunksInvalid++;
    context.counters.malformedCompletionDetails++;
    return;
  }
  ...
  if (!flush) {
    ...
    if (batch || context.seenCompletionSources.has(sourceKey)) {
      if (batch) invalidateCompletionBatch(context, state, batch, true);
      context.counters.completionBatchChunksInvalid++;
      context.counters.malformedCompletionDetails++;
      return;
    }
  ...
  const memberForBatch = [...flush.members.values()].find(
    (candidate) => candidate.batchId === parsed.batchId,
  );
  if (
    (memberAtPosition && memberAtPosition.batchId !== parsed.batchId) ||
  ...
  if (
    batch.batchCount !== parsed.batchCount ||
  ...
  if (parsed.batchIndex === parsed.batchCount - 1) batch.sawFinalChunk = true;
...
function processCompletionNotification(
  ...
  if (!details) {
    ...
    } else context.counters.malformedCompletionDetails++;
    return;
  }

  const runId = telemetry?.run.id ?? extractRunIdFromDetails(details, true);
  const sourceExecution: SubagentExecution =
    telemetry?.run.execution ??
  ...
  const statusValid =
    details.status === "completed" || details.status === "failed" || details.status === "paused";
  const agentValid = details.agent === undefined || safeMetadataString(details.agent) !== undefined;
  if (!statusValid || !agentValid) context.counters.malformedCompletionDetails++;
  ...
  const completionEvidenceValid =
    telemetry !== undefined || (statusValid && details.agent !== undefined && agentValid);
...
function processControlNotification(
  context: WakeupAnalysisContext,
  state: WakeState,
  scanResult: SessionScanResult,
  entryIndex: number,
  ...
  const details = isObject(message.details) ? message.details : undefined;
  const telemetry =
    details && hasOwn(details, "telemetry")
  ...
  const effectiveSource =
    parsed.source === "unknown" ? (telemetry?.run.execution ?? run.execution) : parsed.source;
...
function handleNudge(
  ...
  state.pendingSource = undefined;
  context.counters.wakeupsObserved++;
  const registration: WakeupRegistration = {
    record: makeWakeupRecord(source, false, undefined, null, emptyManagementCalls()),
    finalized: false,
  };
  context.wakeups.push(registration.record);
  for (const flush of state.completionFlushes.values()) {
    if (flush.wakeSourceKey === source.key) flush.wakeupRegistration = registration;
  }
  for (const batch of state.completionBatches.values()) {
    if (batch.wakeSourceKey === source.key) batch.wakeupRegistration = registration;
  }
...
function processOrdinaryUserMessage(context: WakeupAnalysisContext, state: WakeState): void {
  if (state.activeWake) {
...
function processAssistantMessage(
  ...
  if (state.activeWake) {
    ...
    else finishWakeup(context, state.activeWake, undefined, usage, managementCalls, true);
    state.activeWake = undefined;
  }
  if (state.pendingSource) abandonPendingSource(context, state);
}

function processWakeupEntry(
  context: WakeupAnalysisContext,
  state: WakeState,
  scanResult: SessionScanResult,
  entryIndex: number,
  pairedSubagentCallIds: ReadonlySet<string>,
): void {
  ...
  const nudgeKind =
    syntheticNudgeKind(message.role, text) ??
  ...
  if (message.role === "user") processOrdinaryUserMessage(context, state);
  else if (message.role === "assistant") processAssistantMessage(context, state, message);
}
...
export function analyzeWakeupsForScan(
  scanResult: SessionScanResult,
  context: WakeupAnalysisContext,
): void {
  ...
  const pairedSubagentCallIds = new Set(
  ...
  if (state.pendingSource) abandonPendingSource(context, state);
  for (const batch of state.completionBatches.values()) finalizeCompletionBatch(context, batch);
}
