...
export interface InstallerPathConfig {
  agentDir: string;
  binDir?: string;
  wrapperPath?: string;
  wrapperName?: string;
...
export interface InstallerPathOptions {
  homeDir?: string;
  alreadyNormalized?: boolean;
  validUpdateTracks?: readonly string[];
}
...
export { DEFAULT_VALID_UPDATE_TRACKS };
...
export function pathIsProtectedPiConfig(
  path: string,
  { homeDir = homedir(), alreadyNormalized = false }: InstallerPathOptions = {},
): boolean {
  ...
  return (
    pathWithinOrEqual(normalPiRoot, normalizedPath) ||
    pathWithinOrEqual(normalPiAgentRoot, normalizedPath)
  );
}

export function validateInstallerTargets(
  config: InstallerPathConfig,
  {
    homeDir = homedir(),
    validUpdateTracks = DEFAULT_VALID_UPDATE_TRACKS,
  }: InstallerPathOptions = {},
): void {
  ...
  if (!config.binDir || !config.wrapperPath) {
    throw new Error("installer target validation requires binDir and wrapperPath");
  }
  ...
  if (pathIsProtectedPiConfig(normalizedAgent, { homeDir, alreadyNormalized: true })) {
    throw new Error(
      `refusing to place The Last Harness agent dir under normal Pi config root: ${config.agentDir}`,
    );
  }
  if (pathIsProtectedPiConfig(normalizedBin, { homeDir, alreadyNormalized: true })) {
    throw new Error(
      `refusing to place The Last Harness wrapper dir under normal Pi config root: ${config.binDir}`,
    );
  }
  if (pathIsProtectedPiConfig(normalizedWrapper, { homeDir, alreadyNormalized: true })) {
    throw new Error(
      `refusing to place The Last Harness wrapper under normal Pi config root: ${config.wrapperPath}`,
    );
  }
}
...
export function validateProfileRelativePath(
  relativePath: string,
  label = "TLH profile path",
): void {
...
export function assertProfilePathWithinAgent(
  config: InstallerPathConfig,
  path: string,
  label = "TLH profile path",
  { homeDir = homedir() }: InstallerPathOptions = {},
): void {
  const normalizedAgent = realpathForCompare(config.agentDir);
...
export function ensureSafeProfileDir(
  config: InstallerPathConfig,
  relativePath: string,
  label = "TLH profile directory",
...
export function safeProfileFileTarget(
  config: InstallerPathConfig,
  relativePath: string,
...
export function copySafeProfileFile(
  config: InstallerPathConfig,
  source: string,
  relativePath: string,
  label = "TLH profile file",
...
export function assertSafeSettingsTarget(
  ...
  if (isSymlink(config.settingsPath)) {
    throw new Error(
      `refusing to let Pi write through symlinked isolated settings file: ${config.settingsPath}`,
    );
  }
  if (existsSync(config.settingsPath) && !lstatSync(config.settingsPath).isFile()) {
    throw new Error(
      `refusing to let Pi replace non-file isolated settings path: ${config.settingsPath}`,
    );
  }
  ...
  if (existsSync(config.settingsPath) && lstatSync(config.settingsPath).nlink !== 1) {
    throw new Error(
      `refusing to let Pi mutate hard-linked isolated settings file: ${config.settingsPath}`,
    );
  }
