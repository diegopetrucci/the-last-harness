...
import {
  ...
  OUTCOME_STATES,
  TERMINATION_REASONS,
  isStrictTelemetryEnvelope,
} from "./subagent-analysis-validation.mjs";
...
export type RuntimeTelemetryEnvelope = {
  schemaVersion: number;
  run: { id: unknown; execution: unknown; mode: unknown };
...
/** Inputs accepted by the canonical runtime parallel-outcome resolver. */
...
const {
  ...
  resolveParallelSubagentTelemetryOutcome: resolveRuntimeParallelSubagentTelemetryOutcome,
} = require("../../extensions/subagents/src/shared/telemetry.js") as {
  normalizeSubagentRunTelemetry(value: unknown): RuntimeTelemetryEnvelope | undefined;
...
export function resolveParallelTelemetryOutcome(
  inputs: readonly ParallelTelemetryOutcomeInput[],
): TelemetryOutcome | undefined {
...
export const BACKGROUND_COMPLETION_NUDGE =
...
export interface ParsedCompletionBatchEntry {
  agent?: string;
  ...
  telemetry?: unknown;
  /** True when this entry contains malformed fields but bounded sibling evidence remains usable. */
  invalid: boolean;
}

export interface ParsedCompletionBatch {
  batchId: string;
  batchIndex: number;
  batchCount: number;
  flushId: string;
  ...
  completions: ParsedCompletionBatchEntry[];
  /** Nested entry errors invalidate the wakeup without discarding valid siblings. */
  invalid: boolean;
}
...
export function safeMetadataString(value: unknown, maxLength = 128): string | undefined {
...
export function safeOpaqueString(value: unknown, maxLength = 512): string | undefined {
  if (typeof value !== "string") return undefined;
...
export function safeRunId(value: unknown): string | undefined {
...
export function safeOpaqueRunId(value: unknown): string | undefined {
  return safeOpaqueString(value, 512);
}

/** Project optional overall-flush metadata without retaining its raw identity. */
export function projectCompletionBatchFlushFields(value: Record<string, unknown>): {
  ...
  if (!hasFlushId && !hasFlushIndex && !hasFlushCount) return { value: {}, invalid: false };
...
export function isCompletionBatchCandidate(value: unknown): boolean {
  if (!isObject(value)) return false;
  return (
    value.kind === SUBAGENT_COMPLETION_BATCH_KIND ||
    ["schemaVersion", "batchId", "batchIndex", "batchCount", "triggersTurn", "completions"].some(
...
export function parseCompletionBatchDetails(value: unknown): ParsedCompletionBatch | undefined {
  if (!isObject(value) || value.__projectionInvalid === true) return undefined;
  if (
  ...
  let invalid = false;
  const completions: ParsedCompletionBatchEntry[] = [];
  for (const rawEntry of value.completions) {
    ...
    const status =
      ...
      rawEntry.status === "paused"
        ? rawEntry.status
        : undefined;
    if (status === undefined) entryInvalid = true;
    const durationMs =
      rawEntry.durationMs === undefined
        ? undefined
        : finiteNonNegativeNumber(rawEntry.durationMs)
          ? rawEntry.durationMs
          : undefined;
    if (rawEntry.durationMs !== undefined && durationMs === undefined) entryInvalid = true;
    const asyncId =
      rawEntry.asyncId === undefined
        ? undefined
        : safeOpaqueString(rawEntry.asyncId, MAX_COMPLETION_BATCH_ID_LENGTH);
    if (rawEntry.asyncId !== undefined && asyncId === undefined) entryInvalid = true;
...
export function safeStepIndex(value: unknown): number | undefined {
  return typeof value === "number" && Number.isSafeInteger(value) && value >= 0 ? value : undefined;
}

export function parseTelemetryModel(value: unknown): TelemetryModel | undefined {
...
export function parseTelemetryUsage(value: unknown): TelemetryUsage | undefined {
  if (!isObject(value)) return undefined;
  if (
...
export function parseLegacyUsage(value: unknown): TelemetryUsage | undefined {
  if (!isObject(value)) return undefined;
  const hasTelemetryCounters =
  ...
  if (hasTelemetryCounters) return parseTelemetryUsage(value);
  if (
    !finiteNonNegativeNumber(value.input) ||
...
export function parseTelemetryTiming(value: unknown): TelemetryTiming | undefined {
  if (!isObject(value)) return undefined;
  ...
  if (
    (startedAt !== undefined && !finiteNonNegativeNumber(startedAt)) ||
...
function allowedSetString(value: unknown, allowed: ReadonlySet<string>): string | undefined {
  return typeof value === "string" && allowed.has(value) ? value : undefined;
}

export function parseTelemetryOutcome(value: unknown): TelemetryOutcome | undefined {
...
export function parseLegacyOutcome(value: Record<string, unknown>): TelemetryOutcome | undefined {
  ...
  if (
    ...
    state = "completed";
  else if (statusValue && OUTCOME_STATES.has(statusValue)) state = statusValue;
  else if (stateValue && OUTCOME_STATES.has(stateValue)) state = stateValue;
  else if (value.terminationReason === "cancelled") state = "cancelled";
  else if (value.timedOut === true || value.terminationReason === "timed_out") state = "failed";
  else if (value.interrupted === true || value.terminationReason === "interrupted")
    state = "paused";
  else if (value.success === true || value.exitCode === 0) state = "completed";
...
export function parseLegacyModel(value: Record<string, unknown>): TelemetryModel | undefined {
...
export function parseLegacyStepTiming(value: Record<string, unknown>): LegacyStep["timing"] {
...
export function parseNormalizedTelemetry(value: unknown): NormalizedTelemetry | undefined {
  if (isObject(value) && value.__projectionInvalid === true) return undefined;
  if (!isStrictTelemetryEnvelope(value)) return undefined;
  let normalized: RuntimeTelemetryEnvelope | undefined;
  try {
  ...
  for (const rawStep of candidate.steps) {
    if (!isObject(rawStep)) return undefined;
    ...
    const outcome =
      rawStep.outcome === undefined ? undefined : parseTelemetryOutcome(rawStep.outcome);
    steps.push({
      index,
      agent,
      ...(model ? { model } : {}),
  ...
  const timing =
    candidate.timing === undefined ? undefined : parseTelemetryTiming(candidate.timing);
  const outcome =
    candidate.outcome === undefined ? undefined : parseTelemetryOutcome(candidate.outcome);
...
export function parseTelemetryLineage(
  value: unknown,
  childRunId: string,
): TelemetryLineage | undefined {
  if (!isObject(value)) return undefined;
  const continuationFrom = isObject(value.continuationFrom)
    ...
    : undefined;
  const continuations: Array<{ sourceStepIndex?: number; continuationRunId: string }> = [];
  if (Array.isArray(value.continuations)) {
    for (const entry of value.continuations) {
      ...
      if (!continuationRunId) continue;
      const sourceStepIndex =
        entry.sourceStepIndex === undefined ? undefined : safeStepIndex(entry.sourceStepIndex);
      if (entry.sourceStepIndex !== undefined && sourceStepIndex === undefined) continue;
      if (
        continuations.some(
          (candidate) =>
            candidate.continuationRunId === continuationRunId &&
  ...
  let nested: TelemetryLineage["nested"] | undefined;
  if (isObject(value.nested)) {
    ...
    if (rootRunId && parentRunId) {
      ...
      const depth =
        value.nested.depth === undefined ? undefined : safeStepIndex(value.nested.depth);
      nested = {
        rootRunId,
        parentRunId,
        childRunId,
  ...
  if (!continuationFrom && continuations.length === 0 && !nested) return undefined;
  return {
...
export function mergeTelemetryLineage(
  older: TelemetryLineage | undefined,
  newer: TelemetryLineage | undefined,
): TelemetryLineage | undefined {
  if (!older && !newer) return undefined;
  const continuationFrom = newer?.continuationFrom ?? older?.continuationFrom;
  const continuations = [...(older?.continuations ?? []), ...(newer?.continuations ?? [])].filter(
    (entry, index, all) =>
      all.findIndex(
        (candidate) =>
...
export function mergeTelemetryTiming(
  older: TelemetryTiming | undefined,
  newer: TelemetryTiming | undefined,
...
export function mergeTelemetrySnapshots(
  older: NormalizedTelemetry,
  newer: NormalizedTelemetry,
): NormalizedTelemetry {
  const stepsByIndex = new Map<number, TelemetryStep>();
  for (const step of older.steps) stepsByIndex.set(step.index, { ...step });
  for (const step of newer.steps) {
    ...
    if (step.usage && !step.model) delete mergedStep.model;
...
export type ParsedCallArguments = {
  record?: Record<string, unknown>;
  malformed: boolean;
};

export type ParsedCallOperation = {
  action?: string;
  hasAction: boolean;
...
export function parseCallArguments(item: Record<string, unknown>): ParsedCallArguments {
  const raw = hasOwn(item, "arguments") ? item.arguments : item.args;
  if (raw === undefined) return { malformed: false };
  if (isObject(raw)) return { record: raw, malformed: false };
...
export function parsedCallOperation(item: Record<string, unknown>): ParsedCallOperation {
  const empty: ParsedCallOperation = {
    hasAction: false,
    actionUnknown: false,
  ...
  if (isObject(item.operation)) {
    return {
      ...
      roles: Array.isArray(item.operation.roles)
        ? item.operation.roles.filter(
            (role): role is string => safeMetadataString(role) !== undefined,
          )
        : [],
...
export function isLegacyCompletionText(value: unknown): boolean {
...
export function isLegacyControlText(value: unknown): boolean {
  const text = messageText(value);
  return text !== undefined && /^Subagent(?: run)? needs attention\b/.test(text);
}

export function syntheticNudgeKind(
  role: unknown,
  text: string | undefined,
): "completion" | "control" | undefined {
  if (role !== "user") return undefined;
  if (text === BACKGROUND_COMPLETION_NUDGE) return "completion";
  if (text === CONTROL_NOTICE_NUDGE) return "control";
  return undefined;
}

export function parseDirectSubagentControlEvent(
  value: unknown,
  source: SubagentExecution,
): { runId: string; source: SubagentExecution; event: Record<string, unknown> } | undefined {
  if (!isObject(value)) return undefined;
  ...
  const retiredTransition =
    value.type === "active_long_running" &&
    value.to === "active_long_running" &&
    value.reason === "time_threshold";
  if (!runId || (!currentTransition && !retiredTransition)) return undefined;
  if (
    !finiteNonNegativeNumber(value.ts) ||
    ...
    return undefined;
  if (value.index !== undefined && safeStepIndex(value.index) === undefined) return undefined;
  return { runId, source, event: value };
}

export function parseSubagentControlEvent(
  details: Record<string, unknown>,
): { runId: string; source: SubagentExecution; event: Record<string, unknown> } | undefined {
  const source =
    details.source === "foreground" || details.source === "async"
      ? details.source
...
export function controlSourceKey(event: Record<string, unknown>, runId: string): string {
  return tupleKey(
    "control",
    runId,
    finiteNonNegativeNumber(event.ts) ? event.ts : undefined,
    event.index === undefined ? undefined : safeStepIndex(event.index),
    safeMetadataString(event.reason),
    safeMetadataString(event.contextPressureSeverity),
    safeMetadataString(event.contextPressureThreshold),
    safeMetadataString(event.idleEpisodeId),
  );
}

export function telemetryExecutionFromRaw(value: unknown): SubagentExecution {
  if (!isObject(value) || !isObject(value.run)) return "unknown";
