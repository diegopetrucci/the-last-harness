...
export function resolveTlhAgentDir(
  agentDir: string | undefined,
  options: TlhPathOptions = {},
): string {
  return (
...
export function defaultTlhKeybindingsPath({
  ...
  return join(
    resolveTlhAgentDir(agentDir, { env, homeDir, preferTlhAgentDir }),
    "keybindings.json",
  );
}
...
export function defaultTlhBinDir(
  ...
  return (
    expandHomePath(env.TLH_BIN_DIR || join(homeDir, ".local", "bin"), { homeDir }) ||
...
export function readRegularFileForBackup(
  path: string,
  label: string,
  ...
  try {
    ...
    if (
      !sameBackupSourceIdentity(backupSourceIdentity(openedStats), backupSourceIdentity(pathStats))
    ) {
...
export function pathIsInNormalPiConfig(
  ...
  }: { homeDir?: string; alreadyNormalized?: boolean } = {},
): boolean {
