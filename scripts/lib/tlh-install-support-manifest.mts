...
export interface SupportFileDescriptor {
	variable: string;
	requirement: SupportFileRequirement;
	relativePath: string;
	tempPath: string;
	installName: string;
}
...
const REQUIRED: SupportFileRequirement = "required";
const OPTIONAL: SupportFileRequirement = "optional";

const BASE_SUPPORT_FILES = Object.freeze<readonly SupportFileDescriptor[]>([
	{
...
function cloneSupportFile(file: SupportFileDescriptor): SupportFileDescriptor {
	return { ...file };
}

export function supportFileManifest({ noSettings = false }: SupportFileManifestOptions = {}): SupportFileDescriptor[] {
...
export function requiredSupportFiles(options: SupportFileManifestOptions = {}): SupportFileDescriptor[] {
	return supportFileManifest(options).filter((file) => file.requirement === REQUIRED);
}

export function installableSupportFiles(options: SupportFileManifestOptions = {}): SupportFileDescriptor[] {
