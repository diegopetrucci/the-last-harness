...
function warnMissingOptionalSupportFile(
  ...
  if (variable === "TLH_UPDATE_SCRIPT") {
    ...
    callWarn(
      `tlh wrapper support script not found for ref ${config.ref}; wrapper creation will be unavailable`,
      io,
    );
    ...
    callWarn(
      `optional installer support file not found for ref ${config.ref}: ${relativePath}`,
      io,
    );
  }
}
  ...
  for (const file of config.supportFiles) {
    ...
    try {
      ...
      if (file.requirement === "required") {
        throw new Error(
          `required installer support file not found for ref ${config.ref}: ${file.relativePath}`,
        );
      }
...
export async function preflightRuntimeSupportFiles(
  config: SupportFilesConfig,
  io: SupportFilesIo = {},
): Promise<void> {
  ...
  if (missing.length > 0) {
    throw new Error(
      `required installer support files not found for ref ${config.ref}: ${missing.join(" ")}`,
    );
  }
}
