 ...
 */
function resolveGuardedProfilePath(agentDir, relativePath, label, io) {
...
function cleanupRelativeProfileFiles(config, relativePaths, io) {
    for (const relativePath of relativePaths) {
        ...
        const target = resolveGuardedProfilePath(config.agentDir, relativePath, "retired profile file cleanup", io);
        if (target === null)
            continue;
        try {
        ...
        try {
            ...
            io.warn(`failed to remove retired profile file ${target}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
export function cleanupLegacyManagedProfileArtifacts(config, io) {
...
export function cleanupOldSettingsBackups(config, io) {
    ...
    try {
        ...
        io.warn(`Skipping stale settings backup cleanup: cannot read agentDir: ${error instanceof Error ? error.message : String(error)}`);
        return;
    }
    ...
    for (const filename of toDelete) {
        ...
        try {
            ...
            io.warn(`failed to remove stale settings backup ${target}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
export function backupExistingSettingsBeforePiInstall(config, io) {
...
function retiredSourceIsOnDisk(source, agentDir, io) {
    ...
    const target = resolveGuardedProfilePath(agentDir, relativePath, "retired extension residue probe", io);
    if (target === null)
        return false;
    if (isSymlink(target))
        return false;
...
export function reclaimRetiredExtensionResidues(config, io) {
    ...
    for (const source of FORCE_REMOVED_RETIRED_DEFAULT_EXTENSION_SOURCES) {
        ...
        if (!config.dryRun) {
            ...
            if (retiredSourceIsOnDisk(source, config.agentDir, io)) {
                io.warn(`failed to remove retired extension residue ${source}: residue still present after pi remove`);
            }
            else {
