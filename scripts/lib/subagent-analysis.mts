...
import { aggregateCoverage, type ScanCoverage } from "./session-analysis-coverage.mjs";
...
import {
  ...
  collectRunLineage,
  createAggregate,
  createRunCoverageBucket,
  createTelemetryCoverageBucket,
  effectiveRunSteps,
  emptySubagentUsage,
  groupMapToObject,
  incrementAggregateMap,
  modelKey,
  outcomeForRun,
  runContinuation,
  runRuntime,
...
function parseSubagentToolResultDetails(
  message: Record<string, unknown>,
): Record<string, unknown> | undefined {
  return isObject(message.details) ? message.details : undefined;
}
function telemetryExecutionFromRaw(value: unknown): SubagentExecution {
...
function addLegacyDetailsToRun(
  run: MutableRun,
  details: Record<string, unknown>,
  sourceKind: string,
): void {
  ...
  if (mode === "single" || mode === "parallel" || mode === "management") run.mode = mode;
  if (detailsIndicateAsync(details)) run.execution = "async";
  const topModel = parseLegacyModel(details);
  if (topModel) run.models.set(modelKey(topModel), topModel);
  const topOutcome = parseLegacyOutcome(details);
  if (topOutcome) run.legacyOutcome = topOutcome;
  ...
  for (let index = 0; index < limit; index++) {
    ...
    if (role) run.roles.add(role);
    if (model) run.models.set(modelKey(model), model);
  }
}

function addCompletionDetailsToRun(run: MutableRun, details: Record<string, unknown>): void {
  addLegacyDetailsToRun(run, details, "completion");
  const agent = safeMetadataString(details.agent);
  if (agent) run.roles.add(agent);
}

function extractRunIdFromDetails(details: Record<string, unknown> | undefined): string | undefined {
...
function addLaunchRoleHints(run: MutableRun, operation: ParsedCallOperation): void {
  for (const role of operation.roles) run.roles.add(role);
}
 ...
 */
export function analyzeSubagentSessions(
  scanResults: readonly SessionScanResult[],
  baseCoverage: ScanCoverage = aggregateCoverage(scanResults),
  options: { publicRunId?: (runId: string) => string } = {},
): SubagentAnalysisOutput {
  const runs = new Map<string, MutableRun>();
  const telemetrySourceIdentities = new Set<string>();
  const telemetryBuckets: Record<SubagentExecution, SubagentTelemetryCoverageBucket> = {
    foreground: createTelemetryCoverageBucket(),
    async: createTelemetryCoverageBucket(),
    unknown: createTelemetryCoverageBucket(),
  };
  const runBuckets: Record<SubagentExecution, SubagentRunCoverageBucket> = {
  ...
  const operationLaunches: SubagentOperationCounts = {
    total: 0,
    foreground: 0,
    async: 0,
    unknown: 0,
  ...
  const operationManagementByAction = new Map<string, number>();
  const managementTargetRunIds: string[] = [];
  const continuationEdges = new Map<string, ContinuationEdge>();
  const nestedEdges = new Map<string, NestedEdge>();
  const wakeups: SyntheticWakeupRecord[] = [];
  const seenCompletionSources = new Set<string>();
  const seenControlSources = new Set<string>();

  function ensureRun(runId: string): MutableRun {
    ...
    const created: MutableRun = {
      ...
      models: new Map<string, TelemetryModel>(),
      legacySteps: new Map<number, LegacyStep>(),
  ...
  function addTelemetry(raw: unknown, sourceKind: string): NormalizedTelemetry | undefined {
    ...
    telemetryBuckets[normalized.run.execution].stepsWithUsage += normalized.steps.filter(
      (step) => step.usage !== undefined,
    ).length;
    ...
    for (const identity of identityParts) telemetrySourceIdentities.add(identity);
    ...
    for (const step of normalized.steps) {
      ...
      if (step.model) run.models.set(modelKey(step.model), step.model);
    }
    return normalized;
  }

  function applyRunHints(
    run: MutableRun,
  ...
  function processToolResult(message: Record<string, unknown>): void {
  ...
  function processToolCalls(
    ...
    resultByCallId: Map<string, RawToolResult>,
  ): void {
    ...
    for (let entryIndex = 0; entryIndex < entries.length; entryIndex++) {
      ...
      for (let itemIndex = 0; itemIndex < message.content.length; itemIndex++) {
        ...
        const callId =
          typeof rawItem.toolCallId === "string"
            ? rawItem.toolCallId
        ...
        const malformedOperation =
          operation.malformedArguments || operation.asyncInvalid || operation.actionMalformed;
        ...
        if (operation.hasAction) {
          ...
          const normalizedAction =
            operation.action && SUBAGENT_ACTIONS.has(operation.action)
              ? operation.action
              : "unknown";
          operationManagementByAction.set(
            normalizedAction,
            (operationManagementByAction.get(normalizedAction) ?? 0) + 1,
          );
        ...
        if (result) operationLaunches.withResult++;
        else operationLaunches.withoutResult++;
        const telemetry =
          details && hasOwn(details, "telemetry")
            ? parseNormalizedTelemetry(details.telemetry)
            : undefined;
        const runId = telemetry?.run.id ?? extractRunIdFromDetails(details);
        if (runId) operationLaunches.withRunId++;
        else {
          operationLaunches.withoutRunId++;
          counters.launchesWithoutRunId++;
        }
        let execution: SubagentExecution = "unknown";
        if (telemetry) execution = telemetry.run.execution;
        else if (operation.async === true) execution = "async";
        else if (operation.async === false) execution = "foreground";
        else if (details && detailsIndicateAsync(details)) execution = "async";
        ...
        if (runId) {
          ...
          if (!run.telemetry) {
            ...
            if (operation.tasksPresent) run.mode = "parallel";
            else if (operation.agentPresent) run.mode = "single";
  ...
  function parseResultMaps(scanResult: SessionScanResult): Map<string, RawToolResult> {
    const resultByCallId = new Map<string, RawToolResult>();
    ...
    for (const entry of scanResult.entries) {
      ...
      if (!toolCallId || !pairedSubagentCallIds.has(toolCallId)) continue;
      ...
      if (message.toolName !== undefined && message.toolName !== "subagent") continue;
      const subagentMessage =
        message.toolName === "subagent" ? message : { ...message, toolName: "subagent" };
  ...
  const runSummaries: SubagentRunSummary[] = [];
  const byRole = new Map<string, SubagentAggregate>();
  const byModel = new Map<string, SubagentAggregate>();
  const byExecution = new Map<string, SubagentAggregate>();
  const byOutcome = new Map<string, SubagentAggregate>();
  const totalUsage = emptySubagentUsage();
  const totalRuntime: SubagentRuntimeTotals = { durationMs: 0, activeRuntimeMs: 0 };
  let totalSteps = 0;
  ...
  for (const run of runs.values()) {
    ...
    if (runtime.durationMs !== null || runtime.activeRuntimeMs !== null) {
      if (runtime.durationMs !== null) totalRuntime.durationMs += runtime.durationMs;
      if (runtime.activeRuntimeMs !== null) totalRuntime.activeRuntimeMs += runtime.activeRuntimeMs;
    }
  ...
  const roleRunSets = new Map<string, Set<string>>();
  const modelRunSets = new Map<string, Set<string>>();
  const roleRuntimeRunSets = new Map<string, Set<string>>();
  const modelRuntimeRunSets = new Map<string, Set<string>>();
  for (const summary of runSummaries) {
    const runHasRuntime =
      summary.runtime.durationMs !== null || summary.runtime.activeRuntimeMs !== null;
    for (const role of summary.roles) {
      const roleRuns = roleRunSets.get(role) ?? new Set<string>();
      roleRuns.add(summary.runId);
      roleRunSets.set(role, roleRuns);
      if (runHasRuntime) {
        const roleRuntimeRuns = roleRuntimeRunSets.get(role) ?? new Set<string>();
    ...
    for (const model of summary.models) {
      ...
      const modelRuns = modelRunSets.get(key) ?? new Set<string>();
      modelRuns.add(summary.runId);
      modelRunSets.set(key, modelRuns);
      if (runHasRuntime) {
        const modelRuntimeRuns = modelRuntimeRunSets.get(key) ?? new Set<string>();
        modelRuntimeRuns.add(summary.runId);
        modelRuntimeRunSets.set(key, modelRuntimeRuns);
      }
    }
    for (const step of summary.steps) {
      const roleRuns = roleRunSets.get(step.role) ?? new Set<string>();
      roleRuns.add(summary.runId);
      ...
      if (
        ...
        const roleRuntimeRuns = roleRuntimeRunSets.get(step.role) ?? new Set<string>();
        roleRuntimeRuns.add(summary.runId);
  ...
  const publicRunId = (runId: string): string => {
  ...
  const publicContinuation = (
    continuation: SubagentRunSummary["continuation"],
  ): SubagentRunSummary["continuation"] | undefined => {
  ...
  for (const execution of ["foreground", "async", "unknown"] as const) {
    const bucket = runBuckets[execution];
    for (const run of runSummaries.filter((candidate) => candidate.execution === execution)) {
      bucket.runs++;
      if (run.usage) bucket.withUsage++;
      else bucket.withoutUsage++;
      if (run.outcome) bucket.withOutcome++;
      else bucket.withoutOutcome++;
      if (run.runtime.durationMs !== null || run.runtime.activeRuntimeMs !== null)
        bucket.withRuntime++;
      else bucket.withoutRuntime++;
      if (run.telemetryRecordCount > 0) bucket.withTelemetry++;
      else bucket.withoutTelemetry++;
    }
  }
  for (const execution of ["foreground", "async", "unknown"] as const) {
    const telemetryRuns = [...runs.values()].filter(
      (run) => run.execution === execution && run.telemetryRecordCount > 0,
    );
    telemetryBuckets[execution].runs = telemetryRuns.length;
    telemetryBuckets[execution].runsWithUsage = telemetryRuns.filter(
      (run) =>
        run.telemetry?.usage !== undefined ||
        run.telemetry?.steps.some((step) => step.usage !== undefined),
    ).length;
  }
  ...
  const lineageOutput = {
    continuationEdges: [...continuationEdges.values()].sort((a, b) =>
      tupleKey(a.sourceRunId, a.continuationRunId, a.sourceStepIndex).localeCompare(
        tupleKey(b.sourceRunId, b.continuationRunId, b.sourceStepIndex),
      ),
    ),
    nestedEdges: [...nestedEdges.values()].sort((a, b) =>
      tupleKey(a.rootRunId, a.parentRunId, a.childRunId, a.parentStepIndex).localeCompare(
        tupleKey(b.rootRunId, b.parentRunId, b.childRunId, b.parentStepIndex),
      ),
    ),
  };

  const coverage: SubagentCoverage = {
    ...baseCoverage,
    ...
    missingEvidence: {
      ...
      usageForStep: runSummaries.reduce(
        (total, run) => total + run.steps.filter((step) => step.usage === undefined).length,
        0,
      ),
      runtimeForRun: runSummaries.filter(
        (run) => run.runtime.durationMs === null && run.runtime.activeRuntimeMs === null,
      ).length,
  ...
  counters.unresolvedManagementTargets = managementTargetRunIds.filter(
    (target) => !runs.has(target),
  ).length;
    ...
    operations: {
      ...
      management: {
        ...
        byAction: Object.fromEntries(
          [...operationManagementByAction.entries()].sort(([a], [b]) => a.localeCompare(b)),
        ),
        withTarget: counters.managementTargets,
        targetingKnownRun: Math.max(
          0,
          counters.managementTargets - counters.unresolvedManagementTargets,
        ),
