...
import { chmodSync, existsSync, lstatSync, mkdtempSync, readFileSync, renameSync, rmSync, unlinkSync, writeFileSync, } from "node:fs";
import { tmpdir } from "node:os";
import { basename, dirname, join, relative, resolve } from "node:path";
import { criticalGitSourceSpec } from "./tlh-install-package-source.mjs";
import { assertProfilePathWithinAgent, isSymlink, pathWithinOrEqual, realpathForCompare, } from "./tlh-install-paths.mjs";
...
function runCommandInDir(config, dir, commandArgs, io) {
...
function logVerbose(config, message, io) {
    if (config.verbose && !config.quiet && typeof io.log === "function")
        io.log(config, message);
}
...
function emitInstrumentationEvent(io, event) {
    try {
...
function dependencyInputsFromPackageJson(pkgJson) {
    if (!isJsonRecord(pkgJson))
        return undefined;
    const inputs = {};
    for (const key of [
        "dependencies",
        ...
        if (!isJsonRecord(value))
            return undefined;
        inputs[key] = stableJsonValue(value);
    }
    return inputs;
}
function readDependencyInputs(targetDir) {
    try {
        return dependencyInputsFromPackageJson(JSON.parse(readFileSync(join(targetDir, "package.json"), "utf8")));
    }
    catch {
        return undefined;
    }
}
function dependencyInputsEqual(left, right) {
    if (!isJsonRecord(left))
        return false;
    return JSON.stringify(stableJsonValue(left)) === JSON.stringify(stableJsonValue(right));
}
function readNpmInstallMarker(markerPath, head, dependencyInputs) {
    if (!dependencyInputs)
        return false;
    try {
        ...
        if (!isJsonRecord(parsed))
            return false;
        return (parsed.schemaVersion === NPM_INSTALL_MARKER_SCHEMA_VERSION &&
            parsed.head === head &&
 ...
 */
function isSafeDependencyDirectory(nodeModulesDir, dependencyPath) {
    try {
        ...
        return (pathWithinOrEqual(resolvedNodeModulesDir, resolvedDependencyPath) &&
...
function allDirectDepsPresent(targetDir) {
    ...
    if (!isRegularDirectory(nodeModulesDir))
        return false;
...
function directDependencyCount(targetDir) {
    const inputs = readDependencyInputs(targetDir);
    return inputs ? Object.keys(inputs.dependencies).length : 0;
}
function symlinkedMarkerError(action, markerPath) {
    return new Error(`refusing to ${action} symlinked npm install marker: ${markerPath}; remove the symlink (not its target) and rerun the installer`);
}
...
function writeNpmInstallMarker(markerPath, head, dependencyInputs) {
...
function gitEnvironment(extraEnv = {}) {
...
function indexPathForCheckout(config, targetDir, label, io) {
    const rawPath = gitOutput(config, targetDir, ["rev-parse", "--git-path", "index"], io);
    if (!rawPath)
        throw new Error(`could not resolve ${label} Git index path`);
...
function snapshotRealIndex(config, targetDir, label, io) {
...
function assertSafeIndexParent(config, indexPath, label) {
    assertProfilePathWithinAgent(config, indexPath, `${label} Git index`);
    ...
    while (pathWithinOrEqual(agentRoot, current)) {
        ...
        if (stats.isSymbolicLink()) {
            throw new Error(`refusing to restore ${label} Git index through a symlinked directory: ${current}`);
        }
        if (!stats.isDirectory()) {
            throw new Error(`refusing to restore ${label} Git index through a non-directory: ${current}`);
        }
        if (current === agentRoot)
            return parentDir;
...
function restoreRealIndex(config, preparation, force = false) {
    if (!preparation)
        return;
    ...
    const restoreTempPath = join(parentDir, `.${basename(snapshot.path)}.tlh-restore-${randomUUID()}.tmp`);
    try {
        if (snapshot.exists) {
...
function gitOutput(config, targetDir, args, io = {}, env = {}) {
...
function gitSucceeds(config, targetDir, args, io = {}, env = {}) {
    const spawnCapture = io.spawnCapture || defaultSpawnCapture;
...
function cleanCheckout(config, targetDir, io, env = {}) {
    runGitCommand(config, ["git", "-C", targetDir, "clean", "-fd", "-e", ROOT_NODE_MODULES_CLEAN_EXCLUDE], io, { env });
}
function backupRefs(config, targetDir, io = {}) {
    const spawnCapture = io.spawnCapture || defaultSpawnCapture;
    const result = spawnCapture(config, ["git", "-C", targetDir, "for-each-ref", "refs/tlh-backup", "--format=%(refname)"], { allowFailure: true, env: gitEnvironment() });
...
function collisionSafeBackupRef(config, targetDir, kind, io) {
...
function malformedCheckoutError(targetDir, label, refs) {
    if (refs.length > 0) {
        return new Error(`refusing destructive repair of malformed ${label}: ${targetDir}; preserve existing TLH backup refs (${refs.join(", ")}) and repair it manually; no checkout changes were made`);
    }
    return new Error(`unable to safely prepare malformed ${label}: ${targetDir}; no TLH backup refs were found. Repair the Git index or checkout manually, then rerun; no checkout changes were made`);
}
function malformedCheckoutPreparation(config, targetDir, label, io) {
    const refs = backupRefs(config, targetDir, io);
    if (refs.length > 0)
        throw malformedCheckoutError(targetDir, label, refs);
    if (!config.quiet) {
        warn(`unable to safely prepare malformed ${label}: ${targetDir}; no TLH backup refs were found. Repair the Git index or checkout manually, then rerun; no checkout changes were made`, io);
    }
...
function normalizeGitOrigin(config, targetDir, repo, io) {
    if (!repo)
        return;
...
function createAlternateIndex(config, targetDir, label, head, io) {
...
function backupDirtyCheckout(config, targetDir, head, indexFile, io) {
    ...
    const commit = gitOutput(config, targetDir, [
        "-c",
    ...
    if (!config.quiet) {
        ...
        warn(`  recover worktree changes: git -C ${targetDir} diff ${head} ${backupRef} | git -C ${targetDir} apply`, io);
    }
...
function backupPrePiIndex(config, targetDir, head, snapshot, label, io) {
    if (!snapshot?.exists || !snapshot.content || snapshot.mode === undefined)
        return undefined;
    ...
    try {
        ...
        if (!config.quiet) {
            warn(`pre-Pi staged index for ${targetDir} differed from ${head}; backed up to ${backupRef}`, io);
            warn(`  inspect: git -C ${targetDir} show ${backupRef}`, io);
...
function checkoutEnvironment(preparation) {
    return preparation?.indexFile ? { GIT_INDEX_FILE: preparation.indexFile } : {};
}
function currentCheckoutHead(config, targetDir, io) {
...
function synchronizeRealIndexToHead(config, targetDir, head, label, io) {
    const indexPath = indexPathForCheckout(config, targetDir, label, io);
 ...
 */
export function prepareGitCheckout(config, options, io = {}) {
    ...
    if (config.dryRun) {
        ...
        printDryRunCommand(["git", "-C", targetDir, "clean", "-fd", "-e", ROOT_NODE_MODULES_CLEAN_EXCLUDE], io);
    ...
    let gitDir;
    try {
        gitDir = realpathForCompare(gitOutput(config, targetDir, ["rev-parse", "--absolute-git-dir"], io));
    }
    catch {
    ...
    return {
        ...
        head,
        markerPath,
        markerValid: readNpmInstallMarker(markerPath, head, dependencyInputs) && directDependenciesPresent,
...
function assertPreparationStillTargetsSameRepository(config, preparation, targetDir, label, io) {
    if (!preparation)
        return;
    ...
    if (!preparation.gitDir)
        return;
    const currentGitDir = realpathForCompare(gitOutput(config, targetDir, ["rev-parse", "--absolute-git-dir"], io));
 ...
 */
export function finalizeGitCheckout(config, options, preparation, io = {}) {
    const { targetDir, repo, label, missingMessage, warnOnMissing = false } = options;
    if (preparation?.consumed) {
        throw new Error(`checkout preparation for ${label} has already been consumed; call prepareGitCheckout again before finalizing`);
    }
    ...
    try {
        if (!config.dryRun && preparation?.status === "malformed") {
            throw new Error(`refusing to finalize malformed ${label}: ${targetDir}; repair the Git index or checkout manually, then rerun; no checkout changes were made`);
        }
        if (config.dryRun) {
            ...
            printDryRunCommand(["git", "-C", targetDir, "reset", "--hard", "HEAD"], io);
            printDryRunCommand(["git", "-C", targetDir, "clean", "-fd", "-e", ROOT_NODE_MODULES_CLEAN_EXCLUDE], io);
            logDryRun(config, "Would validate the Pi-selected HEAD and expected origin while preserving root node_modules.", io);
            return true;
        }
        if (!safeGitCheckoutDirForMutation(config, targetDir, label, io)) {
            if (warnOnMissing) {
        ...
        const head = gitOutput(config, targetDir, ["rev-parse", "--verify", "HEAD^{commit}"], io, indexEnv);
        if (repo) {
            const actualOrigin = gitOutput(config, targetDir, ["remote", "get-url", "origin"], io);
            if (actualOrigin !== repo) {
                throw new Error(`refusing to finalize ${label} with unexpected origin ${actualOrigin}; expected ${repo}`);
            }
        }
        ...
        cleanCheckout(config, targetDir, io, indexEnv);
        const finalHead = gitOutput(config, targetDir, ["rev-parse", "--verify", "HEAD^{commit}"], io, indexEnv);
        if (finalHead !== head) {
...
export function cleanupGitCheckoutPreparation(preparation) {
    if (!preparation)
        return;
    preparation.consumed = true;
    if (!preparation.indexFile)
    ...
    if (basename(preparation.indexFile) !== "index" ||
        !basename(tempDir).startsWith("tlh-git-index-") ||
        dirname(tempDir) !== tmpdir())
        return;
    try {
        if (lstatSync(tempDir).isSymbolicLink() || !lstatSync(tempDir).isDirectory())
            return;
    }
    catch {
        return;
...
function dependencyInstallCommand(config) {
...
function runDependencyInstall(config, targetDir, io) {
    runCommandInDir(config, targetDir, dependencyInstallCommand(config), io);
}
function persistNpmInstallMarker(config, targetDir, label, head, io) {
    const dependencyInputs = readDependencyInputs(targetDir);
    if (!dependencyInputs) {
...
function instrumentManagedCheckoutIo(io, counts) {
    const originalRunCommand = io.runCommand;
    const originalRunInDir = io.runInDir;
    return {
        ...io,
        runCommand: (config, commandArgs, options) => {
            if (commandArgs[0] === "git" && commandArgs.includes("fetch"))
                counts.tlhGitFetches += 1;
 ...
 */
export function installManagedGitCheckout(config, options, runPi, io = {}) {
    let preparation;
    let failure;
    const commandCounts = {
        tlhGitFetches: 0,
    ...
    try {
        ...
        if (preparation.status === "malformed") {
            throw new Error(`refusing to install into malformed ${options.label}: ${options.targetDir}; repair the Git index or checkout manually, then rerun`);
        }
        let piFailure;
        ...
        if (headChanged) {
            if (!priorHead)
                throw new Error(`could not resolve pre-Pi HEAD for ${options.label}`);
            ...
            backupPrePiIndex(config, options.targetDir, priorHead, preparation.realIndexSnapshot, options.label, managedIo);
            ...
            synchronizeRealIndexToHead(config, options.targetDir, finalizedHead, options.label, managedIo);
    ...
    if (failure) {
        try {
            ...
            failure = new Error(`${String(failure)}; additionally failed to restore the real Git index: ${String(error)}`, { cause: new AggregateError([failure, error]) });
        }
    }
    cleanupGitCheckoutPreparation(preparation);
    if (failure)
...
export function reconcileGitCheckoutDependencies(config, options, preparation, io = {}) {
    const { targetDir, label, missingMessage, warnOnMissing = false } = options;
    if (config.dryRun) {
        logDryRun(config, `Would run ${dependencyInstallCommand(config).join(" ")} when the managed checkout dependency marker is invalid.`, io);
        return true;
    }
    if (!safeGitCheckoutDirForMutation(config, targetDir, label, io)) {
        if (warnOnMissing) {
    ...
    const currentMarkerValid = readNpmInstallMarker(markerPath, head, dependencyInputs) && directDependenciesPresent;
    ...
    const piCouldHaveRepaired = sameHead &&
    ...
    if (sameHead && !currentMarkerValid)
        needsRepair = true;
    if (sameHead && configuredCustomManager && !piCouldHaveRepaired)
        needsRepair = true;
    if (piCouldHaveRepaired) {
        ...
        logVerbose(config, `${label}: Pi repaired the missing direct dependencies; skipping a duplicate package-manager pass`, io);
    }
    if (needsRepair) {
        emitInstrumentationEvent(io, {
            type: "tlh-repair",
        ...
        logVerbose(config, `${label}: dependency marker or direct dependency state is incomplete; running ${dependencyInstallCommand(config).join(" ")}`, io);
        try {
    ...
    if (!finalInputs || !allDirectDepsPresent(targetDir)) {
        throw new Error(`package-manager install did not produce a complete ${label} dependency tree; rerun the installer with --verbose`);
    }
    try {
        persistNpmInstallMarker(config, targetDir, label, head, io);
    }
    catch (error) {
        if (String(error).includes("symlinked npm install marker"))
            throw error;
...
export function refreshGitPackageSource(config, { packageSource, packageRoot, ref, packageSourceIsDefault, }, io = {}) {
    ...
    if (!packageSourceIsDefault && packageSpec?.ref) {
        logVerbose(config, "Pinned custom git package source was refreshed directly; skipping pi update.", io);
    }
    return true;
}
export function refreshGitCheckout(config, { targetDir, repo, ref, label, missingMessage, warnOnMissing = false, }, io = {}) {
    if (config.dryRun) {
        ...
        logDryRun(config, `Would run ${dependencyInstallCommand(config).join(" ")} if package.json is present and the clean unchanged-checkout marker is not valid.`, io);
        return true;
    }
    if (!safeGitCheckoutDirForMutation(config, targetDir, label, io)) {
        if (warnOnMissing) {
    ...
    if (statusOutput !== "") {
        ...
        if (!config.quiet) {
            warn(`dirty checkout at ${targetDir} — worktree changes backed up to ${backupRef}`, io);
            warn(`  inspect: git -C ${targetDir} show ${backupRef}`, io);
            warn(`  recover worktree changes: git -C ${targetDir} diff ${parentOrEmpty} ${backupRef} | git -C ${targetDir} apply`, io);
        }
        if (config.verbose && !config.quiet) {
