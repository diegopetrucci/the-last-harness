...
export interface NpmPreinstallConfig extends InstallerPathConfig {
  agentDir: string;
  dryRun: boolean;
  noSettings: boolean;
...
export interface NpmPreinstallIo {
...
function readNpmPreinstallSettings(
  config: NpmPreinstallConfig,
  io: NpmPreinstallIo,
...
function ensureNpmStageParent(config: NpmPreinstallConfig): string {
...
function markNpmStageIgnoredByCloudSync(
  config: NpmPreinstallConfig,
  stagePath: string,
  io: NpmPreinstallIo,
): void {
  ...
  for (const attribute of attributes) {
    try {
      spawnSync(
        ...
        { env: io.inheritedCommandEnv(), stdio: "ignore" },
      );
    } catch {
...
function assertNpmStageDirectory(
  config: NpmPreinstallConfig,
  stagePath: string,
  phase: string,
): void {
...
function promoteNpmStage(stagePath: string, npmRoot: string, io: NpmPreinstallIo): boolean {
 ...
 */
export function preInstallNpmDefaultExtensions(
  config: NpmPreinstallConfig,
  io: NpmPreinstallIo,
): void {
  ...
  const settings =
    settingsExists || !config.dryRun ? readNpmPreinstallSettings(config, io) : undefined;
  ...
  try {
    ...
    if (promoteNpmStage(stagePath, npmRoot, io)) stagePath = undefined;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    io.warn(
