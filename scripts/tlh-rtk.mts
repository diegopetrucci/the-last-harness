#!/usr/bin/env node
import { accessSync, closeSync, constants, existsSync, fchmodSync, lstatSync, mkdirSync, mkdtempSync, openSync, readFileSync, realpathSync, renameSync, rmSync, statSync, writeFileSync, type Stats } from "node:fs";
...
interface CliArgs extends Record<string, unknown> {
	...
	rtkRepo: string;
	rtkVersion: string;
	unsafeTestDownloadUrl?: string;
	unsafeTestAssetName?: string;
	unsafeTestAssetSha256?: string;
	unsafeTestBinarySha256?: string;
	command?: string;
	commandArgs: string[];
	dryRun: boolean;
	detail: boolean;
...
interface ManagedRtkRelease {
	key: keyof typeof PINNED_RTK_RELEASES;
	version: string;
	assetName: string;
	assetSha256: string;
	binarySha256: string;
...
function expandResolvedPath(path: string): string {
	return resolve(expandHomePath(path) ?? path);
}
...
function safeHelperEnv(extraEnv: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
	return {
		...process.env,
		...extraEnv,
...
function commandPath(command: string, agentDir: string): string | undefined {
	return resolveCommandFromPath(command, helperEnv(agentDir).PATH);
}

function safeHelperCommandPath(command: string): string {
...
function normalizeValidCandidate(candidate: string, agentDir: string): string {
...
function isUnderNormalPiConfig(path: string): boolean {
...
function assertManagedRtkAgentRootSafe({ agentRoot, intendedAgentDir }: Pick<ManagedRtkPlan, "agentRoot" | "intendedAgentDir">): void {
	const stats = lstatIfExists(agentRoot);
...
function managedRtkTargetPath(args: Pick<CliArgs, "target">, agentDir: string): string {
	const agentRoot = expandResolvedPath(agentDir);
	return expandResolvedPath(args.target || join(agentRoot, "bin", "rtk"));
}

function managedRtkTargetPlan(args: Pick<CliArgs, "target">, agentDir: string): ManagedRtkPlan {
	const agentRoot = expandResolvedPath(agentDir);
...
function validateManagedRtkTarget(args: Pick<CliArgs, "target">, agentDir: string): string {
	return managedRtkTargetPlan(args, agentDir).target;
}

function assertManagedRtkTargetParentSafe({ targetParent, intendedAgentDir, intendedTargetParent }: Pick<ManagedRtkPlan, "targetParent" | "intendedAgentDir" | "intendedTargetParent">): void {
...
function assertManagedRtkTempPath(path: string, plan: ManagedRtkPlan, label: string, { mustExist = false, expectDirectory = false, expectFile = false }: { mustExist?: boolean; expectDirectory?: boolean; expectFile?: boolean } = {}): void {
...
function createManagedRtkTempTarget(plan: ManagedRtkPlan): { tempDir: string; tempTarget: string } {
...
function copyFileExclusive(source: string, target: string, mode: number): void {
	let fd;
	try {
		const noFollowFlag = typeof constants.O_NOFOLLOW === "number" ? constants.O_NOFOLLOW : 0;
...
function platformKey(platform: NodeJS.Platform = process.platform, arch: string = process.arch): keyof typeof PINNED_RTK_RELEASES | undefined {
	if (platform === "darwin") {
...
function unsupportedRtkPlatformMessage(): string {
...
function managedRtkRelease(args: Pick<CliArgs, "rtkRepo" | "rtkVersion" | "unsafeTestAssetName" | "unsafeTestAssetSha256" | "unsafeTestBinarySha256" | "unsafeTestDownloadUrl">): ManagedRtkRelease | undefined {
...
async function fetchWithTimeout(url: string, options: RequestInit = {}): Promise<Response> {
	...
	headers.set("User-Agent", "tlh-rtk-installer");
...
function verifyRtkArchive(release: ManagedRtkRelease, archivePath: string): void {
...
function listTarGzipEntries(archivePath: string): string[] {
...
function isRtkBinaryEntry(entry: string): boolean {
	return isSafeArchiveEntry(entry) && !entry.endsWith("/") && basename(entry) === "rtk";
}

function rtkArchiveEntry(archivePath: string): string {
...
function extractRtkBinary(archivePath: string, extractDir: string): string {
	const tarPath = safeHelperCommandPath("tar");
...
function validateRtkCommand(command: string, agentDir: string, release: ManagedRtkRelease): boolean {
	try {
...
function candidateCommands(args: Pick<CliArgs, "target">, agentDir: string): string[] {
...
function findValidRtk(args: Pick<CliArgs, "target">, agentDir: string, release: ManagedRtkRelease): string | undefined {
...
async function installManagedRtk(args: CliArgs, agentDir: string): Promise<string | undefined> {
...
function commandValidate(args: CliArgs, agentDir: string): void {
	const release = managedRtkRelease(args);
	if (!release) {
		process.exitCode = 1;
		return;
