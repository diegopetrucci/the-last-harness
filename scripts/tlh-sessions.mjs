 ...
 */
const TIMING_QUALITY_NOTE = "observedLatencyMs is wall-clock between recorded events; includes queueing, subprocess startup, streaming, supervisor pauses, and in-tool retries. It is not execution time.";
 ...
 * expected prefix.
 */
function projectLabelFromPath(filePath, sessionsDir) {
...
function buildProvenance(agentDir, sessionsDir, includePaths, profileSource) {
    const provenance = {
...
// Per-tool rollup
// ---------------------------------------------------------------------------
function buildPerToolOutput(scanResults, coverage, sessionsDir, agentDir, includePaths, profileSource) {
