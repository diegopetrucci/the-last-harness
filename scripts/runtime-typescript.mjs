...
function resolveConfiguredTargets(repoRoot) {
  const configuredTargetIds = (
    process.env.TLH_RUNTIME_TYPESCRIPT_TARGETS ?? "scripts,extensions,subagents"
  )
    .split(",")
  ...
  if (configuredTargetIds.length === 0) {
    throw new Error(
      "TLH_RUNTIME_TYPESCRIPT_TARGETS must include at least one of: scripts, extensions, subagents.",
    );
  }
  ...
  return uniqueTargetIds.map((targetId) => {
    if (targetId === "scripts") {
      return {
        ...
        sourceDir: realpathSync(
          resolveOverridePath(
            process.env.TLH_RUNTIME_TYPESCRIPT_SCRIPTS_DIR,
            join(repoRoot, "scripts"),
          ),
        ),
        tsconfigPath: realpathSync(
          resolveOverridePath(
            process.env.TLH_RUNTIME_TYPESCRIPT_SCRIPTS_TSCONFIG ??
              process.env.TLH_RUNTIME_TYPESCRIPT_TSCONFIG,
    ...
    return {
      ...
      sourceDir: realpathSync(
        resolveOverridePath(
          process.env.TLH_RUNTIME_TYPESCRIPT_EXTENSIONS_DIR,
...
function loadGeneratedOutputRegistry(repoRoot) {
  ...
  if (!existsSync(registryPath)) {
    throw new Error(
      `Missing generated output registry: ${relativeRepoPath(registryPath, repoRoot)}`,
    );
  }
...
function loadConfig() {
  const repoRoot = realpathSync(
    resolveOverridePath(process.env.TLH_RUNTIME_TYPESCRIPT_REPO_ROOT, defaultRepoRoot),
  );
  return {
    repoRoot,
    tempParentDir: realpathSync(
      resolveOverridePath(process.env.TLH_RUNTIME_TYPESCRIPT_TMPDIR, systemTmpDir()),
    ),
...
function listRuntimeTypescriptSources(dir, sourceExtension, excludedSourceDirs = []) {
  ...
  for (const entry of entries) {
    ...
    if (
      excludedSourceDirs.some((excludedSourceDir) =>
        pathWithinOrEqual(excludedSourceDir, entryPath),
      )
    )
      continue;
...
function spawnTsc(config, target, args, stdio = "inherit") {
  ...
  const result = spawnSync(
    process.execPath,
    [tscEntrypoint, "--project", target.tsconfigPath, ...args],
    {
      cwd: config.repoRoot,
      stdio,
    },
...
function compileTargetToTemp(config, target) {
  const tempOutDir = mkdtempSync(
    join(config.tempParentDir, `tlh-runtime-typescript-${target.id}-`),
  );
...
function generatedOutputPathForSource(config, target, tempOutDir, sourcePath) {
  const outputPath = runtimeOutputPathForSource(
    sourcePath,
...
function sourcePathForRuntimeOutput(repoRoot, target, relativeOutputPath) {
  return join(
    repoRoot,
    relativeOutputPath.slice(0, -target.outputExtension.length) + target.sourceExtension,
  );
}
...
function listRegisteredTargetOutputs(config, target) {
  ...
  return [...config.generatedOutputRegistry]
    .filter(
      (relativePath) =>
        relativePath.startsWith(targetRootPrefix) && relativePath.endsWith(target.outputExtension),
    )
    .filter(
      (relativePath) =>
...
function inspectGeneratedOutputRegistry(config, target, sources) {
  ...
  const unregisteredOutputs = expectedOutputPaths.filter(
    (outputPath) => !registeredOutputSet.has(outputPath),
  );
  ...
  const shippingOrphanOutputs = orphanOutputPaths.filter((outputPath) =>
    existsSync(join(config.repoRoot, outputPath)),
  );
...
function main() {
  ...
  const targetsWithSources = config.targets
    .map((target) => ({
      target,
      sources: listRuntimeTypescriptSources(
        target.sourceDir,
        target.sourceExtension,
        target.excludedSourceDirs,
      ),
    }))
