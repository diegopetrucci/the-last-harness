...
function applyAnthropicWarningOnEnable(settings) {
    const warnings = settings.warnings;
    if (warnings !== undefined && !isPlainObject(warnings))
        return false;
...
function main() {
    ...
    if (args.command === "disable") {
        ...
        if (!id || args.commandArgs.length !== 1)
            throw new Error("Usage: tlh defaults disable <id>");
        ...
        if (warningChanged)
            console.log("Restored upstream warnings.anthropicExtraUsage default (warning will reappear).");
        if (backupPath)
            console.log(`Backed up previous settings to: ${backupPath}`);
        if (!changed)
    ...
    if (args.command === "enable") {
        ...
        if (warningChanged)
            console.log("Suppressed warnings.anthropicExtraUsage (tlh default).");
        if (backupPath)
            console.log(`Backed up previous settings to: ${backupPath}`);
        if (!changed)
            console.log("No settings changes were needed.");
