...
function expandHomePath(pathValue, options = {}) {
...
function realpathForCompare(pathValue) {
...
function resolvePlan(state, args) {
    ...
    if (!track || !isValidTrack(track)) {
        throw new Error(`Could not determine update track from ${installStatePath(args.agentDir)}. Re-run the installer from a working The Last Harness checkout, or pass --track latest-release, --track pinned-tag, or --track ref.`);
    }
    if (track === "custom") {
...
function installManagedRtk(args, env) {
    if (!managedRtkSupportedPlatform()) {
        return;
    }
    ...
    if (args.dryRun) {
        commandArgs.push("--dry-run", "--detail");
    }
    else if (args.verbose) {
        commandArgs.push("--detail");
    }
    if (args.quiet) {
