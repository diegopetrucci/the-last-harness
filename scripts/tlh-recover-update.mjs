...
function assignOptionValue(target, key, argv, index, flags) {
...
function expandHomePath(path, { homeDir = process.env.HOME || homedir() } = {}) {
...
function defaultTlhAgentDir(env = process.env, { homeDir = env.HOME || homedir() } = {}) {
...
function defaultTlhBinDir(env = process.env, { homeDir = env.HOME || homedir() } = {}) {
	return expandHomePath(env.TLH_BIN_DIR || join(homeDir, ".local", "bin"), { homeDir });
}

function parseArgs(argv) {
	const args = {
		agentDir: defaultTlhAgentDir(process.env),
...
function pathIsProtectedPiConfig(path, { homeDir = process.env.HOME || homedir(), alreadyNormalized = false } = {}) {
...
function normalizeState(raw) {
	...
	const repo = typeof raw.repo === "string" && raw.repo.trim() ? raw.repo.trim() : undefined;
	const track = typeof raw.track === "string" && raw.track.trim() ? raw.track.trim() : undefined;
	const ref = typeof raw.ref === "string" && raw.ref.trim() ? raw.ref.trim() : undefined;
	const packageSource = typeof raw.packageSource === "string" && raw.packageSource.trim() ? raw.packageSource.trim() : undefined;
...
function loadState(args) {
	...
	try {
		...
		if (!args.quiet) {
			console.error(`warning: ${path} does not contain usable The Last Harness update metadata; ignoring it`);
		}
	} catch (error) {
		if (!args.quiet) {
...
function assertRecoveryTargetSafe(agentDir) {
