...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    const agentDirIndex = assignOptionValue(args, "agentDir", argv, index, "--agent-dir") as
      | number
      | undefined;
    ...
    const packageRootIndex = assignOptionValue(
      args,
      "packageRoot",
      argv,
      index,
...
function categorizeSettingsDriftLine(change: string): {
  category: string;
  countsAsPendingChange: boolean;
} {
...
function addUnsafeSettingsPathCheck(
  results: CheckResult[],
  agentDir: string,
  settingsPath: string,
): void {
...
function addSettingsDriftCheck(
  ...
  if (lines.some((line) => line === "No settings changes needed.")) {
    recordCheck(
      ...
      "OK",
      "settings drift",
      "packaged defaults already match the isolated profile",
    );
    return;
  }
...
function addBundledSubagentCheck(
  results: CheckResult[],
  packageRoot: string,
  agentDir: string,
): void {
  ...
  if (restoreNeeded.length === 0) {
    recordCheck(
      ...
      "OK",
      "bundled subagent resources",
      `found current prompt bundle at ${subagentDir}`,
    );
    return;
  }
  recordCheck(
...
function readRuntimeMarker(markerPath: string): { detail: string; level: CheckLevel } {
  ...
  try {
    ...
    const runtimeAbsPath =
      typeof marker.runtimeAbsPath === "string" ? marker.runtimeAbsPath : "unknown";
    return {
...
function addRuntimeCheck(
  ...
  if (!existsSync(piPath)) {
    recordCheck(
      ...
      `private runtime pi missing at ${piPath}`,
    );
    return;
  }
  try {
...
function addGnosisCheck(
  ...
  const result = runCommand(
    process.execPath,
    [gnosisScript(packageRoot), "validate", "--agent-dir", agentDir],
    {
      env,
    },
  );
  if (result.status === 0) {
    recordCheck(
      results,
      "OK",
      "managed gn validation",
      `validated ${String(result.stdout || "").trim() || "gn"}`,
    );
    return;
  }
  recordCheck(
    ...
    "WARN",
    "managed gn validation",
    `no valid gn command found (${commandFailureSummary(result)})`,
  );
}
...
function addTicketsCheck(
  ...
  recordCheck(
    ...
    "WARN",
    "managed tk validation",
    `ticket integration inactive (${command || "not found"})`,
  );
}
...
function repairSettings(
  ...
  try {
    ...
    if (cleanup.uninstalledNpmPackages.length > 0) {
      details.push(
        `uninstalled ${cleanup.uninstalledNpmPackages.length} retired TLH subagent npm package(s)`,
      );
    }
    if (cleanup.removedGitPaths.length > 0) {
      details.push(
        `removed ${cleanup.removedGitPaths.length} retired TLH subagent git checkout(s)`,
      );
    }
...
function repairBundledSubagentPrompts(packageRoot: string, agentDir: string): RepairAction {
  ...
  if (restoreNeeded.length === 0) {
    return repairAction(
      "OK",
      "bundled subagent resources",
      `prompt bundle already current at ${subagentDir}`,
    );
  }
...
function repairManagedHelper(
  ...
  if (result.status !== 0) {
    return repairAction(
      "FAIL",
      label,
      `${commandArgs[0]} failed (${commandFailureSummary(result)})`,
    );
  }
...
function runRepairMode(
  ...
  if (pathIsInNormalPiConfig(settingsPath)) {
    console.error(
      `error: refusing repair for normal Pi settings path inside ~/.pi: ${settingsPath}`,
    );
    return 1;
  }
  if (!settingsPathIsWithinAgent(agentDir, settingsPath)) {
    console.error(
      `error: refusing repair for settings path outside isolated TLH profile: ${settingsPath}`,
    );
    return 1;
  }
  ...
  const actions: RepairAction[] = [
    ...
    repairAction(
      ...
      "gh auth, EXA keys, and MCP config remain manual",
    ),
  ];
...
function main(): number {
  ...
  const packageRoot = resolve(
    expandHomePath(args.packageRoot || DEFAULT_PACKAGE_ROOT) || DEFAULT_PACKAGE_ROOT,
  );
  const settingsPath = resolve(
    expandHomePath(args.settingsPath || defaultTlhSettingsPath({ agentDir })) ||
