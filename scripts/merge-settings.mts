...
function packageIdentityExists(
...
function shouldEnsureDefaultExtensionSource(
  ...
  return !extension.replaces.some((oldSource) =>
    packageIdentityExists(existingPackages, packageIdentity(oldSource)),
  );
}
...
function removeDuplicatePackagesByIdentity(
  settings: JsonObject,
  identity: string | undefined,
): string[] {
...
function applyHarnessPackageDedupes(
  settings: JsonObject,
  ensuredSource: string,
  changes: string[],
): void {
  if (!Array.isArray(settings.packages)) return;
  for (const removedSource of removeDuplicatePackagesByIdentity(
    settings,
    HARNESS_PACKAGE_IDENTITY,
  )) {
    changes.push(
...
function applyDefaultExtensionSourceUpdates(
  ...
  for (const extension of defaultExtensions) {
    ...
    if (
      !shouldMigrateManagedDefaultExtensionSource(currentSource, extension, {
        force,
    ...
    if (sourceNeedsUpdate) {
      changes.push(
        `update default extension package source: ${currentSource} -> ${extension.source}`,
      );
    }
...
function removeCriticalDisabledDefaultExtensionOptOuts(
  ...
  const nextValues = values.filter(
    (value: unknown) => !(typeof value === "string" && criticalIds.has(value.trim())),
  );
  if (nextValues.length === values.length) return;
...
function isPersistentTelemetryOptOut(
  ...
  return (
    path.join(".") === "tlh.telemetry.enabled" && currentValue === false && defaultValue === true
  );
}
...
function mergeObject(
  ...
  for (const [key, value] of Object.entries(defaults)) {
    ...
    if (
      (options.force || isInstallerOwnedSetting(path)) &&
...
function main(): void {
  ...
  const sourceUpdatedIdentities = applyDefaultExtensionSourceUpdates(
    next,
    defaultExtensions,
    disabledIds,
    changes,
