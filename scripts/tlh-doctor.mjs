...
import { copyTlhSubagentPrompts, missingTlhSubagentPrompts, settingsRequireTlhSubagentPrompts, } from "./lib/tlh-install-subagents.mjs";
...
function recordCheck(results, level, label, detail) {
...
function summarizeLevels(results) {
    const counts = { OK: 0, WARN: 0, FAIL: 0 };
    for (const result of results)
        counts[result.level] += 1;
    return `${counts.OK} OK, ${counts.WARN} WARN, ${counts.FAIL} FAIL`;
}
function summarizeItems(items, maxItems = MAX_DETAIL_ITEMS) {
    if (items.length <= maxItems)
        return items.join(", ");
    return `${items.slice(0, maxItems).join(", ")}, +${items.length - maxItems} more`;
}
function summarizeCountsByCategory(entries) {
...
function categorizeSettingsDriftLine(change) {
...
function addProtectedAgentDirCheck(results, agentDir) {
    recordCheck(results, "FAIL", "profile isolation/settings", `agent dir is inside ~/.pi; profile not inspected (${agentDir})`);
}
function addUnsafeSettingsPathCheck(results, agentDir, settingsPath) {
    const details = [];
    if (pathIsInNormalPiConfig(agentDir)) {
        details.push("agent dir is inside ~/.pi");
    }
...
function addSettingsDriftCheck(results, packageRoot, settingsPath, env) {
...
function readRuntimeMarker(markerPath) {
...
function readInstallStateHint(agentDir) {
    const path = join(agentDir, "tlh", "install-state.json");
    if (!existsSync(path))
        return "install-state missing";
    try {
        const state = readJsonFile(path);
        const parts = [];
        if (typeof state.track === "string" && state.track)
            parts.push(`track=${state.track}`);
        if (typeof state.ref === "string" && state.ref)
            parts.push(`ref=${state.ref}`);
        if (typeof state.repo === "string" && state.repo)
            parts.push(`repo=${state.repo}`);
...
function parseStatusLine(output, label) {
...
function addTicketsCheck(results, packageRoot, agentDir, settingsPath, env) {
...
function addRtkCheck(results, packageRoot, agentDir, env) {
...
function addGhCheck(results, env) {
...
function extensionById(defaultExtensions, id) {
    return defaultExtensions.find((entry) => entry.id === id);
}
function settingsPackageIdentities(settings) {
...
function extensionEnabled(settings, defaultExtensions, id) {
    return !disabledDefaultExtensionIds(settings, defaultExtensions).has(id);
}
function extensionPresent(settings, extension) {
    if (!extension)
        return false;
...
function readWebSearchSettings(agentDir) {
    ...
    if (!existsSync(settingsPath))
        return { hasStoredKey: false, invalid: false };
    try {
...
function printRepairActions(actions) {
...
function collectHealthResults(agentDir, packageRoot, settingsPath, env) {
...
function settingsPathIsWithinAgent(agentDir, settingsPath) {
    return pathWithinOrEqual(realpathForCompare(agentDir), realpathForCompare(settingsPath));
}
function runRepairMode(agentDir, packageRoot, settingsPath, env) {
    if (pathIsInNormalPiConfig(agentDir)) {
