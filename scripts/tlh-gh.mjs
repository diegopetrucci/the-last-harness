...
function optionValue(argv, index, flag, { allowEmpty = false } = {}) {
...
function parseArgs(argv) {
    ...
    if (args.commandPath.length === 0 && args.positionals.length > 0) {
        args.commandPath.push(args.positionals.shift());
    }
...
function validateRepoParts(owner, repo, source) {
...
function parseGitHubRemoteUrl(remoteUrl) {
    ...
    if (scpLikeMatch) {
        ...
        if (!GITHUB_REMOTE_HOSTS.has(host.toLowerCase()))
            return undefined;
        try {
    ...
    if (!GITHUB_REMOTE_HOSTS.has(parsed.hostname.toLowerCase()))
        return undefined;
...
function requireSuccess(command, args, failureHint) {
...
function inferRepoFromGit() {
    const remoteList = requireSuccess("git", ["remote"], "Failed to inspect git remotes. Use --repo owner/repo outside a GitHub checkout.");
    ...
    for (const remoteName of orderedRemotes) {
        if (!remotes.includes(remoteName))
            continue;
        const remoteUrl = requireSuccess("git", ["remote", "get-url", remoteName], `Failed to inspect git remote ${JSON.stringify(remoteName)}.`);
...
function parseJsonBody(response, expected) {
    try {
...
function ghApiFailure(method, endpoint, detailValue) {
...
function runGhApi(endpoint, options = {}) {
...
function listRestCollection(endpoint, limit, collectionKey, includeItem = () => true) {
    ...
    while (nextEndpoint && remaining > 0) {
        ...
        if (collectionKey) {
            const parsed = parseJsonBody(response, "an object");
        ...
        const parsed = parseJsonBody(response, "an array");
...
function requireNoPositionals(args) {
    if (args.positionals.length !== 0) {
        throw new Error(`Unexpected positional argument: ${JSON.stringify(args.positionals[0])}`);
    }
}
function requireSinglePositional(args, label) {
    if (args.positionals.length !== 1) {
        throw new Error(`Expected exactly one ${label}.`);
    }
    return args.positionals[0];
}
function requirePositiveIntegerIdentifier(args, label) {
...
function ensureBodySource(args, { required }) {
...
function mutationEnvelope(operation, repo, resourceKey, resourceValue) {
    return {
        operation,
...
function runIssueView(context) {
    ...
    return formatJson(parseJsonBody(response, "an object"));
}
...
function runIssueList(context) {
    ...
    return formatJson(listRestCollection(endpoint, context.args.limit, undefined, isIssueItem));
}
function runIssueComments(context) {
    const issueNumber = requirePositiveIntegerIdentifier(context.args, "issue number");
...
function runIssueCreate(context) {
    requireNoPositionals(context.args);
...
function runIssueComment(context) {
    const issueNumber = requirePositiveIntegerIdentifier(context.args, "issue number");
...
function runPrComments(context) {
    const prNumber = requirePositiveIntegerIdentifier(context.args, "pull request number");
...
function runCommandPath(args) {
    ...
    const context = { args, repo: resolveRepo(args) };
    if (key === "repo view")
        return runRepoView(context);
    if (key === "issue view")
        return runIssueView(context);
    if (key === "issue list")
        return runIssueList(context);
    if (key === "issue comments")
        return runIssueComments(context);
    if (key === "issue create")
        return runIssueCreate(context);
    if (key === "issue comment")
        return runIssueComment(context);
    if (key === "pr view")
        return runPrView(context);
    if (key === "pr list")
        return runPrList(context);
    if (key === "pr diff")
        return runPrDiff(context);
    if (key === "pr reviews")
        return runPrReviews(context);
    if (key === "pr comments")
        return runPrComments(context);
    if (key === "pr create")
        return runPrCreate(context);
    if (key === "pr comment")
        return runPrComment(context);
    if (key === "checks")
        return runChecks(context);
    if (key === "statuses")
        return runStatuses(context);
    throw new Error(`Unknown command: ${key}`);
}
export { commandKey, encodeQuery, inferRepoFromGit, listRestCollection, parseArgs, parseGhResponse, parseGitHubRemoteUrl, parseRepoSpec, runCommandPath, sanitizeSecretText, };
function main() {
    try {
