...
import { assignOptionValue, expandHomePath, resolveTlhAgentDir } from "./lib/tlh-install-utils.mjs";
...
function candidateCommands(agentDir: string): string[] {
...
function findValidGnosis(agentDir: string): string | undefined {
...
function managedGnosisTargetPath(args: CliArgs, agentDir: string): string {
  const agentRoot = resolve(expandHomePath(agentDir) ?? agentDir);
  return resolve(
    expandHomePath(args.target || join(agentRoot, "bin", "gn")) ?? join(agentRoot, "bin", "gn"),
  );
}

function findValidGnosisForConfigure(args: CliArgs, agentDir: string): string | undefined {
...
function gnosisPlatform(): GnosisPlatform | undefined {
  let os: GnosisPlatform["os"];
  if (process.platform === "darwin") os = "darwin";
  else if (process.platform === "linux") os = "linux";
  else return undefined;

  let arch: GnosisPlatform["arch"];
...
function unsupportedGnosisPlatformMessage(): string {
...
async function resolveGnosisVersion(args: CliArgs): Promise<string> {
...
function checksumForAsset(checksumsText: string, assetName: string): string | undefined {
...
function extractTarGzip(archivePath: string, extractDir: string): void {
...
function resolvedManagedAgentDir(agentDir: string): string {
  return realpathForCompare(resolve(expandHomePath(agentDir) ?? agentDir));
}

function assertManagedGnosisTempPath(
  path: string,
  agentDir: string,
  label: string,
  {
  ...
  if (!stats) {
    if (mustExist) {
      throw new Error(
        `Refusing to install managed Gnosis because temporary ${label} was not created: ${path}`,
      );
    }
  } else {
    if (stats.isSymbolicLink()) {
      throw new Error(
        `Refusing to install managed Gnosis through symlinked temporary ${label}: ${path}`,
      );
    }
    if (expectDirectory && !stats.isDirectory()) {
      throw new Error(
        `Refusing to install managed Gnosis because temporary ${label} is not a directory: ${path}`,
      );
    }
    if (expectFile && !stats.isFile()) {
      throw new Error(
        `Refusing to install managed Gnosis because temporary ${label} is not a file: ${path}`,
      );
    }
  }
...
async function installManagedGnosis(args: CliArgs, agentDir: string): Promise<string | undefined> {
  ...
  let installTempDir: string | undefined;
  ...
  try {
    ...
    ({ tempDir: installTempDir, tempTarget } = createManagedGnosisTempTarget(
      args,
      agentDir,
      target,
    ));
...
function assertNotNormalPiPath(path: string, label: string): void {
  if (isUnderNormalPiConfig(path)) {
    throw new Error(
      `Refusing to modify normal Pi config from The Last Harness gnosis command (${label}): ${path}`,
    );
  }
}
...
function assertManagedGnosisAgentDir(agentDir: string): void {
  ...
  if (!directoryStats.isDirectory()) {
    throw new Error(
      `Refusing to install managed Gnosis because agent dir is not a directory: ${agentDir}`,
    );
  }
}
...
function assertNoSymlinkedManagedTargetParents(target: string, boundary: string): void {
  ...
  for (const part of relativeParent.split(sep).filter(Boolean)) {
    ...
    if (stats.isSymbolicLink()) {
      throw new Error(
        `Refusing to install managed Gnosis through symlinked target parent component: ${current}`,
      );
    }
    if (!stats.isDirectory()) {
      throw new Error(
...
function validateManagedGnosisTarget(args: CliArgs, agentDir: string): string {
  const agentRoot = resolve(expandHomePath(agentDir) ?? agentDir);
...
function gnosisInstallSkippedByEnv(): boolean {
  const value = process.env.TLH_SKIP_GNOSIS_INSTALL;
  return value === "1" || value?.toLowerCase() === "true" || value?.toLowerCase() === "yes";
}

async function commandConfigureInstall(args: CliArgs, agentDir: string): Promise<void> {
...
function commandValidate(agentDir: string, commandArgs: readonly string[]): void {
