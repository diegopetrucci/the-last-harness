...
import { assertNotInNormalPiConfig, assignOptionValue, renderShellWords, shellQuote, } from "./lib/tlh-install-utils.mjs";
...
function assertNotNormalPiPath(path, label) {
    assertNotInNormalPiConfig(path, `refusing to modify normal Pi config from The Last Harness wrapper command (${label}): ${path}`);
}
...
function renderWrapper(args) {
    const lines = [
        ...
        '}',
        'tlh_package_github_script_is_usable() {',
        '  tlh_package_script_is_usable "${default_tlh_package_root}/scripts/tlh-gh.mjs"',
        '}',
        ...
        'if [[ "${1:-}" == "github" ]]; then',
        "  shift",
        '  if ! tlh_package_github_script_is_usable; then',
        "    printf 'error: tlh github support files are missing or corrupt; run `tlh update` to recover.\\n' >&2",
        "    exit 1",
        "  fi",
        '  tlh_resolve_node',
        '  PATH="${tlh_sanitized_path}" exec "${tlh_node_cmd}" "${default_tlh_package_root}/scripts/tlh-gh.mjs" "$@"',
        "fi",
        "",
        'if [[ "${1:-}" == "doctor" ]]; then',
        "  shift",
