...
function renderWrapper(args) {
	const lines = [
		...
		'  tlh_update_script=""',
		"  for candidate in \\",
		'    "${default_tlh_package_root}/scripts/tlh-update.mjs" \\',
		'    "${default_agent_dir}/tlh/tlh-update.mjs"; do',
		'    if [[ -f "${candidate}" ]]; then',
		...
		'  if [[ -f "${tlh_package_defaults_script}" && -f "${tlh_package_default_extensions}" ]]; then',
		...
		'  tlh_tickets_script=""',
		"  for candidate in \\",
		'    "${default_tlh_package_root}/scripts/tlh-tickets.mjs" \\',
		'    "${default_agent_dir}/tlh/tlh-tickets.mjs"; do',
		'    if [[ -f "${candidate}" ]]; then',
