...
function renderWrapper(args) {
	const lines = [
		...
		`default_pinned_pi_version=${shellQuote(PINNED_PI_VERSION)}`,
		...
		'  exit 1',
		'fi',
		'tlh_pi_version_output=""',
		'if ! tlh_pi_version_output="$(PATH="${tlh_sanitized_path}" "${default_pi_cmd}" --version 2>&1)"; then',
		"  printf 'error: failed to run private pi runtime at %s; run `tlh update`.\\n' \"${default_pi_cmd}\" >&2",
		'  exit 1',
		'fi',
		'if [[ ! "${tlh_pi_version_output}" =~ ([0-9]+\\.[0-9]+\\.[0-9]+) ]]; then',
		"  printf 'error: could not parse pi version from private runtime; run `tlh update`.\\n' >&2",
		'  exit 1',
		'fi',
		'tlh_pi_version="${BASH_REMATCH[1]}"',
		// Hard floor: reject pi versions below the minimum.
		'if ! tlh_pi_version_meets_minimum "${tlh_pi_version}" "${default_min_pi_version}"; then',
		"  printf 'error: private pi version %s is below the minimum required %s; run `tlh update`.\\n' \"${tlh_pi_version}\" \"${default_min_pi_version}\" >&2",
		'  exit 1',
		'fi',
		// Soft warn: non-fatal notice when the runtime drifts from the pinned version.
		'if [[ "${tlh_pi_version}" != "${default_pinned_pi_version}" ]]; then',
		"  printf 'warning: private pi version %s differs from the expected %s; run `tlh update` to repair.\\n' \"${tlh_pi_version}\" \"${default_pinned_pi_version}\" >&2",
		'fi',
