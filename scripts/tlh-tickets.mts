...
function parseArgs(argv: readonly string[]): CliArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    const installPathIndex = assignCliOptionValue(
      args,
      "installPath",
      argv,
      index,
    ...
    const wrapperNameIndex = assignCliOptionValue(
      args,
      "wrapperName",
      argv,
      index,
...
function findValidTkForConfigure(
  args: CliArgs,
  settings: Settings,
  agentDir: string,
): string | undefined {
...
function assertNotNormalPiSettings(settingsPath: string): void {
  if (isUnderNormalPiAgent(settingsPath)) {
    throw new Error(
      `Refusing to modify normal Pi config from The Last Harness tickets command: ${settingsPath}`,
    );
  }
}
...
function assertNotNormalPiPath(path: string, label: string): void {
  if (isUnderNormalPiAgent(path)) {
    throw new Error(
      `Refusing to modify normal Pi config from The Last Harness tickets command (${label}): ${path}`,
    );
  }
}
...
function stableRealpathOfExistingDirectory(path: string, firstStats: Stats, label: string): string {
  ...
  if (!secondStats) {
    throw new Error(
      `Refusing to install managed tk because ${label} changed while planning: ${path}`,
    );
  }
  ...
  while (true) {
    ...
    if (stats && !stats.isSymbolicLink()) {
      if (!stats.isDirectory()) {
        throw new Error(
          `Refusing to install managed tk because ${label} ancestor is not a directory: ${current}`,
        );
      }
      const resolvedAncestor = stableRealpathOfExistingDirectory(
        current,
        stats,
        `${label} ancestor`,
      );
...
function captureIntendedManagedAgentDir(agentRoot: string): string {
  ...
  if (!stats)
    return intendedPathFromNearestExistingNonSymlinkAncestor(agentRoot, "managed agent root");
  if (stats.isSymbolicLink()) {
    throw new Error(
      `Refusing to install managed tk through symlinked managed agent root: ${agentRoot}`,
    );
  }
  if (!stats.isDirectory()) {
    throw new Error(
      `Refusing to install managed tk because managed agent root is not a directory: ${agentRoot}`,
    );
  }
...
function assertManagedTkAgentRootSafe({
  ...
  if (stats) {
    ...
    resolvedAgentDir = intendedPathFromNearestExistingNonSymlinkAncestor(
      agentRoot,
      "managed agent root",
    );
  }
...
function assertNoSymlinkedManagedTargetParents(target: string, boundary: string): void {
  ...
  for (const part of relativeParent.split(sep).filter(Boolean)) {
    ...
    if (stats.isSymbolicLink()) {
      throw new Error(
        `Refusing to install managed tk through symlinked target parent component: ${current}`,
      );
    }
    if (!stats.isDirectory()) {
      throw new Error(
        `Refusing to install managed tk because target parent component is not a directory: ${current}`,
      );
    }
  }
}
...
function managedTkTargetPlan(args: CliArgs, agentDir: string): ManagedTkPlan {
  ...
  if (target === agentRoot) {
    throw new Error(
      `Refusing to install managed tk over the configured tlh profile directory: ${target}`,
    );
  }
  ...
  if (basename(target) !== "tk") {
    throw new Error(
      `Refusing to install managed tk because the target basename must be exactly "tk": ${target}`,
    );
  }

  const intendedAgentDir = captureIntendedManagedAgentDir(agentRoot);
  const planBase: Pick<ManagedTkPlan, "agentRoot" | "intendedAgentDir"> = {
    agentRoot,
    intendedAgentDir,
  };
...
function assertManagedTkTargetParentSafe({
  ...
  if (parentStats) {
    if (parentStats.isSymbolicLink()) {
      throw new Error(
        `Refusing to install managed tk through symlinked target parent: ${targetParent}`,
      );
    }
    if (!parentStats.isDirectory()) {
      throw new Error(
        `Refusing to install managed tk because target parent is not a directory: ${targetParent}`,
      );
    }
...
function stableRealpathOfSafeDirectory(path: string, firstStats: Stats, label: string): string {
  ...
  if (!secondStats) {
    throw new Error(
      `Refusing to create ${label} because a directory component changed while planning: ${path}`,
    );
  }
  if (secondStats.isSymbolicLink()) {
  ...
  if (!secondStats.isDirectory()) {
    throw new Error(
      `Refusing to create ${label} because a directory component is not a directory: ${path}`,
    );
  }
  if (!sameFileStats(firstStats, secondStats)) {
    throw new Error(
...
function validateAnchoredDirectory(
  ...
  if (!stats) {
    throw new Error(
      `Refusing to create ${label} because a directory component is missing: ${path}`,
    );
  }
  if (stats.isSymbolicLink()) {
    throw new Error(`Refusing to create ${label} through symlinked directory component: ${path}`);
...
function ensureDirectorySafely(directory: string, intendedDirectory: string, label: string): void {
  ...
  while (true) {
    ...
    if (parent === current || expectedParent === expectedCurrent) {
      throw new Error(
        `Refusing to create ${label} because no anchored directory ancestor was found: ${directory}`,
      );
    }
...
function validateOpenedFileForDirectWrite(
  fd: number,
  path: string,
  intendedRoot: string,
  label: string,
  ...
  if (!fdStats.isFile()) {
    throw new Error(
      `Refusing to write ${label} because the opened path is not a regular file: ${path}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Refusing to write ${label} because the opened path could not be validated: ${path} (${message})`,
      {
        cause: error,
      },
    );
  ...
  if (!sameFileStats(fdStats, pathStats)) {
    throw new Error(
      `Refusing to write ${label} because the opened file no longer matches the target path: ${path}`,
    );
  }
  if (fdStats.nlink !== 1) {
    throw new Error(
      `Refusing to write ${label} because the target has ${fdStats.nlink} hard links: ${path}`,
    );
  }
  ...
  try {
    ...
    if (targetStats) {
      ...
      if (!targetStats.isFile()) {
        throw new Error(
          `Refusing to write ${label} because the path is not a regular file: ${path}`,
        );
      }
      if (exclusive) {
...
function verifyTicketArchive(args: CliArgs, archivePath: string): void {
  ...
  if (actual !== expected) {
    throw new Error(
      `Ticket source checksum verification failed (expected ${expected}, got ${actual})`,
    );
  }
}
...
function ticketArchiveEntry(archivePath: string, preferredEntry: string, agentDir: string): string {
  ...
  if (candidates.length > 1) {
    throw new Error(
      `Ticket source archive contained multiple ticket script entries; expected ${preferredEntry}`,
    );
  }
...
async function installManagedTk(args: CliArgs, agentDir: string): Promise<string | undefined> {
  ...
  try {
    ...
    const extracted = extractTicketScript(
      archivePath,
      extractDir,
      args.ticketArchiveEntry,
      agentDir,
    );
...
function assertSettingsDirSafe({
  settingsPath,
  settingsDir,
  intendedSettingsDir,
}: SettingsWritePlan): void {
  assertNotNormalPiSettings(settingsPath);
  ...
  if (dirStats) {
    ...
    if (!dirStats.isDirectory()) {
      throw new Error(
        `Refusing to write settings because the settings directory is not a directory: ${settingsDir}`,
      );
    }
...
function setTicketsEnabled(
  ...
  detailLog(
    args,
    `${args.dryRun ? "Would enable" : "Enabled"} tk integration for the tlh profile.`,
  );
  logWriteResult(args, writeResult);
}

function validatedRequestedInstallPath(
  args: CliArgs,
  agentDir: string,
  installPath: string,
): string | undefined {
  ...
  if (!validateTkCommand(normalized, agentDir)) {
    throw new Error(
      `Refusing to enable tk integration because the command did not validate: ${normalized}`,
    );
  }
  return normalized;
}
...
async function commandConfigureInstall(
  ...
  if (validPath) {
    ...
    const sha =
      samePathForCompare(validPath, managedTarget) && managedPinIsFresh
        ? args.ticketSourceSha256
        : undefined;
