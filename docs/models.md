...
The bundled `developer` subagent follows the active primary session provider with Anthropic Claude Sonnet 4.6 at medium effort, OpenAI Codex GPT-5.6 Luna at max effort, or xAI Grok 4.6 at low effort; its OpenRouter default is the session model at medium effort. The opt-in `staff-developer` role, when selected by enabled staff routing, uses OpenAI Codex GPT-6 Astra at low effort, Anthropic Claude Opus 5 at medium effort, or xAI Grok 4.6 at medium effort. The two xAI tiers use the same Grok 4.6 model and differ by effort. Its OpenRouter entry is effort-only at medium effort: OpenRouter requires an explicit, resolvable staff-developer model override rather than the session model. Here, availability means exact resolution in the active runtime model registry; it does not mean entitlement, quota, authentication, or successful launch. If the effective staff model is unavailable, TLH downgrades the target to `developer` before launch, removes staff-specific model/fallback settings, and does not substitute a model from another provider. The execution-only `test-runner` uses OpenAI Codex GPT-5.6 Luna at low effort, Anthropic Claude Haiku 4.5 at low effort, xAI Grok 4.3 at low effort, and low effort on OpenRouter for cheap final validation; its assigned generic MCP calls may invoke configured tools that change server-side state. Other bundled subagents — `web-scout`, `repo-scout`, `librarian`, and `diff-summarizer` — use OpenAI Codex GPT-5.6 Luna at medium effort, Anthropic Claude Haiku 4.5 at high effort, and xAI Grok 4.3 at medium effort. Review roles use OpenAI Codex GPT-5.6 Sol at high effort for `code-reviewer` and `contrarian`, while `oracle` uses OpenAI Codex GPT-6 Astra at medium effort; their OpenRouter effort defaults remain high. `code-reviewer` uses Anthropic Claude Opus 5 at max effort and xAI Grok 4.6 at high effort, while `oracle` and `contrarian` use Anthropic Claude Fable 5.1 at medium effort and xAI Grok 4.6 at xhigh effort.

### Benchmark selection policy

These ranges are date-sensitive selection guidance, not guarantees or verified runtime scores. On **2026-09-13**, TLH reviewed the live [Artificial Analysis Intelligence Index vs. Cost per Intelligence Index Task source](https://artificialanalysis.ai). The method was to target approximately **35 Intelligence Index** for `developer` and approximately **40–45 Intelligence Index** for `staff-developer`, using cost per task as a secondary constraint when choosing among current candidates, then encode provider-specific defaults. TLH effort settings may differ from the settings used by the benchmark runs. Future default changes must review the then-current chart and repeat this method rather than treating these ranges as permanent or as measured runtime scores.

### Staff-developer routing and tier policy

`staff-developer-routing` is disabled by default and does not change the active primary-agent mode. When enabled, the `architect` primary and `disabled` primary mode may dispatch `staff-developer`; `rush`, `product`, and `bug-hunter` cannot. The `architect` owns automatic semantic assignment; `disabled` mode has no semantic auto-routing persona, so only explicit user-directed dispatch applies there. Ordinary `developer` requests remain available without the experiment. Choose `staff-developer` when design judgment remains during implementation: foundational or new-subsystem scaffolding; cross-cutting API or data-shape changes; concurrency, lifecycle, security, or trust boundaries; migration or compatibility sequencing; broad refactors whose target shape is unresolved; or material implementation uncertainty. Choose `developer` for localized and well-specified work, broad-but-mechanical changes, tests, documentation, and repetitive follow-ups. When uncertain, default to `developer`.

`Product` mode does not assign tiers. An explicit human request for `staff-developer` is honored only while the experiment is enabled from `architect` or `disabled` mode; it never overrides the approved-ticket contract. Before ticket approval, the architect records one `Worker: developer|staff-developer — Reason: <one line>` line per implementation ticket and a `Staff tickets: <count>` aggregate. An approved developer-to-staff upgrade requires a changed-risk explanation and renewed approval; a staff-to-developer downgrade is announced with its reason before dispatch. One approved ticket has one writer, and a staff-developer run cannot delegate further or commit.
...
The upstream Pi runtime provides OpenRouter authentication and transport: use `/login openrouter` for OAuth or set `OPENROUTER_API_KEY`. TLH does not implement that provider's transport or authentication. All non-opposite TLH primaries and subagents other than the opt-in `staff-developer` role follow the active OpenRouter session model by default. Staff routing is the exception: `staff-developer` requires an explicit, resolvable model override before launch. For an active primary, a persisted `tlh.primaryAgent.modelOverrides.<primary>` entry takes precedence and is reapplied at the next session or primary-mode boundary; otherwise the primary follows the current session model. Their effort comes from each role's effort-only `openrouter` entry; generic or legacy thinking values do not leak onto this path. Unknown versus explicitly non-reasoning capability checks apply to stored minor-agent effort overrides and generated fallback handling, not pure bundled or primary defaults.
...
Use `/subagent-settings` to persist model or effort choices for the ten canonical TLH minor-agent roles: the nine stable roles `code-reviewer`, `contrarian`, `developer`, `test-runner`, `diff-summarizer`, `librarian`, `oracle`, `repo-scout`, and `web-scout`, plus opt-in `staff-developer`. Configuring staff-developer does not enable staff routing.
...
```json
{
  ...
  "subagents": {
    ...
    "staff-developer": { "model": "openai-codex/gpt-6-astra", "effort": "low" },
...
Valid primary agent names: `architect`, `rush`, `product`, `bug-hunter`. Valid subagent role names: `code-reviewer`, `contrarian`, `developer`, `staff-developer`, `test-runner`, `diff-summarizer`, `librarian`, `oracle`, `repo-scout`, `web-scout`. A `staff-developer` entry configures the opt-in role but does not enable `staff-developer-routing`; enable that feature separately with `/experimental enable staff-developer-routing`.
