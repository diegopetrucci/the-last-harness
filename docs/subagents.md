...
When an otherwise successful child is missing or has a malformed structured acceptance report, TLH may make exactly one bounded, same-context **report-only repair**. The repair resumes the tracked child session, disables tools/extensions/skills, forbids implementation edits or a restart, and evaluates the returned report against the original resolved contract. The original implementation output and artifact remain authoritative; a failed correction is surfaced as `Implementation complete but acceptance unverified`, and its durable attempt marker prevents another automatic repair across pause, resume, or result-artifact recovery. Substantive criterion, evidence, verification, review, interruption, timeout, and protocol failures are not repairable. This behavior has no separate runtime toggle: to disable acceptance (and therefore report repair) for a run, use an explicit `level: "none"` contract or the applicable acceptance-role override; otherwise reload/restart is not needed for a fixed policy change.
