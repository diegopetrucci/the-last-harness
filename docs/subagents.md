...
**Stale-run reconciliation.** The runtime has one stale-run rule: if a persisted run has a non-terminal lifecycle state and its recorded runner PID is proven dead, reconciliation marks the run `failed` and clears active ownership. This includes a `pausing` run whose runner died before pause finalization. The live/unknown age check applies only when the persisted lifecycle state is exactly `running`: the record is retained while it is no older than the 24-hour stale-record bound, using `lastUpdate` or falling back to `startedAt`; after that bound, reconciliation fails it through the generation-locked repair path, clears the PID, and records that PID ownership cannot be verified. Every non-running state—including `queued` and `pausing`—is exempt from this age cutoff, even when pause metadata is present; a pause marker does not exempt a record whose state is `running`. Proven-dead PID repair still applies to every non-terminal state. The age bound is separate from the retired TLH async-parent prompt-cache heartbeat: it sends no refreshes and wakes no parent. Already-terminal runs are not rewritten. A live async runner in the `running` state refreshes `lastUpdate` approximately every 5 minutes via an internal heartbeat, independently of control configuration and child-event activity. A `running` record whose `lastUpdate` (or `startedAt`) is older than 24 hours therefore indicates a likely lost or crashed runner; the heartbeat prevents false staleness caused by child inactivity, assuming the runner process stays scheduled and status persistence succeeds — suspension, clock jumps, or persistent write failures can still leave a live runner looking stale.
