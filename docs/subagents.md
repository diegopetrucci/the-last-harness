...
Most users should delegate in natural language to the active primary agent. The architect chooses the appropriate bundled agent, supervises the run, and judges the result. Implementation tickets normally go to `developer`; ordinary `developer` requests remain available without the experiment. When the opt-in `staff-developer-routing` experiment is enabled, the architect may assign a ticket to `staff-developer` when substantial design judgment remains. A separate final-validation ticket with explicit ordered shell/MCP steps goes to the execution-only `test-runner`; the latter reports results without editing, installing dependencies, fixing failures, or changing tickets. The runtime details below are mainly useful when diagnosing a run or developing TLH.
...
TLH copies its ten canonical minor-agent definitions to `<agent-dir>/tlh/agents/subagents/<role>.md` and loads them through that installer-managed path. Nine stable roles are in the default allowlist; the opt-in `staff-developer` role is added only when `staff-developer-routing` is enabled. No `subagents.agentDirs` default is installed or required for these first-party roles. For primary-agent delegation, TLH forces canonical minor agents to the isolated user scope, except that a mixed dispatch containing an embedded target is resolved in project scope. Every child starts a fresh session and never inherits the primary session transcript. This prevents unrelated project, package, legacy-profile, or extra-directory definitions from shadowing them and prevents the parent's primary-agent or Gnosis context from leaking into a child.

The canonical packaged TLH roles are fourteen roles: the four primaries `architect`, `rush`, `product`, and `bug-hunter`, plus ten canonical minors — nine stable bundled roles (`developer`, `test-runner`, `code-reviewer`, `repo-scout`, `diff-summarizer`, `librarian`, `web-scout`, `oracle`, and `contrarian`) and the opt-in `staff-developer` for implementation requiring substantial design judgment. The built-in definitions that shipped with the upstream runtime have been removed outright. Stable, always-available project custom `embedded.<slug>` agents are a separate exact-root contract available to the architect or disabled primary mode; see [custom-subagents.md](custom-subagents.md).

### Staff-developer routing

`staff-developer-routing` is disabled by default and does not change the primary-agent mode. When enabled, the `architect` primary and `disabled` primary mode may dispatch `staff-developer`; `rush`, `product`, and `bug-hunter` cannot. The `architect` owns automatic semantic assignment; `disabled` mode has no semantic auto-routing persona, so only explicit user-directed dispatch applies there. Ordinary `developer` requests remain available without the experiment. Staff targets are accepted in the primary-agent `subagent` tool only while the feature is enabled. If a selected staff model cannot be resolved in the active runtime model registry, TLH downgrades the target to `developer` before child launch, removes staff-specific model and fallback settings, and reports the downgrade; it does not substitute a model from another provider. On OpenRouter, staff routing requires an explicit, resolvable `staff-developer` model override because the role has no bundled OpenRouter model. Availability here means registry resolution, not entitlement, quota, authentication, or successful launch. See [models.md](models.md#staff-developer-routing-and-tier-policy) for the tier rubric, defaults, and benchmark-selection policy.
...
The code-owned ceilings for the ten canonical minor roles (nine stable roles plus opt-in `staff-developer`) are:
...
**Staff-routing control limitation (v1).** When `staff-developer-routing` is disabled, TLH blocks `resume` and `steer` only when readable active or persisted metadata identifies a `staff-developer` target. Opaque, missing, unreadable, or ambiguous control metadata retains the existing executor behavior rather than guessing; `status` and `interrupt` remain available. Enable the experiment before retrying a blocked staff-targeted `resume` or `steer`.
...
The ten canonical packaged minor roles (nine stable roles plus opt-in `staff-developer`) are the exception to normal frontmatter precedence for `acceptanceRole`: `subagents.agentOverrides.<role>.acceptanceRole` may override their declared default. The selected project entry wins the isolated profile entry, and `false` clears the role so legacy inference resumes. Other fields still let explicit frontmatter win, while project custom/embedded roles remain isolated from these settings.
