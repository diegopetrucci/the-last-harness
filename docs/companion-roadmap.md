...
| Mac launch configuration               | **Implemented in worktree (unpushed); installed and physical proof separately gated**                | Keychain-backed address/port/pairing storage with user-auditable setup, reveal/hide, reset, and forget flows is implemented in the Mac companion worktree (uncommitted). The bridge auto-starts at every launch; saved access works after relaunch or login without a shell environment. Env override still wins and disables management actions. No push, installed, or physical-device proof yet.                                                                                                                                                                                                                    |
...
### Pushed replies-on-by-default evidence

The following pushed commits record the removal of the `session-mirror-replies`
experimental feature gate from the observer wiring. Replies are now active in
any session started while session-mirror-observer is enabled in the isolated
profile; the feature gate is gone and legacy `enabledFeatures` entries for
`session-mirror-replies` are ignored. This is next-session-only because
observer activation itself is next-session-only.

- TLH/root `abf09e5…`: removes the `session-mirror-replies` experimental
  feature from observer wiring; replies are on by default whenever the
  observer is active.
- Mac `873eb75…`: removes the Enable replies menu item,
  `replyAuthorizationEnabled`, `setReplyAuthorization`, and the
  `authorizationWithdrawn` reply-close path. Authenticated devices negotiate
  `reply-text` whenever the peer advertises the capability; no per-launch
  reply switch is required.
- Mac `1fc2c29…`: companion cleanup and follow-on adjustments to the
  replies-on-by-default changes.

These are pushed commits on the respective branches but remain unmerged. The
2026-09-26 physical proof recorded in Phase 2 used the earlier pushed
revisions with `session-mirror-replies` explicitly enabled; these commits have
not yet been exercised on a physical device.
...
Phase 1 also owns the stable Mac launch configuration gate. User-auditable
Keychain-backed address/port/pairing storage with setup, reveal/hide, reset,
and forget flows is implemented in the Mac companion worktree (uncommitted;
no push or installed proof yet). Physical and installed validation remain
...
| High     | Login-managed app configuration is process-environment-only and is not a durable stable Mac profile.                                                                                                                          | Canonical roadmap evidence (2026-09-13; current composition record)       | **Worktree source closure evidence** — Keychain-backed setup/reset/forget implemented in Mac companion worktree (uncommitted); env override still wins and is never persisted; bridge auto-starts at launch. Installed and physical proof remain separately gated (tlhr-mcxm). | Phase 1           | Run the installed app after push; verify saved access survives relaunch and login; confirm reset/forget revoke stored access; keep private values out of source. |
...
| Low      | The claim that an installed app can never receive shell environment was not established.                                                                                                                                                   | Canonical roadmap evidence (2026-09-13; launch-configuration record)    | **Worktree source closure evidence** — Keychain-backed store removes dependence on shell environment; env override still accepted but never required or persisted; bridge auto-starts at launch from saved config or unconfigured. | Phase 1               | Verify login-launch behavior with saved Keychain config after push and install; confirm no shell env dependency.                                                                      |
...
| Stable Mac launch configuration                                                                 | Keychain-backed setup, reveal/hide, reset, and forget flows are implemented in the Mac companion worktree (uncommitted; no push or installed proof yet). Remaining gate items: push, installed-app proof, physical-device validation, and governing-document reconciliation (tracked in tlhr-mcxm).                                                                                                                                                                                                                                                                                                                            |
