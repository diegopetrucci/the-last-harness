...
- [ ] **Delegation and lifecycle smoke:** delegate to the ten canonical TLH minor agents (the nine stable default roles plus opt-in `staff-developer`, with `staff-developer-routing` enabled for the latter), including the execution-only shell/MCP `test-runner`; confirm a non-allowlisted target is blocked, confirm primary delegation uses user scope plus fresh context, and exercise an async run through `status` and `resume`.
