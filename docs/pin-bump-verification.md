...
- [x] **Pi 0.87.1 published-ref update convergence:** update convergence from the published ref passed after publication in a ticket-owned isolated `v0.42.1` latest-release install. It updated to ref `v0.43.0` with `track=latest-release` and private Pi `0.87.1`, then was cleaned up.

All nine checklist items are complete. The first eight were verified against the packaged `v0.43.0` candidate on 2026-09-25 in the `tlhf-u2gu` run, including the live-provider checks; the post-publication published-ref convergence evidence is recorded below. The dated `tlhf-i8ly` and `tlhf-u2gu` records preserve their original at-the-time limitations. Record the release candidate, profile, session evidence, and outcomes in the release validation notes/current release tracking.
...
At the time of this candidate-only run, published-ref update convergence was **not yet verified**: no published ref in this checkout advertised Pi `0.87.1`. The post-publication check recorded below supersedes that candidate-only limitation.

## Pi 0.87.1 published-ref update convergence record — `tlhf-wgno` (2026-09-25 UTC)

The post-publication check used a ticket-owned isolated `v0.42.1` latest-release install. Updating it converged to ref `v0.43.0` with `track=latest-release` and private Pi `0.87.1`. The isolated profile, runtime, wrapper, and temporary artifacts were cleaned up after verification; no secrets or ephemeral credential material were recorded.
