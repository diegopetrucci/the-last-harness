...
The opt-out is written to `~/.the-last-harness/agent/settings.json` and survives `tlh update`. Disabling ticket integration stops TLH primary agents from using `tk` and makes architect/product workflows fall back to conversation-based plans or non-ticket handoff material. It also removes the managed `<agent>/bin` directory from `PATH` for the wrapped `pi` process, so subagents and shells launched from inside TLH no longer see the managed `tk` binary. The `tlh tickets ...` helper subcommand still finds the managed binary so you can re-enable or reconfigure the integration without reinstalling.

Disabling does not delete repo-local `.tickets` data or any managed `tk`; removing `~/.the-last-harness` removes the managed copy. To remove only the managed `tk` binary while keeping the rest of the profile, delete `~/.the-last-harness/agent/bin/tk`. Running `tlh tickets enable` without performing a managed reinstall clears the recorded SHA-256, so the next `tlh tickets configure-install` will reinstall the managed binary once to refresh the pin.

Managed installs download the pinned `wedow/ticket` source tarball (`v0.3.2`) and verify its SHA-256 before extracting the `ticket` script; inherited environment variables cannot override those installer-owned source pins. TLH also records the SHA-256 of the managed binary in `settings.tlh.tickets.installedSha256`. When a future TLH release bumps the pinned SHA, the next `tlh update` or `tlh tickets configure-install` reinstalls the managed binary and refreshes the recorded value. Custom (non-managed) `installPath` values are unaffected by this check.
