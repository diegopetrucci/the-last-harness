...
### Fixed
- Guaranteed that subagent reports survive acceptance-report stripping, so supervisor-facing summaries no longer collapse to a single sentence when the child put its substantive evidence inside the fenced `acceptance-report` block (ps-9pj7). Two defects shared this path. First, `getFinalOutput` walks messages and content parts in reverse document order and early-returned the one part containing the block, discarding any prose part that preceded it in the same assistant message; it now also collects those preceding text parts and joins them in document order, deliberately scoped to a single message so unrelated intermediate chatter is not pulled in, while still returning text that contains the block because downstream ledger parsing depends on finding it; draft blocks in preceding parts are also skipped so a superseded acceptance-report part cannot pollute the collected prose or drive acceptance evaluation. Second, the validation evidence inside the block did not survive stripping at all, so a compact digest of `commandsRun` results plus any non-empty `residualRisks` is now rendered and appended whenever a report is present, with no length threshold or other magic-number gating so a short report cannot silently lose its evidence again. The digest lands on supervisor-facing artifact content only and never on `result.finalOutput`, which is a semantic value feeding user-requested `output:` file contents and chain/parallel output references; `stripAcceptanceReport` itself remains remove-only, so progress/step-tail output and acceptance evaluation are unchanged. Timed-out runs with an acceptance report previously lost the digest because the timeout branch unconditionally replaced the artifact content with plain timeout diagnostics; the artifact now receives the digest while `result.finalOutput` remains exactly the timeout diagnostics string.
- Deliberate exception, not an oversight: when a run saved a user-requested `output:` file, the artifact is a verbatim archive of that deliverable, so it stays byte-exact and receives no digest. A run dispatched with an `output:` file therefore gets no evidence digest on any surface; surfacing evidence for that case needs a separate surface and is intentionally out of scope here (ps-9pj7).

### Removed
- Removed unsupported Windows CI from the Tests workflow. (psn-fghs)
...
- Hid configured output targets, artifact output paths, and full-output artifact paths from collapsed foreground single/parallel subagent result cards, plus retained legacy chain-shaped compatibility cards, while keeping those paths visible in expanded rendering. This is renderer compatibility rather than a supported chain launch path; it preserves the async compact-widget behavior from PR #104 and limits path exposure in the default foreground TUI.
...
- Clarified the deliberate TLH chain non-support boundary in `README.md` and `docs/tlh-patch-inventory.md`: no supported saved-chain discovery/management/UI/append/root-attachment path, existing user `.chain.*` files and historical artifacts left untouched, retained grouped-runner plus legacy chain-shaped compatibility called out explicitly, and #67/#68 follow-up work kept out of scope.
...
### Fixed
- Restore startup scans now quarantine only re-confirmed unchanged async-status corruption with same-filesystem renames, preserve full run artifacts, bound warning text to aggregate counts, and keep valid async jobs restorable when quarantine attempts fail.
