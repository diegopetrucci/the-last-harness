...
This guide is about project custom agents. It is separate from the fourteen canonical packaged TLH roles (four primaries, nine stable bundled minors, and opt-in `staff-developer`) and their project prompt appends; see [Per-agent project guidance](../README.md#per-agent-project-guidance) for those files.
...
| --- | --- | --- |
| Primary role append | At primary session start; all fourteen role candidates share one inventory snapshot, and switching primary roles selects from that snapshot | Edits or newly persisted trust take effect after `/reload`/new session, not in the current primary snapshot |
