...
| `/experimental` | Open the TLH experimental-feature picker in TUI, or list/change TLH experimental features via typed subcommands (`delta-follow-up-reviews`, `ci-failure-investigation`, and `staff-developer-routing` are currently registered) |
...
`/experimental` currently registers `delta-follow-up-reviews`, `ci-failure-investigation`, and `staff-developer-routing`; all three are disabled by default. In the interactive TLH TUI, running `/experimental` with no arguments opens a picker that shows current feature state and lets you toggle flags; outside the TUI it falls back to the status list. Typed subcommands remain available: `/experimental list`, `/experimental status [feature]`, `/experimental enable <feature>`, `/experimental disable <feature>`, and `/experimental toggle <feature>`. `contrarian` is a bundled default minor subagent for sparing pre-ticket planning stress-tests when a proposed change genuinely warrants an adversarial brief; it is not part of the `/experimental` toggle surface, not the routine `code-reviewer` diff pass, and not the broader `oracle` second-opinion path. `delta-follow-up-reviews` is an opt-in flag that adds architect and `code-reviewer` guidance for delta-scoped follow-up reviews after fixes. `ci-failure-investigation` is an opt-in flag that lets the architect primary agent do read-only failed CI/status-check investigation after TLH opens a PR, then summarize and ask whether to proceed before any edits, commits, pushes, reruns, PR changes, or other follow-up changes. `staff-developer-routing` is an opt-in architect-only implementation-routing flag: it adds `staff-developer` to the allowed minor roles and lets the architect assign design-judgment tickets to that tier; it does not change the primary-agent mode. Enable it with `/experimental enable staff-developer-routing`, inspect it with `/experimental status staff-developer-routing`, and undo it with `/experimental disable staff-developer-routing` (or remove that value from `tlh.experimental.enabledFeatures`). The two prompt-only flags are re-read from settings on each agent turn; staff routing is re-read before relevant delegation and control actions, so enabling or disabling a flag applies without a new session or `/reload`. Project custom subagents use the exact direct Git-root `.tlh/agents/custom/<UPPERCASE-SLUG>.md` path and persisted project trust rather than an `/experimental` flag; see [custom-subagents.md](custom-subagents.md). Stale `run-tests-last` and `embedded-subagents` values in `tlh.experimental.enabledFeatures` are inert and do not re-enable retired behavior. Users upgrading to stable custom subagents do not need to edit the stale `embedded-subagents` value; if you choose to clean it up manually, remove only that value and preserve every other `enabledFeatures` entry.

When `staff-developer-routing` is disabled, the v1 safety check blocks `resume` and `steer` only when readable active or persisted metadata identifies a `staff-developer` target. Opaque, missing, unreadable, or ambiguous controls keep the existing executor behavior rather than guessing; `status` and `interrupt` remain available. Enable the feature before retrying a blocked staff-targeted control.

### `/subagent-settings`

`/subagent-settings` configures persistent model and effort overrides for the ten canonical TLH minor-agent roles: the nine stable roles `code-reviewer`, `contrarian`, `developer`, `test-runner`, `diff-summarizer`, `librarian`, `oracle`, `repo-scout`, and `web-scout`, plus opt-in `staff-developer`. These roles are dispatched through TLH's first-party `extensions/subagents` runtime; they are not a separately installed extension. Configuring staff-developer does not enable `staff-developer-routing`. The execution-only `test-runner` is intended for exact ordered shell/MCP steps from final-validation tickets.
