...
| `/experimental`                              | Open the TLH experimental-feature picker in TUI, or list/change TLH experimental features via typed subcommands (`delta-follow-up-reviews`, `ci-failure-investigation`, and `session-mirror-observer` are currently registered) |
...
`/experimental` currently registers `delta-follow-up-reviews`, `ci-failure-investigation`, and `session-mirror-observer`. In the interactive TLH TUI, running `/experimental` with no arguments opens a picker that shows current feature state and lets you toggle flags; outside the TUI it falls back to the status list. Typed subcommands remain available: `/experimental list`, `/experimental status [feature]`, `/experimental enable <feature>`, `/experimental disable <feature>`, and `/experimental toggle <feature>`. `contrarian` is a bundled default minor subagent for sparing pre-ticket planning stress-tests when a proposed change genuinely warrants an adversarial brief; it is not part of the `/experimental` toggle surface, not the routine `code-reviewer` diff pass, and not the broader `oracle` second-opinion path. `delta-follow-up-reviews` is an opt-in flag that adds architect and `code-reviewer` guidance for delta-scoped follow-up reviews after fixes. `ci-failure-investigation` is an opt-in flag that lets the architect primary agent do read-only failed CI/status-check investigation after TLH opens a PR, then summarize and ask whether to proceed before any edits, commits, pushes, reruns, PR changes, or other follow-up changes. Both prompt-only flags are disabled by default. These prompt-only flags are re-read from settings on each agent turn, so enabling or disabling one applies on the next agent turn; no new session or `/reload` is required. `session-mirror-observer` is disabled by default and is opt-in only for the next conversation session: use `/experimental enable session-mirror-observer`, `/experimental status session-mirror-observer`, `/session-mirror-observer status`, and `/experimental disable session-mirror-observer`. A `/reload` keeps the current conversation session's activation snapshot, including when settings changed mid-session; `/new`, `/resume`, and `/fork` recapture the setting for the new conversation session. Its `/session-mirror-observer snapshot` request is explicitly deferred and nonblocking. It captures in-process source data and, when the installed local companion is present, attempts one bounded (provisional five-second) fail-open Unix-socket publication; a successful bridge connection remains open until observer/session shutdown and carries no durable transcript storage. When a complete snapshot exceeds the protocol bounds, bounded fallback publishes only the newest bounded section of the active branch: it omits older active-branch entries and all non-active branches, and replaces over-bound entries or compactions whose kept entry is outside the window with unsupported placeholders; it never truncates completed text or fabricates compaction summaries. The local bridge is a same-user trust boundary only: its Unix socket is not encrypted and provides no cross-user or cryptographic confidentiality, and it provides no same-UID peer authenticity, authentication, or authorization. A missing, unsafe, unavailable, or timed-out bridge never blocks the agent. It does **not** prove helper recovery, cross-process backpressure, Mac/iOS behavior, notifications, or server behavior. Status remains aggregate-only and omits transcript text, paths, session/entry IDs, source metadata, provider/tool details, arbitrary errors, and exact unbounded timings. Stable profile-owned trusted custom subagents use active-profile authorization rather than an `/experimental` flag, while project-owned custom subagents use the canonical `.tlh/agents/<slug>.md` path and project/session approval; see [custom-subagents.md](custom-subagents.md) for both. Stale `run-tests-last` and `embedded-subagents` values in `tlh.experimental.enabledFeatures` are inert and do not re-enable retired behavior. Users upgrading to stable custom subagents do not need to edit the stale `embedded-subagents` value; if you choose to clean it up manually, remove only that value and preserve every other `enabledFeatures` entry.
...
Custom subagents are not a slash command. Profile-owned agents use the active isolated profile's `agents/**/*.md` path and the exact `embedded.<slug>` runtime name. Project-owned agents use `.tlh/agents/<slug>.md`; approve the project when TLH asks, then request one naturally by slug from the architect. See [custom-subagents.md](custom-subagents.md) for both source contracts, including strict frontmatter, precedence, reload behavior, trust boundaries, resume limits, troubleshooting, and removal.
