...
Requires Node.js >=22.19.0 on `PATH`. TLH always installs its own pinned Pi 0.83.0 into a private runtime at `~/.the-last-harness/runtime` — a sibling of the isolated agent dir. A global or pre-installed `pi` on your PATH is never used or modified; tlh and any existing `pi` are fully decoupled. Install or repair failures stop with an actionable error.
...
```sh
curl -fsSL https://github.com/diegopetrucci/the-last-harness/releases/download/v0.34.0/install.sh | bash -s -- --track pinned-tag
```
