...
The official generated GitHub Release `install.sh` asset embeds its release tag and a SHA-256 checksum for every support file it can fetch, including the conditional settings/keybinding files and bundled subagent prompts. For a matching release ref and the default official repository/base URL, stage 0 verifies every successfully fetched file in one pass before it executes stage 1. An explicit `--ref` or `TLH_REF` that repeats the asset's baked tag remains eligible for verification; a different ref or custom raw base is not. A required mismatch aborts and cleans up the temporary download root; an optional mismatch is removed and treated as unavailable, so tampered support bytes are never handed to stage 1. `--no-settings` still skips the conditional files and prompts as before. This authenticates fetched support bytes against the asset's embedded inventory; obtain the stage-0 asset itself from the official HTTPS GitHub Release or another trusted channel.

This guarantee does not apply to raw source installers, mutable or custom refs, forks, custom raw bases, or local checkout files. Those paths retain the existing canonicalization/local behavior and print an explicit integrity-not-enforced notice when they need remote support files; they must not be treated as release-verified. For recovery after a failed or suspect fetch, inspect the reported ref/source and rerun the official latest-release command above, or select a known-good pinned release tag. To roll back an installed profile, use its isolated settings backup or rerun the desired pinned release installer; normal `~/.pi/agent` configuration is never changed.
...
### Switching from a local package source

During install or update, the resolved package source is supplied to the settings merger. When that source is canonical, the settings merge checks each local registration's `<resolved-source>/package.json` and removes confirmed local registrations even when the user does not pass `--package-source`. An update from a local package source does not switch registrations to canonical unless its resolved source is canonical. Relative sources are resolved from the isolated profile directory, not the command's working directory; absolute paths, `~` paths, and supported absolute or localhost `file:` forms are also recognized. TLH removes a registration only when that manifest is valid JSON with the exact name `the-last-harness`. It never executes package code or deletes the package directory. A merger invocation without a source, such as an internal doctor drift check or repair, does not authorize this cleanup.

Missing, unreadable, malformed, non-TLH, and unsupported-path manifests are insufficient evidence and remain registered for manual review. Relative `file:` URLs and `file:` URLs with non-localhost hosts are unsupported by this check and are deliberately left in place. If a duplicate remains, inspect the exact source and remove only the stale settings registration yourself before rerunning the update. The merge changes only the isolated settings and keeps the existing settings backup; to roll back, copy the appropriate `settings.json.backup-*` file over `settings.json` in the same isolated profile. This restores registrations but does not alter package directories.
...
The project custom-agent cutover is separate from retired external-package cleanup. TLH removes generic custom-agent discovery from active-profile `agents/**`, global `~/.agents`, project `.pi/agents/**` and `.agents/**`, configured `subagents.agentDirs`, and installed-package/extra-directory definitions—not merely their ability to authorize `embedded.*`. Those definitions no longer appear in TLH's custom-agent `list`/`get` or direct-dispatch inventory. Settings/default overrides cannot create or authorize a custom target. Canonical installer-managed packaged TLH roles remain available from fixed `<agent-dir>/tlh/agents/subagents/<role>.md` paths; they are not restored through generic discovery or `subagents.agentDirs`. Install/update does not silently migrate or delete the removed-source files. Move any definition you still need to the exact trusted Git-root path `.tlh/agents/custom/<UPPERCASE-SLUG>.md`, derive its lowercase frontmatter `name` from the uppercase filename stem (for example, `REPO-HELPER.md` → `name: repo-helper`), retain `package: embedded` and a non-empty `description`, and declare needed tools explicitly. Then persist project trust before dispatching from `architect` or `disabled`; see [custom-subagents.md](custom-subagents.md) for safety limits, process timing, and undo. If install/update preserves an existing `subagents.agentDirs` setting, that value is inert for TLH custom-agent authorization and canonical minor discovery; remove it manually whenever you want to clean up stale configuration.
...
Pi-native warming is separate from the retired TLH async-parent prompt-cache heartbeat. The heartbeat code has been removed; a legacy `heartbeat` key is ignored at runtime and preserved for manual cleanup, while any historical `heartbeat.jsonl` is left in place. To stop Pi-native warming, set `cacheWarming` to `off`; to restore the upstream default, remove the key or choose `streaming`. See [subagents.md](subagents.md#prompt-cache-warming) for the native warming limits, legacy-key notice, and rollback steps.
...
A direct single run has one shared deadline. A parallel batch has one shared deadline covering queue/concurrency wait, child startup, fallback/retry work, and the rest of the batch in both awaited and async modes; it is not a per-task pool. Models/callers must not send model-facing root `timeoutMs` or public `tasks[].timeoutMs`. Separately, an executable async-runner envelope/config with its own root `timeoutMs`, or a persisted plan with plan-root `timeoutMs`, fails closed before child launch with migration guidance: remove only that retired envelope/plan-root field and restart as a new direct single or parallel run. TLH-written per-step `plan.task.timeoutMs` and `plan.tasks[].timeoutMs` values remain valid trusted role-ceiling metadata and must not be removed. Existing historical records remain readable and are not rewritten. The former six-minute scout exception tracked by issue #420 is retired and is not current behavior.

To undo a persistent change, remove `execution.maxRunTimeMs` and reload/restart to restore the bounded default, or replace `false` with a positive value. Remove a canonical role's settings override to restore its code-owned ceiling, and remove a custom agent's frontmatter field to restore its 4-hour fallback. Preserve a backup and unknown keys, and never edit normal `~/.pi/agent` configuration. There is no compatibility switch for retired caller timeouts; rolling back the package version is the only way to restore that old public behavior. Other timeout fields—such as provider/network/control timeouts and historical status/reader metadata—remain separate and are not migrated into this policy.
