...
To undo a persistent change, remove `execution.maxRunTimeMs` and reload/restart to restore the bounded default, or replace `false` with a positive value. Remove a canonical role's settings override to restore its code-owned ceiling, and remove a custom agent's frontmatter field to restore its 4-hour fallback. Preserve a backup and unknown keys, and never edit normal `~/.pi/agent` configuration. There is no compatibility switch for retired caller timeouts; rolling back the package version is the only way to restore that old public behavior. Other timeout fields—such as `heartbeat.maxDurationMs`, provider/network/control timeouts, and historical status/reader metadata—remain separate and are not migrated into this policy.
