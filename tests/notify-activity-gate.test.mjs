...
const { TLH_EFFECTIVE_ACTIVITY_EVENT } = await jiti.import("../extensions/shared/tlh-effective-activity.ts");
...
test("no notify when child completion wakes parent and agent is no longer idle when timer fires", async () => {
	...
	assert.equal(notifyCount, 0, "expected no notification when agent is not idle (new turn running)");
});
...
test("notify fires when child completes without waking parent and agent stays idle", async () => {
	...
	assert.equal(notifyCount, 1, "expected exactly one notification when agent stays idle after child completes");
});
...
test("[ts-gfzs] falling edge then parent wake then new settle yields exactly one notification", async () => {
	...
	pi.emitChannel(TLH_EFFECTIVE_ACTIVITY_EVENT, { inProgress: false, activeAsyncJobIds: [] });
	assert.equal(notifyCount, 0, "no notification immediately after falling edge — debounce re-armed");
