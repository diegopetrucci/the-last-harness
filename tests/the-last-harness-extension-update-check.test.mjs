...
const { TLH_UPDATE_CHECK_INTERVAL_MS } = await jiti.import(
  "../extensions/the-last-harness/constants.ts",
);
const { getTlhVersion, normalizeTlhVersion } = await jiti.import(
  "../extensions/the-last-harness/package-version.ts",
);
...
function writeSettings(agentDir, updateCheck = {}) {
  writeFileSync(
    ...
    `${JSON.stringify({ tlh: { updateCheck } }, null, 2)}\n`,
  );
}
...
test("update checks honor every documented skip control without fetching or notifying", async (t) => {
  ...
  for (const skipCase of [
    {
      name: "outside isolated profile",
      env: { HOME: fixture.home, PI_CODING_AGENT_DIR: undefined },
    },
    {
      name: "PI_OFFLINE",
      env: { HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent, PI_OFFLINE: "1" },
    },
    {
  ...
  for (const responseCase of [
    {
      ...
      response: { ok: false, json: async () => ({ tag_name: "v9.9.9" }) },
    },
    {
      name: "missing tag_name",
      response: { ok: true, json: async () => ({ html_url: LATEST_RELEASE.releaseUrl }) },
    },
    { name: "blank tag_name", response: { ok: true, json: async () => ({ tag_name: "   " }) } },
    { name: "prefix-only tag_name", response: { ok: true, json: async () => ({ tag_name: "v" }) } },
    {
      name: "arbitrary tag_name",
      response: { ok: true, json: async () => ({ tag_name: "not-a-version" }) },
    },
    ...
    assert.equal(
      ...
      undefined,
      `${responseCase.name} must not persist a release`,
    );
  }
});
...
test("valid older, equal, and latest release responses retain persistence and notification behavior", async (t) => {
  ...
  for (const releaseCase of [
    ...
    assert.equal(
      readStartupState(fixture.agent).updateCheck?.latestVersion,
      normalizeTlhVersion(releaseCase.tagName),
    );
