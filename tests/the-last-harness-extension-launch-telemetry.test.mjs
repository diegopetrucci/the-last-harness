...
test("launch telemetry maps unknown thinkingLevel to unknown and uppercase value to custom (case-sensitivity check)", async (t) => {
	...
	assert.equal(results["missing"], "unknown:unknown", "undefined thinkingLevel should map to unknown:unknown");
	assert.equal(
		results["uppercase"],
		"unknown:custom",
...
test("joinModelEffort degenerate combinations: unknown:unknown, custom:high, and known-model:unknown", async (t) => {
	...
	try {
		await withEnv(
			...
			async () => {
				...
				await sendTlhLaunchTelemetry({ version: "model-known-effort-unknown", modelId: "claude-opus-4-5" });
			},
		);
	} finally {
		globalThis.fetch = previousFetch;
	...
	assert.equal(results["both-unknown"], "unknown:unknown", "no model and no thinking → unknown:unknown");
	assert.equal(results["custom-high"], "custom:high", "non-public model + known thinking → custom:high");
	assert.equal(
