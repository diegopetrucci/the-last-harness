...
test("stage-1 records piInstalledByTlh=true when installing the private runtime", (t) => {
  for (const scenario of [
    ...
    const root = makeTempDir(
      `tlh-install-stage1-pi-runtime-${scenario.name.replace(/\s+/g, "-")}-`,
    );
    const homeDir = join(root, "home");
    const agentDir = join(root, "agent");
    const binDir = join(root, "bin");
...
test("stage-1 installPiIfNeeded: broken npm install (wrong pi version) throws", (t) => {
  ...
  assert.notEqual(
    result.status,
    0,
    `expected installer to fail on wrong-version pi, got exit 0:\n${output}`,
  );
  ...
  assert.equal(
    existsSync(legacyPiPath),
    true,
    "user-owned ~/.local/bin/pi was removed despite installer throwing",
  );
...
test("stage-1 regression (tlht-5php): installer never removes or execs user-owned ~/.local/bin/pi when piInstalledByTlh=true and private runtime is absent", (t) => {
  ...
  assert.equal(
    existsSync(legacyPiPath),
    true,
    "user-owned ~/.local/bin/pi was incorrectly removed by the installer",
  );
...
test("uninstall.sh does not remove legacy ~/.local/bin/pi without --force-include-pi (prints hint); removes it with --force-include-pi", (t) => {
  ...
  assert.match(
    dryOutput,
    /force-include-pi/,
    "dry-run output must mention --force-include-pi hint",
  );
  ...
  assert.equal(
    existsSync(join(legacyBin, "pi")),
    false,
    "legacy pi should have been removed with --force-include-pi",
  );
});
...
test("uninstall.sh regression (tlht-h7vq): migrated runtime marker preserves nested foreign packages during surgical uninstall", (t) => {
  ...
  writeFileSync(
    join(foreignPackageDir, "package.json"),
  ...
  assert.doesNotMatch(
    dryOutput,
    new RegExp(`would remove private runtime: rm -rf ${escapeRegExp(runtimeDir)}`),
  );
  assert.equal(
    existsSync(foreignPackageDir),
    true,
    "dry-run must not remove nested foreign package",
  );
  ...
  const realResult = spawnSync(
    "bash",
    [join(repoRoot, "uninstall.sh"), "--agent-dir", agentDir, "--bin-dir", binDir],
    {
      cwd: repoRoot,
      env: scrubInstallerEnv({ HOME: homeDir, PATH: `${fakeNpmDir}:${process.env.PATH || ""}` }),
      encoding: "utf8",
  ...
  assert.equal(
    ...
    false,
    "TLH runtime launcher should be removed surgically",
  );
  assert.equal(
    existsSync(tlhPackageDir),
    false,
    "TLH runtime package should be removed surgically",
  );
  assert.equal(
    existsSync(markerPath),
    false,
    "migrated runtime ownership marker should be cleared after uninstall",
  );
  assert.equal(
    existsSync(foreignPackageDir),
    true,
    "nested foreign package must survive migrated runtime uninstall",
  );
  assert.equal(
  ...
  assert.equal(
    ...
    true,
    "shared runtime prefix must survive migrated runtime uninstall",
  );
});
...
test("stage-1 refuses to install into a runtime prefix containing a foreign top-level entry", (t) => {
  ...
  for (const scenario of scenarios) {
    const root = makeTempDir(
      `tlh-install-runtime-guard-${scenario.label.replace(/[^a-z0-9]+/g, "-")}-`,
    );
    const homeDir = join(root, "home");
    const agentDir = join(root, "agent");
    const binDir = join(root, "bin");
    ...
    assert.notEqual(
      result.status,
      0,
      `${scenario.label}: expected failure but installer succeeded\n${output}`,
    );
    // Error message must be actionable and name the ownership marker.
    assert.match(
      output,
      /is not TLH-owned/,
      `${scenario.label}: error must name the ownership problem`,
    );
    ...
    assert.equal(
      existsSync(npmLog),
      false,
      `${scenario.label}: npm must not be invoked when guard refuses`,
    );
  }
});
...
test("runtime ownership: non-empty unmarked prefix without provenance is refused", (t) => {
  ...
  assert.equal(
    existsSync(join(runtimeDir, ".tlh-runtime-owned")),
    false,
...
test("runtime ownership: existing valid marker (path-matched) is accepted on reuse", (t) => {
  ...
  assert.equal(
    existsSync(npmLog),
    false,
    "npm must not be called when valid marker present and pi is valid",
  );
...
test("runtime ownership: non-empty unmarked prefix with piInstalledByTlh=true is migrated", (t) => {
  ...
  assert.equal(
    existsSync(npmLog),
    false,
    "npm must not be called during provenance-gated migration",
  );
...
test("runtime ownership: symlinked runtime prefix is refused", (t) => {
  ...
  assert.notEqual(
    result.status,
    0,
    `expected failure on symlinked runtime but installer succeeded\n${output}`,
  );
...
test("uninstall.sh: shared/unmarked runtime prefix is SKIPPED with conditional hint; foreign package preserved", (t) => {
  ...
  writeFileSync(
    join(runtimeDir, "lib", "node_modules", "some-other-tool", "index.js"),
    "// user content\n",
    "utf8",
  );
  ...
  assert.doesNotMatch(
    ...
    "must not plan runtime removal without marker",
  );
...
test("uninstall.sh: malformed or missing marker causes SKIP (fail-closed)", (t) => {
  ...
  for (const scenario of scenarios) {
    const root = makeTempDir(
      `tlh-uninstall-marker-c-${scenario.label.replace(/[^a-z0-9]+/g, "-")}-`,
    );
    const homeDir = join(root, "home");
    const agentDir = join(root, "agent");
    const binDir = join(root, "bin");
...
test("uninstall.sh: runtimeAbsPath mismatch in marker causes SKIP", (t) => {
  ...
  assert.doesNotMatch(
    output,
    /would remove private runtime/,
    "must not plan rm -rf when runtimeAbsPath mismatches",
  );
...
test("uninstall.sh: marker dotfile is in exclusivity allow-list; runtime with bin+lib+node-compile-cache+marker is planned for removal", (t) => {
  ...
  assert.match(
    output,
    /rm -rf/,
    "runtime with bin+lib+node-compile-cache+marker must be planned for rm -rf",
  );
  assert.doesNotMatch(
    output,
    /leave it/i,
    "exclusivity allow-list must not trip on marker dotfile",
  );
});
