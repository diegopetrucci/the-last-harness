...
test("tlh update rejects legacy ticket integration flags", (t) => {
  ...
  const runUpdate = (...extraArgs) =>
    spawnSync(
      process.execPath,
      [
        join(repoRoot, "scripts/tlh-update.mjs"),
        "--dry-run",
        "--agent-dir",
        agentDir,
        "--bin-dir",
...
test("tlh update --extensions dry-run prints the isolated package update plan and rejects installer-only flags", (t) => {
  ...
  writeFakePi(
    fakebin,
    "printf 'pi should not run during dry-run\\n' >\"${DRY_RUN_PI_LOG}\"\nexit 91",
  );
  t.after(() => rmSync(root, { recursive: true, force: true }));
...
test("tlh update --extensions refuses to target normal Pi config via explicit or inherited agent dir selection", (t) => {
  ...
  for (const scenario of scenarios) {
    const result = spawnSync(
      process.execPath,
      [join(repoRoot, "scripts/tlh-update.mjs"), "--extensions", "--dry-run", ...scenario.args],
      {
        cwd: repoRoot,
        env: scrubInstallerEnv({
          HOME: homeDir,
    ...
    assert.ok(
      result.stderr.includes(protectedAgentDir),
      `${scenario.name} stderr should mention the protected agent dir`,
    );
...
test("tlh recovery update refuses to target normal Pi config before reading install-state", (t) => {
  ...
  for (const scenario of scenarios) {
    const result = spawnSync(
      process.execPath,
      [join(repoRoot, "scripts/tlh-recover-update.mjs"), "--dry-run", ...scenario.args],
      {
        cwd: repoRoot,
        env: scrubInstallerEnv({
          HOME: homeDir,
    ...
    assert.match(
      result.stderr,
      /refusing to recover The Last Harness update against normal Pi config root/,
    );
    assert.ok(
      result.stderr.includes(protectedAgentDir),
      `${scenario.name} stderr should mention the protected agent dir`,
...
test("tlh update --extensions uses absolute private runtime pi and hard-fails when missing", (t) => {
  ...
  const missingResult = spawnSync(
    process.execPath,
    [join(repoRoot, "scripts/tlh-update.mjs"), "--extensions", "--agent-dir", agentDir2],
    {
      cwd: repoRoot,
      env: scrubInstallerEnv({
        HOME: homeDir2,
  ...
  assert.notEqual(
    ...
    0,
    "expected hard failure when private runtime pi is missing",
  );
  ...
  assert.equal(
    dryRunResult.status,
    0,
    `dry-run must succeed even without runtime pi: ${dryRunResult.stderr}`,
  );
  assert.match(dryRunResult.stdout, /Would run:/);
  assert.match(
    dryRunResult.stdout,
    new RegExp(join(root2, "runtime", "bin", "pi").replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
  );
});
