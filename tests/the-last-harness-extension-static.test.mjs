...
const annotateGitDiffUiSource = readFileSync(
  new URL("../extensions/annotate-git-diff/ui.ts", import.meta.url),
  "utf8",
);
...
const changelogSource = readFileSync(
  new URL("../extensions/the-last-harness/changelog.ts", import.meta.url),
  "utf8",
);
...
const effortSource = readFileSync(
  new URL("../extensions/the-last-harness/effort.ts", import.meta.url),
  "utf8",
);
...
const promptsSource = readFileSync(
  new URL("../extensions/the-last-harness/prompts.ts", import.meta.url),
  "utf8",
);
const tokensSource = readFileSync(
  new URL("../extensions/the-last-harness/tokens.ts", import.meta.url),
  "utf8",
);
...
const typesSource = readFileSync(
  new URL("../extensions/the-last-harness/types.ts", import.meta.url),
  "utf8",
);
...
function resolveAmdModuleId(fromModuleId, target) {
  ...
  for (const segment of normalizedTarget.split("/")) {
    ...
    if (segment === "..") {
      assert.ok(
        segments.length > 0,
        `cannot resolve Monaco AMD module from ${fromModuleId} via ${target}`,
      );
      segments.pop();
      continue;
    }
...
function extractMonacoModuleSource(runtimeSource, moduleId) {
  ...
  assert.equal(
    amdDefineModuleId(moduleStatement),
    moduleId,
    `Monaco module ${moduleId} must be an AMD define() call`,
  );
...
function discoverBasicLanguageLoaderTarget(runtimeSource, languageId, extensions) {
  const extensionsPattern = extensions
    .map((extension) => `"${escapeRegex(extension)}"`)
    .join("\\s*,\\s*");
  ...
  assert.fail(
    `Monaco basic-language registration for ${languageId} must include an inlined loader target`,
  );
}

function assertRepresentativeMonacoLanguageChunkInlined(
  runtimeSource,
  _contributionModuleId,
  { id, extensions },
) {
  const { fromModuleId, loaderTarget } = discoverBasicLanguageLoaderTarget(
    runtimeSource,
    id,
    extensions,
  );
  ...
  assert.ok(
    targetModuleSource.trim().length > 256,
    `Monaco ${id} loader target ${targetModuleId} must be non-empty`,
  );
  assert.match(
    targetModuleSource,
    /\.conf\s*=|\bconf:/,
    `Monaco ${id} chunk must export language configuration`,
  );
  assert.match(
    targetModuleSource,
...
function existingNestedExtensionEntrypoints(directoryName) {
  return PI_EXTENSION_DIRECTORY_ENTRYPOINT_FILES.map(
    (entrypointFile) => `${directoryName}/${entrypointFile}`,
  ).filter((relativePath) => existsSync(join(extensionsDir, relativePath)));
}

function parseInlineJsonStringAssignment(source, assignmentName) {
  const escapedAssignmentName = assignmentName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const assignment = source.match(
    new RegExp(`${escapedAssignmentName} = ("(?:\\\\.|[^"\\\\])*");`),
  );
...
test("package manifest selects only the ordered generated JS entrypoints", () => {
  ...
  assert.deepEqual(existingNestedExtensionEntrypoints("notify"), [
    "notify/index.ts",
    "notify/index.js",
  ]);
...
test("annotate-git-diff review HTML inlines Monaco assets without file:// URLs into node_modules", () => {
  ...
  assert.doesNotMatch(
    ...
    "Monaco must not dynamically append a CSS link",
  );
  assert.match(
    html,
    /"bootstrapError":null/,
    "packaged review assets should load without a bootstrap error",
  );
  ...
  assert.ok(
    workerSource.trim().length > 1024,
    "editor worker bundle must be a non-empty inlined script",
  );
  assert.match(
    workerSource,
  ...
  assert.match(
    workerSource,
    /\bpostMessage\b/,
    "editor worker bundle must communicate with the host",
  );
  ...
  const contributionModuleMatch = runtimeSource.match(
    /define\("([^"]*basic-languages\/monaco\.contribution)"/,
  );
  ...
  assert.ok(
    setupMonacoSource.indexOf("window.MonacoEnvironment =") <
      setupMonacoSource.indexOf('["vs/index"]'),
  );
...
test("before_agent_start reapplies primary defaults without a one-shot model gate", () => {
  const lifecycleHooks = sourceSection(
    primaryRuntimeSource,
    "function registerLifecycleHooks()",
    "\n\n  return {",
  );
  const beforeAgentStart = sourceSection(
    lifecycleHooks,
  ...
  assert.match(
    ...
    /selectProviderAwareAgentDefaults\(\s*primary,\s*availableModels,\s*ctx\.model\?\.provider,?\s*\)/,
  );
  assert.match(
    applyPrimaryDefaults,
    /resolvePrimaryAutoApplySetting\(primaryConfig, primary, "applyModel"\)/,
  );
  assert.match(
    applyPrimaryDefaults,
    /resolvePrimaryAutoApplySetting\(primaryConfig, primary, "applyThinking"\)/,
  );
  assert.match(
    applyPrimaryModel,
  ...
  assert.match(
    currentThinkingSatisfiesPrimaryFloor,
...
test("before_agent_start activates ticket runtime without disabled-ticket prompt branching", () => {
  ...
  assert.match(
    beforeAgentStart,
    /buildActivePrimarySystemPrompt\(event\.systemPrompt, ctx\.cwd, settings\)/,
  );
...
test("allowed-subagents prompt keeps bundled minor-agent listings and scopes embedded guidance to architect", () => {
  ...
  assert.doesNotMatch(
    architectUndefined,
    embeddedClause,
    "architect+undefined: no embedded clause",
  );
...
test("extension imports extracted shared helpers from nested TypeScript modules", () => {
  ...
  assert.deepEqual(
    staticImportSpecifiers(extensionSource).filter((specifier) =>
      ["./the-last-harness/tokens.js", "./the-last-harness/annotate-last-message.js"].includes(
        specifier,
      ),
    ),
    [],
    "tokens and annotate-last-message must not be top-level static imports",
  );
  assert.match(extensionSource, /from "\.\/the-last-harness\/review\.js"/);
...
test("extension lazy-loads tokens and annotate-last-message with retryable facades; review and changelog are static", () => {
  assert.match(
    extensionSource,
  ...
  assert.match(
    extensionSource,
    /const ANNOTATE_LAST_MESSAGE_COMMAND_DESCRIPTION =\s*"Open a native annotation window for the latest assistant message";/,
  );
  assert.match(
    extensionSource,
    /const TLH_CHANGELOG_COMMAND_DESCRIPTION =\s*"Show TLH release notes from the packaged changelog";/,
  );
  assert.match(
    extensionSource,
    /function createRetryableLazyImport<TModule>[\s\S]*?loader: \(\) => Promise<TModule>[\s\S]*?\(\) => Promise<TModule> \{/,
  );
  assert.match(
    extensionSource,
  ...
  assert.match(
    extensionSource,
    /pi\.registerCommand\("review", \{[\s\S]*getArgumentCompletions: \(\) => null,[\s\S]*handler: reviewCommandHandler,/,
  );
  assert.match(
    extensionSource,
  ...
  assert.match(
    extensionSource,
    /pi\.registerCommand\("tlh-changelog", \{[\s\S]*handler: \(args, ctx\) => handleTlhChangelogCommand\(pi, args, ctx\),/,
  );
  assert.match(
    extensionSource,
...
test("production annotate-last-message facade wires sendUserMessage through to the command builder", () => {
  ...
  assert.match(
    annotateLastMessageSource,
    /\n {10}sendUserMessage\(prompt, \{ deliverAs: "followUp" \}\);/,
  );
...
test("header and footer install before deferred update side effects", () => {
  ...
  assert.doesNotMatch(
    sessionStart,
    /scheduleDeferredStartupTask\(\(\) => \{\s*if \(event\.reason === "startup"\)/,
  );
...
test("thinking alias shares the effort command thinking-level behavior", () => {
  ...
  assert.match(effortSource, /from "\.\/effort-command\.js"/);
...
test("extension delegates launch update and telemetry services to feature modules", () => {
  ...
  assert.match(
    sessionStart,
    /if \(event\.reason === "startup"\) \{[\s\S]*try \{[\s\S]*scheduleTlhLaunchTelemetry\(ctx, primaryAgentRuntime\.activePrimaryAgentPrompt\(\)\?\.name\);[\s\S]*\} catch \{/,
  );
  assert.match(
    sessionStart,
  ...
  assert.match(
    sessionStart,
    /if \(!ctx\.hasUI\) \{[\s\S]*return;[\s\S]*if \(event\.reason === "startup"\)/,
  );
  assert.match(sessionStart, /const headerUpdate = getTlhHeaderUpdate\(\);/);
  assert.match(
    sessionStart,
...
test("extension registers synchronous header-session invalidation before async shutdown handlers", () => {
  ...
  assert.notEqual(
    ...
    -1,
    "expected a synchronous header-session invalidation handler",
  );
  assert.equal(
...
test("extension runs primary session_start work before UI startup in one handler", () => {
  ...
  assert.match(
    sessionStart,
    /await primaryAgentRuntime\.applySessionStart\(ctx\);[\s\S]*if \(!ctx\.hasUI\)/,
  );
  assert.match(
    primaryRuntimeSource,
...
test("extension wires switch-primary-agent and active-primary safety", () => {
  ...
  assert.match(
    switchPrimaryAgentCommand,
    /writeTlhPrimaryAgentDefault\(ctx\.cwd, defaultSelection\)/,
  );
  ...
  assert.match(
    toolCall,
    /if \(event\.toolName === "bash"\) \{[\s\S]*resolveTlhCommitAttribution\(\s*getTlhGlobalSettings\(ctx\.cwd\)\.tlh\?\.attribution,?\s*\)/,
  );
  assert.match(
    toolCall,
    /getTlhGitCommitAttributionBlockReason\(\s*event\.input\.command,\s*commitAttributionState,\s*\)/,
  );
  assert.match(
    toolCall,
  ...
  assert.match(
    toolCall,
    /const allowedSubagents = allowedSubagentsForExperimentalConfig\(\s*getTlhGlobalSettings\(ctx\.cwd\)\.tlh\?\.experimental,?\s*\)/,
  );
  assert.match(
    toolCall,
  ...
  assert.match(
    toolCall,
    /const requestedEmbeddedTargets = collectSubagentCallTargetsMatching\(\s*event\.input,\s*isEmbeddedSubagentTarget,?\s*\)/,
  );
  assert.match(
    toolCall,
  ...
  assert.match(
    toolCall,
    /const reason = validateSubagentToolInput\(\s*event\.input,\s*\{\s*allowedSubagents,\s*allowEmbeddedTargets,?\s*\}\s*\)/,
  );
  assert(
    toolCall.indexOf('if (event.toolName === "bash")') <
      toolCall.indexOf("resolveTlhCommitAttribution"),
  ...
  assert(
    toolCall.lastIndexOf('subagentCallTargetsAgent(event.input, "developer")') <
      genericValidationIndex,
  ...
  const embeddedAuthorizationScanIndex = toolCall.indexOf(
...
test("extension wires subscription usage to lifecycle refreshes and footer", () => {
  ...
  assert.match(
    sessionStart,
    /typeof footerData\?\.onBranchChange === "function"\s*\?\s*\(cb\) => footerData\.onBranchChange\(cb\)\s*:\s*undefined/,
  );
...
test("extension wires TLH changelog static import and release-notes rendering", () => {
  assert.match(
    extensionSource,
    /pi\.registerCommand\("tlh-changelog", \{[\s\S]*description: TLH_CHANGELOG_COMMAND_DESCRIPTION,[\s\S]*handler: \(args, ctx\) => handleTlhChangelogCommand\(pi, args, ctx\),/,
  );
  assert.match(
    changelogSource,
    /export const TLH_CHANGELOG_COMMAND_DESCRIPTION =\s*"Show TLH release notes from the packaged changelog";/,
  );
  assert.match(
    changelogSource,
...
test("extension keeps TLH experimental command wiring with registered ticket, ci, and review feature flags", () => {
  ...
  assert.match(ticketWorkflowUiFacadeSource, /from "\.\/ticket-workflow-ui\.js"/);

  assert.match(experimentalSource, /pi\.registerCommand\("experimental"/);
  assert.match(experimentalSource, /from "\.\/experimental-command\.js"/);
  ...
  assert.doesNotMatch(
    experimentalSource,
    /Enables the contrarian minor agent and primary-agent guidance/,
  );
  ...
  assert.match(attributionSource, /from "\.\/attribution-command\.js"/);
  ...
  assert.match(usageLimitsSource, /from "\.\/usage-limits-command\.js"/);
  ...
  assert.match(
    backupWriteHelper,
    /suffix === 0[\s\S]*?`\$\{settingsPath\}\.bak-\$\{timestamp\}`[\s\S]*?`\$\{settingsPath\}\.bak-\$\{timestamp\}-\$\{suffix\}`/,
  );
  assert.match(
    backupWriteHelper,
