...
test("analyzeSessionEntries summarizes exact assistant usage, estimated tool attribution, and redacted subagent discoveries", () => {
  ...
  assert.equal(
    analysis.tools.byTool.find((tool) => tool.toolName === "bash")?.source.kind,
    "built-in",
  );
  assert.equal(
    analysis.tools.byTool.find((tool) => tool.toolName === "subagent")?.source.kind,
    "extension",
  );
  assert.equal(
    analysis.tools.byTool.find((tool) => tool.toolName === "mcp")?.source.kind,
    "mcp-proxy",
  );
        ...
        results: [
          {
            ...
            sessionFile:
              "/Users/me/.the-last-harness/agent/sessions/--repo--/run-77/researcher.jsonl",
            artifactPaths: {
          ...
          },
          {
            ...
            sessionFile:
              "/Users/me/.the-last-harness/agent/sessions/--repo--/run-77/developer.jsonl",
            artifactPaths: {
  ...
  assert.deepEqual(
    ...
    [
      { agent: "developer", mode: "parallel", runId: "run-77", session: "run-77/developer.jsonl" },
      {
        ...
        mode: "parallel",
        runId: "run-77",
        session: "run-77/researcher.jsonl",
      },
    ],
  );
  assert.deepEqual(analysis.totals.subagents, {
...
test("analyzeCurrentSessionUsage reads session metadata and avoids false subagent discoveries from ordinary tool details", () => {
  const entries = [
    assistantEntry("a1", null, "2026-06-15T13:00:00Z", {
      content: [
        { type: "toolCall", id: "call-bash-1", name: "bash", arguments: { command: "pwd" } },
      ],
    }),
    toolResultEntry(
      "tr1",
...
test("subagents.byAgent groups runs by agent+provider and sums usage across multiple agents", () => {
  ...
  const byAgentInputTotal = analysis.subagents.byAgent.reduce(
...
test("direct MCP tools count toward estimated MCP attribution when the current tool catalog identifies them", () => {
  const entries = [
    assistantEntry("a1", null, "2026-06-15T14:00:00Z", {
      stopReason: "tool_use",
      content: [
        { type: "toolCall", id: "call-exa-1", name: "exa_search", arguments: { query: "secret" } },
      ],
    ...
    current.next =
      depth === 7
        ? {
            agent: "developer",
            sessionFile:
              "/Users/me/.the-last-harness/agent/sessions/--repo--/run-depth/within.jsonl",
          }
        : {};
...
test("self- and cross-referential arrays are read once per edge with cycle dedupe", () => {
  ...
  const right = Array.from({ length: 2 });
...
test("cacheMisses: idle-gap miss is detected when the previous context was not served from cache", () => {
  ...
  assert.equal(
    analysis.cacheMisses.worst[0].turnIndex,
    1,
    "turnIndex is 0-based; second assistant message = index 1",
  );
...
test("cacheMisses: compaction entry clears the prev baseline so no miss is detected across it", () => {
  ...
  assert.equal(
    ...
    0,
    "compaction should clear prev; no miss across it",
  );
...
test("cacheMisses: branch_summary entry also clears prev baseline", () => {
  ...
  assert.equal(
    analysis.cacheMisses.missCount,
    0,
    "branch_summary should clear prev; no miss across it",
  );
});
...
test("cacheMisses: worst list is sorted by missedTokens descending and capped at 10", () => {
  ...
  assert.ok(
    analysis.cacheMisses.missCount >= 10,
    `expected >=10 misses, got ${analysis.cacheMisses.missCount}`,
  );
  assert.equal(
    analysis.cacheMisses.worst.length,
    Math.min(analysis.cacheMisses.missCount, 10),
    "worst capped at 10",
  );
...
test("analyzeSessionEntries leaves observedLatency absent when no toolCallId pairs match", () => {
  ...
  assert.equal(
    bashTool.observedLatency,
    undefined,
    "observedLatency is absent when no toolCallId pairs match",
  );
