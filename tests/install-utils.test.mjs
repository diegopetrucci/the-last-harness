...
test("required CLI value helpers preserve installer option errors", () => {
  ...
  assert.throws(
    () => assignRequiredEqualsValue(target, "key", "", "--flag"),
...
test("shared CLI option helpers parse split, equals, alias, and default path values", () => {
  ...
  assert.deepEqual(
    readOptionValue(["--defaults=manifest.json"], 0, ["--defaults", "--default-extensions"]),
    {
      flag: "--defaults",
      value: "manifest.json",
      nextIndex: 0,
  ...
  assert.equal(
    resolveTlhAgentDir("~/custom-agent", { homeDir: "/tmp/home" }),
    "/tmp/home/custom-agent",
  );
  assert.equal(
  ...
  assert.equal(
    defaultTlhBinDir({ TLH_BIN_DIR: "~/bin" }, { homeDir: "/tmp/home" }),
    "/tmp/home/bin",
  );
});
...
test("readJsonFile handles BOM, empty files, missing fallbacks, and parse errors", (t) => {
  ...
  assert.deepEqual(readJsonFile(join(root, "missing.json"), { missingValue: { missing: true } }), {
    missing: true,
  });
...
test("backup timestamp helpers render existing suffix formats", () => {
  ...
  assert.equal(
    backupPathWithTimestamp("settings.json", { date }),
    "settings.json.backup-2026-05-20T21-22-23-456Z",
  );
  assert.equal(
    backupPathWithTimestamp("settings.json", {
...
test("normal Pi config helpers guard paths under only the normal ~/.pi root", (t) => {
  ...
  assert.equal(
    pathIsInNormalPiConfig(join(homeDir, ".pi", "agent", "settings.json"), { homeDir }),
    true,
  );
...
test("selectExpiredBackups: falls back to mtimeFallback when filename timestamp is unparseable", () => {
  ...
  const result = selectExpiredBackups([unknownOld, unknownNew], {
    now,
    keepNewest: 1,
    mtimeFallback,
  });
...
test("isTlhOwnedBackupFilename: accepts all four known TLH marker forms", () => {
  ...
  assert.equal(
    isTlhOwnedBackupFilename("settings.json.backup-2026-07-11T17-01-16Z", "settings.json"),
    true,
  );
  // Empty marker (with milliseconds)
  assert.equal(
    isTlhOwnedBackupFilename("settings.json.backup-2026-07-11T17-01-16-155Z", "settings.json"),
    true,
  );
  // before-install marker
  assert.equal(
    isTlhOwnedBackupFilename(
      "settings.json.backup-before-install-2026-07-10T20-58-04Z",
      "settings.json",
    ),
    true,
  );
  ...
  assert.equal(
    isTlhOwnedBackupFilename(
      "settings.json.backup-tlh-defaults-2026-07-10T09-42-45-504Z",
      "settings.json",
    ),
    true,
  );
  ...
  assert.equal(
    isTlhOwnedBackupFilename(
      "settings.json.backup-tlh-tickets-2026-07-10T09-42-45-504Z",
      "settings.json",
    ),
    true,
  );
  ...
  assert.equal(
    isTlhOwnedBackupFilename(
      "keybindings.json.backup-2026-07-11T17-01-16-155Z",
      "keybindings.json",
    ),
    true,
  );
...
test("isTlhOwnedBackupFilename: rejects unknown marker even with a parseable trailing timestamp", () => {
  ...
  assert.equal(
    isTlhOwnedBackupFilename(
      "settings.json.backup-work-project-2026-07-11T17-01-16Z",
      "settings.json",
    ),
    false,
  );
...
test("isTlhOwnedBackupFilename: rejects filenames with no timestamp or wrong base", () => {
  ...
  assert.equal(
    isTlhOwnedBackupFilename("settings.json.backup-2026-07-11T17-01-16-155Z", "keybindings.json"),
    false,
  );
...
test("isTlhOwnedBackupFilename: rejects shape-valid but semantically invalid timestamps", () => {
  ...
  assert.equal(
    isTlhOwnedBackupFilename(
      "settings.json.backup-before-install-2026-99-99T99-99-99Z",
      "settings.json",
    ),
    false,
