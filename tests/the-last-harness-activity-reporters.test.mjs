...
test("Herdr reporter shutdown never emits pane.release_agent", async () => {
	...
	assert.deepEqual(releaseCalls, [], "pane.release_agent must never be emitted by the herdr reporter");
	...
	assert.equal(stateCallsAfterShutdown, stateCallsBeforeShutdown, "no state sends should occur after shutdown");
});
...
test("queued Herdr heartbeat reads idle after a real send settles", async () => {
	...
	await flushAsyncWork();
	assert.equal(deliveries.length, 3);
	assert.equal(deliveries[2].request.params.state, "idle", "heartbeat must not replay stale working");
	assert.ok(deliveries[2].request.params.seq > deliveries[1].request.params.seq);
...
test("Herdr reporter heartbeat re-sends last state with strictly increasing seq", async () => {
	...
	assert.equal(preStateReportCalls.length, 0, "heartbeat must not fire before the first state report");
...
test("Herdr reporter heartbeat stops on handleSessionShutdown and dispose", async () => {
	...
	shutdownReporter.handleSnapshot({ inProgress: true, primaryReasons: ["primary:agent-loop"], activeAsyncJobIds: [] });
	await flushAsyncWork();
	...
	disposeReporter.handleSnapshot({ inProgress: true, primaryReasons: ["primary:agent-loop"], activeAsyncJobIds: [] });
	await flushAsyncWork();
