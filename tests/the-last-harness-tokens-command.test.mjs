    ...
    assistantEntry("a1", "u1", "2026-06-15T12:00:05Z", {
      ...
      content: [
        ...
        { type: "toolCall", id: "call-bash-1", name: "bash", arguments: { command: "ls" } },
        {
          ...
          id: "call-subagent-1",
          name: "subagent",
          arguments: { task: "RAW SUBAGENT TASK" },
        },
      ],
      ...
      content: [
        {
          ...
          id: "call-mcp-1",
          name: "mcp",
          arguments: { server: "exa", prompt: "secret query" },
        },
      ],
...
test("analyzeSessionEntries computes non-zero approxTokens for tool calls and results", () => {
  ...
  const toolCatalog = [
    {
      name: "bash",
      sourceInfo: { source: "built-in", path: "core/bash.ts", scope: "user", origin: "top-level" },
    },
    {
      name: "mcp",
      sourceInfo: {
  ...
  assert.equal(
    analysis.tools.mcpApproxTokens,
    mcpTool.approxTokens,
    "mcpApproxTokens matches mcp tool approxTokens",
  );
      ...
      content: [
        {
          ...
          id: "call-mcp-1",
          name: "mcp",
          arguments: { server: "search", query: "example" },
        },
      ],
...
test("analyzeSessionEntries counts result-content characters toward a tool's approxTokens even when tool arguments are empty", () => {
  ...
  const toolCatalog = [
    {
      name: "read-file",
      sourceInfo: { source: "built-in", path: "core/read.ts", scope: "user", origin: "top-level" },
    },
  ];
  const analysis = analyzeSessionEntries(entries, { toolCatalog });
  ...
  assert.ok(
    readTool.approxTokens > 0,
    "result-content text alone contributes non-zero approxTokens",
  );
});
...
test("analyzeSessionEntries byTool sorts by approxTokens descending, with callCount as tiebreaker", () => {
  ...
  assert.equal(
    expensiveTool.approxTokens,
    expectedExpensive,
    `expensive-tool approxTokens is ${expectedExpensive}`,
  );
  assert.equal(
    cheapTool.approxTokens,
    expectedCheap,
    `cheap-tool approxTokens is ${expectedCheap}`,
  );
  ...
  assert.ok(
    expensiveTool.approxTokens > cheapTool.approxTokens,
    "expensive-tool has more approxTokens",
  );
...
test("renderUsageTotalsTable renders per-subagent sub-rows with plain agent name (no arrow prefix) and provider", () => {
  ...
  assert.ok(
    developerIdx < subtotalIdx,
    "developer sub-row appears before Subagents (all) subtotal",
  );
});
...
test("buildTokensReportHtml uses a provided primaryAgentLabel in all four primary-bucket sites", () => {
  ...
  assert.match(
    html,
    /<p class="card-label">architect<\/p>/,
    "site 2: Overview metric card title uses provided label",
  );

  // Site 3: models table heading becomes '<label> models'
  assert.match(
    html,
    /<h3>architect models<\/h3>/,
    "site 3: models table heading uses provided label",
  );
...
test("buildTokensReportHtml falls back to 'Primary assistant' when no primaryAgentLabel is given", () => {
  ...
  assert.match(
    html,
    /<td>Primary assistant<\/td><td>\u2014<\/td>/,
    "site 1: fallback label in usage totals table",
  );

  // Site 2: Overview metric card title
  assert.match(
    html,
    /<p class="card-label">Primary assistant<\/p>/,
    "site 2: fallback label in Overview metric card",
  );

  // Site 3: models table heading
  assert.match(
    html,
    /<h3>Primary assistant models<\/h3>/,
    "site 3: fallback label in models table heading",
  );
  ...
  const cacheSection = html.slice(cacheTableIdx);
  assert.ok(
    cacheSection.indexOf("<td>Primary assistant</td>") !== -1,
    "site 4: fallback label in cache totals table",
  );
});
