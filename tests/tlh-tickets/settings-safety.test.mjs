...
test(
  "settings writes do not follow the old predictable temp-file symlink",
  {
...
test(
  "settings direct write refuses parent swap before open without touching external sentinels",
  {
    skip: process.platform === "win32",
  },
  () => {
...
test(
  "settings direct write removes empty file created by parent swap to normal Pi",
  {
    skip: process.platform === "win32",
  },
  () => {
    ...
    assert.match(
      result.stderr,
      /normal Pi config|outside the intended directory|symlinked settings directory/i,
    );
...
test(
  "settings backup write refuses parent swap before open without copying settings outside",
  {
    skip: process.platform === "win32",
  },
  () => {
...
test(
  "settings write refuses parent swap before intended directory realpath capture",
  {
    skip: process.platform === "win32",
  },
  () => {
...
test(
  "settings directory creation does not clean up a directory when parent revalidation fails",
  {
    skip: process.platform === "win32",
  },
  () => {
...
test(
  "settings directory creation does not remove external directory swapped after mkdir",
  {
    skip: process.platform === "win32",
  },
  () => {
...
test(
  "settings writes create new settings with restrictive mode",
  { skip: process.platform === "win32" },
  () => {
    const fixture = tempFixture();
    const settings = join(fixture.agent, "settings.json");
...
test(
  "settings writes preserve existing settings and backup modes",
  { skip: process.platform === "win32" },
  () => {
    const fixture = tempFixture();
    const settings = join(fixture.agent, "settings.json");
    ...
    const result = runTickets(ticketEnableArgs(fixture, settings, ["--detail"]), {
    ...
    const backups = readdirSync(fixture.agent).filter((entry) =>
      entry.startsWith("settings.json.backup-tlh-tickets-"),
    );
