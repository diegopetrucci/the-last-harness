...
function assistantMsg(
  timestampIso,
  { input = 100, output = 50, cacheRead = 0, cacheWrite = 0, costTotal = 0 } = {},
) {
  return {
    type: "message",
...
test("aggregateSessionUsage: assistant messages without usage are counted in coverage", () => {
  ...
  assert.ok(
    result.caveats.some((c) =>
      c.includes("1 of 2 in-window assistant message(s) had no usage data"),
    ),
  ...
  const entries = [
    {
      ...
      id: "sess-001",
      timestamp: "2026-05-01T10:00:00.000Z",
      cwd: "/Users/foo/real-project-name",
    },
  ];
  const result = aggregateSessionUsage(WINDOW, SESSIONS_ROOT, [
    { filePath: PRIMARY_FILE, entries, malformedLineCount: 0 },
...
test("aggregateSessionUsage: per-message provider wins over stale model_change", () => {
  ...
  const entries = [
    ...
    assistantMsgWithAttribution(TS_INSIDE, "anthropic", "claude-opus-4", {
      input: 100,
      output: 50,
    }),
  ...
  assert.equal(
    stalePt,
    undefined,
    "stale-provider must not appear when per-message fields are present",
  );
...
test("aggregateSessionUsage: per-message modelId is reflected in providerTotals.modelId", () => {
  ...
  const entries = [
    assistantMsgWithAttribution(TS_INSIDE, "anthropic", "claude-3-5-sonnet", {
      input: 50,
      output: 25,
    }),
    assistantMsgWithAttribution(TS_INSIDE, "anthropic", "claude-opus-4", {
