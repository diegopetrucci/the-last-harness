    ...
    try {
      ...
      if (response.type === "text") {
        ...
        stream.push({
          type: "toolcall_delta",
          contentIndex: 0,
          delta: JSON.stringify(response.args),
          partial: output,
        });
...
test(
  "hermetic core architect workflow runs end-to-end with deterministic subagent boundaries",
  {
  ...
  async (t) => {
    ...
    assert.equal(
      fakeGnResult.status,
      1,
      fakeGnResult.stderr || fakeGnResult.stdout || String(fakeGnResult.error),
    );
      ...
      async execute(_toolCallId, params) {
        ...
        const { session } = await withEnv(subagentEnv, async () => {
          const resourceLoader = new DefaultResourceLoader({
            ...
            extensionFactories: [
              theLastHarness,
              createLoggingExtension(toolLogs, promptLogs, role),
            ],
            noContextFiles: true,
        ...
        try {
          ...
          const finalAssistant = session.state.messages
            .filter((message) => message.role === "assistant")
            .at(-1);
          sessionRoles.push({
            role,
            systemPrompt: session.systemPrompt,
            activeTools: [...session.getActiveToolNames()].sort((left, right) =>
    ...
    const { session } = await withEnv(createHermeticRuntimeEnv(fixture), async () =>
      createAgentSession({
        ...
        sessionManager: SessionManager.create(
          fixture.workspace,
          join(fixture.agentDir, "sessions"),
        ),
    ...
    try {
      ...
      await withEnv(
        { ...createHermeticRuntimeEnv(fixture), NODE_TEST_CONTEXT: undefined },
        async () => {
      ...
      const architectMessages = session.state.messages.filter(
      ...
      const gitDiff = run("git", ["diff", "--no-color"], {
        cwd: fixture.workspace,
        env: fixture.gitEnv,
      });
      ...
      assert.match(
        readFileSync(join(fixture.workspace, "src", "index.mjs"), "utf8"),
        /formatGreeting/,
      );
      ...
      assert.deepEqual(
        reviewerRun?.activeTools,
        EXPECTED_ACTIVE_TOOLS["code-reviewer"],
        diagnostics,
      );
      ...
      assert.equal(
        developerTools.some(
          (entry) => entry.toolName === "write" && entry.input.path === "src/greeting.mjs",
        ),
        true,
        diagnostics,
      );
      assert.equal(
        developerTools.some(
          (entry) => entry.toolName === "write" && entry.input.path === "test/greeting.test.mjs",
        ),
        true,
        diagnostics,
      );
      assert.equal(
        architectTools.some(
          (entry) => entry.toolName === "subagent" && entry.input.agent === "developer",
        ),
        true,
        diagnostics,
      );
      assert.equal(
        architectTools.some(
          (entry) => entry.toolName === "subagent" && entry.input.agent === "code-reviewer",
        ),
        true,
        diagnostics,
      );
      ...
      assert.equal(
        reviewerTools.some(
          (entry) => entry.toolName === "read" && entry.input.path === "src/greeting.mjs",
        ),
        true,
        diagnostics,
      );
      assert.equal(
        reviewerTools.some(
          (entry) => entry.toolName === "read" && entry.input.path === "test/greeting.test.mjs",
        ),
        true,
        diagnostics,
      );
      assert.equal(
        developerTools.some(
          (entry) =>
            entry.toolName === "bash" && entry.input.command === "node test/greeting.test.mjs",
        ),
        true,
        diagnostics,
      );
      const developerBashResults =
        developerRun?.toolResults.filter((result) => result.toolName === "bash") ?? [];
      ...
      const reviewerBashResults =
        reviewerRun?.toolResults.filter((result) => result.toolName === "bash") ?? [];
      ...
      const reviewerReadResults =
        reviewerRun?.toolResults.filter((result) => result.toolName === "read") ?? [];
      ...
      assert.equal(
        reviewerReadTexts.some(
          (text) =>
            /export function formatGreeting\(name\)/.test(text) && /Hello, \$\{name\}!/.test(text),
        ),
        true,
        diagnostics,
      );
      ...
      assert.equal(
        readFileSync(join(fixture.workspace, "test", "greeting.test.mjs"), "utf8").includes(
          "formatGreeting",
        ),
        true,
      );
