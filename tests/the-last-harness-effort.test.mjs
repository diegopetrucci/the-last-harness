...
function createInteractiveThinkingContext(model, scopeSelection, pickerCalls, thinkingSelection = "high") {
...
test("interactive thinking scope replays a pending failed model selector before capturing its write", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-thinking-failed-selector-", { cwd: true, test: t });
...
test("interactive thinking cancellation reports a failed restoration and resulting level", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-thinking-cancel-error-", { cwd: true, test: t });
	writeFileSync(join(fixture.agent, "settings.json"), JSON.stringify({ defaultThinkingLevel: "low" }));
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		registerEffortCommand(pi);
		const pickerCalls = [];
		const { notifications, ctx } = createInteractiveThinkingContext(reasoningModel(), undefined, pickerCalls);
...
test("interactive thinking All sessions warns when Pi emits no default write", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		registerEffortCommand(pi);
		const pickerCalls = [];
		const { notifications, ctx } = createInteractiveThinkingContext(reasoningModel(), 1, pickerCalls);
		...
		assert.deepEqual(notifications, [
			{
				message: "Thinking level set to high for this session, but TLH could not update the persistent default.",
				type: "warning",
			},
		]);
	});
...
for (const commandName of ["thinking", "effort"]) {
	test(`${commandName} interactive session-only scope leaves the profile default unchanged`, async (t) => {
		const fixture = createIsolatedProfileFixture(`tlh-${commandName}-session-only-`, { cwd: true, test: t });
		writeFileSync(join(fixture.agent, "settings.json"), JSON.stringify({ defaultThinkingLevel: "low" }));
...
test("interactive thinking All sessions preserves the profile default write", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-thinking-all-sessions-", { cwd: true, test: t });
	writeFileSync(join(fixture.agent, "settings.json"), JSON.stringify({ defaultThinkingLevel: "low" }));
	const model = reasoningModel();
...
test("pending thinking scope does not capture unrelated defaults and survives lifecycle replay", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-thinking-pending-scope-", { cwd: true, test: t });
	writeFileSync(join(fixture.agent, "settings.json"), JSON.stringify({ defaultThinkingLevel: "low" }));
	const model = reasoningModel();
...
test("unchanged interactive and typed thinking selections do not open the scope picker", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		assert.equal(pi.thinkingLevel, "low");
		assert.deepEqual(unchanged.notifications, [{ message: "Thinking level set to low.", type: "info" }]);
