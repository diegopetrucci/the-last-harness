...
test("runtime TypeScript package scripts stay wired into validation before tests and pack", () => {
  ...
  assert.equal(
    pkg.scripts["typecheck:subagents-test"],
    "tsc --noEmit --project tsconfig.subagents-test.json",
  );
  assert.equal(
  ...
  assert.ok(
    validate.indexOf("npm run typecheck:runtime") < validate.indexOf("npm run check:runtime"),
  );
...
test("runtime TypeScript helper tracks generated extension runtime modules", () => {
  ...
  assert.equal(extensionSources.length, 76);
...
test("dedicated subagents runtime target compiles all production modules and rewrites relative TypeScript imports", () => {
  ...
  const entrypoint = readFileSync(
    join(repoRoot, "extensions/subagents/src/extension/index.js"),
    "utf8",
  );
...
test(".gitattributes linguist-generated entries are in sync with runtime TypeScript sources", () => {
  ...
  const mtsFiles = globSync("scripts/**/*.mts", { cwd: repoRoot }).filter(
    (path) => !path.endsWith(".d.mts"),
  );
  ...
  for (const entry of generatedEntries) {
    const sourcePath = entry.endsWith(".mjs")
      ? entry.replace(/\.mjs$/, ".mts")
      : entry.replace(/\.js$/, ".ts");
    assert.ok(
