...
function writeReconcileState(agentDir, state) {
  ...
  writeFileSync(
    join(agentDir, "tlh", "reconcile-state.json"),
...
test("getChangedOverriddenRoles: returns role name when packaged defaults differ from acknowledged snapshot", () => {
  ...
  const snapshot = {
    architect: {
      byProvider: { anthropic: { model: "anthropic/claude-opus-4-5", thinking: "high" } },
    },
  };
  const roles = getChangedOverriddenRoles(
    primaryAgentsMap,
...
test("buildModelEffortNoticeMessage: multiple roles", () => {
  ...
  assert.equal(
    msg,
    "TLH default model/effort changed for architect, developer — run /reconcile to review",
  );
});
...
test("hasAnyModelEffortOverride: false for empty and structurally empty settings", () => {
  for (const settings of [
    ...
    assert.equal(
      ...
      false,
      `expected false for ${JSON.stringify(settings)}`,
    );
  }
});
...
test("hasAnyModelEffortOverride: true for entries computeModelEffortDrift would accept", () => {
  for (const settings of [
    ...
    assert.equal(
      hasAnyModelEffortOverride(settings),
      true,
      `expected true for ${JSON.stringify(settings)}`,
    );
  }
});
...
test("launch path with an override does load packaged agents", async (t) => {
  ...
  assert.equal(
    calls.subagent,
    1,
    "an existing override must still load packaged subagent metadata",
  );
});
...
test("settings with only skippable override entries do not load packaged agents", async (t) => {
  ...
  assert.equal(
    calls.primary,
    0,
    "skippable override entries must not read packaged primary agents",
  );
  assert.equal(
    calls.subagent,
    0,
    "skippable override entries must not read packaged subagent metadata",
  );
  assert.deepEqual(notifications, []);
});
...
test("backfill: pre-existing primary override with no baseline is silently seeded, no notice fires (main backfill test)", async (t) => {
  ...
  const fixture = createIsolatedProfileFixture("tlh-model-effort-notice-backfill-", {
    cwd: true,
    test: t,
  });
  writeSettings(fixture.agent, {
...
test("backfill: subsequent packaged-default change produces notice after backfill (forward detection works)", async (t) => {
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, () => {
    ...
    writeReconcileState(fixture.agent, {
      ...
      acknowledgedSnapshot: {
        ...
        architect: {
          byProvider: {
            anthropic: { model: "anthropic/claude-opus-4-5", thinking: archEntry.thinking },
          },
        },
      },
    });
  ...
  assert.equal(
    notificationsPass2.length,
    1,
    "notice must fire on second launch when packaged default changed",
  );
  assert.ok(
...
test("backfill: merges rather than replaces other roles and providers in existing state", async (t) => {
  ...
  assert.ok(
    devOpenai !== undefined,
    "developer openai-codex baseline must survive the backfill write",
  );
  assert.equal(
    devOpenai.model,
    "openai-codex/gpt-5.6-luna",
    "developer openai-codex model must be unchanged",
  );
...
test("backfill: unknown provider does not write state or produce notice", async (t) => {
  ...
  assert.equal(
    savedState.acknowledgedSnapshot,
    undefined,
    "no baseline must be written when provider is unknown",
  );
});
