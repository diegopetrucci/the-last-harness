...
test("stage-1 hides PATH-adjustment and refresh fallback detail lines unless --verbose", (t) => {
	for (const verbose of [false, true]) {
		...
		const refreshDetailPattern =
			/Running settings-wide extension refresh from merged settings; fallback retries only 10 non-critical bundled default source\(s\) individually\./;
...
test("installer helpers no longer support the removed --no-pi-install opt-out", (t) => {
	...
	assert.match(stage0Help.stdout, /TLH_GNOSIS_VERSION\s+Gnosis version to install \(default: 0\.5\.4\)/);
...
test("stage-1 infers update track unless env or CLI overrides", (t) => {
	...
	assert.equal(
		configFor(["--track", "ref"], { TLH_REF: "v1.2.3", TLH_UPDATE_TRACK: "latest-release" }).updateTrack,
		"ref",
	);
});
...
test("stage-1 explicit --wrapper-name and --agent-dir override main-ref auto-defaults", (t) => {
	...
	const configEnvOverride = buildInstallConfig(parseArgs(["--bin-dir", binDir], envWithExplicit), envWithExplicit);
	assert.equal(configEnvOverride.wrapperName, "my-tlh", "TLH_WRAPPER_NAME env must override main-ref auto-default");
	assert.equal(configEnvOverride.agentDir, customAgentDir, "TLH_AGENT_DIR env must override main-ref auto-default");
	...
	assert.equal(configPartialWrapper.wrapperName, "my-tlh", "TLH_WRAPPER_NAME overrides wrapper name even on main ref");
	assert.ok(
...
test("stage-1 derives packageRoot from custom package source install dirs", (t) => {
	...
	assert.equal(unsupportedConfig.packageRoot, join(agentDir, "git", "github.com", "diegopetrucci", "the-last-harness"));
...
test("stage-1 canonicalizes relative target dirs before deriving wrapper and state paths", (t) => {
	...
	assert.equal(config.packageRoot, join(expectedAgentDir, "git", "github.com", "diegopetrucci", "the-last-harness"));
