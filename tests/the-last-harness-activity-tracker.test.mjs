...
const { createTlhEffectiveActivityTracker, registerTlhEffectiveActivityTracker, TLH_EFFECTIVE_ACTIVITY_EVENT } =
...
test("tracker keeps concurrent async jobs active across duplicate, out-of-order, and shutdown events", () => {
	...
	try {
		...
		tracker.handleAsyncControl({ event: { runId: "job-2", mode: "background" }, asyncDir: asyncDir2 });
...
test("tracker ignores foreground control notices and only tracks safe async control contexts", () => {
	...
	try {
		...
		tracker.handleAsyncControl({ event: { runId: "job-async", mode: "async" }, asyncDir: asyncDirAsync });
...
test("registered tracker listens to pi events and cleans up on session shutdown", () => {
	...
	try {
		...
		const asyncDir1 = makeAsyncDir(root, "job-1", { runId: "job-1", state: "running", pid: process.pid });
...
test("drain: drops job whose status.json reports a terminal state", () => {
	...
	try {
		for (const terminalState of ["complete", "failed", "cancelled", "continued"]) {
			...
			assert.deepEqual(snapshot.activeAsyncJobIds, [], `Expected terminal state '${terminalState}' to cause drain`);
		}
	} finally {
		rmSync(root, { recursive: true, force: true });
	}
...
test("drain: drops job whose pid is dead", () => {
	...
	try {
		const asyncDir = makeAsyncDir(root, "run-dead", { runId: "run-dead", state: "running", pid: 99998 });
		const tracker = createTlhEffectiveActivityTracker({
...
test("drain: drops job when asyncDir is missing or unreadable", () => {
	...
	tracker.handleAsyncStarted({ id: "run-missing", asyncDir: "/nonexistent/path/that/does/not/exist" });
	assert.deepEqual(tracker.getSnapshot().activeAsyncJobIds, []);
});
...
test("periodic drain: dead child is drained and snapshot changes even without external stimulus", () => {
	...
	try {
		...
		assert.deepEqual(tracker.getSnapshot().activeAsyncJobIds, [], "dead child must be drained by periodic timer");
		assert.equal(tracker.isInProgress(), false);
