...
test("model override resolution: stored override is applied when the model is in the registry", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-primary-runtime-test-", { cwd: true, test: t });
	const primaryAgents = new Map([["architect", rushLikePrimary()]]);
	// Bundled default for rushLikePrimary on Anthropic is anthropic/claude-opus-5.
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		// Override should win over the bundled anthropic/claude-opus-5 default.
...
test("model_select listener writes override to settings when user picks a non-default model", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-primary-runtime-test-", { cwd: true, test: t });
	const primaryAgents = new Map([["architect", rushLikePrimary()]]);
	// rushLikePrimary has model: "anthropic/claude-opus-5".
	// The user picks a different Anthropic model that is NOT the bundled default for the architect primary.
	// Available: both claude-opus-5 (bundled default) and claude-sonnet-4-6 (non-default).
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		const ctx = {
			...
			sessionManager: { getBranch: () => [] },
			ui: { notify() {} },
			// Registry includes the bundled default (claude-opus-5) and the override target (claude-sonnet-4-6)
			modelRegistry: {
				getAvailable: () => [
					{ provider: "anthropic", id: "claude-opus-5" },
					{ provider: "anthropic", id: "claude-sonnet-4-6" },
		...
		// bundledKey for provider "anthropic" with rushLikePrimary: "anthropic/claude-opus-5" (the primary's .model field)
...
test("model_select listener clears override when user reselects the primary's bundled default model", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		// rushLikePrimary with only anthropic available: bundled default is anthropic/claude-opus-5
		const bundledDefaultModel = { provider: "anthropic", id: "claude-opus-5" };
		const ctx = {
			cwd: fixture.cwd,
			sessionManager: { getBranch: () => [] },
			ui: { notify() {} },
