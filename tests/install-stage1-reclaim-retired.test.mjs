...
import {
  buildInstallConfig,
  parseArgs,
  reclaimRetiredExtensionResidues,
...
test("reclaimRetiredExtensionResidues case 1: invokes pi remove for retired git source absent from settings and present on disk", (t) => {
  ...
  assert.ok(
    records.some(
      (r) => r.agentDir === agentDir && r.command === "remove git:github.com/diegopetrucci/pi-rtk",
    ),
...
test("reclaimRetiredExtensionResidues case 3: skips pi remove when retired source is not on disk", (t) => {
  ...
  assert.equal(
    existsSync(piLog),
    false,
    "pi should not be invoked when no retired packages are on disk",
  );
});
...
test("reclaimRetiredExtensionResidues finding 3: dry-run prints pi remove for force-removed source even when present in pre-merge settings", (t) => {
  ...
  assert.match(
    stdout,
    /remove.*github\.com\/diegopetrucci\/pi-rtk/,
    "dry-run must print the pi remove command",
  );
  ...
  assert.equal(
    ...
    settingsBefore,
    "settings file must be unchanged in dry-run",
  );
});
