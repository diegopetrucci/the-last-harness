...
test("install-managed dry-run uses the pinned default Gnosis release", () => {
  ...
  assert.match(
    result.stderr,
    /Would download Gnosis 0\.5\.4 from https:\/\/github\.com\/skorokithakis\/gnosis/,
  );
...
test("install-managed dry-run still honors TLH_GNOSIS_VERSION and CLI overrides", () => {
  ...
  assert.match(
    cliOverride.stderr,
    /Would download Gnosis 0\.5\.2 from https:\/\/github\.com\/skorokithakis\/gnosis/,
  );
});
...
test(
  "install-managed revalidates target before creating swapped parent directories",
  {
  ...
  () => {
    ...
    const tarResult = spawnSync("tar", ["-czf", archivePath, "-C", archiveSource, "gn"], {
      encoding: "utf8",
    });
...
test(
  "install-managed ignores a pre-existing predictable temp symlink",
  {
    skip: process.platform === "win32" || !gnosisAssetName("1.2.3"),
  },
  () => {
...
test("configure-install rejects a managed target whose parent is symlinked", () => {
  ...
  const result = runGnosis(
    ["--agent-dir", fixture.agent, "--target", managedTarget, "configure-install"],
    {
...
test(
  "configure-install exits non-zero when release fetch fails",
  { skip: !gnosisAssetName("1.2.3") },
  () => {
    const fixture = tempFixture();
