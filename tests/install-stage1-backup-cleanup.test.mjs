...
test("cleanupOldSettingsBackups removes keybindings.json backups older than 28 days", (t) => {
  ...
  assert.equal(
    existsSync(join(agentDir, oldName)),
    false,
    "old keybindings backup should be removed",
  );
  assert.ok(existsSync(join(agentDir, newer1)), "newest keybindings backup should be retained");
  assert.ok(
    existsSync(join(agentDir, newer2)),
    "second-newest keybindings backup should be retained",
  );
});
...
test("cleanupOldSettingsBackups skips symlinked backup-named entries", (t) => {
  ...
  assert.ok(
    existsSync(join(agentDir, symlinkName)),
    "symlinked backup-named entry should be preserved",
  );
...
test("cleanupOldSettingsBackups is skipped when agentDir is a symlink", (t) => {
  ...
  assert.ok(
    existsSync(join(realDir, oldName)),
    "file in real dir must be untouched when agentDir is a symlink",
  );
...
test("cleanupOldSettingsBackups retains sole old keybindings backup when two recent settings backups exist (mixed-profile regression)", (t) => {
  ...
  assert.ok(
    existsSync(join(agentDir, recentSettings1)),
    "recent settings backup 1 should be retained",
  );
  assert.ok(
    existsSync(join(agentDir, recentSettings2)),
    "recent settings backup 2 should be retained",
  );
});
...
test("cleanupOldSettingsBackups does not delete user files with no parseable timestamp", (t) => {
  ...
  assert.ok(
    existsSync(join(agentDir, newer2)),
    "second-newest timestamped backup should be retained",
  );
});
