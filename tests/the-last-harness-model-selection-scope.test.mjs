...
test("session-only survives turn and tree reapplication, then resets on explicit mode and new session", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		const { beforeAgentStart, modelSelect, pi, runtime, sessionTree } = registerScopeRuntime(primaryAgents);
		...
		assert.deepEqual(pi.model, selectedModel, "next turn must preserve the accepted session-only model");
		await sessionTree({ type: "session_tree" }, context);
		assert.deepEqual(pi.model, selectedModel, "session-tree reapplication must preserve the session-only model");

		await pi.commands.get("switch-primary-agent").handler("architect", context);
		assert.deepEqual(pi.model, previousModel, "an explicit primary mode change reapplies its packaged model");
		...
		await modelSelect({ type: "model_select", model: selectedModel, previousModel, source: "set" }, context);
		await runtime.applySessionStart(context);
		assert.deepEqual(pi.model, previousModel, "a new session must not inherit the prior session-only model");
...
test("reconfirming an active session-only model does not persist it globally", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-model-scope-session-noop-", { cwd: true, test: t });
	const previousModel = { provider: "anthropic", id: "claude-sonnet-4-6" };
	const selectedModel = { provider: "anthropic", id: "claude-opus-5" };
	writeSettings(fixture.agent, { defaultProvider: previousModel.provider, defaultModel: previousModel.id });
...
test("an active-model re-selection persists synchronously without stale state and thinking still persists", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		await thinkingSelect({ type: "thinking_level_select", level: "medium", previousLevel: "low" }, {});
		await manager.flush();
		...
		await thinkingSelect({ type: "thinking_level_select", level: "high", previousLevel: "medium" }, {});
...
test("a failed selector write is bounded and replayed before the next unrelated selection", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		await manager.flush();
		const written = readSettings(fixture.agent);
		assert.equal(written.defaultModel, failedModel.id, "the old upstream write is replayed, not merged");
...
test("session shutdown replays and clears a failed-selector candidate", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-model-scope-failed-shutdown-", { cwd: true, test: t });
	const previousModel = { provider: "anthropic", id: "claude-sonnet-4-6" };
	const failedModel = { provider: "openai-codex", id: "unavailable-model" };
	writeSettings(fixture.agent, { defaultProvider: previousModel.provider, defaultModel: previousModel.id });
...
test("unknown model event sources fail open by replaying unmatched writes", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-model-scope-future-source-", { cwd: true, test: t });
	const previousModel = { provider: "anthropic", id: "claude-sonnet-4-6" };
	const failedModel = { provider: "openai-codex", id: "unavailable-model" };
	writeSettings(fixture.agent, { defaultProvider: previousModel.provider, defaultModel: previousModel.id });
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		await modelSelect(
			{ type: "model_select", model: previousModel, previousModel: failedModel, source: "future-source" },
...
test("/model exact-name and provider-auth-style source=set persist without the picker", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-model-scope-programmatic-", { cwd: true, test: t });
	const previousModel = { provider: "anthropic", id: "claude-sonnet-4-6" };
...
test("model cycling keeps persistent upstream behavior without creating a TLH primary override", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		await modelSelect({ type: "model_select", model: selectedModel, previousModel, source: "cycle" }, context);
...
test("cancel restores the previous active model without persisting attempted or restoration writes", async (t) => {
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		const notifications = [];
		const context = createScopeContext(fixture, "cancel", selectedModel, [previousModel, selectedModel]);
		Object.defineProperty(context, "model", { get: () => pi.model });
		...
		pi.setModel = async (model) => {
			...
			pi.model = model;
			manager.setDefaultModelAndProvider(model.provider, model.id);
			await modelSelect({ type: "model_select", model, previousModel: prior, source: "set" }, context);
			return true;
		};
		await modelSelect({ type: "model_select", model: selectedModel, previousModel, source: "set" }, context);

		assert.deepEqual(pi.model, previousModel);
		assert.deepEqual(notifications, [], "the corrected status waits for upstream's attempted-model status");
...
test("cancel restoration failures are caught and reported without persisting the attempted model", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-model-scope-cancel-error-", { cwd: true, test: t });
	const previousModel = { provider: "anthropic", id: "claude-sonnet-4-6" };
	const selectedModel = { provider: "anthropic", id: "claude-opus-5" };
	writeSettings(fixture.agent, { defaultProvider: previousModel.provider, defaultModel: previousModel.id });
...
test("TLH-internal model application persists defaults without prompting or recording an override", async (t) => {
	...
	writeSettings(fixture.agent, { defaultProvider: initialModel.provider, defaultModel: initialModel.id });
...
test("locked-primary All sessions persists globally without creating a role override", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-model-scope-locked-all-", { cwd: true, test: t });
	const primaryModel = { provider: "anthropic", id: "claude-sonnet-4-6" };
	const selectedModel = { provider: "anthropic", id: "claude-opus-5" };
	writeSettings(fixture.agent, {
...
test("locked-primary session-only is reapplied on the next before_agent_start", async (t) => {
	const fixture = createIsolatedProfileFixture("tlh-model-scope-locked-session-", { cwd: true, test: t });
	const primaryModel = { provider: "anthropic", id: "claude-sonnet-4-6" };
	const selectedModel = { provider: "anthropic", id: "claude-opus-5" };
	writeSettings(fixture.agent, {
		defaultProvider: primaryModel.provider,
	...
	await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
		...
		await beforeAgentStart({ systemPrompt: "base" }, context);
		assert.deepEqual(pi.model, primaryModel, "locked primary must force its model on the next turn");
