...
`);
  ...
  assert.equal(
    formatPullRequestFooterSegment({ number: 42, state: "OPEN", isDraft: false }),
    "PR #42",
  );
  assert.equal(
    formatPullRequestFooterSegment({ number: " 9007199254740993 " }),
    "PR #9007199254740993",
  );
  ...
  assert.equal(
    formatTlhGitFooterSegments(status, { number: 42 }).join(" • "),
    "feature/pr • +1 ~2 ?1 ↑1 • PR #42",
  );
});
