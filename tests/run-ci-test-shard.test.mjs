...
test("spawnBuffered: buffers stdout and writes atomically on completion", async () => {
	...
	await spawnBuffered([process.execPath, "-e", "process.stdout.write('hello stdout')"], process.env, {
		stdout: out.stream,
		stderr: err.stream,
	});
	assert.ok(out.content.includes("hello stdout"), `expected stdout to include 'hello stdout', got: ${out.content}`);
...
test("spawnBuffered: buffers stderr and writes it in the same labeled block as stdout", async () => {
	const out = captureStream();
	const err = captureStream();
	await spawnBuffered([process.execPath, "-e", "process.stderr.write('hello stderr')"], process.env, {
		stdout: out.stream,
		stderr: err.stream,
	});
	...
	assert.ok(out.content.includes("hello stderr"), `expected stderr payload in stdout frame, got: ${out.content}`);
...
test("spawnBuffered: injects env into child process", async () => {
	...
	await spawnBuffered([process.execPath, "-e", "process.stdout.write(process.env.TLH_LANE_CANARY ?? 'missing')"], env, {
		stdout: out.stream,
	});
	assert.ok(out.content.includes("lane-canary-42"), `expected 'lane-canary-42' in output, got: ${out.content}`);
});
...
test("spawnBuffered: emits truncation notice when stdout exceeds buffer limit", async () => {
	...
	await spawnBuffered([process.execPath, "-e", "process.stdout.write('x'.repeat(100))"], process.env, {
		stdout: out.stream,
...
test("spawnBuffered: emits truncation notice when stderr exceeds buffer limit", async () => {
	...
	const out = captureStream();
	const limit = 10;
	await spawnBuffered([process.execPath, "-e", "process.stderr.write('x'.repeat(100))"], process.env, {
		stdout: out.stream,
		maxBufferBytes: limit,
	});
...
test("spawnBuffered: does not emit truncation notice when output is within limit", async () => {
	...
	assert.ok(!out.content.includes("[TLH]"), `unexpected truncation notice in output: ${out.content}`);
});
...
test("runLanes: concurrent stderr-producing lanes each keep stderr attached to their label", async () => {
	...
	assert.ok(content.includes("alpha-stderr-payload"), "alpha stderr payload must be in stdout frame");
...
test("spawnBuffered: flushes partial buffered output to process.stdout on SIGTERM", { timeout: 15000 }, async () => {
	const base = tempDir();
...
`;
	...
	try {
		...
		assert.match(output, /partial output from child/, `Signal handler should flush buffered output; got: ${output}`);
	} finally {
...
test("runLane: merges lane-specific env over process.env", async () => {
	...
	await runLane(
		{
			name: "merge",
			commands: [[process.execPath, "-e", "process.stdout.write(process.env.TLH_LANE_VAR ?? 'missing')"]],
			env: { TLH_LANE_VAR: "custom-value" },
		},
		laneHome,
		{ stdout: out.stream },
	);
	assert.ok(out.content.includes("custom-value"), `expected 'custom-value' in output, got: ${out.content}`);
});
...
test("runLanes: runs lanes concurrently (not sequentially)", { timeout: 15000 }, async () => {
	...
	const code = await runLanes(
		[
			{ name: "a", commands: [[process.execPath, "--input-type=module", "-e", pollCode(markerA, markerB)]] },
			{ name: "b", commands: [[process.execPath, "--input-type=module", "-e", pollCode(markerB, markerA)]] },
		],
		{ baseHomeDir: base },
	);
	assert.equal(code, 0, "Both lanes should find each other's markers, confirming concurrent execution");
});
		...
		for (const lane of lanes) {
			for (const cmd of lane.commands) {
				...
				if (!joined.includes("e2e")) {
					assert.match(joined, new RegExp(`--test-shard=${shard}/2`), `Missing shard option in: ${joined}`);
				}
			}
		}
	}
