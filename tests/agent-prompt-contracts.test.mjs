...
test("scout and research subagent prompts keep bounded scope and tool budgets", () => {
  ...
  assertBodyPattern(
    diffSummarizer,
    "diff-summarizer tool budget",
    /toolBudget:\s*\{"soft":12,"hard":20\}/i,
  );
...
test("OpenAI scout and research subagent defaults pin GPT-5.6 Luna with medium thinking", () => {
  for (const name of ["repo-scout", "web-scout", "librarian", "diff-summarizer"]) {
    ...
    assert.match(
      agent,
      /^tlhOpenaiModels: openai-codex\/gpt-5\.6-luna$/m,
      `${name} OpenAI model default`,
    );
