...
test("LANES: npm run typecheck is present", () => {
	assertHasCommand(
		(c) => c.includes("npm") && c.includes("run") && c.includes("typecheck") && !c.some((a) => a.includes(":")),
		"npm run typecheck",
	);
});
...
test("LANES: npm run lint is present", () => {
	assertHasCommand(
		(c) => c.includes("npm") && c.includes("run") && c.includes("lint") && !c.includes("lint:sh"),
		"npm run lint",
	);
});

test("LANES: npm run lint:sh is present", () => {
	assertHasCommand((c) => c.includes("npm") && c.includes("run") && c.includes("lint:sh"), "npm run lint:sh");
});
...
test("LANES: npm pack --dry-run is present", () => {
	assertHasCommand((c) => c.includes("npm") && c.includes("pack") && c.includes("--dry-run"), "npm pack --dry-run");
});
...
test("LANES: every lane has at least one command", () => {
	for (const lane of LANES) {
		assert.ok(Array.isArray(lane.commands) && lane.commands.length > 0, `Lane "${lane.name}" has no commands`);
	}
});
	...
	for (const lane of LANES) {
		for (const cmd of lane.commands) {
			...
			for (const arg of cmd) {
				assert.equal(typeof arg, "string", `Lane "${lane.name}" command has non-string arg: ${JSON.stringify(cmd)}`);
			}
		}
	}
});
