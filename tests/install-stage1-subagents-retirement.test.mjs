...
test("actual install retries failed managed npm cleanup before merge and cannot resurrect it", (t) => {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        packages: ["npm:@diegopetrucci/pi-subagents@0.31.14", "npm:keep-me"],
  ...
  writeFileSync(
    join(packageDirInNodeModules, "package.json"),
    JSON.stringify({ name: packageName, version: "0.31.14" }),
  );
...
`,
  ...
  const installArgs = ["--agent-dir", agentDir, "--bin-dir", binDir, "--no-wrapper"];
  ...
  assert.match(
    firstOutput,
    /failed to uninstall retired TLH subagent npm package.*simulated package-manager failure/,
  );
  ...
  assert.equal(
    failedSettings.tlh.defaultExtensionProvenance.managedPackageIdentities.includes(
  ...
  assert.equal(
    Object.hasOwn(readJson(join(npmRoot, "package.json")).dependencies, packageName),
    true,
  );
  ...
  assert.equal(
    firstEvents.includes("pi:update --extensions"),
    false,
    "failed cleanup must abort before extension refresh",
  );
  ...
  assert.match(
    secondOutput,
    /Uninstalled retired TLH subagent npm package: @diegopetrucci\/pi-subagents/,
  );

  const allEvents = readFileSync(eventLog, "utf8").trim().split(/\r?\n/);
  const secondEvents = allEvents.slice(firstEvents.length);
  const uninstallIndex = secondEvents.findIndex((line) =>
    line.startsWith("pm:-- pnpm uninstall @diegopetrucci/pi-subagents"),
  );
  const refreshIndex = secondEvents.findIndex((line) => line === "pi:update --extensions");
  assert.ok(
    uninstallIndex >= 0,
    `configured package manager was not retried: ${secondEvents.join("\n")}`,
  );
  assert.ok(
    refreshIndex > uninstallIndex,
    `extension refresh must run after uninstall and settings merge: ${secondEvents.join("\n")}`,
  );
  assert.equal(
    secondEvents.some((line) => line.startsWith("resurrected:")),
    false,
    "post-merge extension refresh must not resurrect subagents",
  );

  const npmPackageJson = readJson(join(npmRoot, "package.json"));
  const npmPackageLock = readJson(join(npmRoot, "package-lock.json"));
  assert.equal(
    Object.hasOwn(npmPackageJson.dependencies, packageName),
    false,
    "package.json dependency must be removed",
  );
  assert.equal(
    Object.hasOwn(npmPackageLock.packages[""].dependencies, packageName),
    false,
    "package-lock root dependency must be removed",
  );
  assert.equal(
    Object.hasOwn(npmPackageLock.packages, `node_modules/${packageName}`),
    false,
    "package-lock package entry must be removed",
  );
  assert.equal(
    existsSync(packageDirInNodeModules),
    false,
    "node_modules package must remain absent after refresh",
  );
  ...
  assert.equal(
    settings.packages.includes("npm:keep-me"),
    true,
    "unrelated configured package must survive",
  );
  assert.equal(
  ...
  assert.deepEqual(
    settings.npmCommand,
    ["corepack", "--", "pnpm"],
    "configured package-manager command must survive",
  );
  assert.ok(
    existsSync(join(runtimeDir, "bin", "pi")),
    "installer must complete with the private runtime intact",
  );
  ...
  assert.equal(
    ...
    malformedSettingsRaw,
    "dry-run must not rewrite settings",
  );
  assert.equal(
    readFileSync(uninstallAttemptsPath, "utf8"),
    "2",
    "no retired candidate means no package-manager invocation",
  );
});
