...
test("subagent-settings set preserves unrelated settings and applies on the next dispatch", async (t) => {
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
    ...
    const event = {
      toolName: "subagent",
      input: { agent: "developer", prompt: "Implement the ticket" },
    };
...
test("subagent tool-call runtime honors persisted false model and thinking sentinels", async (t) => {
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
    ...
    const inheritedEvent = {
      toolName: "subagent",
      input: { agent: "developer", prompt: "Implement" },
    };
    ...
    const disabledThinkingEvent = {
      toolName: "subagent",
      input: { agent: "librarian", prompt: "Research" },
    };
...
test("subagent-settings status shows saved effort after the full exact suffix-like model identity", async (t) => {
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
    ...
    assert.match(
      ...
      /override model=openrouter\/reasoner:high, effort=low/i,
    );
...
test("subagent-settings status reports unavailable stored pins and update/reset guidance", async (t) => {
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
    ...
    assert.match(
      message,
      /override model=openai-codex\/gpt-5\.999, effort=stored nonstandard\/disabled \("turbo"\)/i,
    );
    ...
    assert.equal(
      ...
      "openai-codex/gpt-5.999",
    );
    assert.equal(settings.subagents.agentOverrides["code-reviewer"].thinking, "turbo");
    assert.deepEqual(settings.subagents.agentOverrides["code-reviewer"].fallbackModels, [
      "saved/fallback",
    ]);
...
test("subagent-settings keeps suffix guidance for non-exact recognized effort suffixes", async (t) => {
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
    ...
    assert.match(
      ...
      /Model overrides must omit any :effort suffix/i,
    );
    ...
    const selections = [
      ...
      (options) => {
        const picked = options.find((option) =>
          option.startsWith("openrouter/available-while-picked"),
        );
...
test("subagent-settings status and picker show false sentinels with inherited model and effective off", async (t) => {
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
    ...
    const developerOption = pickerRun.selects[0]?.options.find((option) =>
      option.includes("developer"),
    );
  ...
  await withEnv({ HOME: fixture.home, PI_CODING_AGENT_DIR: fixture.agent }, async () => {
    for (const role of ["code-reviewer", "oracle", "contrarian"]) {
      ...
      assert.equal(
        existsSync(join(fixture.agent, "settings.json")),
        false,
        `no write should occur for ${role}`,
      );
    }
  });
});
