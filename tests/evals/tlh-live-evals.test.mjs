...
test("package scripts keep live evals out of package command surfaces", () => {
  ...
  assert.equal(
    pkg.scripts.test,
    'node --test --test-reporter=dot "tests/**/*.test.mjs" && npm run test:subagents',
  );
  assert.equal(
    pkg.scripts["test:verbose"],
    'node --test "tests/**/*.test.mjs" && npm run test:subagents',
  );
...
test("packaged local development docs exclude repo-only live eval tooling", () => {
  ...
  assert.doesNotMatch(
    docs,
    /eval:live|tlh-live-evals|TLH_RUN_LIVE_EVALS|architect-e2e|install-update-smoke/i,
  );
...
test("contributing docs link to the dedicated workflow eval guide and keep a concise quick reference", () => {
  ...
  assert.doesNotMatch(
    docs,
    /Scenario \| Mode \| Prerequisites \| What it checks|What it prepares or verifies/,
  );
});
...
test("workflow eval guide owns the detailed contributor-facing eval docs", () => {
  ...
  assert.match(
    docs,
    /repo-local contributor tooling only; it does not change packaged TLH runtime behavior/i,
  );
  ...
  assert.match(
    docs,
    /use the importer-backed incident-to-fixture loop above instead of checking in raw exports/i,
  );
  assert.doesNotMatch(docs, /Until that normalization path exists/i);
  assert.match(
    docs,
    /Do not commit `results\.json`, temp workspaces, or per-run score snapshots\./i,
  );
