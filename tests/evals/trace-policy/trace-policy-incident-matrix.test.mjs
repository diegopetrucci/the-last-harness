...
const ALLOWED_SOURCE_KINDS = new Set(["live-smoke", "repo-test", "settings-fixture", "synthetic", "transcribed"]);
...
test("gh-241 milestone incident matrix is present and well-formed", () => {
	...
	for (const entry of TRACE_POLICY_INCIDENT_MATRIX) {
		...
		assert.ok(ALLOWED_SOURCE_KINDS.has(entry.sourceKind), `unsupported source kind for ${entry.id}`);
		assert.ok(entry.coverage);
		assert.ok(ALLOWED_COVERAGE_STATUSES.has(entry.coverage.status), `unsupported coverage status for ${entry.id}`);

		if (entry.coverage.status === "covered") {
			assert.ok(Array.isArray(entry.coverage.fixtureIds));
			assert.ok(entry.coverage.fixtureIds.length > 0, `covered entry missing fixtureIds: ${entry.id}`);
			assert.ok(
...
test("fixture incident metadata resolves to known matrix entries", () => {
	...
	for (const fixture of TRACE_POLICY_FIXTURES) {
		for (const matrixId of fixture.incidentMatrixIds || []) {
			assert.ok(matrixIds.has(matrixId), `fixture ${fixture.id} references unknown matrix id ${matrixId}`);
		}
	}
});
...
test("covered matrix entries resolve to tagged fixtures", () => {
	...
	for (const entry of TRACE_POLICY_INCIDENT_MATRIX) {
		...
		for (const fixtureId of entry.coverage.fixtureIds) {
			assert.ok(fixturesById.has(fixtureId), `matrix entry ${entry.id} references unknown fixture ${fixtureId}`);
		}
	}
});
