...
function createSessionsFixture(
	...
	const sessionHeader = includeCwd
		? { type: "session", id: "session-abc123", name: "My test session", cwd: "/Users/foo/cwd-project" }
...
test(`/${SESSION_LIMIT_REPORT_COMMAND_NAME} happy path: report generated with window metadata and ranked sessions`, async (t) => {
	...
	// Notification
	assert.ok(notifications.at(-1)?.message.includes("Opened local TLH session-limit report"), "success notification");
