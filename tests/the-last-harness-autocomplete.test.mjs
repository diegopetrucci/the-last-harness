...
const { createTlhAutocompleteProvider } = await jiti.import(
  "../extensions/the-last-harness/autocomplete.ts",
);
...
test("autocomplete keeps hidden commands outside slash-command-name context", async () => {
  ...
  const result = await provider.getSuggestions(["/agent quiet"], 0, 12, {
...
test("wrapper has no triggerCharacters property when the underlying provider omits it", () => {
  ...
  assert.ok(
    !("triggerCharacters" in wrapper),
    "wrapper must not have an own triggerCharacters key",
  );
});
