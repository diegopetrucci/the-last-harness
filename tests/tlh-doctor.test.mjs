...
test("generated wrapper doctor preserves an explicit --settings override", (t) => {
  ...
  assert.deepEqual(record.argv.slice(-4), [
    "--agent-dir",
    otherAgentDir,
    "--settings",
...
`,
  ...
  assert.match(
    output,
    /FAIL\s+profile isolation\/settings: agent dir is inside ~\/\.pi; profile not inspected/,
  );
  ...
  assert.equal(
    ...
    false,
    "doctor must not execute helpers from protected profile bin",
  );
  assert.equal(
    existsSync(ghMarkerPath),
    false,
    "doctor must not execute gh from protected profile bin",
  );
...
test("tlh doctor reports repairable settings drift without mutating settings or creating backups", (t) => {
  ...
  assert.equal(
    settingsAfterDoctor,
    settingsBeforeDoctor,
    "plain doctor must not rewrite drifted settings",
  );
  assert.equal(
    backupsAfterDoctor,
    backupsBeforeDoctor,
    "plain doctor must not create settings backups",
  );
...
test("tlh doctor drift summary does not disclose credential-like package sources", (t) => {
  ...
  assert.equal(
    settingsAfterDoctor,
    settingsBeforeDoctor,
    "plain doctor must stay read-only for drift checks",
  );
  assert.equal(
    backupsAfterDoctor,
    backupsBeforeDoctor,
...
test("tlh doctor reports stale bundled subagent prompts as repairable drift", (t) => {
  ...
  assert.match(
    output,
    /FAIL\s+bundled subagent resources: restoration needed for 1 prompt\(s\): contrarian\.md/,
  );
...
test("tlh doctor --repair restores isolated settings drift, preserves user values, and creates a backup", (t) => {
  ...
  assert.equal(
    backupsAfterRepair,
    backupsBeforeRepair + 1,
    "repair should create one additional settings backup",
  );
  assert.equal(
    existsSync(join(fixture.agentDir, "tlh", "agents", "subagents", "developer.md")),
    true,
  );
  assert.equal(
  ...
  assert.match(
    output,
    /OK\s+bundled subagent resources: restored 1 prompt\(s\) from packaged defaults/,
  );
  assert.match(
    output,
...
test("tlh doctor --repair refuses normal Pi targets before reading settings or invoking helpers", (t) => {
  ...
  const result = runDoctor(
    ["--repair", "--agent-dir", protectedAgentDir, "--package-root", packageRoot],
    {
      env: {
        HOME: fixture.home,
        PATH: `${fixture.fakebin}:${process.env.PATH}`,
