...
test("stripLeadingShellCommandPrefixes removes shell control words and assignment tokens", () => {
  ...
  assert.deepEqual(stripLeadingShellCommandPrefixes(["!", "command", "git", "commit"]), [
    "git",
    "commit",
  ]);
  assert.deepEqual(stripLeadingShellCommandPrefixes(["FOO=bar", "git", "commit"]), [
    "git",
    "commit",
  ]);
  assert.deepEqual(stripLeadingShellCommandPrefixes(["do", "FOO=bar", "git", "commit"]), [
    "git",
    "commit",
  ]);
});
...
test("readHereDocBodies reads a single heredoc body", () => {
  const body = "line1\nline2\n";
  const full = `${body}EOF\n`;
  const { bodies, nextIndex } = readHereDocBodies(full, 0, [
    { delimiter: "EOF", allowIndent: false },
  ]);
...
test("unwrapLeadingEnvCommandTokens unwraps simple env invocations", () => {
  ...
  assert.deepEqual(unwrapLeadingEnvCommandTokens(["env", "FOO=bar", "git", "commit"]), [
    "git",
    "commit",
  ]);
  assert.deepEqual(unwrapLeadingEnvCommandTokens(["env", "-i", "git", "commit"]), [
    "git",
    "commit",
  ]);
});
...
test("unwrapLeadingEnvCommandTokens skips to after -- option terminator", () => {
  assert.deepEqual(unwrapLeadingEnvCommandTokens(["env", "--", "git", "commit"]), [
    "git",
    "commit",
  ]);
});
...
test("normalizeShellCommandTokens passes through plain commands unchanged", () => {
  assert.deepEqual(normalizeShellCommandTokens("git commit -m msg"), [
    "git",
    "commit",
    "-m",
    "msg",
...
test("normalizeShellCommandTokens unwraps env and strips variable assignments from remainder", () => {
  ...
  assert.deepEqual(normalizeShellCommandTokens("/usr/bin/env FOO=bar git commit"), [
    "git",
    "commit",
  ]);
});
