...
- **Breaking:** reduced the first-party subagent contract to fresh-only direct single/parallel execution. Every child starts a fresh session with no caller-supplied `context` or agent `defaultContext`; `turnBudget` and its old turn-count controls are not supported. Foreground runs and TLH-tracked `async: true` background runs remain available, along with `timeoutMs`, `maxExecutionTimeMs`, `toolBudget`, native `contact_supervisor`, status/interrupt/resume/steer controls, acceptance, diagnostics, artifacts, and model fallback. Saved chains and the external pi-intercom detach request/result/control integration are unsupported; TLH-tracked async work still uses a detached OS child process managed by TLH. Existing run, session, and saved-chain artifacts are not rewritten or deleted, and their status/history may remain readable. Older paused runs whose persisted configuration depends on retired `steps`, fork/context inheritance, the removed external pi-intercom detach request/result/control integration, or turn-budget behavior cannot be resumed; start a new direct single or parallel run instead. Normal direct-plan durable pause/resume remains supported. Migration: remove retired fields from custom definitions and stale settings, then use the supported fields described in [docs/subagents.md](docs/subagents.md) and [docs/custom-subagents.md](docs/custom-subagents.md).
...
- **Breaking:** retired caller-facing subagent execution overrides for task `reads`/`progress`, top-level `concurrency`/`includeProgress`, and `fallbackModels`. Configure `defaultReads`, `defaultProgress`, and `fallbackModels` in the agent definition, and configure `parallel.concurrency` in `<agent-dir>/extensions/subagent/config.json`; direct calls carrying retired keys fail before a child launches with migration guidance. Agent defaults, provider-aware fallback selection, persisted plans, live progress, and result-state diagnostics remain supported.
