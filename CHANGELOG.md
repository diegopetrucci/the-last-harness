...
- Experimental, default-off prompt-cache heartbeat trial: while async subagent runs are live and the parent session is idle, the trial replays the last provider request and aborts at the first usable cache-usage signal or generation boundary in an attempt to warm the provider prompt cache; observed spend is recorded, but aborted usage does not prove a prompt-cache TTL refresh, and providers that expose usage only after generation may incur incompletely measured whole-request cost before the breaker disables heartbeat. Enable via `heartbeat.enabled` in the subagent extension config; per-beat accounting is written to `<agent-dir>/subagents/heartbeat.jsonl`, each beat-bearing gap emits a session-visible summary entry, and `/subagents-doctor` reports totals. Gap verdicts are `saved`, `wasted`, `lost`, or `unneeded`; the `unneeded` verdict covers zero-beat gaps closed before their first beat, including after a short run, parent turn, lifecycle event, model change, or compaction; telemetry does not record which closure occurred (these are no longer incorrectly counted as `lost`). See the "Prompt-cache heartbeat" section in `docs/subagents.md`.
