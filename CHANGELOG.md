...
- Experimental, default-off prompt-cache heartbeat trial: while async subagent runs are live and the parent session is idle, the trial replays the last provider request and aborts at the first usable cache-usage signal or generation boundary in an attempt to warm the provider prompt cache; observed spend is recorded, but aborted usage does not prove a prompt-cache TTL refresh, and providers that expose usage only after generation may incur incompletely measured whole-request cost; generation-boundary cutoffs are recorded as `generation_cutoff`, distinct from genuine stream/auth `error` outcomes, and both outcomes count toward the existing three-failure session breaker, while a tripped breaker stops later empty gap wiring and remains visible in `/subagents-doctor`. Enable via `heartbeat.enabled` in the subagent extension config; per-beat accounting is written to `<agent-dir>/subagents/heartbeat.jsonl`, each beat-bearing gap emits a session-visible summary entry, and `/subagents-doctor` reports totals. Gap verdicts are `saved`, `wasted`, `lost`, or `unneeded`; the `unneeded` verdict covers zero-beat gaps closed before their first beat, including after a short run, parent turn, lifecycle event, model change, or compaction; telemetry does not record which closure occurred (these are no longer incorrectly counted as `lost`). See the "Prompt-cache heartbeat" section in `docs/subagents.md`.
- TLH now warns in the footer when no providers are available and points to `/login`.
- Rush, Product, Bug-hunter, and Architect now use their provider-aware model and thinking values as editable defaults. Their model selections persist per primary and can be reviewed or reset with `/reconcile`; supported thinking levels remain native to Pi with no TLH primary restrictions. Explicit thinking choices remain retained across turns and clamp safely when a switched model supports fewer levels; model defaults are written by the native model-picker Ctrl+S action, while `/effort` Ctrl+S uses TLH's guarded path for the thinking default.
- Disabled primary-agent mode now retains TLH's architect-equivalent tools and guarded bundled/trusted-subagent access, and `/review` can dispatch a fresh isolated `code-reviewer`; canonical bundled minor agents and persisted-trust-authorized project custom `embedded.<slug>` agents use the validated Git-root project scope and a fresh child session. It still omits the architect persona, project append and experimental guidance, automatic primary model/thinking behavior, and any thinking floor while preserving the current session model and thinking level. See [README.md](README.md), [docs/commands.md](docs/commands.md), and [docs/models.md](docs/models.md).
- Bumped the pinned Pi runtime to `0.85.1`. Pi's native model picker keeps Enter selections session-only and uses the save key (Ctrl+S by default, `app.models.save`) to persist the isolated profile model default; TLH's second model-scope prompt was removed in favor of those native Enter/save-key controls, and TLH primary model application no longer rewrites that profile model default.
- Pi's built-in `/thinking` remains native. TLH registers only `/effort` as a behavioral alias using Pi's exported native picker and supported levels; Enter/session-only, the configurable save key (Ctrl+S by default, `app.thinking.save`)/save-default, and the configurable cancel key (Escape/Ctrl+C by default, `tui.select.cancel`) controls are rendered from keybinding config at runtime. Typed levels and cycles are session-only; `/effort` save key saves `defaultThinkingLevel` only through TLH's guarded isolated-profile settings path, preserving unknown fields and reporting a session-only fallback on failure.
- Inherited from Pi 0.85.1: built-in tools now honor `ctx.cwd` per task; the streaming working indicator moved into the default editor border; Claude thinking effort is preserved per turn on supported Anthropic transports and recovers from signed-thinking mismatches; model and thinking selector save keybindings are now configurable (default Ctrl+S, rendered from keybinding config at runtime); GPT-6 Astra is available via OpenAI API keys and Codex subscriptions; skills are no longer broken when Bash is the only enabled tool; `fd` and `ripgrep` managed downloads are fixed for musl targets and no longer require the GitHub Releases API; the branch-summary token cap is fixed; GPT-5.6+ Responses API prompt-cache requests now use `prompt_cache_options.ttl` (30 minutes) instead of the retired `prompt_cache_retention`.
- Activity reporting now clears failed or aborted compactions and tracks blocking extension UI prompts as waiting without treating them as background work; cmux shows waiting while Herdr receives its supported blocked representation.
- Projects can provide session-scoped model and effort defaults for the packaged primary agents and bundled subagent roles via `.tlh/defaults.json` at the Git worktree root. Defaults use a separate, weaker configuration-trust decision: persisted `/trust` enables both defaults and custom-agent execution, while upstream/default/session approval can enable `.tlh/defaults.json` only and never authorizes or modifies custom agents. Model and effort resolve independently per role with project defaults at layer 2 of a 4-layer precedence stack, above persisted per-user overrides and bundled frontmatter but below explicit in-session actions. Project defaults are applied at runtime only and are never written to user settings. See [docs/models.md § Project model/effort defaults](docs/models.md#project-modeleffort-defaults).
- Added project custom subagents as a stable architect extension point. A valid `.tlh/agents/custom/<UPPERCASE-SLUG>.md` definition with strict frontmatter is loaded only when persisted project trust covers the validated Git worktree root and can be requested naturally by name. Reloads create immutable project-agent generations; same-session resume uses the retained generation, while a process-restarted resume revalidates the current trusted definition, but a resume from a genuine new session fails closed. Persisted markers never grant control authority. See [docs/custom-subagents.md](docs/custom-subagents.md).
...
- **Breaking:** reduced the first-party subagent contract to fresh-only direct single/parallel execution. Every child starts a fresh session with no caller-supplied `context` or agent `defaultContext`; `turnBudget` and its old turn-count controls are not supported. Foreground runs and TLH-tracked `async: true` background runs remain available, along with human-owned execution policy, `maxExecutionTimeMs`, `toolBudget`, native `contact_supervisor`, status/interrupt/resume/steer controls, acceptance, diagnostics, artifacts, and model fallback. Saved chains and the external pi-intercom detach request/result/control integration are unsupported; TLH-tracked async work still uses a detached OS child process managed by TLH. Existing run, session, and saved-chain artifacts are not rewritten or deleted, and their status/history may remain readable. Older paused runs whose persisted configuration depends on retired `steps`, fork/context inheritance, the removed external pi-intercom detach request/result/control integration, turn-budget behavior, or a retired execution timeout at an async-runner envelope/config root or plan root cannot be resumed; start a new direct single or parallel run instead. Normal direct-plan durable pause/resume remains supported. Migration: remove retired fields from custom definitions and stale settings, then use the supported fields described in [docs/subagents.md](docs/subagents.md) and [docs/custom-subagents.md](docs/custom-subagents.md).
- **Breaking:** removed generic custom-agent discovery from active-profile `agents/**`, global `~/.agents`, project `.pi/agents/**` and `.agents/**`, configured `subagents.agentDirs`, installed-package/extra-directory definitions, and settings/default overrides—not merely their ability to authorize `embedded.*`. An exact active-profile `subagents.agentOverrides["embedded.<slug>"].disabled: true` entry remains only as a deny-only tombstone for that captured project target; `disabled: false` and all other fields are inert. These definitions no longer appear in TLH's custom-agent `list`/`get` or direct-dispatch inventory and cannot create or authorize a custom target. Canonical installer-managed packaged TLH roles continue loading from fixed `<agent-dir>/tlh/agents/subagents/<role>.md` paths. Only the persisted-trust-authorized direct Git-root `.tlh/agents/custom/<UPPERCASE-SLUG>.md` contract can authorize a project custom agent now.
- **Breaking:** retired structured-output child contracts and runtime capture/validation. Direct plans carrying `structuredOutput` or `structuredOutputSchema` task properties fail closed with guidance to start a new direct run without a structured output contract; existing historical artifacts remain readable and are not rewritten.
- **Breaking:** retired caller-facing subagent execution overrides for task `reads`/`progress`, top-level `concurrency`/`includeProgress`, `fallbackModels`, and execution timeouts. Configure `defaultReads`, `defaultProgress`, and `fallbackModels` in the agent definition, configure `parallel.concurrency` and the shared `execution.maxRunTimeMs` in `<agent-dir>/extensions/subagent/config.json`, and use the documented role ceilings/settings overrides for per-agent policy; direct calls carrying retired keys fail before a child launches with migration guidance. Model-facing root `timeoutMs` and public `tasks[].timeoutMs` are no longer accepted. An executable async-runner envelope/config with its own root `timeoutMs`, or a persisted plan with plan-root `timeoutMs`, fails closed before launch; remove only that retired envelope/plan-root field and restart as a new direct single or parallel run. TLH-written per-step `plan.task.timeoutMs` and `plan.tasks[].timeoutMs` remain valid trusted role-ceiling metadata and must not be removed. Historical records remain readable and are not rewritten. The former six-minute scout cap tracked by issue #420 is retired. Agent defaults, provider-aware fallback selection, persisted plans, live progress, and result-state diagnostics remain supported.
