...
- Manual `~/.pi/web-search.json` migration docs now resolve the target agent directory before writing and still refuse symlink or `..` paths into the normal `~/.pi/agent` profile.
