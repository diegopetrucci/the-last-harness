...
- Bumped the bundled `pi-anthropic-auth` extension from `2.0.8` to `3.2.2`. The primary motivation is a fix for upstream issue #66: the 2.x request shaper incorrectly reordered assistant tool-use content, causing interleaved-thinking requests to fail with `400: thinking blocks in the latest assistant message cannot be modified` — a bug TLH users hit today. The 3.x release also adds Claude Code version reconciliation (raising `cc_version` to match Pi's `claude-cli` version at the wire) and automatic retry when Anthropic rejects a request with `claude_code_version_too_old`. **Known regression:** Pi 0.87.1 passes per-message effort for `claude-opus-5-5`, `claude-opus-5`, and `claude-fable-5-1` in content-less system messages, but `3.x`'s `shapeSystemRoleMessages()` silently drops content-less system messages, discarding the requested effort — no error is raised. As a result, `code-reviewer` (configured `max` effort on `claude-opus-5`) will run at `high`, which is a capability downgrade; `oracle` and `contrarian` (both configured `medium` on `claude-fable-5-1`) will also run at `high`. The architect, bug-hunter, and product primaries are unaffected because they are already configured at `high`; rush, developer, and the haiku-based agents are not managed-effort models. There is no config workaround — the effort value itself is what gets discarded. Upstream fix is tracked at [gotgenes/pi-anthropic-auth#79](https://github.com/gotgenes/pi-anthropic-auth/pull/79).
...
- You can now append instructions to the system prompt of all primary agents, and subagents by creating `.tlh/agents/builtin/<ROLE>_PROMPT_APPEND.md` files.
- Create your own, custom, subagents by adding `.tlh/agents/custom/<UPPERCASE-SLUG>.md` files! See [docs/custom-subagents.md](docs/custom-subagents.md).
- Tests (tickets) are now run by a very light and quick `test-runner` subagent.
- You can now provide per-project/checkout model/thinking defaults via `.tlh/defaults.json`. See [docs/models.md § Project model/effort defaults](docs/models.md#project-modeleffort-defaults).
