...
- Bumped the pinned Pi runtime to `0.84.4`. Pi's native model picker keeps Enter selections session-only and uses Ctrl+S to persist the isolated profile model default; TLH's second model-scope prompt was removed in favor of those native Enter/Ctrl+S controls, and TLH primary model application no longer rewrites that profile model default.
- Pi's built-in `/thinking` remains native. TLH registers only `/effort` as a behavioral alias using Pi's exported native picker and supported levels; Enter/session-only, Ctrl+S/save-default, and Esc/cancel controls remain visible. Typed levels and cycles are session-only; `/effort` Ctrl+S saves `defaultThinkingLevel` only through TLH's guarded isolated-profile settings path, preserving unknown fields and reporting a session-only fallback on failure.
