...
- Removed the eight builtin subagent definitions (`context-builder`, `delegate`, `oracle`, `planner`, `researcher`, `reviewer`, `scout`, `worker`) from `extensions/subagents/agents/`. TLH ships its own eight agents and sets `disableBuiltins: true`; the bundled definitions were never used. Removed the `models` management action (`handleModels` in `agent-management.ts`, the `"models"` enum member in `schemas.ts` and `SUBAGENT_ACTIONS` in `types.ts`, the action bullet in `tool-description.ts`, and the `"models"` entry from `SAFE_SUBAGENT_ACTIONS` in `the-last-harness-subagent-safety.mjs`). The `models` action was actively misleading: it reported only the now-removed builtins and none of TLH's real agents. The `disableBuiltins` setting remains parsed and stored to preserve TLH settings management; it is now inert since no builtin agents exist. Removed the orphaned `disableThinking` setting from `SubagentSettings` and its parse/validate block in `agents.ts`; it was consumed exclusively inside the now-deleted `applyBuiltinOverrides`/`clearBuiltinThinking` path and is undocumented and unwritten by TLH.

- Removed the dynamic-fanout capability: chain steps that expanded a prior step's structured array output into N parallel children and collected their results. The capability is unreachable in The Last Harness — the subagent tool schema exposes no `chain`, `chainName`, `chainDir`, `expand` or `collect` input, `unsupportedSavedChainInput` rejects all of them at the tool boundary, no chain definition files exist, and an ordinary `tasks[]` dispatch always produces an array-valued `parallel` step that the dynamic predicate excludes by construction. Deletes `dynamic-fanout.ts`/`.js` and removes `DynamicParallelStep`, `DynamicParallelTemplate`, `DynamicExpandSpec`, `DynamicParallelGroup`, `DynamicFanoutError`, `isDynamicParallelStep`, `isDynamicRunnerGroup`, the `dynamicFanoutMaxItems` threading and the `chain.dynamicFanout` setting. Ordinary parallel dispatch (`tasks[]`, `ParallelStep`, `expandChainParallelCounts`, `parallel-groups.ts`) is unaffected, and `workflow-graph.ts` and `chain-outputs.ts` are retained with only their dynamic branches removed. Removed the acceptance residue the removal itself created: `aggregateAcceptanceReport` (whose only callers were in the deleted subagent-runner fanout paths) and the `dynamic`/`dynamicGroup` fields and branches in `acceptance.ts`, `execution.ts`, `async-execution.ts`, and `types.ts` (which were settable only by those same deleted call sites).
...
- Removed the seven unreachable mutating agent-management verbs (`create`, `update`, `delete`, `eject`, `disable`, `enable`, `reset`). The subagent tool's `action` schema enum (`extensions/subagents/src/extension/schemas.ts`) admits only `list`, `get`, `status`, `interrupt`, `resume`, `steer`, and `doctor`; none of the write verbs can be named by any caller. Also removed the now-vestigial `MUTATING_MANAGEMENT_ACTIONS` belt-gate in `subagent-executor.ts`. The `allowMutatingManagementActions` flag was subsequently removed outright by the nested-fanout removal above, which deleted its only setter.

- Removed the immutable upstream-import archive (`docs/subagents-history/`), contributor and maintainer docs (`docs/local-development.md`, `docs/releasing.md`, `docs/dependency-risk.md`, `docs/pin-bump-verification.md`, `docs/upstream-sync-inventory.md`, `docs/pi-startup-investigation-2026-07-15.md`, `docs/web-search-fork-release-cadence.md`, `docs/workflow-evals.md`), and `docs/brand/` assets from the published npm package. All excluded files remain in the repository unchanged; this is a publication-scope change only. Published docs are now exactly 11 end-user files: `commands`, `embedded-subagents`, `git-attribution`, `install`, `integrations`, `mcp`, `models`, `subagents`, `telemetry`, `troubleshooting`, and `web-search`. The package shrinks from 480 files / 6400 KB unpacked / 1481 KB tarball to 449 files / 5699 KB unpacked / 1267 KB tarball. `scripts/check-package-contents.mjs` now enforces a fail-closed `PUBLIC_DOCS` allowlist: it fails if a packed `docs/` path is not allowlisted, and fails if an allowlisted doc is missing. Links in the published `README.md`, `docs/subagents.md`, and `docs/web-search.md` that pointed at now-unpublished paths were repointed to absolute GitHub URLs on `main` so the tarball has no dangling relative links.

### Added

- **TLH now reports effective agent configuration in telemetry.** `Tlh.launched` gains `Tlh.Runtime.thinking` (the primary session's privacy-filtered thinking level), `Tlh.PrimaryAgent.name` (the privacy-filtered primary agent name — `architect`, `bug-hunter`, `product`, or `rush`; other values become `custom`, missing values become `unknown`), and, for each of the eight bundled subagents, `Tlh.Subagent.<name>.thinking` and `Tlh.Subagent.<name>.model` reflecting the effective configured values. Settings overrides take precedence over installed frontmatter. Disabled agents (via `subagents.agentOverrides`) report both keys as `disabled`; a key explicitly cleared via `model: false` or `thinking: false` is reported as `cleared`, distinguishing a deliberate removal from an unconfigured value. Per-subagent override resolution reads `subagents.agentOverrides` from both the isolated profile's user `settings.json` and the nearest project's config-dir `settings.json`, with the project scope taking precedence — mirroring how the subagents runtime resolves custom agents. The winning scope's entry applies wholesale: a project-scope override that sets only `thinking` also discards any `model` from the user-scope entry. The `tlh.telemetry.enabled` opt-out remains user-scope only. All existing opt-outs (`tlh.telemetry.enabled: false`, `PI_OFFLINE`, `TLH_SKIP_TELEMETRY`, `TLH_TELEMETRY_DISABLED`, `PI_TELEMETRY=0`) continue to apply.

### Fixed

- **First-party subagent dispatches now capability-check thinking levels before appending model suffixes** ([#305](https://github.com/diegopetrucci/the-last-harness/pull/305)). On both foreground and background dispatch paths, a known-unsupported level is left off the model string only when positive registry metadata rules it out: `reasoning: false` for a non-`off` level, a present `thinkingLevelMap` that omits `xhigh` or `max`, or the selected level mapped to `null`. The gate fails open in two distinct cases: when the model cannot be resolved in the registry at all, and when a resolved model carries no `thinkingLevelMap`; absent metadata is not evidence of absence, so those cases still receive the suffix. An explicit caller `thinkingOverride` is exempt, and each dropped level emits a note naming the level and model. This gate also applies to dispatches that never touch `/subagent-settings`, so it is runtime behavior and is not feature-gated by that command. The stale model-suffix level subset was removed, so `max` is emitted and parsed like every other level; this matters because `agents/subagents/developer.md` ships `tlhOpenaiThinking: max`.

### Minor details

- **Added `/subagent-settings` for persistent bundled minor-agent model and effort overrides** ([#305](https://github.com/diegopetrucci/the-last-harness/pull/305)). It applies to the eight bundled roles — `code-reviewer`, `contrarian`, `developer`, `diff-summarizer`, `librarian`, `oracle`, `repo-scout`, and `web-scout` — and supports `/subagent-settings status [role]`, `/subagent-settings set <role> [model <provider/id>] [effort <off|minimal|low|medium|high|xhigh|max>]`, `/subagent-settings reset <role> [model|effort]`, and `/subagent-settings reset-all`; invoking it with no arguments opens the TUI picker, while non-TUI/headless use reports status. The optional model and effort pairs can be supplied in either order. Overrides are stored under `subagents.agentOverrides` in the active isolated profile's `settings.json`, writes are refused outside that profile, and normal Pi configuration is not affected. The user-facing `effort` field is stored as the runtime's `thinking` field. Writes that replace existing settings content create a collision-safe `settings.json.bak-*` backup. A caller-supplied dispatch model still wins over a stored model pin, while stored overrides resolve before bundled provider-aware defaults when no model is supplied. Fixed model overrides for `code-reviewer`, `oracle`, and `contrarian` ask for provider-independence confirmation with `Provider independence is not guaranteed when a fixed model override is configured for this role.` and refuse the write in headless or otherwise non-UI mode because that warning cannot be confirmed. The supported effort values are `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`; undo with `/subagent-settings reset <role> model`, `/subagent-settings reset <role> effort`, `/subagent-settings reset <role>`, `/subagent-settings reset-all`, or by restoring the desired `settings.json.bak-*` backup. `reset-all` clears only bundled model and effort overrides and preserves unrelated role keys and unknown entries.
