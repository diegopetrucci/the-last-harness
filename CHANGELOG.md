...
- Installing from `main` now uses isolated defaults so a main-track install no longer collides with a stable-release install: the wrapper command defaults to `tlh-main` (instead of `tlh`) and the isolated profile defaults to `~/.the-last-harness-main/agent` (with a private runtime at `~/.the-last-harness-main/runtime`). Release-tag installs are unchanged, and explicit `--wrapper-name`/`--agent-dir` (or `TLH_WRAPPER_NAME`/`TLH_AGENT_DIR`) always override. Removing a main-track install requires the matching `uninstall.sh --wrapper-name tlh-main --agent-dir ~/.the-last-harness-main/agent`.
