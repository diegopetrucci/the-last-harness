...
- Retired RTK from TLH shipping/runtime support: TLH no longer registers the RTK extension or manages its binary lifecycle; full install/update conservatively remove the two exact legacy RTK regular-file artifacts (`bin/rtk` and `tlh/tlh-rtk.mjs`), while install/update/`tlh doctor --repair` scrub stale RTK settings/package markers from older TLH installs.
