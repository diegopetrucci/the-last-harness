...
- Bumped the bundled critical `pi-subagents` default extension pin from `npm:@diegopetrucci/pi-subagents@0.31.10` to `npm:@diegopetrucci/pi-subagents@0.31.12`.
...
- Packaged librarian, web-scout, repo-scout, and diff-summarizer guidance now defines scoped stop rules, architect async steering via `steer`, and per-agent tool-call budgets: soft tool-call thresholds provide an advisory wrap-up nudge, while at hard thresholds bundled `pi-subagents` 0.31.12's omitted block list blocks only `read`, `grep`, `find`, and `ls`, leaving `bash` and web research tools available.
...
- Bumped the bundled Pi runtime pin from `0.82.1` to `0.83.0`. Validated as behavior-neutral for TLH's compatibility shims. The sole upstream breaking change — TypeBox 1.3.7 removing deprecated APIs such as `Type.Base` and `Value.Mutate` — has no TLH exposure. Users inherit one concrete fix from Pi 0.83.0: context files no longer load twice when a linked Git worktree is nested under its main repository.
- Refreshed development dependencies: `@types/node` to 26.1.2, `eslint` to 10.8.0, and `globals` to 17.9.0.
- `typescript` is intentionally held at `6.0.3`. `typescript-eslint@8.65.0` declares a peer range of `>=4.8.4 <6.1.0`, making TypeScript 7.x incompatible until `typescript-eslint` widens its peer range.
