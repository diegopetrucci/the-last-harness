...
- Experimental, default-off prompt-cache heartbeat: while async subagent runs are live and the parent session is idle, TLH can replay the last provider request and abort it early to keep the provider prompt cache warm at cache-read prices. Enable via `heartbeat.enabled` in the subagent extension config; per-beat accounting is written to `<agent-dir>/subagents/heartbeat.jsonl`, each beat-bearing gap emits a session-visible summary entry, and `/subagents-doctor` reports totals. Gap verdicts are `saved`, `wasted`, `lost`, or `unneeded`; the `unneeded` verdict covers benign zero-beat gaps where the async run finished before the first beat timer fired (these are no longer incorrectly counted as `lost`). See the "Prompt-cache heartbeat" section in `docs/subagents.md`.
