...
- Projects can now provide session-scoped model and effort defaults for the bundled primary agents and bundled subagent roles via `.tlh/defaults.json` at the Git worktree root. Defaults are gated by the same worktree-level trust decision as `.tlh/agents` (the confirmation prompt now describes both agent definitions and model/effort defaults). Model and effort resolve independently per role with project defaults at layer 2 of a 4-layer precedence stack, above persisted per-user overrides and bundled frontmatter but below explicit in-session actions. Project defaults are applied at runtime only and are never written to user settings. See [docs/models.md § Project model/effort defaults](docs/models.md#project-modeleffort-defaults).
- Added project custom subagents as a stable architect extension point. A valid `.tlh/agents/<slug>.md` definition with strict frontmatter is loaded only after project/session approval and can be requested naturally by name. Reloads create immutable project-agent generations; same-session resume uses the retained generation, while resume after a process restart or new session fails closed. Persisted markers never grant control authority. See [docs/custom-subagents.md](docs/custom-subagents.md).
