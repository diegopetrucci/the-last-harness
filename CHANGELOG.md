...
- The `/tokens` report now includes a "Cache misses" section that surfaces significant prompt-cache misses — missed tokens, extra cost, and miss count — with a short explanation of what a cache miss is and when it happens, plus a table of the worst individual misses.
- The `/tokens` report Overview bucket table now shows a Provider column and breaks the subagents bucket into per-subagent rows grouped by agent and provider, while retaining the `Subagents (all)` subtotal.
