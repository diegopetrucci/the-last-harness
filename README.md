...
- **Repo toolkit helper**: TLH bundles a patched `pi-rtk` fork. Its repo-tooling features need an `rtk` binary on your `PATH`; use `/rtk` to check status, and `tlh defaults disable rtk` if you do not want the bundled default. The TLH fork stays compatible with `quiet-tools` because it only rewrites bash command input and leaves Pi's normal bash rendering in place, so collapsed bash rendering stays active without adding a persistent footer indicator.
