...
`tlh github` is an installer-owned wrapper subcommand for REST-first GitHub workflows that TLH covers directly. It runs the packaged `scripts/tlh-gh.mjs` helper without launching upstream Pi, uses your existing `gh` authentication, and currently covers rate-limit inspection, repo metadata, issue and pull-request reads, diffs, reviews, comments, issue/PR creation, issue/PR comment creation, and commit checks/statuses. It does **not** cover GraphQL-only operations such as review threads or `statusCheckRollup`; those stay explicit limitations. If `gh` is missing or unauthenticated, fix that with `gh auth login` / `gh auth status`. If the packaged helper files are missing or corrupt, rerun `tlh update` to recover them.
...
**`gh` CLI (for `tlh github` and `librarian` GitHub research):** Install `gh` from <https://cli.github.com/> and authenticate with `gh auth login` before using `tlh github` or the `librarian` subagent. Run `gh auth status` to confirm. `tlh github` reuses that auth for covered REST workflows, including issue/PR creation and comment mutations, while librarian stays read-only and falls back to plain `git clone` only when checkout inspection is genuinely necessary. Without an authenticated `gh`, TLH reports what it could not verify rather than silently failing.
