...
**`gh` CLI (for GitHub work and `librarian` research):** Install `gh` from <https://cli.github.com/> and authenticate with `gh auth login` before using TLH for GitHub-heavy work. Preflight with `gh auth status` and `gh api rate_limit`. TLH agents prefer direct REST `gh api` usage for covered workflows, with concrete reads such as `gh api repos/OWNER/REPO`, `gh api repos/OWNER/REPO/issues/NUMBER`, `gh api repos/OWNER/REPO/pulls/NUMBER`, `gh api repos/OWNER/REPO/commits/SHA/check-runs`, and `gh api repos/OWNER/REPO/commits/SHA/status`; add `gh api --paginate ...` when lists can span multiple pages. For authorized mutations, send JSON safely on stdin, for example `printf '%s\n' '{"body":"..."}' | gh api repos/OWNER/REPO/issues/NUMBER/comments --method POST --input -`. Review threads and `statusCheckRollup` remain GraphQL-only exceptions, so TLH should call that out explicitly instead of pretending REST coverage. The `librarian` subagent stays read-only and only uses plain `git clone` when checkout inspection is genuinely necessary. Without an authenticated `gh`, TLH reports what it could not verify rather than silently failing.
