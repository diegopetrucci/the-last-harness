...
Advanced users can also add trusted user-owned embedded subagents, gated behind the default-off `embedded-subagents` experimental flag. Only architect may initiate embedded delegation, and only a regular non-symlink `.md` profile file under the active TLH profile's `agents/**/*.md` tree with `package: embedded`, a valid `name`, and a non-empty `description` authorizes it; `.chain.md` files do not. A new session or explicit `/reload` recaptures flag state. Product and Bug-hunter can still resume an architect-started embedded run in the same session under the accepted issue #330 limitation. See [docs/embedded-subagents.md](docs/embedded-subagents.md).
